/*------------------------------------------------------------------------------
--                                                                            --
--       This software is confidential and proprietary and may be used        --
--        only as expressly authorized by a licensing agreement from          --
--                                                                            --
--                            VeriSilicon Inc.                                --
--                                                                            --
--                   (C) COPYRIGHT 2015 VeriSilicon Inc                       --
--                            ALL RIGHTS RESERVED                             --
--                                                                            --
--                 The entire notice above must be reproduced                 --
--                  on all copies and should not be removed.                  --
--                                                                            --
--------------------------------------------------------------------------------
--
--  Abstract  :
--
------------------------------------------------------------------------------*/

/*------------------------------------------------------------------------------

    Table of contents

    1. Include headers
    2. External compiler flags
    3. Module defines
    4. Function prototypes

------------------------------------------------------------------------------*/

#ifndef __ENC_JPEG_INSTANCE_H__
#define __ENC_JPEG_INSTANCE_H__

/*------------------------------------------------------------------------------
    1. Include headers
------------------------------------------------------------------------------*/
#include "enccommon.h"
#include "encpreprocess.h"
#include "encasiccontroller.h"

#include "EncJpeg.h"
#include "rate_control_jpeg.h"
#include "encdec400.h"
#include "encufbc.h"

/*------------------------------------------------------------------------------
    2. External compiler flags
--------------------------------------------------------------------------------

--------------------------------------------------------------------------------
    3. Module defines
------------------------------------------------------------------------------*/
enum JpegEncStatus { ENCSTAT_INIT = 0xA1, ENCSTAT_ENCODE, ENCSTAT_ERROR };

typedef struct {
  void *ctx;
  u32 encStatus;
  asicData_s asic;
  stream_s stream;
  EWLHwConfig_t featureToSupport;
  u32 reserve_core_info;
  preProcess_s preProcess;
  jpegData_s jpeg;
  inputLineBuf_s inputLineBuf;
  streamMultiSeg_s streamMultiSegment;
  jpegEncRateControl_s rateControl; /* Rate control parameters */
  u32 timeIncrement;                /* The previous picture duration in units
                                    * of 1/frameRateNum. 0 for the very first picture
                                    * and typically equal to frameRateDenom for the rest.
                                   */
  i32 fixedQP;                      /*fixed QP to every frame*/
  /** quality settinf for still picture, valid when fixedQP=0 and  */
  JpegEncQuality quality;

  u32 input_alignment;
  u64 lumaSize;
  u64 chromaSize;
  u64 dec400LumaTableSize;
  u64 dec400ChrTableSize;
  u32 dec400Enable;
  u32 axiFEEnable; /*0: axiFE disable   1:axiFE normal mode enable   2:axiFE bypass  3.security mode*/
  u32 invalidBytesInBuf0Tail;
  VCDec400data dec400_data;
  VCDec400data dec400OsdData;

  u32 sliceinfoEn; /* 0 disable poll sliceinfo  1: enable poll sliceinfo */
  u32 sliceinfo_status; /* check if poll sliceinfo timeout*/
  const void *inst; /* used as checksum */

  EncUfbc ufbc;
  ufbc_param ufbcParam;
  u32 ufbc_status; /* check if ufbc timeout*/
} jpegInstance_s;

JpegEncRet JpegEncSetQuailty(JpegEncInst inst, JpegEncQuality *quality);
JpegEncRet JpegEncSetNonRoi(JpegEncInst inst, const u8 *filter);
void JpegSetQTable(u8 *dst, const u8 *src);

#endif
