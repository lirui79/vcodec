...
u32 MjpegEncodeAVIHeader(u32 *outbuf, u32 width, u32 height, u32 frameRateNum,
  ...
  aviheader.Flages =
      0x910;  //indicate idx block contains or not, interaced or not
  ...
  memcpy(&avistreamheader.fccType, "vids",
         4);  // stream type:'auds'(audio),'vids'(video)
  memcpy(&avistreamheader.fccHandler, "MJPG",
         4);  // decoder type: 'MJPG', 'H264' etc
