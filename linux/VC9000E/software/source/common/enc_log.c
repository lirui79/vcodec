...
--                            VeriSilicon Inc.                                --
--                                                                            --
--                   (C) COPYRIGHT 2020 VeriSilicon Inc                       --
--                            ALL RIGHTS RESERVED                             --
--                                                                            --
...
#define VPU_MAX_DEBUG_BUFFER_LEN 1024 * 30
...
static VCEncLogSetting env_log = {LOG_STDOUT, VCENC_LOG_ERROR, 0x003F, 0x0001};

static unsigned int vcenc_log_trace_bitmap[] = {
    1 << VCENC_LOG_TRACE_API, 1 << VCENC_LOG_TRACE_REGS,
    1 << VCENC_LOG_TRACE_EWL, 1 << VCENC_LOG_TRACE_MEM,
    1 << VCENC_LOG_TRACE_RC,  1 << VCENC_LOG_TRACE_CML,
    1 << VCENC_LOG_TRACE_PERF};
static char *log_trace_str[] = {"api", "regs", "ewl", "mem",
                                "rc",  "cml",  "perf"};

static unsigned int vcenc_log_check_bitmap[] = {
    1 << VCENC_LOG_CHECK_RECON, 1 << VCENC_LOG_CHECK_QUALITY,
    1 << VCENC_LOG_CHECK_VBV, 1 << VCENC_LOG_CHECK_RC,
    1 << VCENC_LOG_CHECK_FEATURE};
...
void getNameByPid(pid_t pid, char *task_name) {
  ...
  sprintf(proc_pid_path, "/proc/%lld/status", (long long)pid);
  ...
  if (NULL != fp) {
    ...
    fclose(fp);
    sscanf(buf, "%*s %1023s", task_name);
  }
}

// rainbow fprint
static void _rainbow_fprint(FILE *stream, VCEncLogLevel log_level,
                            char *str_content) {
  switch (log_level) {
...
int VCEncLogInit(unsigned int out_dir, unsigned int out_level,
  ...
  // VCEncLogGetEnvSetting();
  env_log.out_dir = (VCEncLogOutput)out_dir;
  env_log.out_level = (VCEncLogLevel)out_level;
  env_log.k_trace_map = trace_map;
  env_log.k_check_map = check_map;

  if ((LOG_ONE_FILE == env_log.out_dir) &&
      (VCENC_LOG_QUIET != env_log.out_level)) {
...
void VCEncTraceMsg(const void *inst, VCEncLogLevel level,
                   unsigned int log_trace_level, const char *fmt, ...) {
  char arg_buffer[VPU_MAX_DEBUG_BUFFER_LEN - 128] = {0};
  char msg_buffer[VPU_MAX_DEBUG_BUFFER_LEN + 21] = {0};
  ...
  sprintf(msg_buffer, "[%s]%s[%p]%s", log_trace_str[log_trace_level],
          time_stamp_buffer, (inst), arg_buffer);
  if ((LOG_ONE_FILE == env_log.out_dir) &&
      (VCENC_LOG_QUIET != env_log.out_level)) {
    ...
    sprintf(log_out_filename, "vcenc_trace_p%d_t%llu.log", GETPID(),
...
void VCEncCheckMsg(const void *inst, VCEncLogLevel level,
                   unsigned int log_check_level, const char *fmt, ...) {
  char arg_buffer[VPU_MAX_DEBUG_BUFFER_LEN - 128] = {0};
  char msg_buffer[VPU_MAX_DEBUG_BUFFER_LEN + 21] = {0};
  char time_stamp_buffer[128] = {0};
  va_list arg;
  ...
  // add time stamp
  _get_time_stamp(time_stamp_buffer);
  sprintf(msg_buffer, "[%s]%s[%p]%s", log_check_str[log_check_level],
          time_stamp_buffer, (inst), arg_buffer);
  if ((LOG_ONE_FILE == env_log.out_dir) &&
      (VCENC_LOG_QUIET != env_log.out_level)) {
    ...
    //combine the log filename
    sprintf(log_out_filename, "vcenc_check_p%d_t%llu.log", GETPID(),
            (unsigned long long)pthread_self());
    fLog = fopen(log_out_filename, "at+");
	if (fLog == NULL)
		return;
...
#define REGS_LOGI(inst, fmt, ...)                                          \
  VCEncTraceMsg(inst, VCENC_LOG_INFO, VCENC_LOG_TRACE_REGS, "[%s:%d]" fmt, \
...
void VCEncLogGetEnvSetting(VCEncLogSetting *env_log) {
  ...
  if (env_log_tmp) {
    ...
    if (tmp_val_1 < LOG_COUNT) {
      env_log->out_dir = (VCEncLogOutput)tmp_val_1;
    }
  }
  ...
  if (env_log_tmp) {
    ...
    if (tmp_val_1 < VCENC_LOG_COUNT) {
      env_log->out_level = (VCEncLogLevel)tmp_val_1;
    }
  }
