...
void get_align(bits_buffer_s *b, u32 bytes) {
  ...
  if (tail) get_value(b, 8 - tail, HANTRO_FALSE);
...
#if defined(SUPPORT_MEM_STATISTIC) && !defined(SYSTEMSHARED)
...
/*
 * using alloca() to stats stack size.
 * assume only profile one main thread.
 *
 * TODO: stats for different threads
 */
#define STACK_STAT_SIZE      (100*1024)
#define STACK_STAT_MAGICWORD 0xDEADBEEF

unsigned int *g_stack_top = NULL;
unsigned int g_stack_size = 0;
...
void StackStatInit() {
  ...
  if (g_stack_top == NULL)
  	g_stack_top = (unsigned int *)alloca(STACK_STAT_SIZE);  //cppcheck-suppress allocaCalled

  for (i = 0, p = g_stack_top; i < STACK_STAT_SIZE/sizeof(unsigned int); i++) {
    *p++ = STACK_STAT_MAGICWORD;
  }
}

void StackConsumption(const char *func, unsigned int lineNum) {
  int i;
  unsigned int *p;
  unsigned int ss;

  for (i = 0, p = g_stack_top; i < STACK_STAT_SIZE/sizeof(unsigned int); i++) {
    if (*p == STACK_STAT_MAGICWORD) {
      p++;
    } else {
      ss = STACK_STAT_SIZE - (p - g_stack_top) * sizeof(unsigned int);
      if (ss > g_stack_size)
        g_stack_size = ss;
      break;
    }
  }
  /* FIXME: using LOGMSG will add stack, should use LOGMSG if remove such impact */
  printf("%s:line%u Stack Size: %u bytes\n", func, lineNum, g_stack_size);
}
