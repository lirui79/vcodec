...
#define ASIC_FRAME_BUF_CUINFO_MAX \
  (VCENC_LOOKAHEAD_MAX + 2 * MAX_GOP_SIZE + MAX_CORE_NUM - 2)
...
typedef struct {
  /** \brief flag to indicate if use the RPS in sps */
  ...
  /** \brief indicate if use the picture with the negtive delta POC as reference frame */
  u8 u1DeltaPocS0Used[VCENC_MAX_REF_FRAMES];
  /** \brief indicate the negtive delta POC of reference frames, 20 bits are used */
  u32 u20DeltaPocS0[VCENC_MAX_REF_FRAMES];
  /** \brief indicate if use the picture with the positive delta POC as reference frame */
  u8 u1DeltaPocS1Used[VCENC_MAX_REF_FRAMES];
  /** \brief indicate the positive delta POC of reference frames, 20 bits are used */
  u32 u20DeltaPocS1[VCENC_MAX_REF_FRAMES];
} HWRPS_CONFIG;
...
typedef struct {
  ...
  u32 scanType;
  u32 supertileXOffsetX;
  u32 supertileXOffsetY;
  ...
  // 3DNR
  // u32 nrSigmaYCur;   // noise sigma estimation
  // u32 nrSigmaUCur;   // noise sigma estimation
  // u32 nrSigmaVCur;   // noise sigma estimation

  u32 noiseSigmaEst;
  u32 noiseSigmaY; // read only
  u32 noiseSigmaU;
  u32 noiseSigmaV;
  u32 noiseReductionStrength_IntraY;
  u32 noiseReductionStrength_IntraU;
  u32 noiseReductionStrength_IntraV;
  u32 noiseReductionStrength_InterY;
  u32 noiseReductionStrength_InterU;
  u32 noiseReductionStrength_InterV;

  u32 noiseReductionStrength_Y;
  u32 noiseReductionStrength_U;
  u32 noiseReductionStrength_V;
  u32 noiseReduction_ChromaMaxMV;
...
// for HEVC
  ...
  u32 sramPowerdownROPTimeoutFlag;
  ...
  u32 ctbRcThreshold[16];

  /* visual quality prior mode decision */
  /* Trail Detect Related Parameter */
  u32 trailDetectEnable;
  u32 trailDetectMinCuLogSize;
  u32 trailDetectPrevMvTh;
  u32 trailDetectCurMvTh;
  u32 trailSplitThCU16;
  u32 trailSplitThCU32;
  u32 trailUseLambdaMinQp;
  u32 trailQpFactorSSE;
  u32 trailQpFactorSAD;
  u32 trailSaoNumCu16Ctb;
  u32 trailSaoChrMaxQp;
  i32 trailDeltaQp;

  //Mod subject prefer Parameters
  u32 skipBinCostMaxDistTh;
  u32 intraBiasExtraBits;
  u32 iBiasChromaMseThr;
  u32 iBiasCostRatioThr;
  u32 iBiasDistRatioThr1;
  u32 iBiasDistRatioThr2;
  u32 IntraBiasMvThreshold;
  u32 trailAvoidIntraBias;
  u32 disableIntraDistRatioThr;
  u32 intraChrBincostRatio;
  ...
  /* SliceInfo Cfg */
  u32 sliceinfo_poll_time; /* poll time interval */
  u8 sliceinfo_poll_enable; /* set 1 to enable polling, 0 disable*/
  u8 sliceinfo_poll_timeout_int; /* set 1 to enable polling timeout interruput */
  u32 sliceinfo_poll_timeout_cycles; /* timeout max cycles */
  u8 sliceinfo_irqTypeMask_poll_timeout; /* irq type mask, 0=normal interruption, 1= abnormal interrupution*/
...
typedef struct {
  ...
  EWLLinearMem_t ctbRcMem[2+MAX_CORE_NUM*2];     //buffer number = 2*(1+coreNum)
  ...
  //frame Information memory
  EWLLinearMem_t framInfoMem;
  ...
  EWLLinearMem_t decTblLoopbackLineBufMem;
...
typedef struct {
  ...
  /** \brief flag to indicate if output recon or not.
   * For I-frame encoding, beside set numRefBuffsLum and numRefBuffsChr
   * as zero, need to set this flag as zero. */
  u32 writeReconToDDR;
} asicMemAlloc_s;
