...
i32 sw_init_image(struct sw_picture *p, struct sw_data *image, i32 w, i32 h,
  ...
  image->lum_width = w;
  image->lum_height = h;
  image->ch_width  = w >> p->ch_width_shift;
  // VCENC_CHROMA_IDC_422: HW TODO.
  image->ch_height = h >> p->ch_height_shift;
  image->lum_pad_width = padw;
  image->lum_pad_height = padh;
  image->ch_pad_width  = padw >> p->ch_width_shift;
  image->ch_pad_height = padh >> p->ch_height_shift;
  return OK;
}

void create_slices_ctrlsw(struct sw_picture *p, struct pps *pps,
                          u32 sliceSize) {
  ...
  int sliceNum = (sliceSize == 0)
                     ? 1
                     : ((pps->ctb_per_column + (sliceSize - 1)) / sliceSize);
  p->sliceNum = sliceNum;
  ctbs_per_slice = (sliceSize == 0) ? pps->ctb_per_picture
                                    : (pps->ctb_per_row * (i32)sliceSize);
...
/*------------------------------------------------------------------------------
  create_picture
------------------------------------------------------------------------------*/
struct sw_picture *create_picture_ctrlsw(struct vcenc_instance *vcenc_instance,
  ...
  u32 tileId;
  i32 ctbs_per_slice;
  if (!(p = EWLcalloc(1, sizeof(struct sw_picture)))) return NULL;
  ...
  //init chroma size shift before memory malloc
  p->ch_width_shift = CH_WIDTH_SHIFT(p->sps->chroma_format_idc);
  p->ch_height_shift = CH_HEIGHT_SHIFT(p->sps->chroma_format_idc);
  ...
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    ...
    if (sw_init_image(p, &p->input[tileId], w, h, SW_IMAGE_U8, HANTRO_FALSE, 0,
                      0))
      goto out;
  }
  ...
  p->picture_memeory_id = vcenc_instance->created_pic_num++;
...
------------------------------------------------------------------------------*/
void reference_picture_list(struct container *c, struct sw_picture *pic, bool oneRefB) {
  struct sw_picture *p;
  struct slice *s;
  struct node *n;
  struct rps *r;
  ...
  for (n = pic->slice.tail; n; n = n->next) {
    ...
    if (oneRefB && pic->sliceInst->type == B_SLICE) s->active_l1_cnt = MAX(s->active_l1_cnt, 1);
    if (pic->sliceInst->type != B_SLICE) s->active_l1_cnt = 0;
    if (pic->sliceInst->type == I_SLICE) s->active_l0_cnt = 0;
  ...
  if (cnt[0] != r->before_cnt) flag = HANTRO_TRUE;
  if (cnt[1] != r->after_cnt) flag = HANTRO_TRUE;

  for (n = pic->slice.tail; n; n = n->next) {
    s = (struct slice *)n;
    s->active_override_flag = HANTRO_TRUE || flag ||
                              (cnt[0] != s->active_l0_cnt) ||
                              (cnt[1] != s->active_l1_cnt);
...
void show_picture(struct container *c) {
  struct node *n;
  struct sw_picture *p;

  for (n = c->picture.tail; n; n = n->next) {
    p = (struct sw_picture *)n;
    APITRACE_INFO(NULL, "poc=%d ref_cnt=%d is_reference=%d\n",
        p->poc, p->ref_cnt, p->reference);
  }
}
