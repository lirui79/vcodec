...
void HevcBufferingSei(struct buffer *sp, sei_s *sei, vui_t *vui) {
  ...
  put_bit(sp, 0xFF,
          8); /* this will be updated after we know exact payload size */
  COMMENT(sp, "last_payload_size_byte");
  //rbsp_flush_bits(sp);
  //startByteCnt = *sp->cnt;
...
------------------------------------------------------------------------------*/
void HevcMasteringDisplayColourSei(struct buffer *sp,
...
------------------------------------------------------------------------------*/
void HevcContentLightLevelSei(struct buffer *sp,
