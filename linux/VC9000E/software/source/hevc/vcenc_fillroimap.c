...
#define ALIGN_AND_SEGMENT(length, align_unit_length, segment_length)    \
  (((((length) + (align_unit_length)-1) & (~((align_unit_length)-1))) + \
    (segment_length)-1) /                                               \
   (segment_length))
/** @ } */
...
static u8 fill_roimap_fmt5(const RoiRect *rect, u8 *QPCU_Ctrl_Infor) {
  ...
  if (rect->mode != CU_MODE_INTRA) return 1;
  QPCU_Ctrl_Infor[2] |= (rect->info.intra.luma_mode[0] & 0x3f) |
                        ((rect->info.intra.luma_mode[1] & 0x3) << 6);
...
static u8 fill_roimap_fmt6(const RoiRect *rect, u8 *QPCU_Ctrl_Infor) {
  ...
  if (rect->mode != CU_MODE_INTER) return 1;
  if (rect->part == ROI_INTERNAL_AUTOMODE) return 1;
  ...
  if (rect->part == ROI_PART_2N_2N) {
    ...
    if (rect->info.inter[0].dir != rect->info.inter[1].dir) {
      APITRACEERR(
          "QP_CU roimap format 6 error: rect->info.inter[0].dir != "
          "rect->info.inter[1].dir\n");
      return 1;
    }
    /* L0 */
    mvxi[0] =
        (rect->info.inter[0].mvx[0] + rect->info.inter[1].mvx[0] + 2) >> 3;
    mvxf_p0[0] = rect->info.inter[0].mvx[0] - (mvxi[0] << 2);
    mvyi[0] =
        (rect->info.inter[0].mvy[0] + rect->info.inter[1].mvy[0] + 2) >> 3;
    mvyf_p0[0] = rect->info.inter[0].mvy[0] - (mvyi[0] << 2);
    if (((mvxf_p0[0] < 4) && (mvxf_p0[0] >= -4)) ||
        ((mvyf_p0[0] < 4) && (mvyf_p0[0] >= -4))) {
      APITRACEERR(
          "QP_CU roimap format 6 error: (mvxf_p0[0]<4)&&(mvxf_p0[0]>=-4) || "
          "(mvyf_p0[0]<4)&&(mvyf_p0[0]>=-4)\n");
      return 1;
    }
    /* L1 */
    mvxi[1] =
        (rect->info.inter[0].mvx[1] + rect->info.inter[1].mvx[1] + 2) >> 3;
    mvxf_p0[1] = rect->info.inter[0].mvx[1] - (mvxi[1] << 2);
    mvyi[1] =
        (rect->info.inter[0].mvy[1] + rect->info.inter[1].mvy[1] + 2) >> 3;
    mvyf_p0[1] = rect->info.inter[0].mvy[1] - (mvyi[1] << 2);
    if (((mvxf_p0[1] < 4) && (mvxf_p0[1] >= -4)) ||
        ((mvyf_p0[1] < 4) && (mvyf_p0[1] >= -4))) {
      APITRACEERR(
          "QP_CU roimap format 6 error: ((mvxf_p0[1]<4)&&(mvxf_p0[1]>=-4)) || "
          "((mvyf_p0[1]<4)&&(mvyf_p0[1]>=-4))\n");
      return 1;
    }
  }
  ...
  QPCU_Ctrl_Infor[7] |=
      (((mvxi[0] >> 6) & 0x7)) | (((mvyi[0]) & 0x1f) << 3); /* 7bit mvyi */
  QPCU_Ctrl_Infor[8] |=
      (((mvyi[0] >> 5) & 0x3)) | (((mvxi[1]) & 0x3f) << 2); /* 9bit mvxi */
  QPCU_Ctrl_Infor[9] |=
      (((mvxi[1] >> 6) & 0x7)) | (((mvyi[1]) & 0x1f) << 3); /* 7bit mvyi */
  QPCU_Ctrl_Infor[10] |= (((mvyi[1] >> 5) & 0x3) << 0) | (((mvxf_p0[0])) << 2) |
                         (((mvyf_p0[0])) << 5);
...
static u8 fill_roimap_fmt7(const RoiRect *rect, u8 *QPCU_Ctrl_Infor) {
  ...
  if (rect->part == ROI_PART_2N_2N) {
    ...
    if (((mvxf_p1[0] < 4) && (mvxf_p1[0] >= -4)) ||
        ((mvyf_p1[0] < 4) && (mvyf_p1[0] >= -4))) {
      APITRACEERR(
          "QP_CU roimap format 6 error: (mvxf_p0[0]<4)&&(mvxf_p0[0]>=-4) || "
          "(mvyf_p0[0]<4)&&(mvyf_p0[0]>=-4)\n");
      return 1;
    ...
    if (((mvxf_p1[1] < 4) && (mvxf_p1[1] >= -4)) ||
        ((mvyf_p1[1] < 4) && (mvyf_p1[1] >= -4))) {
      APITRACEERR(
          "QP_CU roimap format 6 error: ((mvxf_p0[1]<4)&&(mvxf_p0[1]>=-4)) || "
          "((mvyf_p0[1]<4)&&(mvyf_p0[1]>=-4))\n");
      return 1;
...
static void fill_data(u8 *p, u8 roimap_format, u8 temp_start_x, u8 temp_end_x,
                      u8 temp_start_y, u8 temp_end_y, u8 temp_start_x_offset,
                      u8 temp_start_y_offset, u8 ctb_side_len_steps) {
  u32 i, j, k;
  u8 *p_start;
 ...
 */
VCEncRet VCEncFillRoiMap(const void *inst, const RoiRect *roi, u8 roimap_format,
  ...
  if (pEncInst->codecFormat == VCENC_VIDEO_CODEC_HEVC) {
    ...
    if (roimap_format == 1 && (QPCU_Ctrl_Infor[0] & 0x80) == 0x80 &&
      ...
      if (blk_side_length != 64) {
        APITRACEWRN(
            "Warning: In the IPCM mode, Treat Block Unit as 64 x 64 for HEVC.");
        blk_side_length = 64;
      }
    }
    if (roimap_format == 2 && (QPCU_Ctrl_Infor[0] & 0x80) == 0x80 &&
      ...
      if (blk_side_length != 32) {
        APITRACEWRN(
            "Warning: In the SKIP mode, Treat Block Unit as 32 x 32 for H264.");
        blk_side_length = 32;
      }
    }
  ...
  // caculation example  ( ((416 + 64 - 1) & (~(64 - 1))) + 32 - 1 ) / 32 = ( ((416 + 64 - 1) & (1111 1111 1100 0000) + 32 - 1 ) / 32
  //                                                          =  ( 479 & (1111 1111 1100 0000) + 32 -1 ) / 32
  // Fill a full max_cu_size length at rows trail for new length, then cut off with max_cu_size in new length. Finally seperates
  ...
  u8 cu8_num_in_ctb =
      cu8_num_in_ctb_side * cu8_num_in_ctb_side;  // cu8 number in ctb.
  u32 cu8_num_in_pic_CtbRows =
      ((ALIGN_AND_SEGMENT(pEncInst->width, pEncInst->max_cu_size,
                          blk_side_length)) /
  ...
  u16 ctb_index_of_left_top_x =
      (roi->x0 / blk_side_length) / block_num_in_ctb_side;
  u16 ctb_index_of_left_top_y =
      (roi->y0 / blk_side_length) / block_num_in_ctb_side;
  ...
  u8 *ctb_x_start_p =
      &roimap_delta_qpmemory[(ctb_index_of_left_top_y * cu8_num_in_pic_CtbRows +
                              ctb_index_of_left_top_x * cu8_num_in_ctb) *
                             format_byte_len[roimap_format]];
  u8 *ctb_x_start_p_origin = ctb_x_start_p;
  u8 ctb_start_x = ((roi->x0 / blk_side_length) % block_num_in_ctb_side) *
                   cu8_num_in_block_side;
  u8 ctb_start_x_offset = ctb_start_x * format_byte_len[roimap_format];
  u8 ctb_start_y = ((roi->y0 / blk_side_length) % block_num_in_ctb_side) *
                   cu8_num_in_block_side;
  u8 ctb_start_y_offset =
      ctb_start_y * cu8_num_in_ctb_side * format_byte_len[roimap_format];
  u8 ctb_end_x = ((UNIT_INDEX(roi->x0 + roi->dx, blk_side_length)) %
                  block_num_in_ctb_side) *
                     cu8_num_in_block_side +
                 cu8_num_in_block_side - 1;
  u8 ctb_end_y = ((UNIT_INDEX(roi->y0 + roi->dy, blk_side_length)) %
                  block_num_in_ctb_side) *
                     cu8_num_in_block_side +
                 cu8_num_in_block_side - 1;
  ...
  for (ctb_temp_y = ctb_index_of_left_top_y;
       ctb_temp_y <= ctb_index_of_right_buttom_y; ctb_temp_y++) {
    temp_start_y = 0;
    ...
    for (ctb_temp_x = ctb_index_of_left_top_x;
         ctb_temp_x <= ctb_index_of_right_buttom_x; ctb_temp_x++) {
      temp_start_x = 0;
      ...
      fill_data(ctb_x_start_p, roimap_format, temp_start_x, temp_end_x,
                temp_start_y, temp_end_y, temp_start_x_offset,
                temp_start_y_offset, move_stride);
