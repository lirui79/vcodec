...
static const u8 list0[16] = {16, 16, 16, 16, 16, 16, 16, 16,
                             16, 16, 16, 16, 16, 16, 16, 16};

static const u8 list1[64] = {16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 17, 16, 17,
                             16, 17, 18, 17, 18, 18, 17, 18, 21, 19, 20, 21, 20,
                             19, 21, 24, 22, 22, 24, 24, 22, 22, 24, 25, 25, 27,
                             30, 27, 25, 25, 29, 31, 35, 35, 31, 29, 36, 41, 44,
                             41, 36, 47, 54, 54, 47, 65, 70, 65, 88, 88, 115};

static const u8 list2[64] = {16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 17, 17, 17,
                             17, 17, 18, 18, 18, 18, 18, 18, 20, 20, 20, 20, 20,
                             20, 20, 24, 24, 24, 24, 24, 24, 24, 24, 25, 25, 25,
                             25, 25, 25, 25, 28, 28, 28, 28, 28, 28, 33, 33, 33,
...
static const i32 forward_scaling[6] = {26214, 23302, 20560,
                                       18396, 16384, 14564};

static const i32 inverse_scaling[6] = {40, 45, 51, 57, 64, 72};

static void profile_and_level(struct buffer *b, i32 max_num_sub_layers,
                              u32 profile, u32 level, u32 tier, VCEncInst inst);
...
static void H264SeqParameterSet(struct container *c, struct sps *s,
                                VCEncInst inst);
...
void *create_parameter_set(i32 type) {
  ...
  if (type == VPS_NUT) {
    if (!(p = EWLmalloc(sizeof(struct vps)))) return NULL;
    memcpy(p, &vps, sizeof(struct vps));
  } else if (type == SPS_NUT) {
    if (!(p = EWLmalloc(sizeof(struct sps)))) return NULL;
    memcpy(p, &sps, sizeof(struct sps));
  } else if (type == PPS_NUT) {
    if (!(p = EWLmalloc(sizeof(struct pps)))) return NULL;
    memcpy(p, &pps, sizeof(struct pps));
  } else if (type == RPS) {
    if (!(p = EWLmalloc(sizeof(struct rps)))) return NULL;
...
i32 init_parameter_set(struct sps *s, struct pps *p) {
  ...
  */
  ...
  /* Tiles ... */
  if (tile_init(p, p->tiles_enabled_flag, p->num_tile_columns,
                p->num_tile_rows))
    goto out;
...
i32 set_reference_pic_set(struct rps *r) {
  ...
  for (tmp = 0, i = 0; i < size; i++) {
    if (IS_LONG_TERM_REF_DELTAPOC(poc[0])) break;
    if (poc[0] >= 0) break;
  ...
  if (r->num_negative_pics) {
    if (!(r->ref_pic_s0 =
              qalloc(m, r->num_negative_pics, sizeof(struct ref_pic))))
      goto out;
  ...
  if (r->num_positive_pics) {
    if (!(r->ref_pic_s1 =
              qalloc(m, r->num_positive_pics, sizeof(struct ref_pic))))
      goto out;
...
void video_parameter_set(struct vps *v, VCEncInst inst) {
  ...
  profile_and_level(b, v->max_num_sub_layers, v->general_profile_idc,
                    v->general_level_idc, v->general_tier_flag, inst);
  ...
  if (v->slo_info_present_flag) {
    for (i = 0; i < v->max_num_sub_layers; i++) {
      ...
      if (vcenc_instance->profile == VCENC_HEVC_MAIN_STILL_PICTURE_PROFILE) {
...
/*------------------------------------------------------------------------------
  profile_and_level for VCE 6.0
------------------------------------------------------------------------------*/
void profile_and_level_60(struct buffer *b, i32 max_num_sub_layers, u32 profile,
                          u32 level, VCEncInst inst) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
...
/*------------------------------------------------------------------------------
  profile_and_level
------------------------------------------------------------------------------*/
void profile_and_level(struct buffer *b, i32 max_num_sub_layers, u32 profile,
                       u32 level, u32 tier, VCEncInst inst) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  preProcess_s *preProcess = &vcenc_instance->preProcess;
  i32 i = 0;
  ...
  if (vcenc_instance->profile == VCENC_HEVC_MONOCHROME_PROFILE) {
    // monochrome, table A.2
    // if (vcenc_instance->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_400) {
    ...
    } else if (vcenc_instance->profile == VCENC_HEVC_MAIN_422_10_PROFILE) {
    // Main 4:2:2 10, table A.2
    // else if (vcenc_instance->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_422){
      COMMENT(b, "general_max_12bits_constraint_flag")
      put_bit(b, 1, 1);
      ...
      put_bit(b, 0, 8);
      put_bit(b, 0, 2);
  } else if (vcenc_instance->profile == VCENC_HEVC_MONOCHROME_10_PROFILE) {
    // monochrome 10, table A.2
    COMMENT(b, "general_max_12bits_constraint_flag")
    put_bit(b, 1, 1);
    ...
    put_bit(b, 0, 8);
    put_bit(b, 0, 2);
  } else if (vcenc_instance->profile == VCENC_HEVC_MAIN_444_10_PROFILE) {
    // Main 4:4:4 10, table A.2
    COMMENT(b, "general_max_12bits_constraint_flag")
    put_bit(b, 1, 1);
    ...
    put_bit(b, 0, 8);
    put_bit(b, 0, 2);
  } else if (vcenc_instance->profile == VCENC_HEVC_SCC_MAIN_PROFILE) {
    // Screen-Extended (SCC) Main 4:2:0, table A.5
    COMMENT(b, "general_max_12bits_constraint_flag")
    put_bit(b, 1, 1);
    ...
    COMMENT(b, "general_max_14bits_constraint_flag")
    put_bit(b, 1, 1);

    COMMENT(b, "general_reserved_zero_33bits")
    put_bit(b, 0, 8);
    put_bit(b, 0, 8);
    put_bit(b, 0, 8);
    put_bit(b, 0, 8);
    put_bit(b, 0, 1);
  } else if (vcenc_instance->profile == VCENC_HEVC_SCC_444_10_PROFILE) {
    // Screen-Extended (SCC) Main 4:4:4 10, table A.5
    COMMENT(b, "general_max_12bits_constraint_flag")
    put_bit(b, 1, 1);
...
------------------------------------------------------------------------------*/
static void HEVCSeqParameterSet(struct container *c, struct sps *s,
                                VCEncInst inst) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  ...
  profile_and_level(b, s->max_num_sub_layers, s->general_profile_idc,
                    s->general_level_idc, s->general_tier_flag, inst);
  ...
  if(s->chroma_format_idc == VCENC_CHROMA_IDC_444) {
    COMMENT(b, "separate_colour_plane_flag")
    put_bit(b, 0, 1); /* Bob: Set separate_colour_plane_flag = 0, u(1) */
  }

  ASSERT(s->chroma_format_idc <= VCENC_CHROMA_IDC_444);
  ...
  if (s->slo_info_present_flag) {
    for (i = 0; i < s->max_num_sub_layers; i++) {
      ...
      if (s->general_profile_idc == SYNTAX_PROFILE_HEVC_MAIN_STILL_PICTURE) {
        //main still picture profile
        put_bit_ue(b, 0);
...
void sequence_parameter_set(struct container *c, struct sps *s,
                            VCEncInst inst) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
...
------------------------------------------------------------------------------*/
void init_poc_list(struct rps *r, i32 poc, bool use_ltr_cur, u32 codingType,
                   struct container *c, bool bH264, const u32 *pLTR_used,
                   u8 *rpsModify) {
  struct ref_pic *p;
  i32 tmp, i, j, k = 0, ii;
  ...
  for (i = 0, j = 0; i < r->num_negative_pics; i++) {
    ...
    if (bH264) {
      ...
      if (ii < VCENC_MAX_LT_REF_FRAMES) continue;
    ...
    for (ii = 0; ii < VCENC_MAX_LT_REF_FRAMES; ii++)
      if ((r->long_term_ref_pic_poc[ii] == poc + p->delta_poc)) {
        ...
        if (pLTR_used[ii] && !p->used_by_curr_pic) bModifyBefore = HANTRO_TRUE;
      }
  ...
  for (i = 0, j = 0; i < r->num_positive_pics; i++) {
    ...
    for (ii = 0; ii < VCENC_MAX_LT_REF_FRAMES; ii++)
      if (r->long_term_ref_pic_poc[ii] == poc + p->delta_poc) {
        r->long_term_ref_pic_poc[ii] = -1;
        if (pLTR_used[ii] && !p->used_by_curr_pic) bModifyAfter = HANTRO_TRUE;
      }
  ...
  if ((codingType == VCENC_INTRA_FRAME) && (poc > 0) && (r->before_cnt > 0) &&
      (r->follow_cnt == 0)) {
  ...
  for (i = 0, j = 0, k = 0; i < r->num_lt_pics; i++) {
    p = &r->ref_pic_lt[i];
    if (r->long_term_ref_pic_poc[p->delta_poc] == -1) continue;
...
------------------------------------------------------------------------------*/
i32 ref_pic_marking(struct container *c, struct rps *r, int saveExtPoc, VCEncInst inst, VCEncIn *pEncIn) {
  struct sw_picture *p;
  struct node *n;
  i32 i, ret = OK;
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  ...
  /* update DPB on skipped frames */
  if(IS_H264(vcenc_instance->codecFormat) && vcenc_instance->rateControl.skipGop)
    h264_dpb_update(c, vcenc_instance, pEncIn);
  ...
  for (n = c->picture.tail; n; n = n->next) {
    ...
    if (!p->reference) {
      if (p->poc == saveExtPoc && saveExtPoc != -1) p->reference = 1;
      if (!p->reference && !p->ref_cnt) p->poc = UNUSED_POC;
    }
  }
  return ret;
}
...
static void HEVCPicParameterSet(struct pps *p) {
  ...
  COMMENT(b, "pps_extension_present_flag")
  put_bit(b, p->pps_extension_present_flag, 1);

  if(p->pps_extension_present_flag) {
    COMMENT(b, "pps_range_extension_flag")
    put_bit(b, p->pps_range_extension_flag, 1);

    COMMENT(b, "pps_multilayer_extension_flag")
    put_bit(b, 0, 1);

    COMMENT(b, "pps_3d_extension_flag")
    put_bit(b, 0, 1);

    COMMENT(b, "pps_scc_extension_flag")
    put_bit(b, 0, 1);

    COMMENT(b, "pps_extension_4bits")
    put_bit(b, 0, 4);
  }

  if(p->pps_range_extension_flag) {
    if(p->transform_skip_enabled_flag) {
      COMMENT(b, "log2_max_transform_skip_block_size_minus_2")
      put_bit_ue(b, p->log2_max_transform_skip_block_size_minus_2);
    }

    COMMENT(b, "cross_component_prediction_enabled_flag")
    put_bit(b, 0, 1);

    COMMENT(b, "chroma_qp_offset_list_enabled_flag")
    put_bit(b, 0, 1);

    COMMENT(b, "log2_sao_offset_scale_luma")
    put_bit_ue(b, 0);

    COMMENT(b, "log2_sao_offset_scale_chroma")
    put_bit_ue(b, 0);
  }
...
void scaling_list(i32 foo) {
  foo = foo;		//cppcheck-suppress selfAssignment
  ASSERT(0);
}
...
void VCEncSpsSetVuiTimigInfo(struct sps *sps, u32 timeScale,
                             u32 numUnitsInTick) {
  if (timeScale) sps->vui_parameters_present_flag = ENCHW_YES;
...
void VCEncSpsSetVuiVideoInfo(struct sps *sps, u32 videoFullRange) {
  if (videoFullRange) sps->vui_parameters_present_flag = ENCHW_YES;

  sps->vui.vuiVideoFullRange =
      videoFullRange; /* used as video_signal_type_present_flag */
}
...
void VCEncSpsSetVuiAspectRatio(struct sps *sps, u32 sampleAspectRatioWidth,
  ...
  if (sampleAspectRatioWidth) sps->vui_parameters_present_flag = ENCHW_YES;

  sps->vui.sarWidth =
      sampleAspectRatioWidth; /* used as aspect_ratio_info_present_flag */
  sps->vui.sarHeight = sampleAspectRatioHeight;
}

void VCEncSpsSetVuiSignalType(struct sps *sps,
...
void UpdateVuiPresence(struct sps *sps) {
  if (sps->vui.nalHrdParametersPresentFlag == 0 && sps->vui.timeScale == 0 &&
      ...
      sps->vui.vuiVideoFullRange == 0 &&
      sps->vui.vuiColorDescripPresentFlag == 0 &&
...
const u32 H264LevelIdc[16] = {10, 99, 11, 12, 13, 20, 21, 22,
...
static void H264WriteVui(struct buffer *b, vui_t *vui, i32 maxNumRefFrames,
                         i32 maxNumReorderPics);
...
u32 H264GetLevelIndex(u32 levelIdc) {
  ...
  while (H264LevelIdc[i] != levelIdc) {
    i++;
    if (i >= MAX_LEVEL_INDEX) return INVALID_LEVEL;
  }
...
static void H264WriteVui(struct buffer *b, vui_t *vui, i32 maxNumRefFrames,
  ...
  if (vui->timeScale != 0) {
    ...
    if (0 == vui->field_seq_flag) DeltaTfiDivisor = 2;
  ...
  if (vui->nalHrdParametersPresentFlag == ENCHW_YES) {
...

    {
      ...
      while (4095 < (tmp >> (4 + i++)))
        ;
      cpb_size_scale = i;
      if (cpb_size_scale > 15) cpb_size_scale = 15;
      ...
      while (4095 < (tmp >> (6 + i++)))
        ;
      bit_rate_scale = i;
      if (bit_rate_scale > 15) bit_rate_scale = 15;
...
static void H264SeqParameterSet(struct container *c, struct sps *s,
  ...
  H264NalUnitHdr(b, 1, H264_SPS_NUT,
                 (s->streamMode == VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO);

  put_bit(b, s->general_profile_idc, 8);
  COMMENT(b, "profile_idc");

  /* constraint_set0 streams obey all baseline profile constraints. */
  if (s->general_profile_idc > SYNTAX_PROFILE_H264_BASELINE) s->constraintSet0 = ENCHW_NO;
  /* constraint_set1 streams obey all main profile constraints. */
  if (s->general_profile_idc > SYNTAX_PROFILE_H264_MAIN) s->constraintSet1 = ENCHW_NO;
  /* constraint_set2 streams obey all extended profile constraints. */
  if (s->general_profile_idc > SYNTAX_PROFILE_H264_EXTENDED) s->constraintSet2 = ENCHW_NO;
  if (s->general_profile_idc == SYNTAX_PROFILE_H264_CAVLC_444) {
    s->constraintSet0 = ENCHW_NO;
    s->constraintSet1 = ENCHW_NO;
    s->constraintSet2 = ENCHW_NO;
  }
  if (vcenc_instance->asic.regs.entropy_coding_mode_flag && s->constraintSet2 == ENCHW_YES)
    s->constraintSet2 = ENCHW_NO;
  ...
  if (s->general_profile_idc >= SYNTAX_PROFILE_H264_HIGH) {
    put_bit_ue(b, s->chroma_format_idc);
    COMMENT(b, "chroma_format_idc");

    /* TODO: if (chroma_format_idc == 3)... */
    ASSERT(s->chroma_format_idc <= VCENC_CHROMA_IDC_422);
  ...
  if (s->vui_parameters_present_flag == ENCHW_YES)
    H264WriteVui(b, &s->vui, s->max_dec_pic_buffering[0],
                 s->max_num_reorder_pics[0]);
...
static void H264PicParameterSet(struct pps *p) {
  ...
  /* Nal unit sytax */
  H264NalUnitHdr(b, 1, H264_PPS_NUT,
                 (p->streamMode == VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO);
...
void H264AccessUnitDelimiter(struct buffer *b, u32 byte_stream,
  ...
  H264NalUnitHdr(b, 0, H264_AUD_NUT,
                 (byte_stream == VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO);
