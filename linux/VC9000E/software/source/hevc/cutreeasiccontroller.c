...
------------------------------------------------------------------------------*/
#define MAKE_CMD(cur, last, middle, frame, frame_p0, frame_p1, cie, coe, qoe, \
                 aqoe, clear_first, clear_last, referenced)                   \
  (((u64)frame->cuDataIdx << 32) | ((u64)frame->sliceType << 42) |            \
   ((u64)(clear_first == 0) << 45) | ((u64)(clear_last == 0) << 46) |         \
   ((((u64)frame->outRoiMapDeltaQpIdx >> 4) & 0x3) << 47) |                   \
   ((u64)(referenced & 1) << 49) | ((u64)(cie & 0x01) << 28) |                \
   ((u64)(coe & 0x01) << 29) | ((u64)(qoe & 0x01) << 30) |                    \
...
void CuTreeAsicFrameStart(const void *ewl, regValues_s *val) {
  ...
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_WR_ID_E, 0);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_RD_ID_E, 0);

  /* clear all interrupt */
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_TIMEOUT, 1);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_BUS_ERROR_STATUS, 1);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_FRAME_RDY_STATUS, 1);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_IRQ, 1);

  /* encoder interrupt */
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_IRQ_DIS,
                          val->bVCMDEnable ? 0 : val->irqDisable);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_READ_ID,
                          val->asic_axi_readID);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_WRITE_ID,
                          val->asic_axi_writeID);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_CLOCK_GATE_ENCODER_E,
                          val->asic_clock_gating_encoder);

  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_MODE, val->codingType);

  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_MAX_BURST,
                          val->AXI_burst_max_length);
  EncAsicSetRegisterValue(
      val->regMirror, HWIF_ENC_CUTREE_ROI_MAP_QP_DELTA_MAP_SWAP,
      (~(ENCH2_ROIMAP_QPDELTA_SWAP_8 | (ENCH2_ROIMAP_QPDELTA_SWAP_16 << 1) |
         (ENCH2_ROIMAP_QPDELTA_SWAP_32 << 2) |
         (ENCH2_ROIMAP_QPDELTA_SWAP_64 << 3))) &
          0xf);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_WRITE_OUTSTANDING_NUM,
                          ENCH2_ASIC_AXI_WRITE_OUTSTANDING_NUM);
  EncAsicSetRegisterValue(val->regMirror, HWIF_ENC_CUTREE_AXI_READ_OUTSTANDING_NUM,
...
static int PrepareRoiBuffer(struct cuTreeCtr *m_param, struct Lowres *pFrame) {
  ptr_t busAddress = 0;
  if (pFrame->outRoiMapDeltaQpIdx != INVALID_INDEX) return 0;
  if (GetRoiMapBufferFromBufferPool(m_param, &busAddress) == NULL) return -1;
  pFrame->outRoiMapDeltaQpIdx = (busAddress - m_param->outRoiMapDeltaQp_Base) /
                                m_param->outRoiMapDeltaQp_frame_size;
  return 0;
}

static void collectGopCmds(struct cuTreeCtr *m_param, struct Lowres **frames,
                           i32 cur, i32 last, i32 depth, bool *p_clear_first,
                           bool *p_clear_last, bool *p_clear_middle, bool coe,
                           bool aq_output, int outGopSize) {
  ...
  if (depth == 0)
    m_param->maxHieDepth =
        ((frames[last]->gopSize == 8) && (frames[last]->aGopSize == 4))
            ? 3
  ...
  if (bframes > 1) {
    ...
    bool tmp;
    collectGopCmds(m_param, frames, middle, last, depth + 1, &clear_middle,
                   &clear_last, &tmp, coe, aq_output, outGopSize);
    collectGopCmds(m_param, frames, cur, middle, depth + 1, &clear_first,
                   &clear_middle, &tmp, coe, aq_output, outGopSize);
    if (clear_middle) cie = false;
  }

  if (bframes >= 1) {
    bool referenced = (bframes > 1);
    qoe = coe && (depth == 0);
    if (frames[middle]->p0 == (middle - cur) &&
        frames[middle]->p1 == (last - middle) &&
      ...
      m_param->commands[m_param->num_cmds++] = MAKE_CMD(
          cur, last, middle, frames[middle], frames[cur], frames[last], cie,
          true, qoe, (aq_output && !qoe), clear_first, clear_last, referenced);
      ...
      if ((m_param->aq_mode != AQ_NONE ||
        ...
        PrepareRoiBuffer(m_param, frames[middle]);
      m_param->commands[m_param->num_cmds++] = MAKE_CMD(
          middle, middle, middle, frames[middle], p_fake_frame, p_fake_frame,
          cie, true, false, aq_output, true, true, false);
    }
  ...
  *p_clear_middle = !cie;
  if (depth == 0) {
    ...
    if (frames[last]->p0 == (last - cur)) {
      ...
      m_param->commands[m_param->num_cmds++] = MAKE_CMD(
          cur, last, last, frames[last], frames[cur], invalid_frame, cie,
      ...
      m_param->commands[m_param->num_cmds++] =
          MAKE_CMD(last, last, last, frames[last], p_fake_frame, p_fake_frame,
                   cie, true, false, aqoe, true, true, false);
    }
    clear_last = false;
  }
...
void processGopConvert_4to8_asic(struct cuTreeCtr *m_param,
                                 struct Lowres **frames);
static void collectCmds(struct cuTreeCtr *m_param, struct Lowres **frames,
  ...
  int i = numframes;
  m_param->num_cmds = 0;
  bool bAgop_4to8 = (m_param->lastGopEnd > 8 && (frames[4]->gopEncOrder == 0) &&
                     (frames[4]->gopSize == 4) && (frames[4]->aGopSize == 8) &&
                     (frames[8]->gopEncOrder == 0) &&
                     (frames[8]->gopSize == 4) && (frames[8]->aGopSize == 8));
  int outGopSize =
      (m_param->lastGopEnd > 1 ? (bAgop_4to8 ? 8 : frames[1]->gopSize) : 1);
  bool aq_output = true;

  if (!(bFirst && bAgop_4to8)) processGopConvert_4to8_asic(m_param, frames);

  while (i > 0 && IS_CODING_TYPE_B(frames[i]->sliceType)) i--;
  ...
  while (i-- > 0) {
    ...
    clear_first = true;
    collectGopCmds(
        m_param, frames, curnonb, lastnonb, 0, &clear_first, &clear_last,
        &clear_middle, false,
        (aq_output && lastnonb <= outGopSize && !(bFirst && bAgop_4to8)),
        outGopSize);
    if (curnonb == outGopSize) {
      PrepareRoiBuffer(m_param, frames[curnonb]);
      m_param->commands[m_param->num_cmds++] =
          MAKE_CMD(curnonb, curnonb, curnonb, frames[curnonb], invalid_frame,
                   invalid_frame, (!clear_first), true, true, false, false,
                   false, false);
    }
    lastnonb = i = curnonb;
  ...
  if (bFirst) {
    ...
    m_param->commands[m_param->num_cmds++] =
        MAKE_CMD(0, 0, 0, frames[0], invalid_frame, invalid_frame,
  ...
  if (bFirst && bAgop_4to8) {
    ...
    collectGopCmds(m_param, frames, 0, 8, 0, &clear_first, &clear_last,
                   &clear_middle, false, aq_output, outGopSize);
  }
  if (outGopSize > 1) {
    PrepareRoiBuffer(m_param, frames[outGopSize / 2]);
    m_param->commands[m_param->num_cmds++] =
        MAKE_CMD(outGopSize / 2, outGopSize / 2, outGopSize / 2,
                 frames[outGopSize / 2], invalid_frame, invalid_frame,
                 (!clear_middle), true, true, false, false, false, false);
  }
  ASSERT(m_param->num_cmds > 0);
}

void EncMakeCmdbufDataCuTree(asicData_s *asic, regValues_s *val) {
  const void *ewl = asic->ewl;
  val->vcmd.vcmdBufSize = 0;
  ...
  //stall
  {
    VcmdbufCollectStallDataCuTree(ewl, &val->vcmd);
  }
  //read registers necessary into status cmdbuf.
  if (0) {
    VcmdbufCollectReadRegData(ewl, &val->vcmd, 0, 6);
    VcmdbufCollectReadRegData(ewl, &val->vcmd, 20, 132);
  } else {
    VcmdbufCollectReadRegData(ewl, &val->vcmd, 0, ASIC_SWREG_AMOUNT);
  }
  //stop encoder
  {
...
static u32 CutreeEstimateExecutingTime(u32 width, u32 height, u32 dsRatio)
{
  return width * dsRatio * height * dsRatio;
}
...
VCEncRet VCEncCuTreeStart(struct cuTreeCtr *m_param) {
  ...
  i32 roiAbsQpSupport =
      ((struct vcenc_instance *)enc)->asic.regs.asicCfg->roiAbsQpSupport;
  i32 roiMapVersion =
  ...
  int fpsFactor = (int)(CLIP_DURATION_FIX8(averageDuration) * 256 /
                        CLIP_DURATION_FIX8((m_param->fpsDenom << 8) /
                                           m_param->fpsNum));  // Q24.8

  /* set new frame encoding parameters */
  EncAsicSetRegisterValue(regs->regMirror, HWIF_CUTREE_TIMEOUT_OVERRIDE_E,
                          ENCH2_ASIC_TIMEOUT_OVERRIDE_ENABLE);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_CUTREE_TIMEOUT_CYCLES,
  ...
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_ROIMAPENABLE,
                          m_param->roiMapEnable);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_HWLOOKAHEADDEPTH,
                          m_param->lastGopEnd);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_INLOOPDSRATIO,
                          m_param->dsRatio - 1);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_BPASS1ADAPTIVEGOP,
                          m_param->bUpdateGop);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_CUINFORVERSION,
                          cuInforVersion);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_ROIMAPVERSION,
                          roiMapVersion);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_ROIABSQPSUPPORT,
                          roiAbsQpSupport);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_ROIDELTAQPVER,
                          roiQpDeltaVer);
  u32 cuTreeStrength =
      (((5 * (10000 - (int)(m_param->qCompress * 10000 + 0.5))) << 8) + 5000) /
      10000;  // Q24.8
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_STRENGTH,
                          cuTreeStrength);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_FPSFACTOR,
                          fpsFactor);
  EncAsicSetRegisterValue(
      regs->regMirror, HWIF_ENC_CUTREE_INQPDELTABLKSIZE,
  ...
  EncAsicSetRegisterValue(regs->regMirror,
                          HWIF_ENC_CUTREE_FRAMECOSTREFINEENABLE,
                          m_param->imFrameCostRefineEn);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_AQMODE,
                          m_param->aq_mode);
  ...
  if (m_param->aq_mode) {
    ...
    EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_AQMODESTRENGTH,
                            aqStrength);
  ...
  if(m_param->enableLeadingPictures)
    while(frames - m_param->lookaheadFrames < m_param->nLookaheadFrames && IS_CODING_TYPE_B(frames[0]->sliceType))
      ++frames; // skip leading pictures before first IDR
  int startIdx = frames - m_param->lookaheadFrames;
  collectCmds(m_param, m_param->lookaheadFrames + startIdx, m_param->lastGopEnd - 1 - startIdx,
              frames[0]->sliceType == VCENC_FRAME_TYPE_IDR);
  SET_CUTREE_CMD(0);
  SET_CUTREE_CMD(1);
  SET_CUTREE_CMD(2);
  SET_CUTREE_CMD(3);
  ...
  SET_CUTREE_CMD(54);
  SET_CUTREE_CMD(55);
  EncAsicSetRegisterValue(regs->regMirror, HWIF_ENC_CUTREE_NUM_CMDS,
                          m_param->num_cmds);

#ifdef SUPPORT_AXIFE
  VCEncCfgAxiFe(enc, &m_param->asic, 1, 0);
#endif
  ...
  if (!regs->bVCMDEnable) {
    ...
    if ((EWLReserveHw(m_param->asic.ewl, &reserve_core_info, NULL) ==
         EWL_ERROR) ||
...
#ifdef SUPPORT_AXIFE
#ifdef SEPARATE_AXIFE
    if (m_param->asic.axife_data->mode) {
      VCEncAxiFeEnable(m_param->asic.axife_data);
    }
#endif
#endif
    ...
    u32 executing_time = CutreeEstimateExecutingTime(m_param->width,
            m_param->height, m_param->dsRatio);
    EncSetReseveInfo(m_param->asic.ewl, executing_time, EWL_CLIENT_TYPE_CUTREE);
    EncReseveCmdbuf(m_param->asic.ewl, &m_param->asic.regs.vcmd);
    EncMakeCmdbufDataCuTree(&m_param->asic, &m_param->asic.regs);

    EncLinkRunCmdbuf(m_param->asic.ewl, &m_param->asic.regs.vcmd);
  }
  return VCENC_OK;
}
...
#ifdef SUPPORT_AXIFE
    if (asic->axife_data->mode) {
      VCEncAxiFeDisable(asic->ewl, NULL);
    }
#endif
    } else {
      ...
      switch (status) {
        ...
        case ASIC_STATUS_FRAME_READY:
          if (m_param->lastGopEnd > 8 &&
              ((m_param->lookaheadFrames[4]->gopEncOrder == 0) &&
            ...
            m_param->rem_frames =
                m_param->lastGopEnd -
                (m_param->lastGopEnd > 1 ? m_param->lookaheadFrames[1]->gopSize
                                         : 0);
...
static void markBRef(struct cuTreeCtr *m_param, struct Lowres **frames, i32 cur,
                     i32 last, i32 depth) {
  i32 bframes = last - cur - 1;
  i32 i = last - 1;
  if (cur < 0) return;
...
static void write_asic_gop_cutree(struct cuTreeCtr *m_param,
                                  struct Lowres **frame, i32 size, i32 base);
void processGopConvert_4to8_asic(struct cuTreeCtr *m_param,
                                 struct Lowres **frames) {
  int i;
  if (m_param->lastGopEnd <= 8) return;

  /* convert 2xgop4 to gop8 */
  if ((frames[4]->gopEncOrder == 0) && (frames[4]->gopSize == 4) &&
      (frames[4]->aGopSize == 8) && (frames[8]->gopEncOrder == 0) &&
      (frames[8]->gopSize == 4) && (frames[8]->aGopSize == 8)) {
    for (i = 1; i <= 8; i++) frames[i]->gopSize = 8;
    ...
    for (i = 1; i <= 8; i++) frames[i]->aGopSize = 0;
  }
}
bool processGopConvert_8to4_asic(struct cuTreeCtr *m_param,
                                 struct Lowres **frames) {
  i32 i;
  if (m_param->lastGopEnd <= 8) return false;

  /* convert gop8 to 2xgop4 */
  if ((frames[8]->gopEncOrder == 0) && (frames[8]->gopSize == 8) &&
      (frames[8]->aGopSize == 4)) {
    for (i = 1; i <= 8; i++) frames[i]->gopSize = 4;
    ...
    for (i = 0; i < 4; i++) remove_one_frame(m_param);
    m_param->out_cnt += 4;
    m_param->pop_cnt += 4;
...
static void write_asic_cutree_file(struct cuTreeCtr *m_param,
                                   struct Lowres *frame, int i) {
  struct vcenc_instance *enc = (struct vcenc_instance *)m_param->pEncInst;
  ...
  if (frame->outRoiMapDeltaQpIdx != INVALID_INDEX) {
    ...
    segBusAddress =
        (ptr_t)m_param->segmentCountVirtualBase +
        m_param->outRoiMapDeltaQp_frame_size * frame->outRoiMapDeltaQpIdx +
        m_param->outRoiMapSegmentCountOffset;
  ...
  out->pRoiMapDeltaQpAddr =
      out->encIn
          .roiMapDeltaQpAddr;  //save orignal roiMapDeltaQpAddr to put it back to buffer pool
  ...
  out->frame.cost = (int)(m_param->lookaheadFrames[i]->cost / 256);
  out->frame.gopSize = frame->gopSize;

  for (i = 0; i < 4; i++) {

#ifdef RCP_FIXED_POINT_OPT
    out->frame.costGop[i] = m_param->costGopInt[i];
    out->frame.costAvg[i] = m_param->costAvgInt[i] ;
#else
  ...
  printf("    Lowres gmv (%d,%d),(%d,%d),(%d,%d)\n", out->encIn.gmv[0][0],
         out->encIn.gmv[0][1], out->encIn.gmv[1][0], out->encIn.gmv[1][1],
         out->frame.gmv[0][2], out->frame.gmv[1][2]);
#endif
...
static void write_asic_gop_cutree(struct cuTreeCtr *m_param,
  ...
  for (i = 0; i < size; i++) {
    for (j = 0; j < size; j++) {
      if (frame[j]->gopEncOrder == i) break;
    }
...
VCEncRet VCEncCuTreeInit(struct cuTreeCtr *m_param) {
  ...
  param.enc_dev = EWLGetDevName(m_param->ctx);
  param.mem_dev = EWLGetMemDevName(m_param->ctx);
  param.useVcmd = EWLGetVCMDSupport(m_param->ctx);
  /* Init EWL */
  if ((ewl = EWLInit(&param)) == NULL) {
    return VCENC_EWL_ERROR;
  }
  m_param->ctx = (void *)ewl;

  m_param->asic.regs.bVCMDAvailable = EWLGetVCMDSupport(ewl) ? true : false;
  m_param->asic.regs.bVCMDEnable = EWLGetVCMDMode(ewl) ? true : false;

  if (!(m_param->asic.axife_data = (struct VCAxiFeData *)EWLcalloc(
            1, sizeof(struct VCAxiFeData )))) {
  ...
  //   cuData
  m_param->cuData_Base = enc->asic.cuInfoMem[0].busAddress +
                         enc->asic.cuInfoTableSize + enc->asic.aqInfoSize;
  m_param->cuData_frame_size =
      enc->asic.cuInfoMem[1].busAddress - enc->asic.cuInfoMem[0].busAddress;
  m_param->aqDataBase =
      enc->asic.cuInfoMem[0].busAddress + enc->asic.cuInfoTableSize;
  ...
  if (IS_VP9(m_param->codecFormat)) {
    ...
    m_param->segmentCountVirtualBase =
        m_param->roiMapDeltaQpMemFactory[0].virtualAddress;
  }
  m_param->inRoiMapDeltaBin_Base = 0;
  m_param->inRoiMapDeltaBin_frame_size = 0;
  if (m_param->asic.regs.bVCMDAvailable) {
      m_param->asic.regs.vcmd.core_mask =
          EncAsicGetCoreMask(param.clientType, m_param->ctx, m_param->priority);
    if (m_param->core_mask) {
      m_param->asic.regs.vcmd.core_mask &= m_param->core_mask;
    }
      m_param->asic.regs.vcmd.priority = m_param->priority;
  }
  {
    ...
    i32 num_buf =
        CUTREE_BUFFER_CNT(m_param->lookaheadDepth, m_param->gopSize) + 1;
    ...
    SET_MEM_USAGE(m_param->propagateCostMemFactory[0].mem_type, EWL_MEM_USAGE_TMP_COST,
    ...
    if (EWLMallocLinear(m_param->asic.ewl, block_size * num_buf, 0,
      ...
      memset(m_param->propagateCostMemFactory[0].virtualAddress, 0,
             block_size * num_buf);
      EWLSyncMemData(&m_param->propagateCostMemFactory[0], 0,
                     block_size * num_buf, HOST_TO_DEVICE);
      ...
      for (i = 0; i < num_buf; i++) {
        ...
        m_param->propagateCostMemFactory[i].size =
            (i < num_buf - 1 ? block_size
                             : total_size - (num_buf - 1) * block_size);
    ...
    m_param->propagateCost_frame_size =
        m_param->propagateCostMemFactory[1].busAddress -
        m_param->propagateCostMemFactory[0].busAddress;
  }
  return VCENC_OK;
...
VCEncRet VCEncCuTreeProcessOneFrame(struct cuTreeCtr *m_param) {
  ...
  i = 0;
  while (i < m_param->nLookaheadFrames && frames[i]->sliceType == VCENC_FRAME_TYPE_B)
    ++i;
  int startIdx = i;
  ...
  if (out_frame->sliceType == VCENC_FRAME_TYPE_IDR && !m_param->enableLeadingPictures) {
    statisAheadData(m_param, m_param->lookaheadFrames, m_param->lastGopEnd - 1,
                    HANTRO_TRUE);
    write_asic_gop_cutree(m_param, &out_frame, 1, 0);
    m_param->out_cnt++;
  }
  /* output first IDR with leading pictures */
  if (m_param->enableLeadingPictures && m_param->lookaheadFrames[0]->poc < 0) {
    statisAheadData(m_param, m_param->lookaheadFrames + m_param->lookaheadFrames[0]->gopSize - 1, m_param->lastGopEnd - (m_param->lookaheadFrames[0]->gopSize),
                    HANTRO_TRUE);
    write_asic_gop_cutree(m_param, m_param->lookaheadFrames, m_param->lookaheadFrames[0]->gopSize, 0);
    m_param->out_cnt += m_param->lookaheadFrames[0]->gopSize;
    for (i = 0; i < m_param->lookaheadFrames[0]->gopSize - 1; i++)
      remove_one_frame(m_param);
    m_param->pop_cnt += m_param->lookaheadFrames[0]->gopSize - 1;
  }
  if (processGopConvert_8to4_asic(m_param, frames)) idx += 4;
  if (m_param->lastGopEnd > 1) {
    int gopSize = (m_param->lookaheadFrames[1]->poc == 0
                       ? 1
                       : m_param->lookaheadFrames[1]->gopSize);
    /* subsequent IDR with leading pictures & previous P forms a normal gop */
    if (m_param->lookaheadFrames[1]->sliceType != VCENC_FRAME_TYPE_IDR || m_param->enableLeadingPictures) {
      statisAheadData(m_param, m_param->lookaheadFrames,
                      m_param->lastGopEnd - 1, HANTRO_FALSE);
      write_asic_gop_cutree(m_param, m_param->lookaheadFrames + 1, gopSize, 1);
      m_param->out_cnt += gopSize;
    }
    for (i = 0; i < gopSize; i++) remove_one_frame(m_param);
...
#ifdef TEST_DATA
  trace_sw_cutree_ctrl_flow(size, m_param->out_cnt, m_param->pop_cnt,
                            m_param->qpOutIdx);
#endif
  return VCENC_OK;
}

VCEncRet VCEncCuTreeRelease(struct cuTreeCtr *m_param) {
  const void *ewl;

  ASSERT(m_param);
  ewl = m_param->asic.ewl;
  if (m_param->asic.axife_data) EWLfree(m_param->asic.axife_data);
  m_param->asic.axife_data = NULL;
  if (ewl == NULL) return VCENC_OK;

  EWLFreeLinear(m_param->asic.ewl, &m_param->propagateCostMemFactory[0]);

  if (ewl) {
    EncAsicMemFree_V2(&m_param->asic);
    (void)EWLRelease(ewl);
  }
  m_param->asic.ewl = NULL;
  return VCENC_OK;
