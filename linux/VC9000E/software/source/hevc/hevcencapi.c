...
extern u32 hw_transform_skip_enabled_flag;
extern u32 hw_log2_max_transform_skip_block_size_minus_2;
#endif
#endif
...
extern u32 hwNrSigmaCur;
extern u32 hwNrFrameSigmaCalced;
#endif
#endif
typedef enum {
...
#define VCENC_PROFILE_MAX (VCENC_H264_PROFILE_NUM+1)

const u64 VCEncMaxSBPSHevc[HEVC_LEVEL_NUM] = {
    552960,     3686400,    7372800,   16588800,  33177600,
    66846720,   133693440,  267386880, 534773760, 1069547520,
    1069547520, 2139095040, 4278190080};
// Table A.8 - General tier and level limits
  // Max luma picture size MaxLumaPs
const u32 VCEncMaxPicSizeHevc[HEVC_LEVEL_NUM] = {
    36864,   122880,  245760,  552960,   983040,   2228224, 2228224,
    8912896, 8912896, 8912896, 35651584, 35651584, 35651584};
  // Max CPB size Main tier
const u32 VCEncMaxCPBSMainTierHevc[HEVC_LEVEL_NUM] = {
    350,    1500,   3000,   6000,   10000,   12000,   20000,
    25000, 40000, 60000, 60000, 120000, 240000};
  // Max CPB size High tier
const u32 VCEncMaxCPBSHighTierHevc[HEVC_LEVEL_NUM] = {
    350,    1500,   3000,   6000,   10000,  30000, 50000,
    100000, 160000, 240000, 240000, 480000, 800000};
// Table A.10 - Specification of CpbVclFactor, CpbNalFactor
const u32 VCEncCpbVclFactorHevc[VCENC_PROFILE_MAX] = {
    1000,   1000,   1000,   667,   833,   1000,   1667,
    2500,   1000,   2500,   1000,   1000,   1000, 1000};
const u32 VCEncCpbNalFactorHevc[VCENC_PROFILE_MAX] = {
    1100,   1100,   1100,   733,   917,   1100,   1833,
    2750,   1100,   2750,   1100,   1100,   1100, 1100};
// Table A.9 - Tier and level limits for the video profiles
  // MaxBRSize = BrVclFactor * MaxBR (BrVclFactor = CpbVclFactor * HbrFactor)
  // Max bit rate Main tier
const u32 VCEncMaxBRMainHevc[HEVC_LEVEL_NUM] = {
    128,   1500,  3000,  6000,  10000,  12000, 20000,
    25000, 40000, 60000, 60000, 120000, 240000};
  // Max bit rate Hign tier
const u32 VCEncMaxBRHighTierHevc[HEVC_LEVEL_NUM] = {
    128,    1500,   3000,   6000,   10000,  30000, 50000,
    100000, 160000, 240000, 240000, 480000, 800000};

// Table A-1 -- Level limits on T-REC-H.264-202108-I
  // Max macroblock processing rate (MaxMBPS) (MB/s)
const u64 VCEncMaxSBPSH264[H264_LEVEL_NUM] = {
    1485 * 256,    1485 * 256,    3000 * 256,    6000 * 256,
    11880 * 256,   11880 * 256,   19800 * 256,   20250 * 256,
    40500 * 256,   108000 * 256,  216000 * 256,  245760 * 256,
    245760 * 256,  522240 * 256,  589824 * 256,  983040 * 256,
    2073600 * 256, 4177920 * 256, 8355840 * 256, 16711680ULL * 256};
  // Max Frame size (MaxFS) (MBs)
const u32 VCEncMaxFSH264[H264_LEVEL_NUM] = {
    99 * 256,    99 * 256,    396 * 256,    396 * 256,    396 * 256,
    396 * 256,   792 * 256,   1620 * 256,   1620 * 256,   3600 * 256,
    5120 * 256,  8192 * 256,  8192 * 256,   8704 * 256,   22080 * 256,
    36864 * 256, 36864 * 256, 139264 * 256, 139264 * 256, 139264 * 256};
  // Max CPB size (MaxCPB) (1000 bits/s, 1200 bits/s, cpbBrVclFactor bits/s, or cpbBrNalFactor bits/s)
const u32 VCEncMaxCPBSH264[H264_LEVEL_NUM] = {
    175,    350,    500,    1000,   2000,   2000,  4000,
    4000,   10000,  14000,  20000,  25000,  62500, 62500,
    135000, 240000, 240000, 240000, 480000, 800000};
  // Max video bit rate (MaxBR) (1000 bits/s, 1200 bits/s, cpbBrVclFactor bits/s, or cpbBrNalFactor bits/s)
const u32 VCEncMaxBRH264[H264_LEVEL_NUM] = {
    64,     128,    192,    384,    768,    2000,  4000,
    4000,   10000,  14000,  20000,  20000,  50000, 50000,
    135000, 240000, 240000, 240000, 480000, 800000};

// A.3. Levels on AV1 Bitstream & Decoding Process Specification
  // MaxDecodeRate
const u64 VCEncMaxSBPSAV1[AV1_LEVEL_NUM] = {
    5529600,    10454400,   0,          0,          24969600,   39938400,
    0,          0,          77856768,   155713536,  0,          0,
    273715200,  547430400,  1094860800, 1176502272, 1176502272, 2189721600,
    4379443200, 4706009088, 0,          0,          0,          0};
  // MaxPicSize
const u32 VCEncMaxPicSizeAV1[AV1_LEVEL_NUM] = {
  ...
  // MaxBitrate = MainMbps(or HighMbps)*1000000
  // MaxBufferSize = MaxBitrate * 1 second
...
const u32 VCEncMaxCPBSAV1[AV1_LEVEL_NUM] = {
    1500000,   3000000,   0,        0,        6000000,  10000000,
    0,         0,         12000000, 20000000, 0,        0,
    30000000,  40000000,  60000000, 60000000, 60000000, 100000000,
    160000000, 160000000, 0,        0,        0,        0};
  // HighMbps(Bits/sec)
const u32 VCEncMaxCPBSHighTierAV1[AV1_LEVEL_NUM] = {
    0,         0,         0,         0,         0,         0,
    0,         0,         30000000,  50000000,  0,         0,
    100000000, 160000000, 240000000, 240000000, 240000000, 480000000,
    800000000, 800000000, 0,         0,         0,         0};

// Annex A Levels on VP9 Bitstream & Decoding Process Specification - v0.7
  // Max Luma Sample Rate (samples/sec)
const u64 VCEncMaxSBPSVP9[VP9_LEVEL_NUM] = {
    829440,     2764800,    4608000,    9216000,   20736000,
    36864000,   83558400,   160432128,  311951360, 588251136,
    1176502272, 1176502272, 2353004544, 4706009088};
  // Max CPB Size for Visual Layer** (bits)
const u32 VCEncMaxCPBSVP9[VP9_LEVEL_NUM] = {
    400000,   1000000,  1500000,  2800000, 6000000, 10000000, 16000000,
    18000000, 36000000, 46000000, 0,       0,       0,        0};
  // Max Bitrate (bits/s)
const u32 VCEncMaxBRVP9[VP9_LEVEL_NUM] = {
    200000,   800000,   1800000,   3600000,   7200000,   12000000,  18000000,
    30000000, 60000000, 120000000, 180000000, 180000000, 240000000, 480000000};
  // Max Luma Picture Size (samples)
const u32 VCEncMaxPicSizeVP9[VP9_LEVEL_NUM] = {
    36864,   73728,   122880,  245760,  552960,   983040,   2228224,
    2228224, 8912896, 8912896, 8912896, 35651584, 35651584, 35651584};

static const u32 VCEncIntraPenalty[52] = {
    24,   24,   24,   26,   27,   30,   32,    35,   39,   43,   48,
    53,   58,   64,   71,   78,   85,   93,    102,  111,  121,  131,
    142,  154,  167,  180,  195,  211,  229,   248,  271,  296,  326,
    361,  404,  457,  523,  607,  714,  852,   1034, 1272, 1588, 2008,
    2568, 3318, 4323, 5672, 7486, 9928, 13216, 17648};  //max*3=16bit
...
static const float sqrtPow2QpDiv3[] = {
   ...
   */
    0.250000000,   0.280615512,   0.314980262,   0.353553391,   0.396850263,
    0.445449359,   0.500000000,   0.561231024,   0.629960525,   0.707106781,
    0.793700526,   0.890898718,   1.000000000,   1.122462048,   1.259921050,
    1.414213562,   1.587401052,   1.781797436,   2.000000000,   2.244924097,
    2.519842100,   2.828427125,   3.174802104,   3.563594873,   4.000000000,
    4.489848193,   5.039684200,   5.656854249,   6.349604208,   7.127189745,
    8.000000000,   8.979696386,   10.079368399,  11.313708499,  12.699208416,
    14.254379490,  16.000000000,  17.959392773,  20.158736798,  22.627416998,
    25.398416831,  28.508758980,  32.000000000,  35.918785546,  40.317473597,
    45.254833996,  50.796833663,  57.017517961,  64.000000000,  71.837571092,
    80.634947193,  90.509667992,  101.593667326, 114.035035922, 128.000000000,
    143.675142184, 161.269894387, 181.019335984, 203.187334652, 228.070071844,
...
static const float sqrtQpDiv6[] = {
   ...
   */
    ...
    1.414213562, 1.414213562, 1.414213562, 1.414213562, 1.414213562,
    1.414213562, 1.414213562, 1.414213562, 1.414213562, 1.414213562,
    1.471960144, 1.527525232, 1.581138830, 1.632993162, 1.683250823,
    1.732050808, 1.779513042, 1.825741858, 1.870828693, 1.914854216,
    1.957890021, 2.000000000, 2.000000000, 2.000000000, 2.000000000,
...
#ifdef SYSTEM_BUILD
#ifdef TEST_DATA
extern bool multipass_enabled;
#endif
#endif

#ifdef INTERNAL_TEST
static int last_skip_poc = 0;
static int last_gop_idx = 0;
#endif
...
#define CHECK_API_I(fmt, ...) \
  VCEncCheckMsg(NULL, VCENC_LOG_INFO, VCENC_LOG_CHECK_FEATURE, fmt, ##__VA_ARGS__)
/*------------------------------------------------------------------------------
    4. Local function prototypes
------------------------------------------------------------------------------*/
static i32 out_of_memory(struct vcenc_buffer *n, u32 size);
static i32 get_buffer(struct buffer *, struct vcenc_buffer *, i32 size,
                      i32 reset);
static i32 init_buffer(struct vcenc_buffer *, struct vcenc_instance *);
static i32 set_parameter(struct vcenc_instance *vcenc_instance,
                         const VCEncIn *pEncIn, struct vps *v, struct sps *s,
                         struct pps *p);
static bool_e VCEncCheckCfg(const VCEncConfig *enc_cfg,
                            const EWLHwConfig_t *asic_cfg, void *ctx);
static bool_e SetParameter(struct vcenc_instance *inst,
                           const VCEncConfig *pEncCfg);
static i32 vcenc_set_ref_pic_set(struct vcenc_instance *vcenc_instance);
static i32 vcenc_ref_pic_sets(struct vcenc_instance *vcenc_instance,
                              const VCEncIn *pEncIn);
static i32 VCEncGetAllowedWidth(i32 width, VCEncPictureType inputType);
static u32 VCEncGetMaxWidth(const EWLHwConfig_t *asic_cfg,
                            VCEncVideoCodecFormat codec_format);

#ifdef FIX_POINT_LAMBDA
static void IntraLamdaQpFixPoint(int qp, u32 *lamda_sad, enum slice_type type,
                                 int poc, u32 qpFactorSad, bool depth0);
static void InterLamdaQpFixPoint(int qp, unsigned int *lamda_sse,
                                 unsigned int *lamda_sad, enum slice_type type,
                                 u32 qpFactorSad, u32 qpFactorSse, bool depth0,
                                 u32 asicId);
#else
static void IntraLamdaQp(int qp, u32 *lamda_sad, enum slice_type type, int poc,
                         float dQPFactor, bool depth0);
static void InterLamdaQp(int qp, unsigned int *lamda_sse,
                         unsigned int *lamda_sad, enum slice_type type,
                         float dQPFactor, bool depth0, u32 asicId);
#endif
static void FillIntraFactor(regValues_s *regs);
static void LamdaTableQp(regValues_s *regs, int qp, enum slice_type type,
                         int poc, double dQPFactor, bool depth0, u32 ctbRc);
static void VCEncSetQuantParameters(struct vcenc_instance *vcenc_instance,
                                    struct sw_picture *pic,
                                    const VCEncIn *pEncIn, double qp_factor,
                                    bool is_depth0);
...
static void VCEncHEVCDnfPrepare(struct vcenc_instance *vcenc_instance, VCEncPictureCodingType codingType);
static void VCEncHEVCDnfUpdate(struct vcenc_instance *vcenc_instance, struct sw_picture *pic);
...
static void VCEncParameterSetHandle(struct vcenc_instance *vcenc_instance,
                                    VCEncInst inst, const VCEncIn *pEncIn,
                                    VCEncOut *pEncOut, struct container *c);

static i32 VCEncWriteFillerDataForCBR(struct vcenc_instance *vcenc, i32 stat);
static void VCEncAddFillerNALU(struct vcenc_instance *vcenc, i32 cnt,
                               true_e has_startcode);
...
static void VCEncCollectEncodeStats(struct vcenc_instance *vcenc_instance,
                                    VCEncIn *pEncIn, VCEncOut *pEncOut,
                                    VCEncPictureCodingType codingType,
                                    VCEncStatisticOut *enc_statistic);
static VCEncRet VCEncResetEncodeStatus(struct vcenc_instance *vcenc_instance,
                                       struct container *c, VCEncIn *pEncIn,
                                       struct sw_picture *pic,
                                       VCEncOut *pEncOut,
                                       const VCEncStatisticOut *enc_statistic);
static void VCEncSetReEncodeParameters(struct vcenc_instance *vcenc_instance,
                                     VCEncIn *pEncIn, VCEncOut *pEncOut,
                                     VCEncSliceReady *slice_callback,
                                     NewEncodeParams *new_params,
                                     regValues_s *regs_for2nd_encode);

#ifdef MULTI_FRAME_SUPPORT
static VCEncRet vcencBatchGetCodingCtrl(struct vcenc_instance *pEncInst,
                            VCEncCodingCtrl *pCodeParams);
static true_e vcencBatchCheckCfg(const VCEncConfig *enc_cfg,
                                 const EWLHwConfig_t *asic_cfg,
                                 const void *ctx);
static VCEncRet vcencBatchRunCmdbuf(struct vcenc_instance *vcenc_instance,
                    asicData_s *asic, VcmdDes_t *vcmd,
                    u32 client_type);
...
VCEncBuild VCEncGetBuild(u32 client_type) {
  ...
  //As there is no ewl ctx, hwBuild is read by application now.
...
VCEncRet VCEncInit(const VCEncConfig *config, VCEncInst *instAddr, void *ctx) {
  ...
  u32 batchEnable = config->enc_mode.batch_flag;
  u32 mcEnable = (config->parallelCoreNum > 1);
  ...
  u32 enableTMVP = config->enableTMVP;
  u32 bSendSPSonFPSAdjust = config->enc_mode.bSendSPSonFPSAdjust;
  u32 bInsertIDRonFPSAdjust = config->enc_mode.bInsertIDRonFPSAdjust;
  ...
  APITRACEPARAM(" %s : %d\n", "frameInfo", config->enableFrameInfo);
  APITRACEPARAM(" %s : %d\n", "lumaInfo", config->enablelumaInfo);
...
#error Right bit-shifting (>>) does not preserve the sign
  ...
  for (tileId = 0; tileId < config->num_tile_columns; tileId++) {
    if (config->tiles_enabled_flag == 1 &&
        config->tile_width[tileId] * 64 < 256) {
      APITRACEERR(
          "VCEncInit: Tile column width needs to be greater than or equal to "
          "256\n");
  ...
  for (tileId = 0; tileId < config->num_tile_rows; tileId++) {
    if (config->tiles_enabled_flag == 1 &&
        config->tile_height[tileId] * 64 < 64) {
      APITRACEERR(
          "VCEncInit: Tile row height needs to be greater than or equal to 64 "
          "\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  if (config->tiles_enabled_flag == 1 && config->num_tile_columns > 1 &&
      cb_try_new_params) {
    APITRACE("VCEncInit: Disable re-encode for multi-tile\n");
    cb_try_new_params = NULL;
  }

  /** multi-core should enable only when resoltuion is big enough. */
  if (mcEnable && config->width * config->height < 256 * 256) {
...
#ifdef MULTI_FRAME_SUPPORT
  if (batchEnable) {
    /* batch mode don't allow multi-core */
    if (mcEnable) {
      APITRACE("VCEncInit: Use single core for small resolution\n");
      mcEnable = 0;
    }
    if (parallelCoreNum < MIN_AGGREGATED_FRAMES ||
        parallelCoreNum > MAX_AGGREGATED_FRAMES) {
      APITRACEERR("VCEncInit: Wrong Parallel Number for batch mode\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
#endif /* MULTI_FRAME_SUPPORT */
  ...
  if (parallelCoreNum > 1 && cb_try_new_params) {
    APITRACEERR(
    "VCEncInit: WARNING Re-Encode is not supported when parallelCoreNum > 1,"
    "expect when process buffer overflow issue with a larger output buffer\n");
  }

  if (bSendSPSonFPSAdjust && !bInsertIDRonFPSAdjust) {
    APITRACE("VCEncInit: WARNING SPS can only updated before IDR\n");
    bInsertIDRonFPSAdjust = 1;
  }
  ...
  param.enc_dev = config->enc_dev;
  param.mem_dev = config->mem_dev;
  param.useVcmd = config->useVcmd;
  ...
  *instAddr = (VCEncInst)instance;
  ...
  for (coreId = 0; coreId < MAX_CORE_NUM; coreId++) {
    if (config->num_tile_columns > 1) {
      vcenc_inst->EncIn[coreId].tileExtra = (VCEncInTileExtra *)calloc(
          config->num_tile_columns - 1, sizeof(VCEncInTileExtra));
  ...
  vcenc_inst->asic.regs.bVCMDAvailable = EWLGetVCMDSupport(ctx) ? true : false;
  vcenc_inst->asic.regs.bVCMDEnable = EWLGetVCMDMode(ewl) ? true : false;

  if (cb_try_new_params) {
    if (!(vcenc_inst->regs_bak = (regValues_s *)EWLcalloc(
              1, parallelCoreNum * sizeof(regValues_s)))) {
      ret = VCENC_MEMORY_ERROR;
      goto error;
    }
  }

  if (!(vcenc_inst->asic.dec400_data = (VCDec400data *)EWLcalloc(
            1, sizeof(VCDec400data)))) {
    ret = VCENC_MEMORY_ERROR;
    goto error;
  }

#ifdef SUPPORT_AXIFE
  if (!(vcenc_inst->asic.axife_data =
  ...
  if (!(vcenc_inst->asic.ufbc = (EncUfbc *)EWLcalloc(
            1, sizeof(EncUfbc)))) {
    ret = VCENC_MEMORY_ERROR;
    goto error;
  }
...
#ifdef INTERNAL_TEST
  vcenc_inst->testId = config->testId;
#endif
  ...
  if (vcenc_inst->asic.regs.bVCMDAvailable) {
      vcenc_inst->asic.regs.vcmd.core_mask =
          EncAsicGetCoreMask(param.clientType, ctx, config->priority);
    if (config->core_mask) {
      vcenc_inst->asic.regs.vcmd.core_mask &= config->core_mask;
    }
    vcenc_inst->asic.regs.vcmd.priority = config->priority;
  }
  ...
  if ((hw_id != HW_ID_PRODUCT_H2) && (hw_id != HW_ID_PRODUCT_VC8000E) &&
      ...
      (hw_id != HW_ID_PRODUCT_VC9800E)) {
    if (instance != NULL) EWLfree(instance);
  ...
  EncUfbcInit(vcenc_inst->asic.ufbc, ewl, asic_cfg->ufbcSupport);
  ...
  */
  ASSERT((config->exp_of_input_alignment == 10) &&
         (config->exp_of_ref_alignment == 10) &&
  ...
  vcenc_inst->asic.secure_mode = config->secure_mode;
  (void)EncAsicControllerInit(&vcenc_inst->asic, ctx, param.clientType);

  vcenc_inst->aif.enable = config->aifEnable;

  u32 lookaheadDepth = config->lookaheadDepth;
  u32 pass = config->pass;
  if (lookaheadDepth > 0 &&
      (asic_cfg->cuInforVersion < 2 || !asic_cfg->bMultiPassSupport)) {
    // lookahead needs bFrame support & cuInfo version 2 & HW multi-pass support
    lookaheadDepth = 0;
    pass = 0;
  }

  vcenc_inst->ori_width = vcenc_inst->width = config->width;
  vcenc_inst->ori_height = vcenc_inst->height =
      config->height / (vcenc_inst->interlaced + 1);
  ...
  vcenc_inst->num_tile_rows = vcenc_inst->asic.regs.num_tile_rows =
  ...
  vcenc_inst->asic.regs.lowlatGatingDisable = 1;
  vcenc_inst->asic.regs.lowlatGatingType = 0;
  vcenc_inst->asic.regs.lowlatGatingCyc = IS_H264(config->codecFormat) ? 7 : 31;
  ...
  allocCfg.writeReconToDDR = config->writeReconToDDR;
  ...
  allocCfg.cuinfoAlignment = config->AXIAlignment == -1
                                 ? asic_cfg->axi_burst_align_wr_cuinfo
                                 : (config->AXIAlignment >> 32) & 0x000f;
  ...
  allocCfg.frameInfo = config->enableFrameInfo;
  allocCfg.enablelumaInfo = config->enablelumaInfo;
  ...
  allocCfg.numCuInfoBuf = vcenc_inst->numCuInfoBuf =
      (pass == 1 && asic_cfg->bMultiPassSupport ? lookAheadBufCnt : 3) +
      parallelCoreNum - 1;
  allocCfg.numCtbBitsBuf = parallelCoreNum;
  allocCfg.parallelCoreNum = parallelCoreNum;
  allocCfg.pass = pass;
  ...
  if (config->P010RefEnable) {
    if (config->bitDepthLuma > 8) allocCfg.bitDepthLuma = 16;
  ...
  vcenc_inst->ufbcParam.mode = config->ufbcParam.mode;
  memcpy(&vcenc_inst->ufbcParam.param, &config->ufbcParam.param, sizeof(config->ufbcParam.param));
  ...
  vcenc_inst->frameInfo = config->enableFrameInfo;
  vcenc_inst->enablelumaInfo = config->enablelumaInfo;

  // TODO, internal memory size alloc according to codedChromaIdc
  // VCENC_CHROMA_IDC_422: HW TODO
  allocCfg.codedChromaIdc =
      VCEncGetChromaIdc(config->codedChromaIdc, vcenc_inst);

  vcenc_inst->enableTMVP = enableTMVP;
  allocCfg.tmvpEnable = enableTMVP;
  allocCfg.ctbRcTrailStrength = (config->gopSize == 1 && (config->ctbRcMode & 1) &&
                                 asic_cfg->ctbRcVersion == 2)? config->ctbRcTrailStrength : 0;
  allocCfg.is_malloc = 1;
  vcenc_inst->asic.regs.mc_sync_enable = mcEnable;
  ...
  if (config->tiles_enabled_flag) {
    vcenc_inst->tileHeights =
        (u16 *)vcenc_inst->asic.tileHeightMem.virtualAddress;
    vcenc_inst->asic.regs.tileHeightBase =
        vcenc_inst->asic.tileHeightMem.busAddress;
    ...
    EWLSyncMemData(&vcenc_inst->asic.tileHeightMem, 0,
        vcenc_inst->asic.tileHeightMem.size, HOST_TO_DEVICE);
#endif
  }
  ...
  vcenc_inst->ctbRcTrailStrength = config->ctbRcTrailStrength;
  vcenc_inst->ctbRcTrailStrengthPrev = config->ctbRcTrailStrength;
  ...
  vcenc_inst->rateControl.intraQpDelta = INTRA_QPDELTA_DEFAULT
                                         << QP_FRACTIONAL_BITS;
  vcenc_inst->rateControl.fixedIntraQp = 0 << QP_FRACTIONAL_BITS;
  vcenc_inst->rateControl.tolMovingBitRate = 100;
  vcenc_inst->rateControl.tolCtbRcInter = vcenc_inst->asic.regs.asicCfg->ctbRcVersion == 2 ? 0.2 : -1.0;
  vcenc_inst->rateControl.tolCtbRcIntra = vcenc_inst->asic.regs.asicCfg->ctbRcVersion == 2 ? 19 : -1.0;
  vcenc_inst->rateControl.longTermQpDelta = 0;
  //  vcenc_inst->rateControl.f_tolMovingBitRate = 100.0;		//floating tolMovingBitRate seems no use
  vcenc_inst->rateControl.smooth_psnr_in_gop = 0;
  vcenc_inst->rateControl.maxIprop = 100;
  vcenc_inst->rateControl.minIprop = 1;
  vcenc_inst->rateControl.changePos = -1;
  ...
  vcenc_inst->rateControl.minPicSizeB =
      rcCalculate(vcenc_inst->rateControl.virtualBuffer.bitRate,
                  ...
                  vcenc_inst->rateControl.outRateNum) /
      (1 + 20);
  vcenc_inst->rateControl.virtualBuffer.bitPerPic =
      rcCalculate(vcenc_inst->rateControl.virtualBuffer.bitRate,
                  vcenc_inst->rateControl.outRateDenom,
                  vcenc_inst->rateControl.outRateNum);
//for xdx jira
  if (vcenc_inst->rateControl.virtualBuffer.bitPerPic) {
    vcenc_inst->rateControl.bitVarRangeI =
        (i32)(((i64)vcenc_inst->rateControl.maxPicSizeI) *
        100 / vcenc_inst->rateControl.virtualBuffer.bitPerPic - 100);

    vcenc_inst->rateControl.bitVarRangeP =
        (i32)(((i64)vcenc_inst->rateControl.maxPicSizeP) *
        100 / vcenc_inst->rateControl.virtualBuffer.bitPerPic - 100);

    vcenc_inst->rateControl.bitVarRangeB =
        (i32)(((i64)vcenc_inst->rateControl.maxPicSizeB) *
        100 / vcenc_inst->rateControl.virtualBuffer.bitPerPic - 100);
  } else {
    vcenc_inst->rateControl.bitVarRangeI = 10000;
    vcenc_inst->rateControl.bitVarRangeP = 10000;
    vcenc_inst->rateControl.bitVarRangeB = 10000;
  }
  ...
  #endif
  coreNum = parallelCoreNum;

  u32 num_ctbrcMem = 2 + coreNum;

  for (i = 0; i < num_ctbrcMem; i++) {
  ...
  for (i = 0; i < coreNum; i++) {
    vcenc_inst->rateControl.ctbRateCtrl.Cur[i].ctbMemCurAddr =
        vcenc_inst->asic.ctbRcMem[num_ctbrcMem+i].busAddress;
    vcenc_inst->rateControl.ctbRateCtrl.Cur[i].ctbMemCurVirtualAddr =
        vcenc_inst->asic.ctbRcMem[num_ctbrcMem+i].virtualAddress;
  }

  vcenc_inst->rateControl.ctbRateCtrl.qpStep = asic_cfg->ctbRcVersion == 2 ? 2 :
      (((4 << CTB_RC_QP_STEP_FIXP) + vcenc_inst->rateControl.ctbCols / 2) /
      vcenc_inst->rateControl.ctbCols);

  vcenc_inst->rateControl.ctbRateCtrl.ctbRcRowQpDeltaRange = asic_cfg->ctbRcVersion == 2 ? 1 : 0;

  vcenc_inst->rateControl.pass = vcenc_inst->pass = pass;
  vcenc_inst->rateControl.coreNum = coreNum;

  vcenc_inst->rateControl.ctbRateCtrl.models[0].idxType = B_SLICE;
  vcenc_inst->rateControl.ctbRateCtrl.models[1].idxType = P_SLICE;
  vcenc_inst->rateControl.ctbRateCtrl.models[2].idxType = I_SLICE;
  ...
  RC_LOG_I("rateControl init done. rcMode = %d\n",
           vcenc_inst->rateControl.rcMode);
  ...
  if (IS_H264(vcenc_inst->codecFormat)) {
    ...
    vcenc_inst->tr_depth_inter = (vcenc_inst->max_cu_size == 64)
                                     ? 4
                                     : 3; /* Max transform hierarchy depth */
    vcenc_inst->log2_qp_size = 6;
    if (asic_cfg->tu32Enable) {
      vcenc_inst->max_tr_size = 32;
      vcenc_inst->tr_depth_inter =
          (asic_cfg->tu32Enable && asic_cfg->dynamicMaxTuSize &&
                   vcenc_inst->rdoLevel == 0
               ? 3
               : 2);
    }
    if ((IS_HEVC(vcenc_inst->codecFormat) && asic_cfg->hevcCuRdoSupport) ||
        (IS_AV1(vcenc_inst->codecFormat) && asic_cfg->av1CuRdoSupport)) {
      /* improve base quality by parallel rdo */
      vcenc_inst->tr_depth_inter = 1;
      APITRACEERR(
          "VCEncInit: WARNING rdoLevel 0/1 NOT support in current Parallel rdo hevc, set rdoLevel to 2\n");
      vcenc_inst->rdoLevel = vcenc_inst->asic.regs.rdoLevel = 2;
    }
  }
  ...
  regs->ssim = (config->enableSsim || config->enableFrameInfo); // enable psnr & ssim by default when enableFrameInfo
  if (regs->ssim) {
    if (!IS_AV1(config->codecFormat) && !regs->asicCfg->ssimSupport) {
      APITRACEERR(
          "VCEncInit: WARNING SSIM calculation not supported by HW, force it "
          "disable\n");
      regs->ssim = 0;
    } else if (IS_AV1(config->codecFormat) && !regs->asicCfg->ssimSupportAV1) {
      APITRACEERR(
          "VCEncInit: WARNING SSIM calculation not supported for AV1, force it "
          "disable\n");
      regs->ssim = 0;
    }
  }
  ...
  regs->psnr = (config->enablePsnr || config->enableFrameInfo); // enable psnr & ssim by default when enableFrameInfo
  if (regs->psnr) {
    if (!IS_AV1(config->codecFormat) && !regs->asicCfg->psnrSupport) {
      APITRACEERR(
          "VCEncInit: WARNING PSNR calculation not supported by HW, force it "
          "disable\n");
      regs->psnr = 0;
    } else if (IS_AV1(config->codecFormat) && !regs->asicCfg->psnrSupportAV1) {
      APITRACEERR(
          "VCEncInit: WARNING PSNR calculation not supported for AV1, force it "
          "disable\n");
      regs->psnr = 0;
    }
  }

  if (vcenc_inst->cuTreeCtl.aq_mode > 0 &&
      regs->asicCfg->encVisualTuneSupport == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR("VCEncInit ERROR: aq_mode is not supported by HW\n");
    return VCENC_INVALID_ARGUMENT;
  }

  regs->vcmd.vcmdBufSize = 0;
  ...
  /* AXI alignment */
  /* Use maxAlignment to fill alignment if user doesn't fill "--AXIAlignment" in cml,
     or use data they fill */
  if (config->AXIAlignment == -1) {
    regs->AXI_burst_align_wr_cuinfo = asic_cfg->axi_burst_align_wr_cuinfo;
    regs->AXI_burst_align_wr_common = asic_cfg->axi_burst_align_wr_common;
    regs->AXI_burst_align_wr_stream = asic_cfg->axi_burst_align_wr_stream;
    regs->AXI_burst_align_wr_chroma_ref =
        asic_cfg->axi_burst_align_wr_chroma_ref;
    regs->AXI_burst_align_wr_luma_ref = asic_cfg->axi_burst_align_wr_luma_ref;
    regs->AXI_burst_align_rd_common = asic_cfg->axi_burst_align_rd_common;
    regs->AXI_burst_align_rd_prp = asic_cfg->axi_burst_align_rd_prp;
    ...
    if (config->AXIAlignment != 0) {
      ...
      if (axi_align_fuse == 0) {
        APITRACEERR(
            "VCEncInit: Error, configured AXI is not supported by hardware\n");
        return VCENC_INVALID_ARGUMENT;
      } else {
        if ((asic_cfg->axi_burst_align_wr_common <
             regs->AXI_burst_align_wr_common) ||
            (asic_cfg->axi_burst_align_wr_stream <
             regs->AXI_burst_align_wr_stream) ||
            (asic_cfg->axi_burst_align_wr_chroma_ref <
             regs->AXI_burst_align_wr_chroma_ref) ||
            (asic_cfg->axi_burst_align_wr_luma_ref <
             regs->AXI_burst_align_wr_luma_ref) ||
            (asic_cfg->axi_burst_align_rd_common <
             regs->AXI_burst_align_rd_common) ||
            ...
            (asic_cfg->axi_burst_align_rd_lu_ref_prefetch <
             regs->AXI_burst_align_rd_lu_ref_prefetch) ||
            (asic_cfg->axi_burst_align_wr_cuinfo <
             regs->AXI_burst_align_wr_cuinfo)) {
          APITRACEERR(
              "VCEncInit: Error, configured AXI is not matched with "
              "hardware\n");
          return VCENC_INVALID_ARGUMENT;
        }
      }
  ...
  /* Check multi-core for vp9*/
  if (IS_VP9(config->codecFormat) && mcEnable) {
    APITRACEERR("VCEncInit: Multi-core is not supported by vp9 \n");
    return VCENC_INVALID_ARGUMENT;
  }

  vcenc_inst->jobCnt = 0;
  vcenc_inst->readCnt = 0;
  ...
  vcenc_inst->vuiColorDescription.vuiColorPrimaries = 2;
  vcenc_inst->vuiColorDescription.vuiTransferCharacteristics = 2;
  vcenc_inst->vuiColorDescription.vuiMatrixCoefficients = 2;
...
#ifdef MULTI_FRAME_SUPPORT
  vcenc_inst->batchEnable = batchEnable;
  vcenc_inst->batchCount = parallelCoreNum - 1;
  vcenc_inst->sendCnt = 0;
  vcenc_inst->cacheCnt = 0;
#endif
  /* Parallel Encoding */
  vcenc_inst->parallelCoreNum = parallelCoreNum;
  vcenc_inst->reservedCore = 0;
  vcenc_inst->maxReservedCoreNum = 0;
  vcenc_inst->mcEnable = mcEnable;

  /* dynamic FPS */
  vcenc_inst->bInsertIDRonFPSAdjust = bInsertIDRonFPSAdjust;
  vcenc_inst->bSendSPSonFPSAdjust = bSendSPSonFPSAdjust;

  /* RDOQ. enable RDOQ by default if HW supported */
  if ((vcenc_inst->pass != 1) && (config->tune != VCENC_TUNE_SHARP_VISUAL)) {
    regs->bRDOQEnable =
        (IS_H264(config->codecFormat) && regs->asicCfg->RDOQSupportH264) ||
  ...
  /* Separate 2 type of motion score */
  vcenc_inst->motionScoreEnable = regs->asicCfg->motionScoreEnable;
  vcenc_inst->multiPassMotionScoreEnable = 0;
  ...
  vcenc_inst->gopMaxBSize = config->gopMaxBSize;
  vcenc_inst->numRefP = config->numRefP;
  /* HW can not support tmvp and 2 reference p frame at the same time */
  if (asic_cfg->twoRefPTMVPSupport == 0 && vcenc_inst->numRefP == 2 &&
       vcenc_inst->enableTMVP)
  {
    vcenc_inst->enableTMVP = 0;
    APITRACEERR("VCEncInit: WARNING buildID[0x%x] can not support tmvp and 2 reference "
      "p frame at the same time, disable TMVP\n", asic_cfg->hw_build_id);
    ret = VCENC_INVALID_ARGUMENT;
    goto error;
  }
  ...
  vcenc_inst->enableLeadingPictures = config->enableLeadingPictures;

  if (vcenc_inst->pass == 1) {
    /* init pass1 -> cutree job' bufferpool */
    u32 nodeNum = VCEncGetEncodeMaxDelay(vcenc_inst) + 1;
    nodeNum += 1; //Additional 1, since cutree store one more lookaheadFrames for propagate cost
    ret = InitBufferPool(&vcenc_inst->cutreeJobBufferPool,
                         sizeof(struct Lowres), nodeNum);
    if (VCENC_OK != ret) {
    ...
    /* enable multi pass motion score in pass 1 for agop decision */
    vcenc_inst->multiPassMotionScoreEnable = regs->asicCfg->bMultiPassSupport;
    ...
    ret =
        InitBufferPool(&vcenc_inst->lookahead.jobBufferPool, nodeSize, nodeNum);
    if (ret != VCENC_OK) {
    ...
    if (vcenc_inst->extDSRatio) {
      ...
      if (config->tune == VCENC_TUNE_SHARP_VISUAL) {
        inLoopDsRatio = 0;
        APITRACE(
            "VCEncInit: Disable input downsampe in look-ahead encoding for "
      ...
      if (regs->asicCfg->inLoopDSRatio == 0) inLoopDsRatio = 0;
    }
    ...
    new_cfg.width = ((new_cfg.width + 1) >> 1) << 1;
    new_cfg.height = ((new_cfg.height + 1) >> 1) << 1;

    new_cfg.exteralReconAlloc =
        0;  //this should alloc internarlly when in the first pass.
    if (cb_try_new_params) {
      new_cfg.cb_try_new_params = cb_try_new_params;
    }
    ...
    u32 cuTreeOutQpBlockUnit =
        ((struct vcenc_instance *)vcenc_inst->lookahead.priv_inst)
            ->cuTreeCtl.outRoiMapDeltaQpBlockUnit;
    vcenc_inst->log2_qp_size =
        MIN(vcenc_inst->log2_qp_size, 6 - cuTreeOutQpBlockUnit);

    /* for pass1 cutree put buffer back to bufferpool */
    ((struct vcenc_instance *)vcenc_inst->lookahead.priv_inst)
        ->lookahead.jobBufferPool = vcenc_inst->lookahead.jobBufferPool;
  ...
  u32 nodeNum = MAX_GOP_SIZE+vcenc_inst->parallelCoreNum-1+config->aifEnable;
  ret = InitBufferPool(&vcenc_inst->task_pool, sizeof(VCEncInfo),nodeNum);
  if (ret != VCENC_OK) {
    APITRACEERR("VCEncInit: Init task pool failed\n");
    goto error;
  }
  ...
  if (IS_AV1(config->codecFormat)) {
    ret = VCEncInitAV1(config, vcenc_inst);
    if (VCENC_OK!=ret)
      goto error;
  }
#endif
...
#ifdef SUPPORT_VP9
  if (IS_VP9(config->codecFormat)) VCEncInitVP9(config, vcenc_inst);
#endif
  ...
  /*init rate ctrl buffer pool & queue */
  queue_init(&vcenc_inst->rateCtrl.rateCtrlBufPool);
  queue_init(&vcenc_inst->rateCtrl.rateCtrlQueue);
  queue_init(&vcenc_inst->rateCtrl.fpsRequestBufPool);
  queue_init(&vcenc_inst->rateCtrl.fpsRequestQueue);
  vcenc_inst->rateCtrl.pRateCtrlParam = NULL;

  /* sw skip frame */
  vcenc_inst->sw_skip_flag = 0;
...
VCEncRet VCEncRelease(VCEncInst inst) {
  ...
  ReleaseBufferPool(&pEncInst->task_pool);
  ...
  if (pEncInst->pass == 0) {
    ...
    //release rate ctrl buffer
    DynamicReleasePool(&pEncInst->rateCtrl.rateCtrlBufPool,
                       &pEncInst->rateCtrl.rateCtrlQueue);
    //release fps adjust request buffer
    DynamicReleasePool(&pEncInst->rateCtrl.fpsRequestBufPool,
                       &pEncInst->rateCtrl.fpsRequestQueue);
  }
  ...
  if (pEncInst->pass == 2 && pEncInst->lookahead.priv_inst) {
    ...
    ReleaseBufferPool(&pEncInst_priv->task_pool);
    /* Terminate lookahead thread first, put correct status */
    ret = TerminateLookaheadThread(&pEncInst->lookahead,
    ...
    if ((c = get_container(pEncInst_priv))) {
      ...
      EWLFreeLinear(pEncInst_priv->asic.ewl,
                    &(pEncInst_priv->lookahead.internal_mem.mem));
...
VCEncRet VCEncSetCodingCtrl(VCEncInst instAddr,
  ...
  APITRACEPARAM(" %s : %d\n", "ipcm1Area.enable",
                pCodeParams->ipcm1Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm1Area.top", pCodeParams->ipcm1Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm1Area.bottom",
                pCodeParams->ipcm1Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm1Area.left", pCodeParams->ipcm1Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm1Area.right", pCodeParams->ipcm1Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm2Area.enable",
                pCodeParams->ipcm2Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm2Area.top", pCodeParams->ipcm2Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm2Area.bottom",
                pCodeParams->ipcm2Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm2Area.left", pCodeParams->ipcm2Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm2Area.right", pCodeParams->ipcm2Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm3Area.enable",
                pCodeParams->ipcm3Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm3Area.top", pCodeParams->ipcm3Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm3Area.bottom",
                pCodeParams->ipcm3Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm3Area.left", pCodeParams->ipcm3Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm3Area.right", pCodeParams->ipcm3Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm4Area.enable",
                pCodeParams->ipcm4Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm4Area.top", pCodeParams->ipcm4Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm4Area.bottom",
                pCodeParams->ipcm4Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm4Area.left", pCodeParams->ipcm4Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm4Area.right", pCodeParams->ipcm4Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm5Area.enable",
                pCodeParams->ipcm5Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm5Area.top", pCodeParams->ipcm5Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm5Area.bottom",
                pCodeParams->ipcm5Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm5Area.left", pCodeParams->ipcm5Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm5Area.right", pCodeParams->ipcm5Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm6Area.enable",
                pCodeParams->ipcm6Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm6Area.top", pCodeParams->ipcm6Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm6Area.bottom",
                pCodeParams->ipcm6Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm6Area.left", pCodeParams->ipcm6Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm6Area.right", pCodeParams->ipcm6Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm7Area.enable",
                pCodeParams->ipcm7Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm7Area.top", pCodeParams->ipcm7Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm7Area.bottom",
                pCodeParams->ipcm7Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm7Area.left", pCodeParams->ipcm7Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm7Area.right", pCodeParams->ipcm7Area.right);
  APITRACEPARAM(" %s : %d\n", "ipcm8Area.enable",
                pCodeParams->ipcm8Area.enable);
  APITRACEPARAM(" %s : %d\n", "ipcm8Area.top", pCodeParams->ipcm8Area.top);
  APITRACEPARAM(" %s : %d\n", "ipcm8Area.bottom",
                pCodeParams->ipcm8Area.bottom);
  APITRACEPARAM(" %s : %d\n", "ipcm8Area.left", pCodeParams->ipcm8Area.left);
  APITRACEPARAM(" %s : %d\n", "ipcm8Area.right", pCodeParams->ipcm8Area.right);
  APITRACEPARAM(" %s : %d\n", "rect0.enable", pCodeParams->rect0.enable);
  APITRACEPARAM(" %s : %d\n", "rect0.top", pCodeParams->rect0.top);
  APITRACEPARAM(" %s : %d\n", "rect0.bottom", pCodeParams->rect0.bottom);
  APITRACEPARAM(" %s : %d\n", "rect0.left", pCodeParams->rect0.left);
  APITRACEPARAM(" %s : %d\n", "rect0.right", pCodeParams->rect0.right);
  APITRACEPARAM(" %s : %d\n", "rect1.enable", pCodeParams->rect1.enable);
  APITRACEPARAM(" %s : %d\n", "rect1.top", pCodeParams->rect1.top);
  APITRACEPARAM(" %s : %d\n", "rect1.bottom", pCodeParams->rect1.bottom);
  APITRACEPARAM(" %s : %d\n", "rect1.left", pCodeParams->rect1.left);
  APITRACEPARAM(" %s : %d\n", "rect1.right", pCodeParams->rect1.right);
  APITRACEPARAM(" %s : %d\n", "rect2.enable", pCodeParams->rect2.enable);
  APITRACEPARAM(" %s : %d\n", "rect2.top", pCodeParams->rect2.top);
  APITRACEPARAM(" %s : %d\n", "rect2.bottom", pCodeParams->rect2.bottom);
  APITRACEPARAM(" %s : %d\n", "rect2.left", pCodeParams->rect2.left);
  APITRACEPARAM(" %s : %d\n", "rect2.right", pCodeParams->rect2.right);
  APITRACEPARAM(" %s : %d\n", "rect3.enable", pCodeParams->rect3.enable);
  APITRACEPARAM(" %s : %d\n", "rect3.top", pCodeParams->rect3.top);
  APITRACEPARAM(" %s : %d\n", "rect3.bottom", pCodeParams->rect3.bottom);
  APITRACEPARAM(" %s : %d\n", "rect3.left", pCodeParams->rect3.left);
  APITRACEPARAM(" %s : %d\n", "rect3.right", pCodeParams->rect3.right);
  APITRACEPARAM(" %s : %d\n", "rect4.enable", pCodeParams->rect4.enable);
  APITRACEPARAM(" %s : %d\n", "rect4.top", pCodeParams->rect4.top);
  APITRACEPARAM(" %s : %d\n", "rect4.bottom", pCodeParams->rect4.bottom);
  APITRACEPARAM(" %s : %d\n", "rect4.left", pCodeParams->rect4.left);
  APITRACEPARAM(" %s : %d\n", "rect4.right", pCodeParams->rect4.right);
  APITRACEPARAM(" %s : %d\n", "rect5.enable", pCodeParams->rect5.enable);
  APITRACEPARAM(" %s : %d\n", "rect5.top", pCodeParams->rect5.top);
  APITRACEPARAM(" %s : %d\n", "rect5.bottom", pCodeParams->rect5.bottom);
  APITRACEPARAM(" %s : %d\n", "rect5.left", pCodeParams->rect5.left);
  APITRACEPARAM(" %s : %d\n", "rect5.right", pCodeParams->rect5.right);
  APITRACEPARAM(" %s : %d\n", "rect6.enable", pCodeParams->rect6.enable);
  APITRACEPARAM(" %s : %d\n", "rect6.top", pCodeParams->rect6.top);
  APITRACEPARAM(" %s : %d\n", "rect6.bottom", pCodeParams->rect6.bottom);
  APITRACEPARAM(" %s : %d\n", "rect6.left", pCodeParams->rect6.left);
  APITRACEPARAM(" %s : %d\n", "rect6.right", pCodeParams->rect6.right);
  APITRACEPARAM(" %s : %d\n", "rect7.enable", pCodeParams->rect7.enable);
  APITRACEPARAM(" %s : %d\n", "rect7.top", pCodeParams->rect7.top);
  APITRACEPARAM(" %s : %d\n", "rect7.bottom", pCodeParams->rect7.bottom);
  APITRACEPARAM(" %s : %d\n", "rect7.left", pCodeParams->rect7.left);
  APITRACEPARAM(" %s : %d\n", "rect7.right", pCodeParams->rect7.right);
  ...
  APITRACEPARAM(" %s : %d\n", "RoimapCuCtrl_ver",
                pCodeParams->RoimapCuCtrl_ver);
  APITRACEPARAM(" %s : %d\n", "RoiQpDelta_ver", pCodeParams->RoiQpDelta_ver);
  APITRACEPARAM(" %s : %d\n", "pcm_enabled_flag",
                pCodeParams->pcm_enabled_flag);
  ...
  APITRACEPARAM(" %s : %d\n", "intraReconEnable", pCodeParams->intraReconEnable);
  APITRACEPARAM(" %s : %d\n", "gopMaxBSize", pCodeParams->gopMaxBSize);
  /* Check for existing instance */
  ...
  do {
    ...
    if (pLastCodingCtrlParam) {
      if (pLastCodingCtrlParam->refCnt > 0) pLastCodingCtrlParam->refCnt--;
      ...
      if (0 == pLastCodingCtrlParam->refCnt) {
        ...
        pEncInst->codingCtrl.pCodingCtrlParam = NULL;
    ...
    if (VCENC_OK != EncCheckCodingCtrlParam(pEncInst, pCodingCtrl)) break;
    ...
    if (pEncInst->encStatus == VCENCSTAT_INIT) {
      ...
      if (2 ==
          pEncInst
              ->pass)  //a few coding ctrl paramters need to be set in pass1/cutree
        EncUpdateCodingCtrlForPass1(pEncInst->lookahead.priv_inst,
                                    pEncCodingCtrlParam);
    }
  ...
  for (i = 0; i < gopCfg->size; i++) {
    ...
    if (cfg->codingType == VCENC_BIDIR_PREDICTED_FRAME) {
      ...
      for (iRefPic = 0; iRefPic < cfg->numRefPics; iRefPic++) {
        if (cfg->refPics[iRefPic].used_by_cur &&
            cfg->refPics[iRefPic].ref_pic > 0)
          lowdelayB = HANTRO_FALSE;
      }
      if (lowdelayB) break;
    }
  }
  return (lowdelayB || pEncIn->bIsPeriodUpdateLTR || pEncIn->flexRefsEnable)
             ? 1
             : 0;
}

u32 GetHevcProfileIdc(VCEncProfile profile) {
  switch ((int)profile) {
    case 0:
      //main profile
      return SYNTAX_PROFILE_HEVC_MAIN;
    case 1:
      //main still picture profile.
      return SYNTAX_PROFILE_HEVC_MAIN_STILL_PICTURE;
    case 2:
      //main 10 profile.
      return SYNTAX_PROFILE_HEVC_MAIN_10;
    case 3:
      //monochrome profile.
      return SYNTAX_PROFILE_HEVC_REXT;
    case 4:
      //monochrome 10 profile.
      return SYNTAX_PROFILE_HEVC_REXT;
    case 5:
      //main 422 10 still picture profile.
      return SYNTAX_PROFILE_HEVC_REXT;
    case 6:
      //monochrome 422 still picture profile.
      return SYNTAX_PROFILE_HEVC_REXT;
    case 7:
      //Main 444 10 bit profile.
      return SYNTAX_PROFILE_HEVC_REXT;
    case 8:
      //SCC main profile.
      return SYNTAX_PROFILE_HEVC_SCC;
    case 9:
      //SCC 444 10 profile.
      return SYNTAX_PROFILE_HEVC_SCC;

    default:
      return SYNTAX_PROFILE_HEVC_MAIN; //force to MAIN profile if invalid
  }
}

u32 GetH264ProfileIdc(VCEncProfile profile) {
  switch ((int)profile) {
    case 9:
      //H.264 baseline profile
      return SYNTAX_PROFILE_H264_BASELINE;
    case 10:
      //H.264 main profile
      return SYNTAX_PROFILE_H264_MAIN;
    case 11:
      //H.264 high profile
      return SYNTAX_PROFILE_H264_HIGH;
    case 12:
      //H.264 high 10 profile
      return SYNTAX_PROFILE_H264_HIGH_10;
    default:
      return SYNTAX_PROFILE_H264_HIGH;
  }
}

/** VCEncGetProfile will output profile value from VCEncConfig when profile
is 'ADAPTIVE', output will use to check config and warning report */
i32 VCEncGetProfile(VCEncInst pEncCfg, i32 config_struct_flag, const VCEncIn *pEncIn) {
  u32 profile;
  u32 codedChromaIdc;
  u32 bitDepthLuma;
  u32 codecFormat;
  i32 outProflie = 0;
  u32 enableCabac_flag = 1;
  i32 B_frame_exist_flag = 0;
  u32 transform_skip_enabled_flag = 0;

  if (config_struct_flag == 1) {
    VCEncConfig *pCfg = (VCEncConfig*)pEncCfg;
    profile = pCfg->profile;
    codecFormat = pCfg->codecFormat;
    bitDepthLuma = pCfg->bitDepthLuma;
    codedChromaIdc = pCfg->codedChromaIdc;
  } else {
    struct vcenc_instance * pCfg = (struct vcenc_instance *)pEncCfg;
    // in VCEncStrmStart(), and is adaptive, get profile again with updated info
    profile = (pCfg->adaptive_profile == -1)?
        pCfg->profile : VCENC_ADAPTIVE_PROFILE;
    codecFormat = pCfg->codecFormat;
    bitDepthLuma = pCfg->sps->bit_depth_luma_minus8 + 8;
    codedChromaIdc = pCfg->asic.regs.codedChromaIdc;
    if (pCfg->codingCtrl.pCodingCtrlParam != NULL) {
      if (pCfg->codingCtrl.pCodingCtrlParam->encCodingCtrl.enableCabac == 0) {
        enableCabac_flag = 0;
      }
    }
    for (u32 i=0; i<pEncIn->gopConfig.size; i++) {
      if (pEncIn->gopConfig.pGopPicCfg[i].codingType == VCENC_BIDIR_PREDICTED_FRAME ||
          pEncIn->gopConfig.ltrcnt != 0) {
        B_frame_exist_flag = 1;
        break;
      }
    }
    if (pCfg->enableTS) {
  ...
  if (VCENC_ADAPTIVE_PROFILE == profile) {
    switch (codecFormat) {
      case VCENC_VIDEO_CODEC_HEVC:
        if(VCENC_CHROMA_IDC_400 == codedChromaIdc){ // 400
          if (bitDepthLuma == 10) { // 400 10 bits
            outProflie = VCENC_HEVC_MONOCHROME_10_PROFILE;
          } else { //400 8 bits
            outProflie = VCENC_HEVC_MONOCHROME_PROFILE;
          }
        } else if (VCENC_CHROMA_IDC_422 == codedChromaIdc) { // 422
          outProflie = VCENC_HEVC_MAIN_422_10_PROFILE;
        } else if (VCENC_CHROMA_IDC_444 == codedChromaIdc) { // 444
          if (transform_skip_enabled_flag) { // SCC
            outProflie = VCENC_HEVC_SCC_444_10_PROFILE;
          } else {
            outProflie = VCENC_HEVC_MAIN_444_10_PROFILE;
          }
        } else { // 420
          if (bitDepthLuma == 10) { // 420 10 bits
            outProflie = VCENC_HEVC_MAIN_10_PROFILE;
          } else if (transform_skip_enabled_flag) { // 420 SCC
            outProflie = VCENC_HEVC_SCC_MAIN_PROFILE;
          } else {
            outProflie = VCENC_HEVC_MAIN_PROFILE;
          }
        }
        break;
      case VCENC_VIDEO_CODEC_H264:
        if (VCENC_CHROMA_IDC_400 == codedChromaIdc) {
          outProflie = VCENC_H264_HIGH_10_PROFILE;
        } else if ((enableCabac_flag == 0) && (B_frame_exist_flag == 0)) {
          outProflie = VCENC_H264_BASE_PROFILE;
        } else {
          if(bitDepthLuma == 10){
            outProflie = VCENC_H264_HIGH_10_PROFILE;
          } else {
            outProflie = VCENC_H264_HIGH_PROFILE;
          }
        }
        break;
      case VCENC_VIDEO_CODEC_AV1:
        outProflie = VCENC_AV1_MAIN_PROFILE;
        break;
      case VCENC_VIDEO_CODEC_VP9:
        if(bitDepthLuma == 10){
          outProflie = VCENC_VP9_2_PROFILE;
        } else {
          outProflie = VCENC_VP9_0_PROFILE;
        }
        break;
      default:
        outProflie = VCENC_HEVC_MAIN_PROFILE;
...
i32 set_parameter(struct vcenc_instance *vcenc_instance, const VCEncIn *pEncIn,
  ...
  if (checkRange(tmp, 3, 3)) goto out;
  ...
  if (checkRange(tmp, log2_ctu_size, log2_ctu_size)) goto out;
  ...
  if (checkRange(tmp, 2, 2)) goto out;
  s->log2_min_tr_size = tmp;

  if (log2i(vcenc_instance->max_tr_size, &tmp)) goto out;
  if (checkRange(tmp, s->log2_min_tr_size, MIN(p->log2_ctb_size, 5))) goto out;
  ...
  if (checkRange(vcenc_instance->tr_depth_intra, 0, tmp)) goto out;
  s->max_tr_hierarchy_depth_intra = vcenc_instance->tr_depth_intra;

  if (checkRange(vcenc_instance->tr_depth_inter, 0, tmp)) goto out;
  s->max_tr_hierarchy_depth_inter = vcenc_instance->tr_depth_inter;

  s->scaling_list_enable_flag = vcenc_instance->enableScalingList;

  /* Parameter set id */
  if (checkRange(vcenc_instance->vps_id, 0, 15)) goto out;
  v->ps.id = vcenc_instance->vps_id;

  if (checkRange(vcenc_instance->sps_id, 0, 15)) goto out;
  s->ps.id = vcenc_instance->sps_id;
  s->vps_id = v->ps.id;

  if (checkRange(vcenc_instance->pps_id, 0, 63)) goto out;
  ...
  p->deblocking_filter_override_enabled_flag =
      vcenc_instance->enableDeblockOverride;

  p->cb_qp_offset = vcenc_instance->chromaQpOffset;
  p->cr_qp_offset = vcenc_instance->chromaQpOffset;
  p->transform_skip_enabled_flag = vcenc_instance->enableTS;
  p->log2_max_transform_skip_block_size_minus_2 =
     vcenc_instance->log2MaxTSBlockSizeMinus2;

  if(vcenc_instance->enableTS && vcenc_instance->log2MaxTSBlockSizeMinus2 > 0) {
    p->pps_extension_present_flag = 1;
    p->pps_range_extension_flag = 1;
  }
  ...
  u32 profile_idc = (IS_H264(vcenc_instance->codecFormat))?
                    GetH264ProfileIdc(vcenc_instance->profile) :
                    GetHevcProfileIdc(vcenc_instance->profile);
  CHECK_API_I(" Profile_cfg=%d, Profile_idc=%d\n", vcenc_instance->profile, profile_idc);
  v->general_profile_idc = profile_idc;
  s->general_profile_idc = profile_idc;
  ...
  s->pcm_sample_bit_depth_luma_minus1 =
      vcenc_instance->sps->bit_depth_luma_minus8 + 7;
  s->pcm_sample_bit_depth_chroma_minus1 =
      vcenc_instance->sps->bit_depth_chroma_minus8 + 7;
  s->pcm_loop_filter_disabled_flag =
      vcenc_instance->pcm_loop_filter_disabled_flag;
  ...
  if (IS_H264(vcenc_instance->codecFormat)) {
    ...
    s->picHeightInMapUnitsMinus1 =
        height / (16 * (1 + vcenc_instance->interlaced)) - 1;
    ...
    if (vcenc_instance->interlaced >= 1 &&
        s->general_profile_idc == SYNTAX_PROFILE_H264_BASELINE)
      s->general_profile_idc = SYNTAX_PROFILE_H264_MAIN;

    p->transform8x8Mode =
        (s->general_profile_idc >= SYNTAX_PROFILE_H264_HIGH ?
          ENCHW_YES : ENCHW_NO);
    p->entropy_coding_mode_flag =
        vcenc_instance->asic.regs.entropy_coding_mode_flag;

    /* always CABAC => main profile */
    if (s->general_profile_idc == SYNTAX_PROFILE_H264_BASELINE) {
      ASSERT(p->entropy_coding_mode_flag == 0);
    }

    /*  always 8x8 transform enabled => high profile */
    ASSERT(p->transform8x8Mode == ENCHW_NO ||
            s->general_profile_idc >= SYNTAX_PROFILE_H264_HIGH);
...
VCEncRet VCEncGetCodingCtrl(VCEncInst inst, VCEncCodingCtrl *pCodeParams) {
  ...
  EncCodingCtrlParam *pLastCodingCtrlParam =
      (EncCodingCtrlParam *)queue_head(&pEncInst->codingCtrl.codingCtrlQueue);
  if (pLastCodingCtrlParam) {
    memcpy(pCodeParams, &pLastCodingCtrlParam->encCodingCtrl,
           sizeof(VCEncCodingCtrl));
    return VCENC_OK;
  }
  ...
  pCodeParams->sramPowerdownMode = regs->sramPowerdownMode;
  pCodeParams->sramPowerdownTimerDiv32 = regs->sramPowerdownTimerDiv32;
  ...
  pCodeParams->vuiVideoFullRange =
      (pEncInst->sps->vui.vuiVideoFullRange == ENCHW_YES) ? 1 : 0;
  ...
  if (pEncInst->gdrDuration > 0) pCodeParams->intraArea.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm1Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm2Area.enable = 0;

  pCodeParams->gopMaxBSize = pEncInst->gopMaxBSize;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm3Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm4Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm5Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm6Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm7Area.enable = 0;
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->ipcm8Area.enable = 0;

  pCodeParams->rect0.top = regs->sse0Top;
  pCodeParams->rect0.left = regs->sse0Left;
  pCodeParams->rect0.bottom = regs->sse0Bottom;
  pCodeParams->rect0.right = regs->sse0Right;
  if ((pCodeParams->rect0.top <= pCodeParams->rect0.bottom &&
    ...
    pCodeParams->rect0.enable = 1;
  } else {
    pCodeParams->rect0.enable = 0;
  }

  pCodeParams->rect1.top = regs->sse1Top;
  pCodeParams->rect1.left = regs->sse1Left;
  pCodeParams->rect1.bottom = regs->sse1Bottom;
  pCodeParams->rect1.right = regs->sse1Right;
  if (pCodeParams->rect1.top <= pCodeParams->rect1.bottom &&
    ...
    pCodeParams->rect1.enable = 1;
  } else {
    pCodeParams->rect1.enable = 0;
  }

  pCodeParams->rect2.top = regs->sse2Top;
  pCodeParams->rect2.left = regs->sse2Left;
  pCodeParams->rect2.bottom = regs->sse2Bottom;
  pCodeParams->rect2.right = regs->sse2Right;
  if ((pCodeParams->rect2.top <= pCodeParams->rect2.bottom &&
    ...
    pCodeParams->rect2.enable = 1;
  } else {
    pCodeParams->rect2.enable = 0;
  }

  pCodeParams->rect3.top = regs->sse3Top;
  pCodeParams->rect3.left = regs->sse3Left;
  pCodeParams->rect3.bottom = regs->sse3Bottom;
  pCodeParams->rect3.right = regs->sse3Right;
  if ((pCodeParams->rect3.top <= pCodeParams->rect3.bottom &&
    ...
    pCodeParams->rect3.enable = 1;
  } else {
    pCodeParams->rect3.enable = 0;
  }

  pCodeParams->rect4.top = regs->sse4Top;
  pCodeParams->rect4.left = regs->sse4Left;
  pCodeParams->rect4.bottom = regs->sse4Bottom;
  pCodeParams->rect4.right = regs->sse4Right;
  if ((pCodeParams->rect4.top <= pCodeParams->rect4.bottom &&
    ...
    pCodeParams->rect4.enable = 1;
  } else {
    pCodeParams->rect4.enable = 0;
  }

  pCodeParams->rect5.top = regs->sse5Top;
  pCodeParams->rect5.left = regs->sse5Left;
  pCodeParams->rect5.bottom = regs->sse5Bottom;
  pCodeParams->rect5.right = regs->sse5Right;
  if ((pCodeParams->rect5.top <= pCodeParams->rect5.bottom &&
    ...
    pCodeParams->rect5.enable = 1;
  } else {
    pCodeParams->rect5.enable = 0;
  }

  pCodeParams->rect6.top = regs->sse6Top;
  pCodeParams->rect6.left = regs->sse6Left;
  pCodeParams->rect6.bottom = regs->sse6Bottom;
  pCodeParams->rect6.right = regs->sse6Right;
  if ((pCodeParams->rect6.top <= pCodeParams->rect6.bottom &&
    ...
    pCodeParams->rect6.enable = 1;
  } else {
    pCodeParams->rect6.enable = 0;
  }

  pCodeParams->rect7.top = regs->sse7Top;
  pCodeParams->rect7.left = regs->sse7Left;
  pCodeParams->rect7.bottom = regs->sse7Bottom;
  pCodeParams->rect7.right = regs->sse7Right;
  if ((pCodeParams->rect7.top <= pCodeParams->rect7.bottom &&
    ...
    pCodeParams->rect7.enable = 1;
  } else {
    pCodeParams->rect7.enable = 0;
  }
  ...
  //GDR will use this area, so we need to turn it off when get from instance
  if (pEncInst->gdrDuration > 0) pCodeParams->roi1Area.enable = 0;
  ...
  pCodeParams->pcm_loop_filter_disabled_flag =
      pEncInst->pcm_loop_filter_disabled_flag;
  ...
  pCodeParams->inputLineBufCbData = pEncInst->inputLineBuf.cbData;
    ...
    pCodeParams->inputSliceInfoPollEn = pEncInst->inputSliceInfoPollEn;
#endif

  /* sbi */
  pCodeParams->sbi_id_0 = regs->sbi_id_0;
  pCodeParams->sbi_id_1 = regs->sbi_id_1;
  pCodeParams->sbi_id_2 = regs->sbi_id_2;
  pCodeParams->segmentUnitHeight = pEncInst->inputLineBuf.segmentUnitHeight;
  ...
  /* ctbRc Mode1 */
  pCodeParams->ctbRcSkinQPDelta = pEncInst->ctbRcSkinQPDelta;
  pCodeParams->ctbRcSkinMinQPDelta = pEncInst->ctbRcSkinMinQPDelta;
  pCodeParams->ctbRcSkinCbMin = pEncInst->ctbRcSkinCbMin;
  pCodeParams->ctbRcSkinCbMax = pEncInst->ctbRcSkinCbMax;
  pCodeParams->ctbRcSkinCrMin = pEncInst->ctbRcSkinCrMin;
  pCodeParams->ctbRcSkinCrMax = pEncInst->ctbRcSkinCrMax;
  pCodeParams->ctbRcSkinLumMin = pEncInst->ctbRcSkinLumMin;
  pCodeParams->ctbRcSkinLumMax = pEncInst->ctbRcSkinLumMax;
  pCodeParams->ctbRcSkinGradTh = pEncInst->ctbRcSkinGradTh;
  pCodeParams->ctbRcTrailStrengthMax =
   ((pEncInst->ctbRcMode & 1) && (pEncInst->gopSize == 1) && regs->asicCfg->ctbRcVersion == 2) ?
                  MIN(pEncInst->ctbRcTrailStrengthMax, 3) : 0;
  pCodeParams->ctbRcTrailDeltaQp = pEncInst->ctbRcTrailDeltaQp;
  pCodeParams->ctbRcDirection = pEncInst->ctbRcDirection;
  pCodeParams->ctbRcEdgeTh = pEncInst->ctbRcEdgeTh;
  pCodeParams->ctbRcEdgeMaxQpDelta = pEncInst->ctbRcEdgeMaxQpDelta;
  for (int idx = 0; idx < 16; idx++) {
    pCodeParams->ctbRcThresholdI[idx] = pEncInst->ctbRcThresholdI[idx];
    pCodeParams->ctbRcThresholdB[idx] = pEncInst->ctbRcThresholdB[idx];
    pCodeParams->ctbRcThresholdP[idx] = pEncInst->ctbRcThresholdP[idx];
  }

  /*visual quality prior mode decision*/
  pCodeParams->visualBitRateTolerance = pEncInst->visualBitRateTolerance;
  pCodeParams->IntraBiasChromaStrength = pEncInst->IntraBiasChromaStrength;
  pCodeParams->IntraBiasStrength = pEncInst->IntraBiasStrength;
  pCodeParams->IntraBiasMvThreshold = pEncInst->IntraBiasMvThreshold;
  pCodeParams->bTrailAvoidIntraBias = pEncInst->bTrailAvoidIntraBias;
  ...
  pCodeParams->Hdr10Display.hdr10_display_enable =
      (pEncInst->Hdr10Display.hdr10_display_enable == (u8)HDR10_NOCFGED) ? 0
                                                                         : 1;
  ...
  pCodeParams->Hdr10LightLevel.hdr10_lightlevel_enable =
      (pEncInst->Hdr10LightLevel.hdr10_lightlevel_enable == (u8)HDR10_NOCFGED)
          ? 0
          : 1;
  ...
  /* intra recon*/
  pCodeParams->intraReconEnable = pEncInst->intraReconEnable;

#ifdef MULTI_FRAME_SUPPORT
  vcencBatchGetCodingCtrl(pEncInst, pCodeParams);
#endif /* MULTI_FRAME_SUPPORT */

  /* sw skip frame*/
  pCodeParams->sw_skip_flag = pEncInst->sw_skip_flag;
...
------------------------------------------------------------------------------*/
VCEncRet VCEncSetRateCtrl(VCEncInst inst, const VCEncRateCtrl *pRateParams) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;

  /* Check for illegal inputs */
  if ((vcenc_instance == NULL) || (pRateParams == NULL)) {
    APITRACEERR("VCEncSetRateCtrl: ERROR Null argument\n");
    return VCENC_NULL_ARGUMENT;
  }

  APITRACE("VCEncSetRateCtrl#\n");
  APITRACEPARAM(" %s : %d\n", "pictureRc", pRateParams->pictureRc);
  APITRACEPARAM(" %s : %d\n", "pictureSkip", pRateParams->pictureSkip);
  APITRACEPARAM(" %s : %d\n", "qpHdr", pRateParams->qpHdr);
  APITRACEPARAM(" %s : %d\n", "qpMinPB", pRateParams->qpMinPB);
  APITRACEPARAM(" %s : %d\n", "qpMaxPB", pRateParams->qpMaxPB);
  APITRACEPARAM(" %s : %d\n", "qpMinI", pRateParams->qpMinI);
  APITRACEPARAM(" %s : %d\n", "qpMaxI", pRateParams->qpMaxI);
  APITRACEPARAM(" %s : %d\n", "bitPerSecond", pRateParams->bitPerSecond);
  APITRACEPARAM(" %s : %d\n", "rcMode", pRateParams->rcMode);
  APITRACEPARAM(" %s : %d\n", "hrd", pRateParams->hrd);
  APITRACEPARAM(" %s : %d\n", "hrdCpbSize", pRateParams->hrdCpbSize);
  APITRACEPARAM(" %s : %d\n", "bitrateWindow", pRateParams->bitrateWindow);
  APITRACEPARAM(" %s : %d\n", "intraQpDelta", pRateParams->intraQpDelta);
  APITRACEPARAM(" %s : %d\n", "fixedIntraQp", pRateParams->fixedIntraQp);
  APITRACEPARAM(" %s : %d\n", "bitVarRangeI", pRateParams->bitVarRangeI);
  APITRACEPARAM(" %s : %d\n", "bitVarRangeP", pRateParams->bitVarRangeP);
  APITRACEPARAM(" %s : %d\n", "bitVarRangeB", pRateParams->bitVarRangeB);
  APITRACEPARAM(" %s : %d\n", "staticSceneIbitPercent",
                pRateParams->u32StaticSceneIbitPercent);
  APITRACEPARAM(" %s : %d\n", "ctbRc", pRateParams->ctbRc);
  APITRACEPARAM(" %s : %d\n", "blockRCSize", pRateParams->blockRCSize);
  APITRACEPARAM(" %s : %d\n", "rcQpDeltaRange", pRateParams->rcQpDeltaRange);
  APITRACEPARAM(" %s : %d\n", "rcBaseMBComplexity",
                pRateParams->rcBaseMBComplexity);
  APITRACEPARAM(" %s : %d\n", "picQpDeltaMin", pRateParams->picQpDeltaMin);
  APITRACEPARAM(" %s : %d\n", "picQpDeltaMax", pRateParams->picQpDeltaMax);
  APITRACEPARAM(" %s : %d\n", "vbr", pRateParams->vbr);
  APITRACEPARAM(" %s : %d\n", "ctbRcQpDeltaReverse",
                pRateParams->ctbRcQpDeltaReverse);
  ...
  EncRateCtrlParam *pEncRateCtrlParam = NULL;
  ...
  do {
    ...
    //before new parameter enqueue as new head of queue, adjust refCnt of queue's last head
    EncRateCtrlParam *pLastRateCtrlParam =
        (EncRateCtrlParam *)queue_head(&vcenc_instance->rateCtrl.rateCtrlQueue);
    if (pLastRateCtrlParam) {
      if (pLastRateCtrlParam->refCnt > 0) pLastRateCtrlParam->refCnt--;
      //remove useless parameters from queue
      if (0 == pLastRateCtrlParam->refCnt) {
        queue_remove(&vcenc_instance->rateCtrl.rateCtrlQueue,
                     (struct node *)pLastRateCtrlParam);
        vcenc_instance->rateCtrl.pRateCtrlParam = NULL;
        DynamicPutBufferToPool(&vcenc_instance->rateCtrl.rateCtrlBufPool,
                               (void *)pLastRateCtrlParam);
      }
    }

    //get buffer
    pEncRateCtrlParam = (EncRateCtrlParam *)DynamicGetBufferFromPool(
        &vcenc_instance->rateCtrl.rateCtrlBufPool, sizeof(EncRateCtrlParam));
    if (!pEncRateCtrlParam) {
      APITRACEERR("VCEncSetRateCtrl: ERROR Get rate ctrl buffer failed\n");
      break;
    }
    pRateCtrl = &pEncRateCtrlParam->encRateCtrl;
    //copy parameters to buffer
    memcpy(pRateCtrl, pRateParams, sizeof(VCEncRateCtrl));
    if(pRateParams->frameRateUpdatePicCnt >= 0) {
      EncFpsRequest *req = (EncFpsRequest *)DynamicGetBufferFromPool(
          &vcenc_instance->rateCtrl.fpsRequestBufPool, sizeof(EncFpsRequest));
      if (!req) {
        APITRACEERR("VCEncSetRateCtrl: ERROR Get rate ctrl buffer failed\n");
        break;
      }
      req->frameRateDenom = pRateParams->frameRateDenom;
      req->frameRateNum = pRateParams->frameRateNum;
      req->frameRateUpdatePicCnt = pRateParams->frameRateUpdatePicCnt;
      pRateCtrl->frameRateUpdatePicCnt = -1;
      /* insert or update req */
      if(!EnqueueFpsRequest(&vcenc_instance->rateCtrl.fpsRequestQueue, req))
        DynamicPutBufferToPool(&vcenc_instance->rateCtrl.fpsRequestBufPool,
            (void *)req);
    }

    //check and modify parameters
    if (VCENC_OK != EncCheckRateCtrlParam(vcenc_instance, pRateCtrl)) break;

    /*The newest parameter's refCnt need be realRefCnt+1,
      so that next frame can also use these parameters if no new parameters are set*/
    pEncRateCtrlParam->refCnt = 1;
    pEncRateCtrlParam->startPicCnt = -1;
    queue_put(&vcenc_instance->rateCtrl.rateCtrlQueue,
              (struct node *)pEncRateCtrlParam);

    /*when init, set parameters into instance*/
    if (vcenc_instance->encStatus == VCENCSTAT_INIT) {
      EncUpdateRateCtrlParam(vcenc_instance, pEncRateCtrlParam, NULL);
	  //a few rate ctrl paramters need to be set in pass1/cutree
      if (2 == vcenc_instance->pass)
        EncUpdateRateCtrlParam((struct vcenc_instance *)vcenc_instance->lookahead.priv_inst,
                                    pEncRateCtrlParam, NULL);
    }

    ret = VCENC_OK;
  } while (0);

  if (VCENC_OK != ret && pEncRateCtrlParam) {
    DynamicPutBufferToPool(&vcenc_instance->rateCtrl.rateCtrlBufPool,
                           (void *)pEncRateCtrlParam);
    return ret;
  }
...
VCEncRet VCEncGetRateCtrl(VCEncInst inst, VCEncRateCtrl *pRateCtrl) {
  ...
  // get last rate ctrl parameter set
  EncRateCtrlParam *pLastRateCtrlParam = (EncRateCtrlParam *)queue_head(&vcenc_instance->rateCtrl.rateCtrlQueue);
  if (pLastRateCtrlParam)
  {
    memcpy(pRateCtrl, &pLastRateCtrlParam->encRateCtrl, sizeof(VCEncRateCtrl));
    return VCENC_OK;
  }
  ...
  if (rc->virtualBuffer.bitPerPic) {
    pRateCtrl->bitVarRangeI = rc->bitVarRangeI;
    pRateCtrl->bitVarRangeP = rc->bitVarRangeP;
    pRateCtrl->bitVarRangeB = rc->bitVarRangeB;
  } else {
  ...
  pRateCtrl->tolRcUnderflow = rc->tolUnderflow;
  ...
  pRateCtrl->ctbRcRowQpStep = vcenc_instance->asic.regs.asicCfg->ctbRcVersion == 2 ?
                              rc->ctbRateCtrl.qpStep :
                              (rc->ctbRateCtrl.qpStep
                                  ? ((rc->ctbRateCtrl.qpStep * rc->ctbCols +
                                      (1 << (CTB_RC_QP_STEP_FIXP - 1))) >>
                                     CTB_RC_QP_STEP_FIXP)
                                  : 0);
  pRateCtrl->ctbRcRowQpDeltaRange = vcenc_instance->asic.regs.asicCfg->ctbRcVersion == 2 ?
                              rc->ctbRateCtrl.ctbRcRowQpDeltaRange : 0;
  pRateCtrl->longTermQpDelta = rc->longTermQpDelta >> QP_FRACTIONAL_BITS;
  pRateCtrl->maxIprop = rc->maxIprop;
  pRateCtrl->minIprop = rc->minIprop;
  pRateCtrl->changePos = rc->changePos;
  ...
  pRateCtrl->hieQpDeltaEnable = rc->hieQpDeltaEnable;
...
i32 VSCheckSize(u32 inputWidth, u32 inputHeight, u32 stabilizedWidth,
  ...
  if ((inputWidth < 104) || (inputHeight < 104)) return 1;

  /* Stabilized picture minimum  values */
  if ((stabilizedWidth < 96) || (stabilizedHeight < 96)) return 1;

  /* Stabilized dimensions multiple of 4 */
  if (((stabilizedWidth & 3) != 0) || ((stabilizedHeight & 3) != 0)) return 1;
...
------------------------------------------------------------------------------*/
VCEncRet osd_overlap(const VCEncPreProcessingCfg *pPreProcCfg, u8 id,
                     VCEncVideoCodecFormat format) {
  int i, tmpx, tmpy;
  int blockW = 64;
  ...
  for (i = 0; i < MAX_OVERLAY_NUM; i++) {
    ...
    if ((area0.xoffset + area0.cropWidth) <= area1.xoffset &&
      ...
      tmpx = ((area0.xoffset + area0.cropWidth - 1) / blockW) * blockW;
      if (tmpx + blockW > area1.xoffset) return VCENC_ERROR;
    } else if ((area0.yoffset + area0.cropHeight) <= area1.yoffset) {
      tmpy = ((area0.yoffset + area0.cropHeight - 1) / blockH) * blockH;
      if (tmpy + blockH > area1.yoffset) return VCENC_ERROR;
    } else if (area0.xoffset >= (area1.xoffset + area1.cropWidth)) {
      tmpx = ((area1.xoffset + area1.cropWidth - 1) / blockW) * blockW;
      if (tmpx + blockW > area0.xoffset) return VCENC_ERROR;
    } else if (area0.yoffset >= (area1.yoffset + area1.cropHeight)) {
      tmpy = ((area1.yoffset + area1.cropHeight - 1) / blockH) * blockH;
      if (tmpy + blockH > area0.yoffset) return VCENC_ERROR;
    }
  }
...
VCEncRet VCEncSetPreProcessing(VCEncInst inst,
  ...
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    origWidth = (tileId == 0) ? pPreProcCfg->origWidth
                              : pPreProcCfg->tileExtra[tileId - 1].origWidth;
    origHeight = (tileId == 0) ? pPreProcCfg->origHeight
                               : pPreProcCfg->tileExtra[tileId - 1].origHeight;
    xOffset = (tileId == 0) ? pPreProcCfg->xOffset
                            : pPreProcCfg->tileExtra[tileId - 1].xOffset;
    yOffset = (tileId == 0) ? pPreProcCfg->yOffset
                            : pPreProcCfg->tileExtra[tileId - 1].yOffset;
    APITRACEPARAM(" %s : %d\n", "origWidth", origWidth);
  ...
  APITRACEPARAM(" %s : %d\n", "busAddressScaledBuff",
                pPreProcCfg->busAddressScaledBuff);
  ...
  if (cfg->rgbEnabled == EWL_HW_CONFIG_NOT_SUPPORTED &&
      ...
      (pPreProcCfg->inputType >= VCENC_RGBX8888 &&
      pPreProcCfg->inputType <= VCENC_BGRX1010102))) {
  ...
  if (cfg->scalingEnabled == EWL_HW_CONFIG_NOT_SUPPORTED &&
    ...
    APITRACEERR(
        "VCEncSetPreProcessing: WARNING Scaling core not supported, disabling "
        "output\n");
    return VCENC_INVALID_ARGUMENT;
  }
  if (cfg->cscExtendSupport == EWL_HW_CONFIG_NOT_SUPPORTED &&
      (pPreProcCfg->colorConversion.type >= VCENC_RGBFULL_TO_YUVLIMIT_BT601) &&
      (pPreProcCfg->colorConversion.type <= VCENC_RGBFULL_TO_YUVLIMIT_BT709)) {
    APITRACEERR(
        "VCEncSetPreProcessing: WARNING color conversion extend not supported, "
        "set to BT601_l.mat\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if ((cfg->NVFormatOnlySupport == EWL_HW_CONFIG_ENABLED) &&
    ...
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR input format only support NV12 or NV21 "
        "by HW\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if ((pPreProcCfg->colorConversion.type == VCENC_RGBTOYUV_USER_DEFINED) &&
      ...
      ((pPreProcCfg->colorConversion.coeffG !=
        pPreProcCfg->colorConversion.coeffE) ||
       (pPreProcCfg->colorConversion.coeffH !=
        pPreProcCfg->colorConversion.coeffF) ||
       (pPreProcCfg->colorConversion.LumaOffset != 0))) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR color conversion extend not supported, "
        "invalid coefficient\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (cfg->scaled420Support == EWL_HW_CONFIG_NOT_SUPPORTED &&
      pPreProcCfg->scaledOutput != 0 && pPreProcCfg->scaledOutputFormat == 1) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Scaling to 420SP core not supported\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (cfg->inLoopDSRatio == EWL_HW_CONFIG_ENABLED &&
      ...
      (pPreProcCfg->inputType >= VCENC_YUV422_SEMIPLANAR_888 &&
      pPreProcCfg->inputType < VCENC_YUV420_8BIT_TILE_8_8) ||
      (pPreProcCfg->inputType >= VCENC_YUV420_UV_8BIT_TILE_64_2 &&
      pPreProcCfg->inputType< VCENC_RGB888_24BIT) ||
      (pPreProcCfg->inputType >= VCENC_YVU422_SEMIPLANAR_888 &&
      pPreProcCfg->inputType < VCENC_RGBX8888))) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR in-loop down-scaler not supported by "
        "this format\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (cfg->scaled420Support == EWL_HW_CONFIG_ENABLED &&
      pPreProcCfg->scaledOutput != 0 &&
      (!((((double)vcenc_instance->width / pPreProcCfg->scaledWidth) >= 1) &&
         ...
         (((double)vcenc_instance->height / pPreProcCfg->scaledHeight) <=
          32)))) {
  ...
  if (cfg->prpMirrorSupport == EWL_HW_CONFIG_NOT_SUPPORTED &&
    ...
    APITRACEERR("VCEncSetPreProcessing: ERROR mirror not supported\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (vcenc_instance->pass != 1) {
    /* HW limit: width/height should be 8-pixel aligned for AV1 and VP9 */
    if ((IS_AV1(vcenc_instance->codecFormat) || IS_VP9(vcenc_instance->codecFormat)) &&
        ((vcenc_instance->preProcess.lumWidth & 7) ||
         (vcenc_instance->preProcess.lumHeight & 7))) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR support only 8-pixel aligned "
          "width/height for AV1 and VP9 \n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    ...
    if (origWidth > VCENC_MAX_PP_INPUT_WIDTH) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR Input image width is too big\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
#if 1
  ...
  if ((pPreProcCfg->inputType == VCENC_YUV420_PLANAR_10BIT_I010 ||
       ...
       pPreProcCfg->inputType == VCENC_YVU420_PLANAR_10BIT_P010 ||
       ...
       pPreProcCfg->inputType == VCENC_YUV422P10bWL ||
       pPreProcCfg->inputType ==
           VCENC_YUV420_PLANAR_8BIT_TILE_16_16_PACKED_4) &&
      ((HW_ID_MAJOR_NUMBER(asic->regs.asicHwId) < 4) &&
       HW_PRODUCT_H2(asic->regs.asicHwId))) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid YUV type, HW not support I010 "
        "format.\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if ((pPreProcCfg->inputType > VCENC_YUV420_10BIT_PACKED_Y0L2 &&
       pPreProcCfg->inputType != VCENC_YUV420_SEMIPLANAR_8BIT_TILE_4_4 &&
       ...
       pPreProcCfg->inputType != VCENC_YVU420_PLANAR) &&
      (HW_PRODUCT_SYSTEM60(asic->regs.asicHwId) ||
       HW_PRODUCT_VC9000LE(asic->regs.asicHwId))) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid YUV type, HW not support "
        "format.\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (asic->regs.asicHwId >= MIN_ASIC_ID_WITH_BUILD_ID) {
    ...
    if (!cfg->tile64x4_8bFormatSupport &&
        (pPreProcCfg->inputType == VCENC_YUV420_8BIT_TILE_64_4 ||
         pPreProcCfg->inputType == VCENC_YUV420_UV_8BIT_TILE_64_4)) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR Invalid YUV type, HW not support "
    ...
    if (!cfg->tile32x4_10bFormatSupport &&
        pPreProcCfg->inputType == VCENC_YUV420_10BIT_TILE_32_4) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR Invalid YUV type, HW not support "
          "tile32x4_10b format.\n");
  ...
  if ((pPreProcCfg->inputType == VCENC_YUV420_UV_8BIT_TILE_64_2 ||
    ...
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid rotation, tile FBC format does "
        "not support rotate.\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (pPreProcCfg->inputType == VCENC_YUV420_UV_8BIT_TILE_64_2 &&
      pPreProcCfg->mirror == VCENC_MIRROR_YES) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid mirror, tile FBC format does not "
        "support rotate.\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if ((cfg->superTileXSupport == EWL_HW_CONFIG_NOT_SUPPORTED) &&
      (pPreProcCfg->scanType == VCENC_SUPERTILEX_SCAN)) {
    APITRACEERR("VCEncSetPreProcessing: ERROR superTileX scanType not supported by HW\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (pPreProcCfg->scanType > VCENC_SUPERTILEX_SCAN) {
    APITRACEERR("VCEncSetPreProcessing: ERROR Invalid scan type\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (!((pPreProcCfg->inputType >= VCENC_YUV422_INTERLEAVED_YUYV && pPreProcCfg->inputType <= VCENC_BGR101010) ||
       (pPreProcCfg->inputType >= VCENC_YUV444_XYUV8888 &&
       pPreProcCfg->inputType <= VCENC_BGRX1010102) ||
       pPreProcCfg->inputType >= VCENC_YUV422_INTERLEAVED_YVYU ||
       pPreProcCfg->inputType >= VCENC_YUV422_INTERLEAVED_VYUY) &&
      pPreProcCfg->scanType == VCENC_SUPERTILEX_SCAN) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid scanType, format does not "
        "support SUPERTILEX scan type.\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (pPreProcCfg->scanType == VCENC_SUPERTILEX_SCAN && (pPreProcCfg->rotation != 0)) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid rotation, SUPERTILEX scan "
        "does not support rotate.\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if (pPreProcCfg->scanType == VCENC_SUPERTILEX_SCAN && vcenc_instance->interlaced) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid option, SUPERTILEX scan "
        "does not support interlacedFrame.\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (vcenc_instance->num_tile_rows * vcenc_instance->num_tile_columns > 1 &&
      pPreProcCfg->rotation > VCENC_ROTATE_0) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid rotation, multi-tile does not "
        "support rotate.\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if ((vcenc_instance->interlaced) && (pPreProcCfg->rotation != 0)) {
    APITRACEERR(
        "VCEncSetPreProcessing: ERROR Invalid rotation, interlacedFrame does "
        "not support rotate.\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if ((vcenc_instance->inputLineBuf.inputLineBufEn) &&
    ...
    APITRACE(
        "VCEncSetPreProcessing: ERROR Cannot do rotation, stabilization, and "
  ...
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    ...
    if (pPreProcCfg->inputType == VCENC_YUV420_SEMIPLANAR_101010 &&
        (xOffset % 6) != 0) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR Invalid offset for this format\n");
      return VCENC_INVALID_ARGUMENT;
    }
    ...
    if (((pPreProcCfg->inputType == VCENC_YUV420_SEMIPLANAR_8BIT_TILE_4_4) ||
      ...
      APITRACEERR(
          "VCEncSetPreProcessing: the source size not supported by the 4x4 "
          "tile format\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  ...
  if ((pPreProcCfg->inputType >= VCENC_YUV420_SEMIPLANAR_8BIT_TILE_4_4 &&
      ...
      pPreProcCfg->rotation != 0) {
    APITRACEERR(
        "VCEncSetPreProcessing: rotation not supported by this format\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if ((pPreProcCfg->inputType > VCENC_BGR101010 &&
      ...
      pPreProcCfg->videoStabilization != 0) {
    APITRACEERR(
        "VCEncSetPreProcessing: stabilization not supported by this format\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (pPreProcCfg->constChromaEn) {
    ...
    if ((pPreProcCfg->constCb > maxCh) || (pPreProcCfg->constCr > maxCh)) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR Invalid constant chroma pixel value\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  ...
  if ((IS_HEVC(vcenc_instance->codecFormat)) &&
    ...
    if (((pPreProcCfg->inputType >= VCENC_RGB565) &&
      ...
      if (pPreProcCfg->colorConversion.type != VCENC_RGBTOYUV_BT2020) {
        APITRACEERR(
            "VCEncSetPreProcessing: Invalid color conversion in HDR10\n\n");
        return VCENC_INVALID_ARGUMENT;
      }
    }
    ...
    if ((vcenc_instance->sps->bit_depth_luma_minus8 != 2) ||
      ...
      APITRACEERR(
          "VCEncSetPreProcessing: Invalid bit depth for main10 profile in "
          "HDR10\n\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  ...
  /*Overlay Area check*/
  for (i = 0; i < MAX_OVERLAY_NUM; i++) {
    if (!pPreProcCfg->overlayArea[i].enable) continue;
    if (!cfg->OSDSupport) {
    ...
    if (asic->regs.asicHwId >= MIN_ASIC_ID_WITH_BUILD_ID &&
        i >= cfg->maxOsdNum) {
      APITRACEERR(
          "VCEncSetPreProcessing: OSD region exceed supported number\n\n");
      return VCENC_INVALID_ARGUMENT;
    }
    if (vcenc_instance->pass != 1) {
      int fillingSize = IS_H264(vcenc_instance->codecFormat) ? 16 : 8;
      u32 alignedPicWidth = STRIDE(vcenc_instance->width, fillingSize);//check area with size after filling.
      u32 alignedPicHeight = STRIDE(vcenc_instance->height, fillingSize);
      if (pPreProcCfg->overlayArea[i].xoffset +
                  pPreProcCfg->overlayArea[i].cropWidth >
              alignedPicWidth ||
          pPreProcCfg->overlayArea[i].yoffset +
                  pPreProcCfg->overlayArea[i].cropHeight >
              alignedPicHeight) {
        APITRACEERR("VCEncSetPreProcessing: ERROR Invalid overlay%2d area\n\n", i + 1);
        return VCENC_INVALID_ARGUMENT;
      }

      if ((pPreProcCfg->overlayArea[i].enable != 0) &&
	  	  (pPreProcCfg->overlayArea[i].cropWidth == 0 ||
          pPreProcCfg->overlayArea[i].cropHeight == 0)) {
        APITRACEERR("VCEncSetPreProcessing: ERROR Invalid overlay%2d size\n\n", i + 1);
        return VCENC_INVALID_ARGUMENT;
      }

      if (pPreProcCfg->overlayArea[i].cropXoffset +
                  pPreProcCfg->overlayArea[i].cropWidth >
              pPreProcCfg->overlayArea[i].width ||
          pPreProcCfg->overlayArea[i].cropYoffset +
                  pPreProcCfg->overlayArea[i].cropHeight >
              pPreProcCfg->overlayArea[i].height) {
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR Invalid overlay%2d cropping size\n\n", i + 1);
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].format == 2 &&
        ...
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR Invalid overlay%2d cropping width for "
            "bitmap \n\n", i + 1);
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (osd_overlap(pPreProcCfg, i, vcenc_instance->codecFormat)) {
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR overlay%2d area overlapping \n\n", i + 1);
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 &&
        ...
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR Super tile only support ARGB8888 "
            "format \n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 && i != 0) {
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR Super tile only support for channel "
            "1 \n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if ((pPreProcCfg->overlayArea[i].superTile == 0 &&
          ...
          (pPreProcCfg->overlayArea[i].scaleHeight !=
           ...
           i != 0)) {
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR Up scale only work for special usage "
            "\n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 &&
        ...
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR super tile cropping offset must be "
            "64 aligned \n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 &&
          (pPreProcCfg->overlayArea[i].scaleWidth <
           ...
           pPreProcCfg->overlayArea[i].scaleHeight <
               pPreProcCfg->overlayArea[i].cropHeight)) {
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR osd only support upscaling but no "
            "downscaling \n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 &&
        ...
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR osd upscaling width or height "
            "constraint \n\n");
        return VCENC_INVALID_ARGUMENT;
      }
      ...
      if (pPreProcCfg->overlayArea[i].superTile != 0 &&
        ...
        APITRACEERR(
            "VCEncSetPreProcessing: ERROR osd upscaling size has to be less "
      ...
      switch (pPreProcCfg->overlayArea[i].format) {
        case 0:
          if (pPreProcCfg->overlayArea[i].superTile != 0)
            strideError =
                pPreProcCfg->overlayArea[i].Ystride <
                STRIDE(pPreProcCfg->overlayArea[i].width, 64) * 64 * 4;
          else
            strideError = pPreProcCfg->overlayArea[i].Ystride <
                          pPreProcCfg->overlayArea[i].width * 4;
          break;
        case 1:
          strideError = pPreProcCfg->overlayArea[i].Ystride <
                        pPreProcCfg->overlayArea[i].width;
          strideError += pPreProcCfg->overlayArea[i].UVstride <
                         pPreProcCfg->overlayArea[i].width;
          break;
        case 2:
          strideError = pPreProcCfg->overlayArea[i].Ystride <
                        pPreProcCfg->overlayArea[i].width / 8;
          break;
      }
      if (strideError) {
  ...
  for (i = 0; i < MAX_MOSAIC_NUM; i++) {
    ...
    int mosfuse = cfg->mosaicVersion;
    if(mosfuse == 1) mbSize = 8 * (1 << pPreProcCfg->mosSizeIndex);
    ...
    if (asic->regs.asicHwId >= MIN_ASIC_ID_WITH_BUILD_ID &&
        i >= cfg->maxMosaicNum) {
      APITRACEERR(
          "VCEncSetPreProcessing: Mosaic region exceed supported number\n\n");
      return VCENC_INVALID_ARGUMENT;
    }
    u32 alignedPicWidth = STRIDE(vcenc_instance->width, mbSize);
    u32 alignedPicHeight = STRIDE(vcenc_instance->height, mbSize);
    if (pPreProcCfg->mosXoffset[i] + pPreProcCfg->mosWidth[i] >
            alignedPicWidth ||
        pPreProcCfg->mosYoffset[i] + pPreProcCfg->mosHeight[i] >
            alignedPicHeight) {
      APITRACEERR("VCEncSetPreProcessing: ERROR Invalid mosaic area\n\n");
      return VCENC_INVALID_ARGUMENT;
    }

    if (pPreProcCfg->mosXoffset[i] % mbSize ||
        pPreProcCfg->mosYoffset[i] % mbSize ||
        pPreProcCfg->mosWidth[i] % mbSize ||
        pPreProcCfg->mosHeight[i] % mbSize) {
      APITRACEERR(
          "VCEncSetPreProcessing: ERROR mosaic region should be ctb/mb "
          "alligned \n\n");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  ...
  /* OSDMap checker */
  if (pPreProcCfg->osdMapEnable) {
    u32 osd_chan = 0, osdmap_chan = 0;
    if (!cfg->OSDMapSupport) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap not supported\n");
      return VCENC_INVALID_ARGUMENT;
    }

    if (pPreProcCfg->osdMapBlockSize != 8 && pPreProcCfg->osdMapBlockSize != 4) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap don't support block size %d\n", pPreProcCfg->osdMapBlockSize);
      return VCENC_INVALID_ARGUMENT;
    }

    if (!cfg->OSDMapVersion && pPreProcCfg->osdMapBlockSize == 4) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap version [%d] don't support block size %d\n", cfg->OSDMapVersion, pPreProcCfg->osdMapBlockSize);
      return VCENC_INVALID_ARGUMENT;
    }

    if (pPreProcCfg->overlayArea[0].superTile) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap conflict with overlay superTile\n");
      return VCENC_INVALID_ARGUMENT;
    }

    if (pPreProcCfg->osdMapStride < ((vcenc_instance->width + (pPreProcCfg->osdMapBlockSize * 2 - 1)) >> (pPreProcCfg->osdMapBlockSize == 8 ? 4 : 3))) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap invalid stride\n");
      return VCENC_INVALID_ARGUMENT;
    }

    if (pPreProcCfg->osdMapStride % 16 != 0) {
      APITRACEERR("VCEncSetPreProcessing: OSDMap stride must be 16 aligned.\n");
      return VCENC_INVALID_ARGUMENT;
    }

    for (i = 0; i < MAX_OVERLAY_NUM; i++) {
      if (pPreProcCfg->overlayArea[i].enable | pPreProcCfg->mosEnable[i]) {
        osd_chan++;
      }
      if (pPreProcCfg->osdMapAlpha[i]) {
        osdmap_chan++;
      }
    }
    if (osd_chan + osdmap_chan > MAX_OVERLAY_NUM) {
      APITRACEERR("VCEncSetPreProcessing: The number of colors set for osdmap exceeds the available channels. Overlay and"
  ...
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    if (pPreProcCfg->videoStabilization == 0) {
      pp_tmp.horOffsetSrc[tileId] =
          (tileId == 0) ? pPreProcCfg->xOffset
                        : pPreProcCfg->tileExtra[tileId - 1].xOffset;
      pp_tmp.verOffsetSrc[tileId] =
          (tileId == 0) ? pPreProcCfg->yOffset
                        : pPreProcCfg->tileExtra[tileId - 1].yOffset;
    } else {
    ...
    pp_tmp.lumWidthSrc[tileId] =
        (tileId == 0) ? pPreProcCfg->origWidth
                      : pPreProcCfg->tileExtra[tileId - 1].origWidth;
    pp_tmp.lumHeightSrc[tileId] =
        (tileId == 0) ? pPreProcCfg->origHeight
                      : pPreProcCfg->tileExtra[tileId - 1].origHeight;
  }
  //VCENC_CHROMA_IDC_422: HW TODO
  pp_tmp.subsample_x =
      (vcenc_instance->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_420 ||
       vcenc_instance->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_422)
          ? 1
          : 0;
  pp_tmp.subsample_y =
      (vcenc_instance->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_420) ? 1
                                                                         : 0;
  ...
  pp_tmp.scanType = pPreProcCfg->scanType;
  pp_tmp.videoStab = (pPreProcCfg->videoStabilization != 0) ? 1 : 0;
  pp_tmp.alignLumWidth = pPreProcCfg->alignmentWidth;
  pp_tmp.alignLumHeight = pPreProcCfg->alignmentHeight;
  ...
  if (EncPreProcessCheck(&pp_tmp, vcenc_instance->num_tile_columns) !=
      ENCHW_OK) {
    APITRACEERR("VCEncSetPreProcessing: ERROR Invalid cropping values\n");
    return VCENC_INVALID_ARGUMENT;
  }

  VCEncSetCropOffset(vcenc_instance, &pp_tmp);
...
#ifdef VIDEOSTAB_ENABLED
    VSAlgInit(&vcenc_instance->vsSwData, pp_tmp.lumWidthSrc[0], heightSrc,
              width, height);

    VSAlgGetResult(&vcenc_instance->vsSwData, &pp_tmp.horOffsetSrc[0],
  ...
  vcenc_instance->asic.regs.ufbcMode = vcenc_instance->ufbcParam.mode;
  ...
  pp_tmp.mosSizeIndex = pPreProcCfg->mosSizeIndex;
  asic->regs.mosSizeIndex = pPreProcCfg->mosSizeIndex;
  /* OSD map parameters */
  pp_tmp.osdMapEnable = pPreProcCfg->osdMapEnable;
  pp_tmp.osdMapStride = pPreProcCfg->osdMapStride;
  pp_tmp.osdMapBlockSize = pPreProcCfg->osdMapBlockSize;
  for (i = 0; i < MAX_OSDMAP_COLOR_NUM; i++) {
    pp_tmp.osdMapAlpha[i] = pPreProcCfg->osdMapAlpha[i];
    pp_tmp.osdMapY[i] = pPreProcCfg->osdMapY[i];
    pp_tmp.osdMapU[i] = pPreProcCfg->osdMapU[i];
    pp_tmp.osdMapV[i] = pPreProcCfg->osdMapV[i];
  }
  ...
  if (vcenc_instance->pass == 2) {
    ...
    VCEncRet ret =
        VCEncSetPreProcessing(vcenc_instance->lookahead.priv_inst, &new_cfg);
    if (ret != VCENC_OK) {
...
VCEncRet VCEncGetPreProcessing(VCEncInst inst,
  ...
  pPreProcCfg->scanType = (VCEncPictureType)pPP->scanType;
  ...
  pPreProcCfg->mosSizeIndex = pPP->mosSizeIndex;

  /* Get OSD map parameters */
  pPreProcCfg->osdMapEnable = pPP->osdMapEnable;
  pPreProcCfg->osdMapStride = pPP->osdMapStride;
  pPreProcCfg->osdMapBlockSize = pPP->osdMapBlockSize;
  for (i = 0; i < MAX_OSDMAP_COLOR_NUM; i++) {
    pPreProcCfg->osdMapAlpha[i] = pPP->osdMapAlpha[i];
    pPreProcCfg->osdMapY[i] = pPP->osdMapY[i];
    pPreProcCfg->osdMapU[i] = pPP->osdMapU[i];
    pPreProcCfg->osdMapV[i] = pPP->osdMapV[i];
  }
...
VCEncRet VCEncCreateNewPPS(VCEncInst inst, const VCEncPPSCfg *pPPSCfg,
  ...
  if (c == NULL) {
    APITRACEERR("VCEncCreateNewPPS: ERROR get_container return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
  p0 = (struct pps *)get_parameter_set(c, PPS_NUT, 0);
  if (p0 == NULL) {
    APITRACEERR("VCEncCreateNewPPS: ERROR get_parameter_set return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
  /*get ppsId*/
  ppsId = 0;
  ...
  *newPPSId = ppsId;
  ...
  p = (struct pps *)create_parameter_set(PPS_NUT);
  if (p == NULL) {
    APITRACEERR("VCEncCreateNewPPS: ERROR create_parameter_set return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
...
VCEncRet VCEncModifyOldPPS(VCEncInst inst, const VCEncPPSCfg *pPPSCfg,
  ...
  c = get_container(vcenc_instance);
  if (c == NULL) {
    APITRACEERR("VCEncModifyOldPPS: ERROR get_container return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
...
VCEncRet VCEncGetPPSData(VCEncInst inst, VCEncPPSCfg *pPPSCfg, i32 ppsId) {
  ...
  c = get_container(vcenc_instance);
  if (c == NULL) {
    APITRACEERR("VCEncGetPPSData: ERROR get_container return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
...
VCEncRet VCEncActiveAnotherPPS(VCEncInst inst, i32 ppsId) {
  ...
  c = get_container(vcenc_instance);
  if (c == NULL) {
    APITRACEERR("VCEncActiveAnotherPPS: ERROR get_container return NULL\n");
    return VCENC_INVALID_ARGUMENT;
  }
...
i32 calculate_gop_reorder_frames(struct sps *sps, VCEncGopPicConfig *pcfg,
                                 int gopSize) {
  for (int i = 0; i < gopSize; i++) {
    ...
    if (numReorder > sps->max_num_reorder_pics[pcfg[i].temporalId]) {
      sps->max_num_reorder_pics[pcfg[i].temporalId] =
          MIN(numReorder, sps->max_dec_pic_buffering[pcfg[i].temporalId] - 1);
    }
  }
  return OK;
}
...
------------------------------------------------------------------------------*/
i32 vcenc_calculate_num_reorder_frames(struct vcenc_instance *inst,
                                       const VCEncIn *pEncIn) {
  ...
  for (i = 1; i <= MAX_GOP_SIZE; i++) {
    if (gopCfg->gopCfgOffset[i] == 0 && i > 1) continue;
    calculate_gop_reorder_frames(
        inst->sps, gopCfg->pGopPicCfg + gopCfg->gopCfgOffset[i], i);
  }
  /* update max_num_reorder_pics & max_dec_pic_buffering to full layer param */
  for (int i = 1; i < inst->sps->max_num_sub_layers; i++) {
    inst->sps->max_num_reorder_pics[0] = MAX(
        inst->sps->max_num_reorder_pics[0], inst->sps->max_num_reorder_pics[i]);
    if (inst->sps->max_dec_pic_buffering[0] <
        inst->vps->max_num_reorder_pics[i] + 1)
...
VCEncRet VCEncStrmStart(VCEncInst inst, const VCEncIn *pEncIn,
  ...
  if (pEncIn->gopConfig.idr_interval < vcenc_instance->gdrDuration ||
      (pEncIn->gopConfig.idr_interval == 1 &&
       vcenc_instance->gdrDuration > 0)) {
    APITRACEERR("VCEncStrmStart: ERROR intraPicRate and gdrDuration\n");
    return VCENC_INVALID_ARGUMENT;
  }

  /*Check idr_interval and intraPeriod*/
  if (pEncIn->gopConfig.idr_interval > 0 &&
      pEncIn->gopConfig.idr_interval <= vcenc_instance->intraPeriod) {
    APITRACEERR("VCEncStrmStart: ERROR intraPicRate and intraPeriod\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  vcenc_instance->nextIdrCnt = (vcenc_instance->enableLeadingPictures ? pEncIn->gopSize - 1 : 0);
  vcenc_instance->intraPeriod = 0;
  ...
  if (vcenc_instance->pass == 1) {
    ...
    SET_MEM_USAGE(vcenc_instance->lookahead.internal_mem.mem.mem_type,
                  EWL_MEM_USAGE_OUT_STRM, vcenc_instance->asic.secure_mode);
    ...
    if (vcenc_instance->lookahead.internal_mem.mem.busAddress == 0) {
      ...
      if (ret != EWL_OK) return VCENC_ERROR;
    }
    ...
    vcenc_instance->stream.stream_bus =
        vcenc_instance->lookahead.internal_mem.busOutBuf =
            vcenc_instance->lookahead.internal_mem.mem.busAddress;
    vcenc_instance->stream.size =
        vcenc_instance->lookahead.internal_mem.outBufSize =
  ...
  if (pEncIn->hashType && IS_AV1(vcenc_instance->codecFormat)) {
    APITRACEERR("VCEncStrmStart: ERROR hashType is not supported for AV1\n");
    return VCENC_INVALID_ARGUMENT;
  }
  hash_init(&vcenc_instance->hashctx, pEncIn->hashType);

  vcenc_instance->temp_size = 1024 * 10;
  vcenc_instance->temp_buffer = vcenc_instance->stream.stream +
                                vcenc_instance->stream.size -
                                vcenc_instance->temp_size;
  ...
  if ((v == NULL) || (s == NULL) || (p == NULL)) {
    Error(2, ERR, "get_parameter_set() fails");
    return NOK;
  }

  i32 temp_profile = vcenc_instance->profile;
  vcenc_instance->profile = VCEncGetProfile(vcenc_instance, 0, pEncIn);
  if (temp_profile != vcenc_instance->profile) {
    APITRACEWRN("Instance Profile had been changed between set param.\n");
  }
  if ((vcenc_instance->profile != vcenc_instance->adaptive_profile)
        && (vcenc_instance->adaptive_profile != -1)) {
    APITRACEWRN("Adaptive profile updated in stream start encode.\n");
  }
  ...
  if ((pEncIn->gopConfig.size == 1) &&
      (pEncIn->gopConfig.pGopPicCfg[0].temporalId > 0)) {
  ...
  VCEncSpsSetVuiAspectRatio(s, vcenc_instance->sarWidth,
  ...
  if (vcenc_instance->vuiVideoSignalTypePresentFlag)
    VCEncSpsSetVuiSignalType(
        s, vcenc_instance->vuiVideoSignalTypePresentFlag,
  ...
  /* Init SEI */
  HevcInitSei(
      &vcenc_instance->rateControl.sei, s->streamMode == ASIC_VCENC_BYTE_STREAM,
  ...
  if (IS_H264(vcenc_instance->codecFormat) && s->max_num_sub_layers > 1 &&
    ...
    H264NalUnitHdr(b, 0, H264_SEI,
                   (v->streamMode == VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO);
  ...
  if (!IS_HEVC(vcenc_instance->codecFormat)) cnt_ref_pic_set(c, s);
  ...
  if (vcenc_instance->pass == 2) {
    ...
    if (vcenc_instance->num_tile_columns > 1) {
      ...
      if (encOut.tileExtra == NULL) goto out;
    } else
      encOut.tileExtra = NULL;
    encIn.gopConfig.pGopPicCfg = pEncIn->gopConfig.pGopPicCfgPass1;
    VCEncRet ret =
        VCEncStrmStart(vcenc_instance->lookahead.priv_inst, &encIn, &encOut);
    if (ret != VCENC_OK) {
    ...
    vcenc_instance->nextGopSize = vcenc_instance->lookahead.encIn.gopSize;
    ...
    vcenc_instance->encInFor1pass.gopSize =
        (vcenc_instance->gopSize ? vcenc_instance->gopSize : vcenc_instance->rateControl.hieQpDeltaEnable ? 4 : 1);
    vcenc_instance->nextGopSize =
        vcenc_instance->encInFor1pass.gopSize;  //init next gop
  ...
  if ((IS_HEVC(vcenc_instance->codecFormat) ||
    ...
    Enc_add_stream_header_trace(
        vcenc_instance->stream.stream - pEncOut->streamSize,
        pEncOut->streamSize);
  }
#endif
...
/*------------------------------------------------------------------------------
  vcenc_ref_pic_sets
------------------------------------------------------------------------------*/
i32 vcenc_replace_rps(struct vcenc_instance *vcenc_instance,
                      const VCEncIn *pEncIn, VCEncGopPicConfig *cfg,
                      i32 rps_id) {
#define RPS_TEMP_BUF_SIZE (RPS_SET_BUFF_SIZE + 60)
  ...
  /* Note: cppcheck reports "error: Address of local auto-variable assigned to a function parameter" here,
   * it seems that vcenc_instance->temp_buffer is useless here.
   */
  vcenc_instance->temp_buffer = temp_buf; //cppcheck-suppress inconclusive
  vcenc_instance->temp_size = RPS_TEMP_BUF_SIZE;
  vcenc_instance->temp_bufferBusAddress = 0;
  if (pEncIn) gopCfg = &pEncIn->gopConfig;
...
struct sw_picture *get_ref_picture(struct vcenc_instance *vcenc_instance,
                                   VCEncGopPicConfig *cfg, i32 idx,
                                   bool bRecovery, bool *error) {
  struct node *n;
      ...
      */
      if (bRecovery && (!p->long_term_flag) && (p->encOrderInGop == 0) &&
        ...
        if (!ref)
          ...
          if (ABS(iDelta) < ABS(lastDelta)) ref = p;
        }
      }
    }
  }
...
static i32 check_multi_references(struct vcenc_instance *vcenc_instance,
                                  struct sps *s, VCEncGopPicConfig *pConfig,
                                  const VCEncIn *pEncIn) {
  struct sw_picture *p;
  //  struct container *c = get_container(vcenc_instance);
  i32 i, j;
  ...
  for (i = 0; i < (i32)cfg->numRefPics; i++) {
    bool iErr;
    if (IS_LONG_TERM_REF_DELTAPOC(
            cfg->refPics[i].ref_pic))  // skip long term reference check
      continue;
    i32 refPoc = vcenc_instance->poc + cfg->refPics[i].ref_pic;
    p = get_ref_picture(vcenc_instance, cfg, i, (pEncIn->gopPicIdx == 0),
                        &iErr);
    ...
    if (!p)
      ...
      for (j = 0; j < (i32)cfg->numRefPics; j++) {
        ...
        if ((j != i) && (cfg->refPics[j].ref_pic == cfg->refPics[i].ref_pic)) {
          if (cfg->refPics[i].used_by_cur) cfg->refPics[j].used_by_cur = 1;
          remove = HANTRO_TRUE;
          break;
        }
      }
  ...
  /* numRefCnt should be vaild && usedByCurrent */
  i32 numRefUsedByCur = 0;
  for (i = 0; i < (i32)cfg->numRefPics; i++)
  {
    if (1 == cfg->refPics[i].used_by_cur)
      numRefUsedByCur ++ ;
  }
  ...
  if (error) {
    if (cfg->numRefPics == 0) {
      ...
      vcenc_instance->rps_id = rps_id;
      if (!IS_H264(vcenc_instance->codecFormat) &&
          !(HW_PRODUCT_SYSTEM60(vcenc_instance->asic.regs.asicHwId) ||
            HW_PRODUCT_VC9000LE(vcenc_instance->asic.regs.asicHwId)))
        vcenc_instance->RpsInSliceHeader = HANTRO_TRUE;
    }
  }

  return numRefUsedByCur;
}

static VCEncPictureCodingType check_references(
    struct vcenc_instance *vcenc_instance, const VCEncIn *pEncIn, struct sps *s,
    VCEncGopPicConfig *cfg, VCEncPictureCodingType codingType) {
  u32 i;
  ...
  if (1) {
    ...
    else if (numRef == 1 && pEncIn->poc >= 0) /* allow 1-ref B frames for leading pictures */
...
static u32 h264_refIdc_val(const VCEncIn *pEncIn, struct sw_picture *pic) {
  ...
  for (int i = 1; i < pEncIn->gopConfig.special_size; i++) {
    if (pEncIn->gopConfig.special_size != 0 && (pEncIn->poc >= 0) &&
        (pEncIn->poc % pEncIn->gopConfig.pGopPicSpecialCfg[i].i32Interval ==
         0)) {
...
--------------------------------------------------------------------------------*/
static int h264_ref_in_use(int delta_poc, int poc, struct rps *r,
                           const i32 *long_term_ref_pic_poc) {
  struct ref_pic *p;
  int i;
  ...
  for (i = 0; i < r->num_negative_pics; i++) {
    p = &r->ref_pic_s0[i];
    if (delta_poc == p->delta_poc) return 1;
  }
...
--------------------------------------------------------------------------------*/
bool h264_mmo_collect(struct vcenc_instance *vcenc_instance,
                      struct sw_picture *pic, const VCEncIn *pEncIn,
                      struct rps *r_next, int delta_poc) {
  struct container *c = get_container(vcenc_instance);
  ...
  if (pEncIn->flexRefsEnable) {
    ...
    if (id == 255)
      id = pic->sps->num_short_term_ref_pic_sets - 1 -
           ((pEncIn->gopConfig.special_size == 0)
                ? 1
                : pEncIn->gopConfig.special_size);
    if (-1 != pEncIn->i8SpecialRpsIdx_next)
      id = pic->sps->num_short_term_ref_pic_sets - 1 -
           pEncIn->gopConfig.special_size + pEncIn->i8SpecialRpsIdx_next;
  ...
  if (pic->sliceInst->type == I_SLICE) {
    ...
    pic->curLongTermIdx =
        (pic->markCurrentLongTerm ? (pEncIn->u8IdxEncodedAsLTR - 1) : -1);
  ...
  for (i = 0; i < r->before_cnt + r->after_cnt + r->lt_current_cnt; i++) {
    ...
    int inuse = h264_ref_in_use(
        ref->poc - poc2, ref->poc, r2,
        pEncIn->long_term_ref_pic); /* whether used as ref of next pic */
    if (inuse == 0) {
      vcenc_instance->h264_mmo_unref[vcenc_instance->h264_mmo_nops] =
          ref->frameNum;
      vcenc_instance->h264_mmo_unref_ext[vcenc_instance->h264_mmo_nops] =
          ref->frameNumExt;
      bool is_long = (h264_ref_in_use(ref->poc - pic->poc, ref->poc, r,
                                      pEncIn->long_term_ref_pic) >= 2);
      vcenc_instance->h264_mmo_long_term_flag[vcenc_instance->h264_mmo_nops] =
          is_long;
      vcenc_instance->h264_mmo_ltIdx[vcenc_instance->h264_mmo_nops++] =
          -1;  // unref short/long
    } else if (inuse > 1 &&
               h264_ref_in_use(ref->poc - pic->poc, ref->poc, r,
                               pEncIn->long_term_ref_pic) == 1 &&
  ...
  for (i = 0; i < r->lt_follow_cnt; i++) {
    ...
    int inuse = h264_ref_in_use(ref->poc - poc2, ref->poc, r2,
                                pEncIn->long_term_ref_pic);
    if (inuse == 0) {
      vcenc_instance->h264_mmo_unref[vcenc_instance->h264_mmo_nops] =
          ref->frameNum;
      vcenc_instance->h264_mmo_unref_ext[vcenc_instance->h264_mmo_nops] =
  ...
  if (vcenc_instance->svctEnable == 1) {
    if (pEncIn->gopCurrPicConfig.temporalId !=
        vcenc_instance->sps->max_num_sub_layers - 1) {
      pic->nalRefIdc = 1;
    } else {
      pic->nalRefIdc = 0;
    }
    ...
    pic->nalRefIdc =
        h264_ref_in_use(pic->poc - poc2, pic->poc, r2, long_term_ref_pic_poc_2);
  }
  ...
  if (vcenc_instance->svctEnable == 1) {
    if (pic->nalRefIdc == 0 && vcenc_instance->prefixNalSvcFlag) {
      pic->prefixNalSvcFlag = 1;
    } else {
      pic->prefixNalSvcFlag = 0;
    }
  }
  return HANTRO_TRUE;
}
...
/*------------------------------------------------------------------------------
collect DPB update on skipped frames for H.264 MMO
--------------------------------------------------------------------------------*/
bool h264_dpb_update(struct container *c, struct vcenc_instance *vcenc_instance, VCEncIn *pEncIn)
{
  u32 old_rps_id = vcenc_instance->skipGopRpsId;
  i32 old_poc = vcenc_instance->skipGopPoc;
  struct rps *rps_old = (struct rps *)get_parameter_set(c, RPS, old_rps_id);
  if(!rps_old) return false;
  struct sw_picture *p;
  struct node *n;
  int inuse;
  ...
  {
    p = (struct sw_picture *)n;
    if(p->reference || p->poc == UNUSED_POC)
      continue;
    inuse = h264_ref_in_use(p->poc - old_poc, p->poc, rps_old, pEncIn->long_term_ref_pic);
    if(inuse) {
      vcenc_instance->h264_mmo_unref[vcenc_instance->h264_mmo_nops] =
        p->frameNum;
      vcenc_instance->h264_mmo_unref_ext[vcenc_instance->h264_mmo_nops] =
...
/*------------------------------------------------------------------------------
Extent collect mmo ref frames by external info for H.264 MMO
--------------------------------------------------------------------------------*/
bool h264_ext_mmo_collect(struct vcenc_instance *vcenc_instance,
                          struct sw_picture *pic, const VCEncIn *pEncIn,
                          struct rps *r_next, int delta_poc) {
  struct container *c = get_container(vcenc_instance);
  if (!c) return HANTRO_FALSE;
...
--------------------------------------------------------------------------------*/
void h264_mmo_mark_unref(regValues_s *regs, int frame_num, int ltrflag,
                         int ltIdx, int cnt[2], struct sw_picture *pic) {
  int i;
...
--------------------------------------------------------------------------------*/
void h264_mmo_mark_unref_ext(regValues_s *regs, int frame_num, int ltrflag,
                             int ltIdx, int cnt[2],
  ...
  // search frame_num in current reference list
  for (i = 0;
       i < pEncExtParaIn->params.h264Para.num_ref_idx_l0_active_minus1 + 1;
       i++) {
  ...
  if (pEncExtParaIn->params.h264Para.slice_type == SLICE_TYPE_B) {
    for (i = 0;
         i < pEncExtParaIn->params.h264Para.num_ref_idx_l1_active_minus1 + 1;
         i++) {
...
static VCEncRet getCtbRcParamsInMultiTiles(struct vcenc_instance *inst,
                                           enum slice_type type) {
  if (inst == NULL) return VCENC_ERROR;
  ...
  pCtbRateCtrl->models[type].x0 = asic->regs.ctbRcModelParam0 =
      ctbRcX0 / inst->num_tile_columns;
  pCtbRateCtrl->models[type].x1 = asic->regs.ctbRcModelParam1 =
      ctbRcX1 / inst->num_tile_columns;
...
static i32 getValidModelsIdx(vcencRateControl_s *rc) {

  i32 idx,idxType,i;

  i32 coreIdx = (rc->frameCnt)%rc->coreNum;
  i32 preMemIdx = rc->ctbRateCtrl.Cur[coreIdx].ctbRcModelsIdx;
  rc->ctbRateCtrl.models[preMemIdx].refCount = rc->ctbRateCtrl.models[preMemIdx].refCount - 1;
  idxType = rc->ctbRateCtrl.models[preMemIdx].idxType;

  idx = -1;
  if (rc->ctbRateCtrl.models[idxType].refCount == 0)
    idx = idxType;
  else{
    for(i = 3; i < (2 + rc->coreNum); i++){
      if (rc->ctbRateCtrl.models[i].refCount == 0){
        idx = i;
        break;
      }
    }
...
static VCEncRet getCtbRcParams(struct vcenc_instance *inst,
                               enum slice_type type) {
  if (inst == NULL) return VCENC_ERROR;
  ...
  if (inst->tiles_enabled_flag) return VCENC_OK;

  i32 idxType = inst->rateControl.predLayerId < 3 ? inst->rateControl.predLayerId : 0;

  i32 idx = getValidModelsIdx(rc);
  if ((idx < 0) || (idx > (1 + rc->coreNum))){
    APITRACEERR("invalid index for ctbrc memory.\n");
    ASSERT(0);
    return VCENC_ERROR;
  }

  rc->ctbRateCtrl.updateModelIdx = idx;

  rc->ctbRateCtrl.models[idx].x0 = asic->regs.ctbRcModelParam0 =
      EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                              HWIF_ENC_CTB_RC_MODEL_PARAM0);

  rc->ctbRateCtrl.models[idx].x1 = asic->regs.ctbRcModelParam1 =
      EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                              HWIF_ENC_CTB_RC_MODEL_PARAM1);

  rc->ctbRateCtrl.models[idx].preFrameMad = asic->regs.prevPicLumMad =
      EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                              HWIF_ENC_PREV_PIC_LUM_MAD);

  rc->ctbRateCtrl.qpSumForRc = asic->regs.ctbQpSumForRc =
      EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
  ...
  rc->ctbRateCtrl.models[idx].started = 1;

  return VCENC_OK;
}

static void onSliceReady(struct vcenc_instance *inst,
                         VCEncSliceReady *slice_callback) {
  if ((!inst) || (!slice_callback)) return;
  ...
  if (inst->sliceReadyCbFunc &&
    ...
    slice_callback->streamBufs.bufLen[0] = asic->regs.outputStrmSize[0];
    slice_callback->streamBufs.bufLen[1] = asic->regs.outputStrmSize[1];
...
VCEncRet VCEncStrmWaitReady(VCEncInst inst, const VCEncIn *pEncIn,
  ...
  do {
    ...
    if (!asic->regs.bVCMDEnable) {
      if ((out = (EWLCoreWaitJob_t *)EWLDequeueCoreOutJob(asic->ewl, waitId)) ==
          NULL)
        break;
      ...
      memcpy(asic->regs.regMirror, out->VCE_reg,
             sizeof(u32) * ASIC_SWREG_AMOUNT);
      EWLPutJobtoPool(asic->ewl, (struct node *)out);
    } else {
      ewl_ret = EncWaitCmdbuf(asic->ewl, (u16)waitId, &status);
      EncGetRegsByCmdbuf(asic->ewl, (u16)waitId, asic->regs.regMirror,
                         asic->dumpRegister);
      if ((asic->regs.sliceNum > 1) && vcenc_instance->sliceReadyCbFunc) {
        slices_rdy = (asic->regs.regMirror[7] >> 17) & 0xFF;
        slice_callback->slicesReady = slices_rdy;
      }
    }
    ...
    if (ewl_ret != EWL_OK && asic->regs.bVCMDEnable) {
      ...
      APITRACEERR(
          "VCEncStrmEncode: ERROR Fatal system error ewl_ret != EWL_OK.\n");
...
#ifdef INTERNAL_TEST
      if (vcenc_instance->cb_try_new_params &&
          ((vcenc_instance->testId == TID_RE_ENCODE_FOR_BUFFER_FULL) ||
          (vcenc_instance->testId == TID_RE_ENCODE_FOR_TIMEOUT))) {
          status = VCEncReEncodeTest(vcenc_instance, vcenc_instance->testId, status);
      }
#endif
      switch (status) {
        ...
        case ASIC_STATUS_UFBC_CFG_ERR:
        case ASIC_STATUS_POLL_SLICEINFO_TIMEOUT:
          APITRACEERR("VCEncStrmEncode: ERROR HW/IRQ timeout\n");
          asic->regs.regValueAXIWritePending =
            EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                    HWIF_ENC_AXI_WRITE_CHANNEL_PENDING);
          asic->regs.regValueAXIReadPending =
            EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                    HWIF_ENC_AXI_READ_CHANNEL_PENDING);
          asic->regs.regValueAXITotalPending =
            EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                    HWIF_ENC_AXI_TOTAL_PENDING);
          vcenc_instance->encodeTimeout = 1;
#ifdef INTERNAL_TEST
          if (vcenc_instance->testId == TID_RE_ENCODE_FOR_TIMEOUT &&
              vcenc_instance->cb_try_new_params) {
            VCEncSetReEncodeTestParameter(vcenc_instance);
          }
#endif
          if (vcenc_instance->asic.regs.bVCMDEnable)
            EWLReleaseCmdbuf(asic->ewl, (u16)waitId);
#ifdef SUPPORT_UFBC
          if (status & ASIC_STATUS_UFBC_DEC_ERR) {
            APITRACEERR("VCEncStrmEncode: ERROR HW ufbc decode error.\n");
            ret = VCENC_HW_UFBC_ERROR;
            break;
         }
          if (status & ASIC_STATUS_UFBC_CFG_ERR) {
            APITRACEERR("VCEncStrmEncode: ERROR HW ufbc config error.\n");
            ret = VCENC_HW_UFBC_ERROR;
            break;
         }
#endif
#ifdef LOW_LATENCY_SLICEINFO_SUPPORT
          if (status & ASIC_STATUS_POLL_SLICEINFO_TIMEOUT) {
            APITRACEERR("VCEncStrmEncode: ERROR HW polling sliceinfo timeout.\n");
            ret = VCENC_HW_POLL_SLICEINFO_TIMEOUT;
            break;
          }
#endif
          ret = VCENC_HW_TIMEOUT;
        ...
        case ASIC_STATUS_SEGMENT_READY:
          sizeTable = &asic->sizeTbl[(vcenc_instance->jobCnt + 1) %
                                     vcenc_instance->parallelCoreNum];
          MemSyncOffset = pEncOut->PreNaluNum * sizeof(u32);
          MemSyncDataSize = asic->sizeTblSize;
          if (EWLSyncMemData(sizeTable, MemSyncOffset, MemSyncDataSize,
                             DEVICE_TO_HOST) != EWL_OK) {
          ...
          if (status & ASIC_STATUS_SEGMENT_READY) {
            ...
            while (vcenc_instance->streamMultiSegment.streamMultiSegmentMode !=
                       0 &&
        ...
        case ASIC_STATUS_BUFF_FULL:
          ...
          ret = VCENC_OK;
          break;
        case ASIC_STATUS_SBI_OUT_OF_SYNC:
        case ASIC_STATUS_SBI_TIMEOUT:
          APITRACEERR("VCEncStrmEncode: ERROR SBI error.\n");
          if (vcenc_instance->asic.regs.bVCMDEnable)
            EWLReleaseCmdbuf(asic->ewl, (u16)waitId);
          ret = VCENC_SBI_ERROR;
          break;
        ...
        case ASIC_STATUS_FRAME_READY:
          ...
          pEncOut->sliceHeaderSize = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_SLICE_HEADER_SIZE);
          ...
          if (IS_AV1(vcenc_instance->codecFormat)) {
            ...
            pEncOut->PostDataSize += temporalDelimiterSize;
          }
#endif
          ...
          u32 *sizeTbl = vcenc_instance->asic.regs.tiles_enabled_flag
                             ...
                             : asic->sizeTbl[(vcenc_instance->jobCnt + 1) %
                                   ...
                                   .virtualAddress;
          u32 naluIndex =
              vcenc_instance->asic.regs.tiles_enabled_flag ? tileIndex : 0;
          u32 numNalus = naluIndex == 0
                             ? pEncOut->numNalus
                             : pEncOut->tileExtra[naluIndex - 1].numNalus;
          if (sizeTbl) {
            ...
            while (HANTRO_TRUE) {
              ...
              APIPRINT(vcenc_instance->verbose, "POC %3d slice %d size=%u\n",
                       vcenc_instance->poc, sliceNum, sliceBytes);
              sliceNum++;
              pEncOut->maxSliceStreamSize =
                  MAX(pEncOut->maxSliceStreamSize, sliceBytes);
              if (vcenc_instance->tiles_enabled_flag)
                i++;
              else
                n = n->next;
          ...
          u32 hashval = EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                                HWIF_ENC_HASH_VAL);
          u32 hashoffset = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_HASH_OFFSET);
          ...
          hash_init(&vcenc_instance->hashctx,
                    vcenc_instance->hashctx.hash_type);
          ...
          vcenc_instance->rateControl.PBFrame4NRdCost =
              asic->regs.PBFrame4NRdCost = EncAsicGetRegisterValue(
                  asic->ewl, asic->regs.regMirror, HWIF_ENC_PBFRAME4NRDCOST);

          //Trail related statics
          if (vcenc_instance->ctbRcTrailStrengthMax) {
            asic->regs.movingCu8Num =
                EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                        HWIF_ENC_NUM_MOVINGCU8);
            asic->regs.trailAreaUnit16 =
                EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                        HWIF_ENC_TRAILAREA_UNIT16);
          }

          if (asic->regs.asicCfg->modSubjPrefer) {
            asic->regs.intraBiasExtraBits =
              EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                      HWIF_ENC_INTRABIAS_EXTRA_BITS);
          }

          // video stabilization
          asic->regs.stabMotionMin = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MINIMUM);
          asic->regs.stabMotionSum = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MOTION_SUM);
          asic->regs.stabGmvX = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_GMVX);
          asic->regs.stabGmvY = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_GMVY);
          asic->regs.stabMatrix1 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX1);
          asic->regs.stabMatrix2 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX2);
          asic->regs.stabMatrix3 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX3);
          asic->regs.stabMatrix4 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX4);
          asic->regs.stabMatrix5 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX5);
          asic->regs.stabMatrix6 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX6);
          asic->regs.stabMatrix7 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX7);
          asic->regs.stabMatrix8 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX8);
          asic->regs.stabMatrix9 = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_STAB_MATRIX9);

          //CTB_RC
          asic->regs.ctbBitsMin = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_CTBBITSMIN);
          asic->regs.ctbBitsMax = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_CTBBITSMAX);
          asic->regs.totalLcuBits = EncAsicGetRegisterValue(
              asic->ewl, asic->regs.regMirror, HWIF_ENC_TOTALLCUBITS);
          ...
          if (vcenc_instance->pass != 1) {
            ...
            i32 qp = EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                             HWIF_ENC_PIC_QP);
            i32 poc = EncAsicGetRegisterValue(asic->ewl, asic->regs.regMirror,
                                              HWIF_ENC_POC);
          ...
          if (s->long_term_ref_pics_present_flag) {
            if (0 != pEncIn->u8IdxEncodedAsLTR) {
              if (IS_H264(vcenc_instance->codecFormat)) {
                ...
                i32 old_poc =
                    pEncIn->long_term_ref_pic[(pEncIn->u8IdxEncodedAsLTR - 1)%VCENC_MAX_LT_REF_FRAMES];
  ...
  } while (status & (ASIC_STATUS_SLICE_READY | ASIC_STATUS_LINE_BUFFER_DONE |
                     ASIC_STATUS_SEGMENT_READY));
...
void useExtPara(const VCEncExtParaIn *pEncExtParaIn, asicData_s *asic,
                VCEncVideoCodecFormat codec_format, struct vcenc_instance *vcenc_instance) {
  i32 i, j;
  const VCEncAv1Para *param = NULL;
  ...
  regs->recon_luma_compress_tbl_base =
      pEncExtParaIn->recon.compressTblReconLuma;
  regs->recon_chroma_compress_tbl_base =
      pEncExtParaIn->recon.compressTblReconChroma;
  ...
  regs->cuInfoDataBase =
      pEncExtParaIn->recon.cuInfoMemRecon + asic->cuInfoTableSize;
  ...
  case VCENC_VIDEO_CODEC_H264:
    ...
    if (pEncExtParaIn->params.h264Para.slice_type == SLICE_TYPE_I ||
      ...
      if ((pEncExtParaIn->params.h264Para.idr_pic_flag)) {
        ...
        regs->markCurrentLongTerm =
            !!(pEncExtParaIn->recon.flags & LONG_TERM_REFERENCE);
      } else {
        //curLongTermIdx
        regs->markCurrentLongTerm =
            !!(pEncExtParaIn->recon.flags & LONG_TERM_REFERENCE);
    ...
    if ((pEncExtParaIn->params.h264Para.slice_type == SLICE_TYPE_P) ||
      ...
      for (i = 0;
           i <
           (pEncExtParaIn->params.h264Para.num_ref_idx_l0_active_minus1 + 1);
           i++) {
        regs->l0_delta_framenum[i] =
            regs->frameNum - pEncExtParaIn->reflist0[i].frame_num;
      ...
      for (i = 0;
        ...
        regs->pRefPic_recon_l0[1][i] =
            pEncExtParaIn->reflist0[i].busReconChromaUV;
        regs->pRefPic_recon_l0_4n[i] = pEncExtParaIn->reflist0[i].reconLuma_4n;

        regs->l0_delta_poc[i] =
            pEncExtParaIn->reflist0[i].poc - pEncExtParaIn->recon.poc;
    ...
    if (pEncExtParaIn->params.h264Para.slice_type == SLICE_TYPE_B) {
      ...
      for (i = 0;
           ...
           i++) {
        regs->l1_delta_framenum[i] =
            regs->frameNum - pEncExtParaIn->reflist1[i].frame_num;
      ...
      for (i = 0;
        ...
        regs->pRefPic_recon_l1[1][i] =
            pEncExtParaIn->reflist1[i].busReconChromaUV;
        regs->pRefPic_recon_l1_4n[i] = pEncExtParaIn->reflist1[i].reconLuma_4n;

        regs->l1_delta_poc[i] =
            pEncExtParaIn->reflist1[i].poc - pEncExtParaIn->recon.poc;
    ...
    if (IS_H264(vcenc_instance->codecFormat) && regs->nalRefIdc &&
      ...
      if (!vcenc_instance->flexRefsEnable) {
        for (i = 0; i < vcenc_instance->h264_mmo_nops; i++)
          h264_mmo_mark_unref_ext(
              &asic->regs, vcenc_instance->h264_mmo_unref_ext[i],
        ...
        for (i = 0; i < vcenc_instance->h264_mmo_nops; i++)
          h264_mmo_mark_unref_ext(
              &asic->regs, vcenc_instance->h264_mmo_unref[i],
              vcenc_instance->h264_mmo_long_term_flag[i],
              vcenc_instance->h264_mmo_ltIdx[i], cnt, pEncExtParaIn);
        vcenc_instance->h264_mmo_nops = 0;
      }
  ...
  case VCENC_VIDEO_CODEC_HEVC:
    ...
    if ((pEncExtParaIn->params.hevcPara.slice_type == SLICE_TYPE_P) ||
      ...
      for (i = 0;
           i <
           (pEncExtParaIn->params.hevcPara.num_ref_idx_l0_active_minus1 + 1);
           i++) {
        regs->l0_delta_framenum[i] =
            regs->frameNum - pEncExtParaIn->reflist0[i].frame_num;
        regs->l0_referenceLongTermIdx[i] = pEncExtParaIn->reflist0[i].frame_idx;
    ...
    if (pEncExtParaIn->params.hevcPara.slice_type == SLICE_TYPE_B) {
      ...
      for (i = 0;
           i <
           (pEncExtParaIn->params.hevcPara.num_ref_idx_l1_active_minus1 + 1);
           i++) {
        regs->l1_delta_framenum[i] =
            regs->frameNum - pEncExtParaIn->reflist1[i].frame_num;
        regs->l1_referenceLongTermIdx[i] = pEncExtParaIn->reflist1[i].frame_idx;
    ...
    if (regs->num_long_term_pics) {
      ...
      if(regs->l1_long_term_flag[0]) {
          regs->ref_pic_list_modification_flag_l1 = 1;
          regs->list_entry_l1[0] = 1;
          regs->list_entry_l1[1] = 0;
      }
    }
    ...
    if(!vcenc_instance->flexRefsEnable) {
      ...
      ASSERT(regs->rps_neg_pic_num + regs->rps_pos_pic_num <=
      ...
      for (i = 0; i < (i32)(regs->rps_neg_pic_num + regs->rps_pos_pic_num); i++) {
        ...
        regs->rps_used_by_cur[i] =
            pEncExtParaIn->params.hevcPara.rps_used_by_cur[i];
      }
      for (i = regs->rps_neg_pic_num + regs->rps_pos_pic_num;
          i < VCENC_MAX_REF_FRAMES; i++) {
  ...
  case VCENC_VIDEO_CODEC_AV1:
    param = &pEncExtParaIn->params.av1Para;
    if(regs->frameCodingType == 1) { //I
      if(param->idr_pic_flag) {
        asic->regs.nal_unit_type = IDR_W_RADL;
        regs->idrPicId = 1;
      } else {
        asic->regs.nal_unit_type = TRAIL_R;
    ...
    //need to fill valid value even if it is only usefule for H264
    regs->idrPicId = param->idr_pic_flag;
    regs->nalRefIdc = param->reference_pic_flag;
    ...
    if((param->slice_type == SLICE_TYPE_P)||(param->slice_type == SLICE_TYPE_B)) {
      for (i = 0; i < (param->num_ref_idx_l0); i ++) {
        regs->l0_delta_framenum[i] = 0;
        regs->l0_referenceLongTermIdx[i] = 0;
      }
      regs->markCurrentLongTerm = 0;
      ...
      for (i = 0; i < (param->num_ref_idx_l0); i++) {
        ...
        regs->l0_delta_poc[i] = 0;//pEncExtParaIn->reflist0[i].poc - pEncExtParaIn->recon.poc;
        if (1) { //(!vcenc_instance->flexRefsEnable){
            regs->l0_long_term_flag[i] = 0;
            regs->num_long_term_pics += 0;
          }
        //compress
        regs->ref_l0_luma_compressed[i] = regs->recon_luma_compress;
    ...
    if (param->slice_type == SLICE_TYPE_B) {
      regs->colctbs_load_base = 0;
      for (i = 0; i < (param->num_ref_idx_l1); i++) {
        regs->l1_delta_framenum[i] = 0;
        regs->l1_referenceLongTermIdx[i] = 0;
      }
      //libva not support mmco so
      regs->markCurrentLongTerm = 0;
      ...
      for (i = 0; i < (param->num_ref_idx_l1); i++) {
        ...
        regs->l1_delta_poc[i] = 0;//pEncExtParaIn->reflist1[i].poc - pEncExtParaIn->recon.poc;
        if(!vcenc_instance->flexRefsEnable) {
            regs->l1_long_term_flag[i] = 0;
            regs->num_long_term_pics += 0;
        }
        //compress
        regs->ref_l1_luma_compressed[i] = regs->recon_luma_compress;
    ...
    regs->rps_neg_pic_num = 0;
    regs->rps_pos_pic_num = 0;
...
------------------------------------------------------------------------------*/
static void LambdaTuningSubjective(asicData_s *asic, struct sw_picture *pic,
                                   const VCEncIn *pEncIn) {
  i32 gopDepth = 0;
...
/** prepare stream multi-segment configuration */
static void AsicSetStrmSegment(struct vcenc_instance *vcenc_instance,
                               asicData_s *asic)
{
  asic->regs.streamMultiSegEn =
      vcenc_instance->streamMultiSegment.streamMultiSegmentMode != 0;
...
static void InitReconPicture(struct vcenc_instance *inst, i32 useExtFlag,
                             const VCEncExtParaIn *pEncExtParaIn,
                             struct sw_picture *pic)
{
  i32 id = pic->picture_memeory_id;
  asicData_s *asic = &inst->asic;

  //FIXME: when asic->internalrecon_num is zero, should not set recon.
  //  in such condition, although it is set as zero here, the address
  //  should not be access by HW.

  pic->recon.id = id;
  pic->recon.lum = asic->internalreconLuma[id].busAddress;
  pic->recon.cb = asic->internalreconChroma[id].busAddress;
  pic->recon.cr = pic->recon.cb + asic->regs.recon_chroma_half_size;
  pic->recon_4n_base = asic->internalreconLuma_4n[id].busAddress;
  pic->framectx_base = asic->internalFrameContext[id].busAddress;
  pic->framectx_virtual_addr =
          (ptr_t)asic->internalFrameContext[id].virtualAddress;

  /* for multi-core sync word */
  if (useExtFlag && inst->mcEnable) {
    pic->mc_sync_addr = pEncExtParaIn->recon.mc_sync_pa;
    pic->mc_sync_ptr = (ptr_t)pEncExtParaIn->recon.mc_sync_va;
  } else {
    pic->mc_sync_addr = asic->mc_sync_word[id].busAddress;
    pic->mc_sync_ptr = (ptr_t)asic->mc_sync_word[id].virtualAddress;
  }

  if (useExtFlag) {
    pic->framectx_base = pEncExtParaIn->recon.framectx_pa;
    pic->framectx_virtual_addr = pEncExtParaIn->recon.framectx_va;
  }

  /* for compress */
  {
    u32 lumaTblSize = 0, chromaTblSize = 0;
    EncGetCompressTableSize(asic->regs.codedChromaIdc, inst->compressor,
        pic->sps->width, pic->sps->height, &lumaTblSize, &chromaTblSize);
    pic->recon_compress.lumaTblBase = asic->compressTbl[id].busAddress;
    pic->recon_compress.chromaTblBase =
        (pic->recon_compress.lumaTblBase + lumaTblSize);
  }

  /* for H.264 collocated mb */
  pic->colctbs_store =
      (struct h264_mb_col *)asic->colBuffer[id].virtualAddress;
  pic->colctbs_store_base = asic->colBuffer[id].busAddress;

  /* To collect MV info for TMVP */
  pic->mvInfoBase = asic->mvInfo[id].busAddress;

  /* set the initialize done flag */
  pic->picture_memeory_init = 1;
}


#ifdef SUPPORT_MEM_STATISTIC
VCEncRet _VCEncStrmEncode(VCEncInst inst, VCEncIn *pEncIn, VCEncOut *pEncOut,
					 ...
					 void *pAppData)
{
  ...
  StackStatInit();
  ret = _VCEncStrmEncode(inst, pEncIn, pEncOut,sliceReadyCbFunc,pAppData);
  StackConsumption(__func__, __LINE__);

  return ret;
}

#define VCEncStrmEncode _VCEncStrmEncode
#endif
...
VCEncRet VCEncStrmEncode(VCEncInst inst, VCEncIn *pEncIn, VCEncOut *pEncOut,
  ...
  VCEncRet ret = VCENC_ERROR;
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  vcenc_instance->intraPeriod = pEncIn->gopConfig.intraPeriod;
  VCEncInfo *info;

  if (vcenc_instance->aif.enable) {
    ret = VceTaskCreate(vcenc_instance, pEncIn);
    if (VCENC_OK != ret) {
      return ret;
    }
  }
  ...
  if (0 == vcenc_instance->pass && !pEncIn->flexRefsEnable) {
    ...
    if (!vcenc_instance->bFlush) {  //enqueue job
      ...
      VCEncGopConfig *gopCfg = (VCEncGopConfig *)(&(pEncInFor1pass->gopConfig));
      int next_picture_cnt = pLastPicCfg->picture_cnt - gopCfg->pGopPicCfg[pLastPicCfg->gopPicIdx].poc + pLastPicCfg->gopSize + 1;
      bool fpsChange = pEncInFor1pass->gopPicIdx == 0 && checkFpsUpdate(vcenc_instance, next_picture_cnt);
      ...
      if ((vcenc_instance->nextIdrCnt > 0 &&
           vcenc_instance->nextIdrCnt <= pEncInFor1pass->picture_cnt && !vcenc_instance->enableLeadingPictures) ||
          (0 != pEncIn->picture_cnt && 0 != gopSizeFromUser &&
           gopSizeFromUser != *pNextGopSize) || fpsChange) {
        ...
        FindNextPic(inst, pEncInFor1pass, *pNextGopSize,
                    ...
                    vcenc_instance->nextIdrCnt, fpsChange);
        if(fpsChange) {
          VCEncJob *nextJob = findJob(&vcenc_instance->jobQueue, pEncInFor1pass->picture_cnt);
          VCEncRet _ret = EncFpsUpdate(vcenc_instance, &vcenc_instance->jobQueue, nextJob->pRateCtrlParam, pEncInFor1pass);
          if (VCENC_OK != _ret) {
            APITRACEERR("EncFPSUpdate Failed. \n\n");
            return _ret;
          }
        }
      }

      /* schedule aif process */
      if (vcenc_instance->aif.enable) {
        VCEncAifSchedule(inst, pEncIn);
      }
    } else {
      //flush
      ...
      i32 bNornalEnd = (((pEncInFor1pass->gopConfig.lastPic -
                          pEncInFor1pass->gopConfig.firstPic + 1) *
    ...
    do {
      ...
      if (job == NULL)  //no job to process, return VCENC_FRAME_ENQUEUE
      {
        ...
        u32 is_sideline = 0;
        if (job->encIn.internal!=0) {
          info = (VCEncInfo *)job->encIn.internal;
          is_sideline = info->aif.is_sideline;
          if (is_sideline == 0)  {
            info->encode_cnt = vcenc_instance->encode_cnt;
            vcenc_instance->encode_cnt++;
          }
        }

        if (!is_sideline) {
          SetPictureCfgToJob(pEncInFor1pass, &job->encIn,
          ...
          if (job->encIn.bIsIDR) {
            ...
            if (curIdrCnt <= job->encIn.picture_cnt) {
              if (pEncInFor1pass->gopConfig.idr_interval >
                  0)  // exist default idr interval
              {
        ...
        EncUpdateCodingCtrlParam(vcenc_instance, job->pCodingCtrlParam,
                                job->encIn.picture_cnt);

        /* Update gopConfig on FPS change */
        vcencRateControl_s *rc = &vcenc_instance->rateControl;
        //set rate ctrl parameters into instance
        EncUpdateRateCtrlParam(vcenc_instance, job->pRateCtrlParam,
                                &job->encIn);

        //process picture

        ret = VCEncStrmEncodeExt(inst, &job->encIn, NULL, pEncOut,
                                sliceReadyCbFunc, pAppData, 0);

        if (ret!=VCENC_FRAME_CONTINUE) {

          /* pic skip by RC or buffer overflow */
      #define NOT_PARALLEL_JOB(ret) ((ret!=VCENC_FRAME_ENQUEUE)&&(ret!=VCENC_FRAME_CACHE))
      if ((NOT_PARALLEL_JOB(ret) && pEncOut->streamSize == 0) ||
          pEncOut->codingType == VCENC_NOTCODED_FRAME ||
              ret == VCENC_SBI_ERROR ||
              ret == VCENC_OUTPUT_BUFFER_OVERFLOW) {
            //save last encIn
            SavePicCfg(pEncInFor1pass, pLastPicCfg);
            job = handleSkipFrame(vcenc_instance, pNextGopSize, pEncOut, pEncInFor1pass, job);
          } else {
            //Adaptive GOP size decision
            if (0 == vcenc_instance->gopSize &&
                pEncInFor1pass->codingType != VCENC_INTRA_FRAME &&
                0 == gopSizeFromUser)
            {
              if (vcenc_instance->rateControl.hieQpDeltaEnable)
                AGopDecisionRefine(vcenc_instance, pEncInFor1pass, pEncOut, pNextGopSize, pAgop);
              else
                AGopDecision(vcenc_instance, pEncInFor1pass, pEncOut, pNextGopSize,
                       pAgop);
            }
            else if (0 == vcenc_instance->gopSize &&
                pEncInFor1pass->codingType == VCENC_INTRA_FRAME &&
                0 == gopSizeFromUser && vcenc_instance->rateControl.hieQpDeltaEnable)
            {
               pAgop->gop_frm_num = 0;
            }
            //save last encIn
            SavePicCfg(pEncInFor1pass, pLastPicCfg);
            VCEncJob *nextJob = (VCEncJob *)queue_tail(&vcenc_instance->jobQueue);
            bool fpsChange = false;
            if(nextJob)
            {
              VCEncRateCtrl *pRateCtrl = &nextJob->pRateCtrlParam->encRateCtrl;
              int nextGopPicIdx = (pEncInFor1pass->gopPicIdx + 1) % pEncInFor1pass->gopSize;
              VCEncGopConfig *gopCfg = (VCEncGopConfig *)(&(pEncInFor1pass->gopConfig));
              int next_picture_cnt = pEncInFor1pass->picture_cnt - gopCfg->pGopPicCfg[pEncInFor1pass->gopPicIdx].poc + pEncInFor1pass->gopSize + 1;
              fpsChange = (nextGopPicIdx == 0 && checkFpsUpdate(vcenc_instance, next_picture_cnt));
#ifdef INTERNAL_TEST
             if ((vcenc_instance->testId == TID_FPS_ADJUST) && fpsChange && vcenc_instance->bInsertIDRonFPSAdjust)
             {
               VCEncAdjustFPSTest(pEncInFor1pass, NULL, 0);
             }
#endif
            }
            //find next picture
            FindNextPic(inst, pEncInFor1pass, *pNextGopSize,
                        ...
                        vcenc_instance->nextIdrCnt, fpsChange);
            if(fpsChange) {
              nextJob = findJob(&vcenc_instance->jobQueue, pEncInFor1pass->picture_cnt);
              if(nextJob) {
                VCEncRet _ret = EncFpsUpdate(vcenc_instance, &vcenc_instance->jobQueue, nextJob->pRateCtrlParam, pEncInFor1pass);
                if (VCENC_OK != _ret) {
                  APITRACEERR("EncFPSUpdate Failed. \n\n");
    ...
    } while (ret==VCENC_FRAME_CONTINUE);
  ...
  //flexRef: not need to manage encode order
    ...
    if ((!pEncIn->flexRefsEnable) || (!vcenc_instance->bFlush))
      ret = VCEncStrmEncodeExt(inst, pEncIn, NULL, pEncOut, sliceReadyCbFunc,
                               pAppData, 0);
    else
      ret = VCENC_OK;

    if (vcenc_instance->pass == 2)
      VCEncPass2CheckAgop(inst, pEncIn, pEncOut, ret);
  }

  return ret;
}

// recover streamer to state before HW run
i32 streamRollback(struct buffer *stream, VCEncOut *pEncOut, i32 bytes)
{
  i32 nals_size = 0;
  i32 nals_num = 0;

  stream->byteCnt -= bytes;
  stream->stream -= bytes;
  do {
    nals_num++;
    nals_size += pEncOut->pNaluSizeBuf[pEncOut->numNalus-nals_num];
    pEncOut->pNaluSizeBuf[pEncOut->numNalus-nals_num] = 0;
  } while (nals_size<bytes);

  pEncOut->numNalus-=nals_num;

  return 0;
}

/**
 * estimate the executing time
 */
static u32 vcencEstimateExecutingTime(u32 width, u32 height, u32 rdoLevel,
                           u32 bRDOQEnable)
{
  return width * height * (rdoLevel+1) * (bRDOQEnable+1);
}
/**
 * reserve cmd buffer and run it.
 */
static VCEncRet vcencVcmdStart(struct vcenc_instance *vcenc_instance,
                              struct sw_picture *pic,
                              asicData_s *asic)
{
  VCEncRet ret = VCENC_OK;
  ...
  u32 exectuting_time = vcencEstimateExecutingTime(vcenc_instance->width,
                  vcenc_instance->height, vcenc_instance->rdoLevel,
                  vcenc_instance->asic.regs.bRDOQEnable);

  EncSetReseveInfo(asic->ewl, exectuting_time, client_type);

  EncReseveCmdbuf(asic->ewl, &asic->regs.vcmd);
  pic->cmdbufid = asic->regs.vcmd.cmdbufid;
//  printf("At %d: pic->cmdbufid = %d\n", __LINE__, pic->cmdbufid);
  if (EncMakeCmdbufData(asic, &asic->regs) == VCENC_INVALID_ARGUMENT) {
    APITRACEERR(
        "VCEncStrmEncode: DEC400 doesn't exist or format not "
        "supported\n");
...
#ifdef MULTI_FRAME_SUPPORT
  /* enable cache jobs */
  if (vcenc_instance->batchEnable) {
    ret = vcencBatchRunCmdbuf(vcenc_instance, asic, &asic->regs.vcmd,
                              client_type);
    return ret;
  }
#endif /* MULTI_FRAME_SUPPORT */

  EncLinkRunCmdbuf(asic->ewl, &asic->regs.vcmd);
  return ret;
}

void handleFrameDiscard(struct vcenc_instance *vcenc_instance,
    VCEncIn *pEncIn, VCEncOut *pEncOut, struct sw_picture *pic, bool bSkipGop) {
  u32 tileId;
  asicData_s *asic = &vcenc_instance->asic;
  ...
  /* process gop skip */
  if(bSkipGop) {
    if(pEncIn->poc != 0 && pEncIn->gopPicIdx > 0)
      vcenc_instance->rateControl.skipGop = ENCHW_YES;
    else
      vcenc_instance->rateControl.skipGop = vcenc_instance->preSkipGop;
    /* restore h264 mmo */
    if (IS_H264(vcenc_instance->codecFormat))
      vcenc_instance->h264_mmo_nops = vcenc_instance->last_h264_mmo_nops;

    /* save for DPB management on skipGop case */
    vcenc_instance->skipGopPoc = pEncIn->poc;
    vcenc_instance->skipGopRpsId = vcenc_instance->rps_id;
    if(vcenc_instance->rateControl.skipGop) {
      /* mark current frame as unavailable */
      pic->reference = 0;
      pic->poc = UNUSED_POC;
    }
  }
...
/*------------------------------------------------------------------------------
    ...
    Function name : VCEncStrmEncodeExt
    Description   : Encodes a new picture
    Return type   : VCEncRet
    Argument      : inst - encoder instance
    Argument      : pEncIn - user provided input parameters
...

{
  ...
  u32 iBuf, tileId;
  u32 numNalusTmp = 0;
  VCEncInfo *info = (VCEncInfo *)pEncIn->internal;
  u32 is_sideline = 0;
  ...
  if (vcenc_instance->pass == 2) {
    if (pEncExtParaIn != NULL) {  //for extra parameters.
      VCEncExtParaIn *pExtParaInBuf =
          (VCEncExtParaIn *)malloc(sizeof(VCEncExtParaIn));
    ...
    if (vcenc_instance->idelay > vcenc_instance->lookaheadDelay ||
        vcenc_instance->bFlush) {
      outframe = GetLookaheadOutput(&vcenc_instance->lookahead,
                                    vcenc_instance->bFlush, vcenc_instance);
      if (outframe == NULL) {
      ...
      EncUpdateCodingCtrlParam(
          vcenc_instance, (EncCodingCtrlParam *)(outframe->pCodingCtrlParam),
    ...
    if (outframe->status != VCENC_FRAME_READY) {
      ...
      if (status == VCENC_ERROR) {
        ...
        PutBufferToPool(vcenc_instance->lookahead.jobBufferPool,
                        (void **)&outframe);
      return status;
    }
    if (vcenc_instance->bIOBufferBinding) {
      ...
      ptr_t busOutBuf =
          pEncIn->busOutBuf[0]; /* Bus address of output stream buffer */
      u32 outBufSize =
          pEncIn->outBufSize[0]; /* Size of output stream buffer in bytes */
    ...
    if (outframe->frame.poc != 0)
      vcenc_instance->nextGopSize = nextGopSize;
    ...
    if (vcenc_instance->enableLeadingPictures && outframe->encIn.bIsIDR && vcenc_instance->lookahead.picture_cnt == 0) {
      /* first IDR, init picture_cnt from encIn */
      vcenc_instance->lookahead.picture_cnt = outframe->encIn.picture_cnt;
      vcenc_instance->lookahead.last_idr_picture_cnt =
        outframe->encIn.last_idr_picture_cnt;
    }
    ...
    outframe->encIn.codingType =
        (outframe->frame.frameNum == 0)
            ? VCENC_INTRA_FRAME
            : FindNextPic(inst, &outframe->encIn, nextGopSize,
                          pEncIn->gopConfig.gopCfgOffset,
                          vcenc_instance->nextIdrCnt, false);
    EncUpdateRateCtrlParam(vcenc_instance, (EncRateCtrlParam *)(outframe->pRateCtrlParam),
        &outframe->encIn);
    ...
    struct vcenc_instance *instPass1 =
        (struct vcenc_instance *)vcenc_instance->lookahead.priv_inst;
    i32 costScale = (instPass1 && !instPass1->preProcess.inLoopDSRatio &&
                     asic->regs.asicCfg->inLoopDSRatio)
                        ? 2
                        : 1;
    ...
    for (i = 0; i < 4; i++) {
      vcenc_instance->cuTreeCtl.costAvg[i] =
          outframe->frame.costAvg[i] * costScale;
      vcenc_instance->cuTreeCtl.FrameTypeNum[i] =
          outframe->frame.FrameTypeNum[i];
      vcenc_instance->cuTreeCtl.costGop[i] =
          outframe->frame.costGop[i] * costScale;
  ...
  /* ErrorChecker */
  client_type = VCEncGetClientType(vcenc_instance->codecFormat);
  if ((ret = StrmEncodeCheckPara(vcenc_instance, pEncIn, pEncOut, asic,
                                 client_type)) != VCENC_OK) {
    return ret;
  }
  ...
  if (pEncIn->bSkipFrame) {
    ...
    } else if (asic->regs.asicHwId >= 0x80006010 &&
               asic->regs.asicHwId < 0x80006200) {
      bSkipFrame = pEncIn->bSkipFrame;
    } else {
    ...
    if (pEncIn->bIsIDR) {
      APITRACEWRN(
          "Disable Frame Skip setting for current frame when conflict with IDR "
          "setting.\n");
  ...
  const EWLHwConfig_t *cfg =
      EncAsicGetAsicConfig(client_type, vcenc_instance->ctx);
  if (!cfg) {
    APITRACEERR("VCEncStrmEncodeExt: ERROR Null argument\n");
    return VCENC_NULL_ARGUMENT;
  }
  ...
  if ((cfg->bFrameEnabled == EWL_HW_CONFIG_NOT_SUPPORTED) &&
    ...
    APITRACEERR(
        "VCEncStrmEncode: Invalid coding format, B frame not supported by "
        "core\n");
    return VCENC_INVALID_ARGUMENT;
  }

  if ((cfg->IframeOnly == EWL_HW_CONFIG_ENABLED) &&
      ((pEncIn->codingType) != VCENC_INTRA_FRAME)) {
    APITRACEERR(
        "VCEncStrmEncode: Invalid coding format, only I frame supported by "
        "core\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  asic->regs.RoimapCuCtrl_index_enable =
      vcenc_instance->RoimapCuCtrl_index_enable;
  ...
  true_e secondMap = (vcenc_instance->RoimapCuCtrl_ver >= 4) &&
                     vcenc_instance->RoimapCuCtrl_enable &&
                     (pEncIn->RoimapCuCtrlAddr != 0);
  if ((((asic->regs.rcRoiEnable & 2) &&
        (vcenc_instance->RoimapCuCtrl_enable == 0)) ||
       ((ENCHW_NO == secondMap) &&
        (asic->regs.ipcmMapEnable || asic->regs.skipMapEnable)) ||
  ...
  if (vcenc_instance->pass == 1) {
    vcenc_instance->stream.stream =
        (u8 *)vcenc_instance->lookahead.internal_mem.pOutBuf;
    vcenc_instance->stream.stream_bus =
        vcenc_instance->lookahead.internal_mem.busOutBuf;
    vcenc_instance->stream.size =
        vcenc_instance->lookahead.internal_mem.outBufSize;
  ...
  vcenc_instance->stream.av1pre_size =
      asic->av1PreCarryBuf[cur_core_index].size;
  vcenc_instance->stream.stream_av1pre_bus =
      asic->av1PreCarryBuf[cur_core_index].busAddress;
  ...
  pEncOut->pNaluSizeBuf =
      (u32 *)vcenc_instance->asic
          .sizeTbl[vcenc_instance->num_tile_columns > 1 ? 0 : cur_core_index]
          .virtualAddress;
//  printf("At %s:%d pEncOut->pNaluSizeBuf = %p, pEncOut->pNaluSizeBuf[0] = %u\n", __FUNCTION__, __LINE__,
//  	pEncOut->pNaluSizeBuf, *pEncOut->pNaluSizeBuf);
  ...
  if (vcenc_instance->num_tile_columns > 1 && pEncIn->tileExtra[0].busLuma) {
    for (tileId = 1; tileId < vcenc_instance->num_tile_columns; tileId++) {
      ...
      if ((lumaAddress + chromaUAddress + chromaVAddress) != 3) {
        APITRACEERR(
            "VCEncStrmStart: ERROR Too few input address for multi-tile\n");
        return VCENC_INVALID_ARGUMENT;
      }
    }
  }
  ...
  u32 defaultInputFlag =
      (vcenc_instance->num_tile_columns > 1 && pEncIn->tileExtra[0].busLuma)
          ? 0
          : 1;
  for (tileId = 1; tileId < vcenc_instance->num_tile_columns; tileId++) {
  ...
  for (i = 0; i < MAX_STRM_BUF_NUM; i++) {
    vcenc_instance->streamBufs[cur_core_index].buf[i] =
        (u8 *)pEncIn->pOutBuf[i];
    vcenc_instance->streamBufs[cur_core_index].bufLen[i] =
        pEncIn->outBufSize[i];
  }
  ...
  if (pEncOut->pNaluSizeBuf != NULL) pEncOut->pNaluSizeBuf[0] = 0;
  for (tileId = 1; tileId < vcenc_instance->num_tile_columns; tileId++) {
  ...
  asic->regs.Av1PreoutputStrmBase =
      (ptr_t)vcenc_instance->stream.stream_av1pre_bus;
  ...
  pEncOut->scaledPicture =
      (u8 *)vcenc_instance->asic.scaledImage.virtualAddress;
  /* Get parameter sets */
  v = (struct vps *)get_parameter_set(c, VPS_NUT, 0);
  ...
  if (pEncIn->flexRefsEnable) {
    ...
    vcenc_replace_rps(vcenc_instance, NULL,
                      (VCEncGopPicConfig *)&(pEncIn->gopCurrPicConfig), 0);
    r = (struct rps *)get_parameter_set(c, RPS, vcenc_instance->rps_id);
    if (r == NULL) return VCENC_ERROR;
    ...
    use_ltr_cur =
        (pEncOut->boolCurIsLongTermRef = (pEncIn->u8IdxEncodedAsLTR != 0));
  ...
  init_poc_list(r, vcenc_instance->poc, use_ltr_cur, codingType, c,
                ...
                &rpsInSliceHeader);
  /* 6.0 not support */
  if (vcenc_instance->asic.regs.asicHwId < 0x80006010) r->lt_follow_cnt = 0;

  if ((r->before_cnt + r->after_cnt + r->lt_current_cnt == 1) &&
      !pEncIn->bIsIntraOnly && pEncIn->poc >= 0) /* allow 1-ref B frames for leading pictures */
    codingType = VCENC_PREDICTED_FRAME;

#if defined(DENOISE_3DNR) && DENOISE_3DNR_INTRAUSENLM3D
  // move below code from VCEncHEVCDnfPrepare() to here, to pass GOP long-term case 8740
  if (vcenc_instance->uiNoiseReductionEnable && vcenc_instance->lastPicNLM) {
    if (codingType == VCENC_INTRA_FRAME) {
    //   /* use lastPicNLM as 3DNR reference */
    } else {
      /* release lastPicNLM if unused */
      vcenc_instance->lastPicNLM->ref_cnt--;
      vcenc_instance->lastPicNLM = NULL;
    }
  }
#endif

  if (vcenc_instance->has_sideline==0) {
    /* mark unused picture, so that you can reuse those. TODO: IDR flush */
    if (GenralRefPicMarking(vcenc_instance, c, r, codingType, pEncIn)) {
      Error(2, ERR, "RPS error: reference picture(s) not available");
      return VCENC_ERROR;
    }
  } else {
    /* mark unused picture to avoid buffer pool overflow */
    if (vcenc_instance->aif.recon_next->picture_cnt == vcenc_instance->nextIdrCnt) {
      IdrRefPicFlush(vcenc_instance, c, vcenc_instance->aif.refer_poc, pEncIn);
    }
  }

  /* Get free picture (if any) and remove it from picture store */
  if (!(pic = get_picture(c, UNUSED_POC))) {
    if (!(pic = create_picture_ctrlsw(vcenc_instance, v, s, p,
                                      asic->regs.sliceSize)))
      return VCENC_ERROR;
  }
  ...
  if (pic->pps != p) pic->pps = p;

  /* store input addr */
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    pic->input[tileId].lum =
        (tileId == 0) ? pEncIn->busLuma : pEncIn->tileExtra[tileId - 1].busLuma;
    pic->input[tileId].cb = (tileId == 0)
                                ? pEncIn->busChromaU
                                : pEncIn->tileExtra[tileId - 1].busChromaU;
    pic->input[tileId].cr = (tileId == 0)
                                ? pEncIn->busChromaV
                                : pEncIn->tileExtra[tileId - 1].busChromaV;
  }

  if (pic->picture_memeory_init == 0) {
    InitReconPicture(vcenc_instance, useExtFlag, pEncExtParaIn, pic);
  }

  if (vcenc_instance->has_sideline) {
    pic->ref_cnt++;
  }

  /* frame info */
  pic->frameInfoDataBase = asic->framInfoMem.busAddress;
  ...
  /* cu info output */
  pic->cuInfoTableBase =
      (useExtFlag) ? pEncExtParaIn->recon.cuInfoMemRecon :
      asic->cuInfoMem[vcenc_instance->cuInfoBufIdx].busAddress;
  pic->cuInfoDataBase =
      pic->cuInfoTableBase + asic->cuInfoTableSize + asic->aqInfoSize;

  if (vcenc_instance->pass == 1) waitCuInfoBufPass1(vcenc_instance);
  if (asic->regs.enableOutputCuInfo) {
    ...
    u32 *tableBase =
        (u32 *)asic->cuInfoMem[vcenc_instance->cuInfoBufIdx].virtualAddress;
    u8 *dataBase =
        (u8 *)asic->cuInfoMem[vcenc_instance->cuInfoBufIdx].virtualAddress +
    ...
    for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
      ...
      vcenc_instance->tileCtrl[tileId].cuinfoTableBase =
          pic->cuInfoTableBase + (vcenc_instance->tileCtrl[tileId].tileLeft *
                                  (vcenc_instance->height + 63) / 64) *
                                     CU_INFO_TABLE_ITEM_SIZE;
      vcenc_instance->tileCtrl[tileId].cuinfoDataBase =
          pic->cuInfoDataBase +
          ((vcenc_instance->tileCtrl[tileId].tileLeft * 64 / 8) *
           ((vcenc_instance->height + 63) / 64 * 64 / 8)) *
              infoSizePerCu;
    }
  } else {
    ...
    for (tileId = 1; tileId < vcenc_instance->num_tile_columns; tileId++)
      memset(&pEncOut->tileExtra[tileId - 1].cuOutData, 0,
             sizeof(VCEncCuOutData));
  }
  ...
  if (asic->regs.aqInfoOutputMode != AQ_NONE) {
    u32 aqStrength = (u32)(vcenc_instance->cuTreeCtl.aqStrength *
                           (1 << AQ_STRENGTH_SCALE_BITS));  // to Q2.7
  ...
  if (asic->regs.enableOutputCtbBits) {
    ...
    pEncOut->ctbBitsData =
        (u16 *)asic->ctbBitsMem[cur_core_index].virtualAddress;
  } else {
  ...
  StrmEncodeGradualDecoderRefresh(vcenc_instance, asic, pEncIn, &codingType,
                                  cfg);

  /* setting TRAIL_N/TRAIL_R based on nonReference setting */
  pic->temporal_id = pEncIn->gopCurrPicConfig.temporalId;
  u32 hevc_nal_unit_type =
      pEncIn->gopCurrPicConfig.nonReference ? TRAIL_N : TRAIL_R;
  /* leading pictures */
  if (pEncIn->poc < 0)
    hevc_nal_unit_type = RADL_N;
  if (codingType == VCENC_INTRA_FRAME) {
    asic->regs.frameCodingType = 1;
    asic->regs.nal_unit_type =
        (IS_H264(vcenc_instance->codecFormat) ? H264_IDR : IDR_W_RADL);
    ...
    asic->regs.nal_unit_type =
        (IS_H264(vcenc_instance->codecFormat) ? H264_NONIDR
                                              : hevc_nal_unit_type);
  ...
  hash_getstate(&vcenc_instance->hashctx, &asic->regs.hashval,
                &asic->regs.hashoffset);

  /* Generate reference picture list of current picture using active
   * reference picture set */
  reference_picture_list(c, pic, pEncIn->poc < 0); /* allow 1-ref B frames for leading pictures */
  if (vcenc_instance->aif.enable) {
    /* force aif P frame to 1 reference frame only */
    struct slice *s;
    struct node *n;
    VCEncInfo *info = (VCEncInfo *)pEncIn->internal;
    if (info!=NULL && info->aif.is_sideline) {
  ...
  if (pic->sliceInst->active_l0_cnt > 1) {
    ...
    if (cfg->maxRefNumList0Minus1 < 1) {
      APITRACEERR(
          "VCEncStrmEncodeExt: The buildID[0x%x] not support multi-reference "
          "for P.\n",
          cfg->hw_build_id);
      return VCENC_ERROR;
    }
    ...
    if ((vcenc_instance->codecFormat != VCENC_VIDEO_CODEC_HEVC) &&
      ...
      APITRACEERR(
          "VCEncStrmEncodeExt: The codecFormat not support multi-reference for "
          "P now.\n");
      return VCENC_ERROR;
    }
    /* for buildid 0x2139 not support tmvp and 2refp at the same time. */
    if (cfg->hw_build_id == 0x2139 && vcenc_instance->enableTMVP)
    {
      APITRACEERR(
          "VCEncStrmEncodeExt: The buildID[0x%x] can not support tmvp and 2 "
      "reference p frame at the same time.\n", cfg->hw_build_id);
      return VCENC_ERROR;
    }
  }
       ...
       (!IS_H264(vcenc_instance->codecFormat))))  // check only hevc?
  {
    ...
    for (i = 0; i < used_cnt; i++) asic->regs.l0_used_by_curr_pic[i] = 1;
    ...
    for (i = 0; i < pic->sliceInst->active_l0_cnt; i++) {
      ...
      //compress
      asic->regs.ref_l0_luma_compressed[i] =
          pic->rpl[0][i]->recon_compress.lumaCompressed;
    ...
    if (pic->sliceInst->type == B_SLICE) {
      ...
      for (i = 0; i < used_cnt; i++) asic->regs.l1_used_by_curr_pic[i] = 1;
    ...
    if (!IS_H264(vcenc_instance->codecFormat)) {
      for (i = pic->sliceInst->active_l0_cnt, j = 0;
           i < 2 && j < r->lt_follow_cnt; i++, j++) {
        asic->regs.l0_delta_poc[i] = r->lt_follow[j] - pic->poc;
        asic->regs.l0_long_term_flag[i] = 1;
      }
      for (i = pic->sliceInst->active_l1_cnt; i < 2 && j < r->lt_follow_cnt;
           i++, j++) {
        asic->regs.l1_delta_poc[i] = r->lt_follow[j] - pic->poc;
        asic->regs.l1_long_term_flag[i] = 1;
      }
  ...
  asic->regs.lists_modification_present_flag =
      pic->pps->lists_modification_present_flag;
  ...
  //frame info
  asic->regs.frameInfoDataBase = pic->frameInfoDataBase;
  ...
  vcenc_instance->rateControl.hierarchial_bit_allocation_GOP_size =
      pEncIn->gopSize;
  /* Rate control */
  if (pic->sliceInst->type == I_SLICE) {
    ...
    if(pEncIn->poc < 0) /* leading picture */
      vcenc_instance->rateControl.gopPoc += pEncIn->gopSize;
  ...
  asic->regs.ctbRcQpDeltaReverse =
      vcenc_instance->rateControl.ctbRcQpDeltaReverse;
  asic->regs.rcQpDeltaRange = vcenc_instance->rateControl.rcQpDeltaRange;
  asic->regs.rcBaseMBComplexity =
      vcenc_instance->rateControl.rcBaseMBComplexity;
  ...
  if (vcenc_instance->pass == 2) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      vcenc_instance->rateControl.pass1AvgCost[i] =
          ((((i64)vcenc_instance->cuTreeCtl.costAvg[i]) * nBlk +
            costScale / 2) / costScale + 128) / 256;
      vcenc_instance->rateControl.pass1GopCost[i] =
          ((((i64)vcenc_instance->cuTreeCtl.costGop[i]) * nBlk +
            costScale / 2) / costScale + 128) / 256;
#else
      vcenc_instance->rateControl.pass1AvgCost[i] =
      ...
      vcenc_instance->rateControl.agopPass1GopPicIdx =
          vcenc_instance->lookahead.agopPass1GopPicIdx;
    }
  }
  ...
  asic->regs.bMotionScoreEnable =
    vcenc_instance->motionScoreEnable || vcenc_instance->multiPassMotionScoreEnable;

  /* reset skipGop on IDR or new GOP start */
  vcenc_instance->preSkipGop = vcenc_instance->rateControl.skipGop;
  if(pEncIn->poc == 0 || pEncIn->gopPicIdx == 0)
    vcenc_instance->rateControl.skipGop = ENCHW_NO;
  i32 PrevQP = vcenc_instance->rateControl.qpHdrPrev;//previous frame QP
  i32 frame_qp = 0;
  if (vcenc_instance->has_sideline) {
    frame_qp = vcenc_instance->aif.refer_qp + TO_HQP(vcenc_instance->aif.qp_delta);
  } else {
    VCEncBeforePicRc(&vcenc_instance->rateControl, pEncIn->timeIncrement,
                   pic->sliceInst->type, use_ltr_cur, pic);
    frame_qp = vcenc_instance->rateControl.qpHdr;
  }
  ...
  if (VCENC_OK != VCEncCodecPrepareEncode(vcenc_instance, pEncIn, pEncOut,
  ...
  if (vcenc_instance->has_sideline==0) {
    /* not need mmco for side line encode */
    if (IS_H264(vcenc_instance->codecFormat)) {
      /* save DPB update for restore later in case current frame is dropped */
      vcenc_instance->last_h264_mmo_nops = vcenc_instance->h264_mmo_nops;

      if (useExtFlag) {
        if (!h264_ext_mmo_collect(vcenc_instance, pic, pEncIn, r2, delta_poc))
          return VCENC_ERROR;
      } else {
      ...
      } else if (pic->nalRefIdc || vcenc_instance->svctEnable == 1) {
        vcenc_instance->frameNum++;
      }
      ...
      if (pic->sliceInst->type != I_SLICE) {
        for (i = 0; i < pic->sliceInst->active_l0_cnt; i++) {
          asic->regs.l0_delta_framenum[i] =
              pic->frameNum - pic->rpl[0][i]->frameNum;
          asic->regs.l0_referenceLongTermIdx[i] =
              (pic->rpl[0][i]->long_term_flag ? pic->rpl[0][i]->curLongTermIdx
                                              : 0);
        }

        if (pic->sliceInst->type == B_SLICE) {
          for (i = 0; i < pic->sliceInst->active_l1_cnt; i++) {
            asic->regs.l1_delta_framenum[i] =
                pic->frameNum - pic->rpl[1][i]->frameNum;
            asic->regs.l1_referenceLongTermIdx[i] =
                (pic->rpl[1][i]->long_term_flag ? pic->rpl[1][i]->curLongTermIdx
                                                : 0);
          }
        }
      }
  ...
  if (asic->regs.asicCfg->ctbRcVersion &&
    ...
    vcencRateControl_s *rc = &(vcenc_instance->rateControl);
    u32 num_ctbrcMem = 2 + rc->coreNum;

    if (EncAsicGetBuffers(asic, asic->ctbRcMem, (num_ctbrcMem + rc->coreNum),
                          vcenc_instance->ctbPerFrame,
    ...
    for (i = 0; i < num_ctbrcMem; i++) {
      ...
      rc->ctbRateCtrl.models[i].ctbMemPreVirtualAddr =
          asic->ctbRcMem[i].virtualAddress;
    }

    for (i = 0; i < rc->coreNum; i++) {
    rc->ctbRateCtrl.Cur[i].ctbMemCurAddr = asic->ctbRcMem[i+num_ctbrcMem].busAddress;
    rc->ctbRateCtrl.Cur[i].ctbMemCurVirtualAddr = asic->ctbRcMem[i+num_ctbrcMem].virtualAddress;
    }
  }
#endif
...
#ifdef LOW_LATENCY_SLICEINFO_SUPPORT
  asic->regs.SliceInfoBase = pEncIn->SliceInfo_Base;
#endif
  ...
  asic->regs.sliceReadyInterrupt =
      vcenc_instance->inputLineBuf.enable_slice_irq & ((asic->regs.sliceNum > 1));
  asic->regs.picInitQp = pic->pps->init_qp;
#ifndef CTBRC_STRENGTH
  asic->regs.qp = frame_qp;
#else
  //printf("float qp=%f\n",vcenc_instance->rateControl.qpHdr);
  asic->regs.qp = (frame_qp >> QP_FRACTIONAL_BITS);
  asic->regs.qpfrac = (frame_qp - (asic->regs.qp << QP_FRACTIONAL_BITS))
  ...
  asic->regs.rcBlockSize = vcenc_instance->blockRCSize;
  {
    ...
    qpDeltaGainFloat =
        (float)vcenc_instance->rateControl.complexity / Q31_FIXED_ONE;
    ...
    // strength & offset for new ctbrc (aq based)
    if (asic->regs.asicCfg->ctbRcVersion == 3) {
      asic->regs.qpDeltaMBGain =
          (int)((asic->regs.frameCodingType == 1 ? 1.4 : 2.0) * 256 + 0.5);
      asic->regs.rcBaseMBComplexity =
          (int)((asic->regs.frameCodingType == 1 ? 1.3 : 1.5) * 8 + 0.5);
    }
  }
#endif
  ...
  asic->regs.maxTransHierarchyDepthInter =
      pic->sps->max_tr_hierarchy_depth_inter;
  asic->regs.maxTransHierarchyDepthIntra =
      pic->sps->max_tr_hierarchy_depth_intra;
  ...
  asic->regs.long_term_ref_pics_present_flag =
      pic->sps->long_term_ref_pics_present_flag;

  //ring buffer for reference and recon data using common buffer.
  if (vcenc_instance->refRingBufEnable) {
    if (pic->sliceInst->active_l0_cnt > 1 || (pic->rpl[0][0] && (pic->rpl[0][0]->poc - pic->poc != -1)))
    {
       APITRACEERR(
           "VCEncStrmEncodeExt: refRingBuffer only support single P frame with deltaPOC = -1\n");
       return VCENC_ERROR;
    }
  ...
  asic->regs.idrPicId =
      (codingType == VCENC_INTRA_FRAME ? (vcenc_instance->idrPicId & 1) : 0);
  asic->regs.nalRefIdc = pic->nalRefIdc;
  asic->regs.nalRefIdc_2bit = pic->nalRefIdc_2bit;
  asic->regs.prefixNalSvcFlag = pic->prefixNalSvcFlag;
  ...
  asic->regs.EnableTS = pic->pps->transform_skip_enabled_flag;
  asic->regs.FastTSThreshold = 133;
  if (rpsInSliceHeader || IS_H264(vcenc_instance->codecFormat)) {
    asic->regs.rpsId = 0;
  } else {
  ...
  if (IS_H264(vcenc_instance->codecFormat) && pic->nalRefIdc &&
    ...
    for(i = vcenc_instance->h264_mmo_nops - 1; i >= 0; i--) {
      h264_mmo_mark_unref(&asic->regs, vcenc_instance->h264_mmo_unref[i],
                          vcenc_instance->h264_mmo_long_term_flag[i],
                          vcenc_instance->h264_mmo_ltIdx[i], cnt, pic);
      vcenc_instance->h264_mmo_nops--;
      // (rare case) if hit HW limitation, postpone rest MMCO operations to next frame
      if(cnt[0] == 2 && cnt[1] == 2)
        break;
    }
    ...
    h264_mmo_mark_unref(
        &asic->regs,
        vcenc_instance->h264_mmo_unref[--vcenc_instance->h264_mmo_nops], cnt,
        pic);
#endif
  }
  ...
  /* strong_intra_smoothing_enabled_flag */
  asic->regs.strong_intra_smoothing_enabled_flag =
      pic->sps->strong_intra_smoothing_enabled_flag;

  /* constrained_intra_pred_flag */
  asic->regs.constrained_intra_pred_flag =
      pic->pps->constrained_intra_pred_flag;
  ...
  if (asic->regs.skip_frame_enabled) asic->regs.rdoLevel = 0;

  /* set rdoLevel=0 when HW not support progRdo */
  if (!asic->regs.asicCfg->progRdoEnable) asic->regs.rdoLevel = 0;
  ...
  asic->regs.compress_coeff_scan_base =
      asic->compress_coeff_SACN[cur_core_index].busAddress;

  /* HW Base must be 64-bit aligned */
  ASSERT(asic->regs.sizeTblBase % 8 == 0);

  /*inter prediction parameters*/
  double dQPFactor = (HW_PRODUCT_SYSTEM60(asic->regs.asicHwId) ||
                      HW_PRODUCT_VC9000LE(asic->regs.asicHwId))
                         ? 0.894
  ...
  asic->regs.prefixnal_svc_ext =
      IS_H264(vcenc_instance->codecFormat) && (s->max_num_sub_layers > 1) &&
      (vcenc_instance->asic.regs.asicHwId >= 0x80006010) &&
      (vcenc_instance->svctEnable == 1);
  if (asic->regs.prefixnal_svc_ext == 1) {
    asic->regs.priority_id = asic->regs.nuh_temporal_id;
  }

  asic->regs.av1_extension_flag = (asic->regs.nuh_temporal_id > 0) ? 1 : 0;

  u32 *ctbrcThreshold = codingType == VCENC_INTRA_FRAME ? vcenc_instance->ctbRcThresholdI :
          (codingType == VCENC_PREDICTED_FRAME ? vcenc_instance->ctbRcThresholdP : vcenc_instance->ctbRcThresholdB);
  for (i32 i=0; i<16; i++)
    vcenc_instance->ctbRcThresholdCurFrame[i] = ctbrcThreshold[i];

  if (vcenc_instance->rateControl.hieQpDeltaEnable) {
    //Use same QP Factor for all layers, let QP controls all
    if (vcenc_instance->pass == 0)
    {
      depth0 = HANTRO_TRUE;
      dQPFactor = 0.760263112; //sqrt(0.578)
    }
  }

  /*config tuning for cases excess bitrate*/
  vcenc_instance->qpfactorSSE = dQPFactor * dQPFactor;

  /* AIF frame not tune qpfactor or qpMin */
  if (vcenc_instance->has_sideline == 0)
    VCEncExcessBitrateConfigTuning(vcenc_instance, &dQPFactor, codingType);
...
#if 0 /* Not limit qp range since this will raise overall QP */
  ...
  if (vcenc_instance->pass == 1) {
    ...
    pic->outForCutree.qp = frame_qp;
    pic->outForCutree.codingType = pEncOut->codingType;
    pic->outForCutree.hierarchial_bit_allocation_GOP_size =
        vcenc_instance->rateControl.hierarchial_bit_allocation_GOP_size;
    pic->outForCutree.encoded_frame_number =
        vcenc_instance->rateControl.encoded_frame_number;
    ...
    pic->outForCutree.roiMapEnable =
        vcenc_instance->roiMapEnable && pEncIn->pRoiMapDelta;
  ...
  /*clip qp
    clip qp after qpMin and qpMax had been setted*/
#ifdef CTBRC_STRENGTH
  ...
  /* multi-segment stream buffer */
  AsicSetStrmSegment(vcenc_instance, asic);

  /*intra recon*/
  asic->regs.intraReconEnable = vcenc_instance->intraReconEnable;
  //TODO, current golden value 13 of CU RDO(he), configurable by option later.
  StrmEncodeMeqnSimpleBinCostConfig(vcenc_instance, asic);

  /* smart */
  StrmEncodeSmartskipConfig(PrevQP, asic, pEncIn, vcenc_instance);

  /* Global MV */
  StrmEncodeGlobalmvConfig(asic, pic, pEncIn, vcenc_instance);

  /* multi-core sync */
  asic->regs.mc_sync_enable = (vcenc_instance->mcEnable);
//  //printf("asic->regs.mc_sync_enable = %d\n", asic->regs.mc_sync_enable);

  if (asic->regs.mc_sync_enable) {
    asic->regs.mc_sync_l0_addr =
        (codingType == VCENC_INTRA_FRAME ? 0 : pic->rpl[0][0]->mc_sync_addr);
    asic->regs.mc_sync_l1_addr = (codingType != VCENC_BIDIR_PREDICTED_FRAME
                                      ? 0
                                      : pic->rpl[1][0]->mc_sync_addr);
    asic->regs.mc_sync_rec_addr = pic->mc_sync_addr;
    asic->regs.mc_ref_ready_threshold = 64;
    asic->regs.mc_ddr_polling_interval =
        (IS_H264(vcenc_instance->codecFormat) ? 8 : 32) *
        pic->pps->ctb_per_row;
    /* clear sync address */
    memset((u32 *)pic->mc_sync_ptr, 0,
           MC_SYNC_WORD_BYTES);  //hw read 16Byte a time
    EWLSyncMemData(&asic->mc_sync_word[pic->picture_memeory_id], 0,
                   MC_SYNC_WORD_BYTES, HOST_TO_DEVICE);
  }
  ...
  pic->recon_compress.chromaCompressed =
      asic->regs.recon_chroma_compress ? 1 : 0;

  VCEncHEVCDnfPrepare(vcenc_instance, codingType);

  /* set rdoLevel=0 when disableRDO */
  if (asic->regs.PredModeBySatdEnable && asic->regs.RefFrameUsingInputFrameEnable) {
    asic->regs.rdoLevel = 0;
  }
  vcenc_instance->preProcess.bottomField =
      (vcenc_instance->poc % 2) == vcenc_instance->fieldOrder;
  HevcUpdateSeiPS(&vcenc_instance->rateControl.sei,
                  vcenc_instance->preProcess.interlacedFrame,
                  vcenc_instance->preProcess.bottomField);
  ...
  for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
    asic->regs.inputLumBase =
        (tileId == 0) ? pEncIn->busLuma : pEncIn->tileExtra[tileId - 1].busLuma;
    asic->regs.inputCbBase = (tileId == 0)
                                 ? pEncIn->busChromaU
                                 : pEncIn->tileExtra[tileId - 1].busChromaU;
    asic->regs.inputCrBase = (tileId == 0)
                                 ? pEncIn->busChromaV
                                 : pEncIn->tileExtra[tileId - 1].busChromaV;
    ...
    for (int i = 0; i < MAX_OVERLAY_NUM; i++) {
      vcenc_instance->preProcess.overlayInputYAddr[i] = pEncIn->overlayInputYAddr[i];
      vcenc_instance->preProcess.overlayInputUAddr[i] = pEncIn->overlayInputUAddr[i];
      vcenc_instance->preProcess.overlayInputVAddr[i] = pEncIn->overlayInputVAddr[i];
    }
    EncPreProcess(asic, &vcenc_instance->preProcess, vcenc_instance->ctx,
                  tileId);
    ...
    vcenc_instance->tileCtrl[tileId].inputChromaBaseOffset =
        asic->regs.inputChromaBaseOffset;
    vcenc_instance->tileCtrl[tileId].inputLumaBaseOffset =
        asic->regs.inputLumaBaseOffset;
    vcenc_instance->tileCtrl[tileId].stabNextLumaBase =
        asic->regs.stabNextLumaBase;
  }
  vcenc_instance->preProcess.lumWidth = reservedLumWidth;
u32 heightStride = vcenc_instance->preProcess.lumHeightSrc[0];

  /* configure OSDMap parameter */
  StrmEncodeOSDMapConfig(asic, pEncIn, vcenc_instance);

  if (vcenc_instance->aif.enable) {
    VCEncAifPrepare(inst, pEncIn, pic);
  }
  EncUfbcParam ufbc_param;
  if (VCEncCfgUfbc(pEncIn, vcenc_instance, asic, &ufbc_param)) {
    return VCENC_INVALID_ARGUMENT;
  }
  VCEncCfgDec400(pEncIn, vcenc_instance, asic, 0);

#ifdef SYSTEM_BUILD
#ifdef TEST_DATA
#ifdef SUPPORT_UFBC
  if (vcenc_instance->ufbcParam.mode) {
    int asic_format = EncPreGetHwFormat(vcenc_instance->preProcess.inputFormat);
    ...
    if (trace_cam_data_ufbc(pEncIn->busLuma, pEncIn->busChromaU,
                            pEncIn->busChromaV, asic_format,
                            asic->regs.input_luma_stride,
                            asic->regs.input_chroma_stride,
                            vcenc_instance->preProcess.lumWidthSrc[0],
                            vcenc_instance->preProcess.lumHeightSrc[0],
                            ufbc_param.height,
                            vcenc_instance->frameCnt) == -1) {
      return VCENC_ERROR;
    }
  }
#endif /* TEST_DATA */
  ...
  VCEncResetCallback(&slice_callback, vcenc_instance, pEncIn, pEncOut,
                     next_core_index, 0);
  ...
  if (useExtFlag) {
    if (vcenc_instance->pass == 2 && pExtParaQ != NULL)
      useExtPara(pExtParaQ, asic, vcenc_instance->codecFormat,
                 vcenc_instance);
    else if (vcenc_instance->pass == 0)
      useExtPara(pEncExtParaIn, asic, vcenc_instance->codecFormat,
                 vcenc_instance);
  }
  ...
  VCEncVisualGenConfig(vcenc_instance, pic, pEncIn);
  ...
  vcenc_instance->asic.regs.tileStrmSizeAlignmentExp =
      vcenc_instance->tileStrmSizeAlignmentExp;
  if (vcenc_instance->num_tile_columns > 1) {
    u32 ctu_size = (1 << (IS_H264(vcenc_instance->codecFormat) ? 4 : 6));
    int tile_sync_size =
        (vcenc_instance->num_tile_columns - 1) *
    ...
    memset(
        (u8 *)vcenc_instance->asic.deblockCtx.virtualAddress + tile_sync_size,
        0, 4 * (vcenc_instance->num_tile_columns - 1));
#ifdef SUPPORT_MEM_SYNC
    EWLSyncMemData(&vcenc_instance->asic.deblockCtx, tile_sync_size,
                   4 * (vcenc_instance->num_tile_columns - 1), HOST_TO_DEVICE);
#endif
    ...
    HevcConfigureTestBeforeStart(vcenc_instance, pic);
#endif
  ...
  do {
    ...
    for (tileId = 0; tileId < vcenc_instance->num_tile_columns; tileId++) {
      if (!vcenc_instance->cb_try_new_params || re_encode_counter == 0) {
        asic->dec400_data->streams[0].width =
            vcenc_instance->preProcess.lumWidthSrc[tileId];
        asic->dec400_data->streams[0].height =
            vcenc_instance->preProcess.lumHeightSrc[tileId];
      ...
      if (vcenc_instance->cb_try_new_params && re_encode_counter == 0 &&
          tileId == 0) {
        memcpy(&vcenc_instance->regs_bak[cur_core_index], &asic->regs,
               sizeof(regValues_s));
      }

      vcenc_instance->axiFEEnable = pEncIn->axiFEEnable;
#ifdef SUPPORT_AXIFE
      VCEncCfgAxiFe(vcenc_instance, asic, 0, tileId);
#endif

re_encode:
      if (!vcenc_instance->asic.regs.bVCMDEnable) {
        /* Try to reserve the HW resource */
        if (EWLReserveHw(vcenc_instance->asic.ewl,
        ...
        VCEncSetSubsystem(vcenc_instance, pEncIn, asic, pic, tileId);
    ...
    if (vcenc_instance->parallelCoreNum > 1) {
      if (re_encode_counter == 0) {
        ...
        memcpy(&vcenc_instance->streams[cur_core_index],
               &vcenc_instance->stream, sizeof(struct buffer));
        ...
        if (vcenc_instance->pass == 1) {
          ASSERT(vcenc_instance->EncOut[cur_core_index].tileExtra ==
                 pEncOut->tileExtra);
          vcenc_instance->EncOut[cur_core_index] = *pEncOut;
          for (tileId = 1; tileId < vcenc_instance->num_tile_columns;
               tileId++) {
            vcenc_instance->EncOut[cur_core_index]
                .tileExtra[tileId - 1]
                .pNaluSizeBuf = pEncOut->tileExtra[tileId - 1].pNaluSizeBuf;
            vcenc_instance->EncOut[cur_core_index]
                .tileExtra[tileId - 1]
                .streamSize = pEncOut->tileExtra[tileId - 1].streamSize;
            vcenc_instance->EncOut[cur_core_index]
                .tileExtra[tileId - 1]
                .numNalus = pEncOut->tileExtra[tileId - 1].numNalus;
            vcenc_instance->EncOut[cur_core_index]
                .tileExtra[tileId - 1]
                .cuOutData = pEncOut->tileExtra[tileId - 1].cuOutData;
          }
        ...
        if (vcenc_instance->reservedCore >=
          ...
          /* when all parallel slot are used, will wait the first job done */
          queue_put(&c->picture, (struct node *)pic);
          pic = vcenc_instance->pic[next_core_index];
          queue_remove(&c->picture, (struct node *)pic);

          memcpy(&vcenc_instance->stream,
                 &vcenc_instance->streams[next_core_index],
          ...
          if (vcenc_instance->pass == 1) {
            ...
            *pEncIn = vcenc_instance->EncIn[next_core_index];
            ...
            pEncOut->consumedAddr.osdMapInputBusAddr =
                vcenc_instance->EncIn[next_core_index].osdMapInputAddr;
          ...
          if (vcenc_instance->pass != 2) pEncIn->tileExtra = tmpTileExtra;
          for (tileId = 1; tileId < vcenc_instance->num_tile_columns; tileId++)
            for (iBuf = 0; iBuf < MAX_STRM_BUF_NUM; iBuf++) {
              pEncIn->tileExtra[tileId - 1].pOutBuf[iBuf] =
                  vcenc_instance->EncIn[cur_core_index]
        ...
        if (vcenc_instance->pass == 2 && outframe)
          pEncOut->consumedAddr.roiMapDeltaQpBusAddr =
              outframe->pRoiMapDeltaQpAddr;
      } else {
        pEncOut->consumedAddr.outbufBusAddr = pEncIn->busOutBuf[0];
      }
    } else {
      /* single pass and pass2 need to record outputbufferAddr in pEncout */
      if (vcenc_instance->pass != 1)
      {
        ...
        memcpy(pEncOut->consumedAddr.overlayInputBusAddr,
               pEncIn->overlayInputYAddr, MAX_OVERLAY_NUM * sizeof(ptr_t));
        pEncOut->consumedAddr.osdMapInputBusAddr =
...
#ifdef MULTI_FRAME_SUPPORT
	  if (vcenc_instance->batchEnable != true)
#endif
	  ret = VCENC_FRAME_ENQUEUE;

      // update frame count after one frame is ready.
      vcenc_instance->frameCnt++;
    ...
    if (vcenc_instance->cb_try_new_params) {
      ...
      VCEncCollectEncodeStats(vcenc_instance, pEncIn, pEncOut, codingType,
                              &enc_statistic);
      use_new_parameters =
          vcenc_instance->cb_try_new_params(&enc_statistic, &new_params);
      if (use_new_parameters) {
        pEncOut->numNalus = numNalusTmp;
        VCEncResetEncodeStatus(vcenc_instance, c, pEncIn, pic, pEncOut,
                               &enc_statistic);
        VCEncSetReEncodeParameters(vcenc_instance, pEncIn, pEncOut,
                                 &slice_callback, &new_params,
                                 regs_bak_for2nd_encode);
      }
      re_encode_counter++;
    }
  ...
  vcenc_instance->readCnt++;
  vcenc_instance->jobCnt++;  // update job count after one frame is ready.
  pEncOut->pNaluSizeBuf =
      (u32 *)vcenc_instance->asic
          .sizeTbl[vcenc_instance->jobCnt % vcenc_instance->parallelCoreNum]
          .virtualAddress;
  vcenc_instance->outGopSize = pEncIn->gopSize;

#ifdef INTERNAL_TEST
    if( vcenc_instance->testId == TID_SBI_ERROR 
        && (pEncIn->poc!=0) ) {                                         /* not skip IDR frame */
      /* pretend RC overflow on certain frames for internal test */
      if ( (pEncIn->poc != last_skip_poc)                                /* prevent repeat drop */
          && (pEncIn->gopPicIdx == last_gop_idx) ) { /* for all gopPicIdx nubmer */
        ret = VCENC_SBI_ERROR;
        last_skip_poc = pEncIn->poc;
        last_gop_idx++;
        if (last_gop_idx == pEncIn->gopSize)
          last_gop_idx = 0;
      }
    }
#endif
  /* process SBI error */
  if (ret == VCENC_SBI_ERROR && vcenc_instance->pass != 2) {
    handleFrameDiscard(vcenc_instance, pEncIn, pEncOut, pic, true);
    APITRACE("VCEncStrmEncode: WARNING, Frame discarded (SBI error)\n");
    //return VCENC_FRAME_READY;
  }

  if (ret != VCENC_OK) goto out;

  if (vcenc_instance->aif.enable) {
    VCEncInfo *info = (VCEncInfo *)pEncIn->internal;
    /* sideline work don't require post RC and stream */
    if (info!=NULL && info->aif.is_sideline) {
      is_sideline = 1;
      goto out;
    }
  ...
  if (vcenc_instance->output_buffer_over_flow == 1) {
    handleFrameDiscard(vcenc_instance, pEncIn, pEncOut, pic, false);
    ret = VCENC_OUTPUT_BUFFER_OVERFLOW;
  ...
  vcenc_instance->rateControl.complexity =
      (RCP_64bit)vcenc_instance->rateControl.rcPicComplexity *
      (vcenc_instance->rateControl.reciprocalOfNumBlk8);
#endif

  const enum slice_type _slctypes[3] = {P_SLICE, I_SLICE, B_SLICE};
  vcenc_instance->rateControl.sliceTypeCur =
      _slctypes[asic->regs.frameCodingType];
  if (vcenc_instance->motionScoreEnable) {
    memcpy(vcenc_instance->rateControl.curMotionScores, asic->regs.motionScore,
           sizeof(asic->regs.motionScore));
  }
  rcVirtualBuffer_s vb_save =
      vcenc_instance->rateControl.virtualBuffer;//save rc state.
  stat = VCEncAfterPicRc(&vcenc_instance->rateControl, 0,
                         vcenc_instance->stream.byteCnt, asic->regs.sumOfQP,
                         asic->regs.sumOfQPNumber);
#ifdef INTERNAL_TEST
    if( vcenc_instance->testId == TID_RCOVERFLOW
        && (pEncIn->poc!=0) ) {                                         /* not skip IDR frame */
      /* pretend RC overflow on certain frames for internal test */
      if ( (pEncIn->poc != last_skip_poc)                                /* prevent repeat drop */
          && (pEncIn->gopPicIdx == last_gop_idx) ) { /* for all gopPicIdx nubmer */
        vcenc_instance->rateControl.frameCoded = ENCHW_NO;
        stat = VCENCRC_OVERFLOW;
        last_skip_poc = pEncIn->poc;
        last_gop_idx++;
        if (last_gop_idx == pEncIn->gopSize)
          last_gop_idx = 0;
  ...
  i32 lastISize = vcenc_instance->rateControl.rcPred[I_SLICE].prevFrameBits;
  i32 idrInterval = pEncIn->gopConfig.idr_interval;
  if (pEncIn->gopConfig.idr_interval == 0)
  {
    u64 denom = (u64)pEncIn->gopConfig.inputRateNumer * (u64)pEncIn->gopConfig.outputRateDenom;
    u64 numer = (u64)pEncIn->gopConfig.inputRateDenom * (u64)pEncIn->gopConfig.outputRateNumer;
    idrInterval = MAX(1, ((pEncIn->gopConfig.lastPic - pEncIn->gopConfig.firstPic + 1) * numer / denom) - pEncIn->last_idr_picture_cnt);
  }
  i32 lastISizeModified = lastISize * (pEncIn->picture_cnt - pEncIn->last_idr_picture_cnt + 1) / idrInterval;
  vcenc_instance->currentActualBitRate = (vcenc_instance->rateControl.encodedFramesBits - lastISize + lastISizeModified) * pEncIn->gopConfig.outputRateNumer
      / (MAX(1, vcenc_instance->rateControl.frameCnt) * pEncIn->gopConfig.outputRateDenom);

  if (pEncIn->codingType != VCENC_INTRA_FRAME && pEncIn->codingType != VCENC_BIDIR_PREDICTED_FRAME)
    VCEncVisualStrengthUpdate(vcenc_instance, pEncIn);
  ...
  if (stat == VCENCRC_OVERFLOW && vcenc_instance->pass != 2) {
    ...
    if(vcenc_instance->sw_skip_flag){
      streamRollback(&vcenc_instance->stream,
                      pEncOut, vcenc_instance->asic.regs.outputStrmSize[0]);
      u32 prev_bytes = vcenc_instance->stream.byteCnt;
      ...
      if(vcenc_instance->lookaheadDepth != 0) {
        ...
        if(pEncIn->codingType == VCENC_INTRA_FRAME) {
          if(asic->regs.qp == 51) {
            APITRACE_INFO(vcenc_instance, "=== The QP is 51, but the cpb buffer still overflows,\n");
            APITRACE_INFO(vcenc_instance, "=== Encoder stops...\n");
            return VCENC_OUTPUT_BUFFER_OVERFLOW;
          } else {
            APITRACE_INFO(vcenc_instance, "=== Set Qp to 51, re-encoding...\n");
            asic->regs.qp = 51;
            vcenc_instance->rateControl.virtualBuffer = vb_save;
            vcenc_instance->rateControl.frameCoded = ENCHW_YES;
            sw_ref_cnt_decrease(pic);
            goto re_encode;
          }
        } else {
          ASSERT((pEncIn->codingType == VCENC_PREDICTED_FRAME) ||
                 (pEncIn->codingType == VCENC_BIDIR_PREDICTED_FRAME));
          sw_skip_frame(inst, pic, pExtParaQ);
          /* update the nal unit size buffer */
          VCEncAddNaluSize(pEncOut, vcenc_instance->stream.byteCnt-prev_bytes, 0);
          /* assume stream is not overflow after insert the skip frame */
          stat = 0;
          /* recover the vbv state. TODO: should consider the skip frame size in VBV. */
          vcenc_instance->rateControl.virtualBuffer  = vb_save;
          vcenc_instance->rateControl.frameCoded = ENCHW_YES;
        }
      }
      APITRACE("VCEncStrmEncode: Insert Skip Frame when HRD overflow)\n");
    }else {
      handleFrameDiscard(vcenc_instance, pEncIn, pEncOut, pic, true);
      APITRACE("VCEncStrmEncode: WARNING, Frame discarded (HRD overflow)\n");
    }
    //return VCENC_FRAME_READY;
  } else
  ...
  /* get frame info  */
  VCEncGetFrameInfo(vcenc_instance, pEncIn, asic, pEncOut);

  /* get luma info */
  if (asic->regs.enablelumaInfo)
  {
    pEncOut->lumaInfo = (u16*)((ptr_t)asic->framInfoMem.virtualAddress + asic->regs.lumaInfo_offset);
  }

  // add filler when
  //  - vbv buffer underflow (stat > 0)
  //  - HRD is enable and in cbr mode or force to add fillerData by app
  if ((stat > 0) && ((vcenc_instance->rateControl.hrd &&
                      vcenc_instance->rateControl.cbr_flag) ||
  ...
  if (stat != VCENCRC_OVERFLOW) {
    ...
    if (vcenc_instance->gdrEnabled == 1) {
      if (vcenc_instance->gdrStart) vcenc_instance->gdrCount++;
      if (vcenc_instance->gdrCount == (vcenc_instance->gdrDuration *
                                       (1 + (i32)vcenc_instance->interlaced))) {
        vcenc_instance->gdrStart = 0;
  ...
  if (vcenc_instance->preProcess.videoStab) {
    ...
    VSReadStabData(vcenc_instance->asic.regs.regMirror,
                   &vcenc_instance->vsHwData);

    no_motion =
        VSAlgStabilize(&vcenc_instance->vsSwData, &vcenc_instance->vsHwData);
    if (no_motion) {
      VSAlgReset(&vcenc_instance->vsSwData);
    }

    /* update offset after stabilization */
    VSAlgGetResult(&vcenc_instance->vsSwData,
                   &vcenc_instance->preProcess.horOffsetSrc[0],
                   &vcenc_instance->preProcess.verOffsetSrc[0]);
  ...
  VCEncHEVCDnfUpdate(vcenc_instance, pic);
...
out:
  ...
  for (tileId = 1; tileId < asic->regs.num_tile_columns; tileId++) {
    pEncOut->tileExtra[tileId - 1].streamSize =
        asic->regs.tiles_enabled_flag
            ? vcenc_instance->tileCtrl[tileId].streamSize
            : vcenc_instance->stream.byteCnt;
  }
  if (VCENC_OK != VCEncCodecPostEncodeUpdate(vcenc_instance, pEncIn, pEncOut,
                                             codingType, pic))
    return VCENC_ERROR;

  sw_ref_cnt_decrease(pic);
  if (vcenc_instance->reservedCore) vcenc_instance->reservedCore--;
  if (vcenc_instance->pass == 2) {
    ReleaseLookaheadPicture(&vcenc_instance->lookahead, outframe,
                            pEncOut->consumedAddr.inputbufBusAddr);
  ...
  /** AIF */
  if (vcenc_instance->aif.enable) {
    VCEncRet aif_ret = VCEncAifUpdate(inst, pEncIn);
    if (aif_ret == VCENC_FRAME_CONTINUE) {
      VceTaskRelease(vcenc_instance, pEncIn);
      return aif_ret;
    }
    VceTaskRelease(vcenc_instance, pEncIn);
  }
...
/*------------------------------------------------------------------------------
    ...
    Function name : VCEncStrmGetOutput
                    ...
                    void *pAppData)
{
  ...
  APITRACE("VCEncStrmGetOutput#\n");

  /* Check for illegal inputs */
  if ((vcenc_instance == NULL) || (pEncIn == NULL) || (pEncOut == NULL)) {
    APITRACEERR("VCEncStrmGetOutput: ERROR Null argument\n");
    return VCENC_NULL_ARGUMENT;
  }

  /* Check for existing instance */
  if (vcenc_instance->inst != vcenc_instance) {
    APITRACEERR("VCEncStrmGetOutput: ERROR Invalid instance\n");
    return VCENC_INSTANCE_ERROR;
  }
  /* Check status, INIT and ERROR not allowed */
  if ((vcenc_instance->encStatus != VCENCSTAT_START_STREAM) &&
      (vcenc_instance->encStatus != VCENCSTAT_START_FRAME)) {
    APITRACEERR("VCEncStrmGetOutput: ERROR Invalid status\n");
    return VCENC_INVALID_STATUS;
  }
  ...
  if (vcenc_instance->batchEnable)
  {
	  if (vcenc_instance->readCnt >= vcenc_instance->sendCnt) {
		  return VCENC_OK;
	  }
  }
#endif
  if (!(c = get_container(vcenc_instance))) return VCENC_ERROR;
    cur_core_index = vcenc_instance->readCnt % vcenc_instance->parallelCoreNum;
    next_core_index = cur_core_index;
  ...
  do {
    if (re_encode_counter != 0) {
      vcenc_instance->axiFEEnable = pEncIn->axiFEEnable;
#ifdef SUPPORT_AXIFE
      VCEncCfgAxiFe(vcenc_instance, asic, 0, 0);
#endif
      if (!vcenc_instance->asic.regs
               .bVCMDEnable)  //multi-tile do not support re-encode in VCEncStrmGetOutput
      {  //vcenc_instance->waitJobid[current_core_index]==vcenc_instance->tileCtrl[n].job_id -> n=tileId
        ...
        /* Enable L2Cache/dec400/AXIFE/APBFilter */
        VCEncSetSubsystem(vcenc_instance, pEncIn, asic, pic, 0);
        ...
        do {
          ...
          if (EncAsicFrameStart(asic->ewl, &asic->regs, asic->dumpRegister) == 0)
            break;
        } while(1);
      } else {
        ret = vcencVcmdStart(vcenc_instance, pic, asic);
    ...
    for (tileId = 0; tileId < vcenc_instance->asic.regs.num_tile_columns;
      ...
      */
      ...
      if (vcenc_instance->cb_try_new_params) {
        regs_bak_for2nd_encode = &vcenc_instance->regs_bak[next_core_index];
        VCEncCollectEncodeStats(vcenc_instance, pEncIn, pEncOut,
                                pEncOut->codingType, &enc_statistic);
        use_new_parameters =
            vcenc_instance->cb_try_new_params(&enc_statistic, &new_params);
        if (use_new_parameters) {
          VCEncResetEncodeStatus(vcenc_instance, c, pEncIn, pic, pEncOut,
  ...
  i32 stat;
  {
    ...
    vcenc_instance->rateControl.sliceTypeCur =
        _slctypes[asic->regs.frameCodingType];
    if (vcenc_instance->motionScoreEnable) {
      memcpy(vcenc_instance->rateControl.curMotionScores,
             asic->regs.motionScore, sizeof(asic->regs.motionScore));
    }
    stat = VCEncAfterPicRc(&vcenc_instance->rateControl, 0,
                           vcenc_instance->stream.byteCnt, asic->regs.sumOfQP,
                           asic->regs.sumOfQPNumber);
...
#ifdef INTERNAL_TEST
	//printf("TID_RCOVERFLOW frameCnt=%d\n", vcenc_instance->frameCnt);
	if( vcenc_instance->testId == TID_RCOVERFLOW
		&& (pEncIn->poc!=0) ) { 										/* not skip IDR frame */
...
//	  static int last_gop_idx = 0;
  ...
  pEncOut->pNaluSizeBuf =
      (u32 *)vcenc_instance->asic
          .sizeTbl[vcenc_instance->readCnt % vcenc_instance->parallelCoreNum]
          .virtualAddress;
  vcenc_instance->readCnt++;  // update frame count after one frame is ready.
...
VCEncRet VCEncFlush(VCEncInst inst, VCEncIn *pEncIn, VCEncOut *pEncOut,
  ...
  if (vcenc_instance->reservedCore > 0) {
    ...
    asicData_s *asic = &vcenc_instance->asic;
    /* enable cache jobs */
    if ((vcenc_instance->batchEnable)&&(vcenc_instance->cacheCnt>0)) {

  	u32 client_type = VCEncGetClientType(vcenc_instance->codecFormat);
  	vcenc_instance->batchCount = 1;
  	ret = vcencBatchRunCmdbuf(vcenc_instance, asic, NULL,
  							  client_type);
    }
#endif /* MULTI_FRAME_SUPPORT */
    ret = VCEncStrmGetOutput(inst, pEncIn, pEncOut, sliceReadyCbFunc, NULL);
    return ret;
  }
...
VCEncRet VCEncStrmEnd(VCEncInst inst, const VCEncIn *pEncIn,
  ...
  if (vcenc_instance->pass == 2 && vcenc_instance->lookahead.priv_inst) {
    ...
    if (vcenc_instance->num_tile_columns > 1) {
      encOut.tileExtra = vcenc_instance->EncOut[0].tileExtra;
      if (encOut.tileExtra == NULL) return VCENC_ERROR;
    } else
      encOut.tileExtra = NULL;
    encIn.gopConfig.pGopPicCfg = pEncIn->gopConfig.pGopPicCfgPass1;
...
/*------------------------------------------------------------------------------
  out_of_memory
------------------------------------------------------------------------------*/
i32 out_of_memory(struct vcenc_buffer *n, u32 size) { return (size > n->cnt); }
...
/*------------------------------------------------------------------------------
  vcenc_ref_pic_sets
------------------------------------------------------------------------------*/
i32 vcenc_ref_pic_sets(struct vcenc_instance *vcenc_instance,
                       const VCEncIn *pEncIn) {
  struct vcenc_buffer *vcenc_buffer;
  ...
  for (i = 0; i < gopCfg->size; i++) {
    ...
    u32 iRefPic, idx = 0;
    for (iRefPic = 0; iRefPic < cfg->numRefPics; iRefPic++) {
      if (IS_LONG_TERM_REF_DELTAPOC(cfg->refPics[iRefPic].ref_pic)) continue;
      poc[idx++] = cfg->refPics[iRefPic].ref_pic;
      poc[idx++] = cfg->refPics[iRefPic].used_by_cur;
    }
...
u32 VCEncGetRoiMapVersion(u32 core_id, void *ctx) {
  const EWLHwConfig_t *cfg = EWLReadAsicConfig(core_id, ctx);
  if (cfg)
 ...
 */
bool_e VCEncCheckCfg(const VCEncConfig *enc_cfg, const EWLHwConfig_t *asic_cfg,
                     void *ctx) {
  i32 height = enc_cfg->height;
  u32 client_type, asicHwId;
  true_e ret = true;
  i32 cfg_profile = VCEncGetProfile(enc_cfg, 1, NULL);
  ...
  if (enc_cfg->aifEnable) {
    VCEncRet vcret = VCEncAifCheckConfig(enc_cfg, asic_cfg);
    if (VCENC_OK != vcret)
      return ENCHW_NOK;
  }
...
/* Supported codec check */
  ...
  if (IS_AV1(enc_cfg->codecFormat) || IS_VP9(enc_cfg->codecFormat)) {
    if (enc_cfg->streamType != VCENC_BYTE_STREAM) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid stream type, need byte stream when AV1 or "
          "VP9\n");
      return ENCHW_NOK;
    }
    ...
    if (IS_AV1(enc_cfg->codecFormat)) {
      if (enc_cfg->width > VCENC_AV1_MAX_ENC_WIDTH) {
        APITRACEERR(
            "VCEncCheckCfg: Invalid width, need 4096 or smaller when AV1\n");
        return ENCHW_NOK;
      }

      if (enc_cfg->width * enc_cfg->height > VCENC_AV1_MAX_ENC_AREA) {
        APITRACEERR(
            "VCEncCheckCfg: Invalid area, need 4096*2304 or below when AV1\n");
        return ENCHW_NOK;
      }
    } else {
      ...
      */
      if (enc_cfg->width > VCENC_VP9_MAX_ENC_WIDTH) {
        APITRACEERR(
            "VCEncCheckCfg: Invalid width, need 4096 or smaller when only one "
            "tile in the VP9\n");
        return ENCHW_NOK;
      } else if (enc_cfg->height > VCENC_VP9_MAX_ENC_HEIGHT) {
        APITRACEERR(
            "VCEncCheckCfg: Invalid height, need 8384 or smaller when VP9\n");
        return ENCHW_NOK;
      }
    }
  }
  ...
  if (enc_cfg->num_tile_rows == 1 &&
      (height < VCENC_MIN_ENC_HEIGHT || height > VCENC_MAX_ENC_HEIGHT_EXT ||
       (height & 0x1) != 0)) {
  ...
  /* check profile */
  {
    ...
    u8 h264Constrain = (IS_H264(enc_cfg->codecFormat) &&
                        (cfg_profile > VCENC_H264_HIGH_10_PROFILE ||
                         cfg_profile < VCENC_H264_BASE_PROFILE));
    u8 h2Constrain =
        (((HW_ID_MAJOR_NUMBER(asicHwId)) < 4) && HW_PRODUCT_H2(asicHwId) &&
         (cfg_profile == VCENC_HEVC_MAIN_10_PROFILE));
    u8 hevcConstrain = (IS_HEVC(enc_cfg->codecFormat) &&
                        (cfg_profile >= VCENC_HEVC_PROFILE_NUM));
    if ((i32)cfg_profile < VCENC_HEVC_MAIN_PROFILE || h264Constrain ||
        hevcConstrain || h2Constrain) {
      APITRACEERR("VCEncCheckCfg: Invalid profile\n");
      return ENCHW_NOK;
    }
    if (IS_AV1(enc_cfg->codecFormat) && (cfg_profile > VCENC_AV1_MAIN_PROFILE)) {
      APITRACEERR("VCEncCheckCfg: Invalid profile\n");
      return ENCHW_NOK;
    }
  }
  ...
  if (VCENC_CHROMA_IDC_444 == enc_cfg->codedChromaIdc &&
      asic_cfg->HevcYUV444Support == EWL_HW_CONFIG_ENABLED) {
    if (!(IS_HEVC(enc_cfg->codecFormat))) {
      APITRACEERR("VCEncInit: codedChromaIdc 444 Only supported by HEVC\n");
      return ENCHW_NOK;
    } else if (cfg_profile < VCENC_HEVC_MAIN_444_10_PROFILE) {
      APITRACEERR("VCEncInit: WARNING profile for HevcYUV444\n");
    }
  }
  if (VCENC_CHROMA_IDC_422 == enc_cfg->codedChromaIdc &&
      asic_cfg->HevcYUV422Support == EWL_HW_CONFIG_ENABLED) {
    if (!(IS_HEVC(enc_cfg->codecFormat))) {
      APITRACEERR("VCEncInit: codedChromaIdc 422 Only supported by HEVC\n");
      return ENCHW_NOK;
    } else if (cfg_profile < VCENC_HEVC_MAIN_422_10_PROFILE) {
      APITRACEERR("VCEncInit: WARNING profile for HevcYUV422\n");
    }
  }
  if (VCENC_CHROMA_IDC_400 == enc_cfg->codedChromaIdc &&
      asic_cfg->MonoChromeSupport == EWL_HW_CONFIG_ENABLED) {
    if (IS_HEVC(enc_cfg->codecFormat) &&
        cfg_profile != VCENC_HEVC_MONOCHROME_PROFILE &&
        cfg_profile != VCENC_HEVC_MONOCHROME_10_PROFILE) {
      APITRACEERR("VCEncInit: WARNING profile for MonoChrome\n");
    } else if (IS_H264(enc_cfg->codecFormat) &&
               cfg_profile < VCENC_H264_HIGH_PROFILE) {
      APITRACEERR("VCEncInit: WARNING profile for MonoChrome\n");

    } else if (IS_AV1(enc_cfg->codecFormat)) {
      APITRACEERR("VCEncInit: codedChromaIdc 400 not supported by AV1\n");
      return ENCHW_NOK;
    } else if (IS_VP9(enc_cfg->codecFormat)) {
      APITRACEERR("VCEncInit: codedChromaIdc 400 not supported by VP9\n");
      return ENCHW_NOK;

    }
  }

  /* check tile in VCENC_CHROMA_IDC_422/444, CModel/HW TODO */
  if ((enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_422
	|| enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_444)
	&& enc_cfg->tiles_enabled_flag) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid, tile not supported by codedChromaIdc 422/444\n");
      return ENCHW_NOK;
  }

  /* check TMVP in VCENC_CHROMA_IDC_444 */
  if (enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_444 &&
      enc_cfg->enableTMVP) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid, TMVP not supported by codedChromaIdc 444\n");
      return ENCHW_NOK;
  }

  /* check 2-pass in VCENC_CHROMA_IDC_444 */
  if (enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_444 &&
      enc_cfg->lookaheadDepth != 0) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid, 2-pass not supported by codedChromaIdc 444\n");
      return ENCHW_NOK;
  }

  /* check ssim in VCENC_CHROMA_IDC_444 */
  if (enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_444 && enc_cfg->enableSsim) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid, SSIM not supported by codedChromaIdc 444\n");
      return ENCHW_NOK;
  }

  /* check psnr in VCENC_CHROMA_IDC_444 */
  if (enc_cfg->codedChromaIdc == VCENC_CHROMA_IDC_444 && enc_cfg->enablePsnr) {
      APITRACEERR(
          "VCEncCheckCfg: Invalid, PSNR not supported by codedChromaIdc 444\n");
      return ENCHW_NOK;
  }

  /* check bit depth for main8 and main still profile */
  if (cfg_profile < (IS_H264(enc_cfg->codecFormat)
      ...
      (IS_HEVC(enc_cfg->codecFormat) || IS_H264(enc_cfg->codecFormat)) &&
       enc_cfg->codedChromaIdc != VCENC_CHROMA_IDC_422 &&
       enc_cfg->codedChromaIdc != VCENC_CHROMA_IDC_444) {
  ...
  if ((enc_cfg->bitDepthLuma != 8 || enc_cfg->bitDepthChroma != 8) &&
    ...
    APITRACEERR(
        "VCEncCheckCfg: Not support higher bit depth in current hardware\n");
    return ENCHW_NOK;
  }

  if (enc_cfg->ctbRcMode != 0  && asic_cfg->ctbRcSupport == 0) {
    APITRACEERR(
        "VCEncCheckCfg: CTBRC is not supported due to HW limitation\n");
    return ENCHW_NOK;
  }
  if (enc_cfg->ctbRcMode > 1  && asic_cfg->ctbRcVersion < 1) {
    APITRACEERR(
        "VCEncCheckCfg: CTBRC Mode is not supported due to HW limitation\n");
    return ENCHW_NOK;
  }

  if (enc_cfg->compressor == 1 || enc_cfg->compressor == 2) {
    APITRACEERR(
        "VCEncCheckCfg: Compressor not support 1 or 2 due to HW limitation\n");
    return ENCHW_NOK;
  }
  ...
  if (enc_cfg->codedChromaIdc > VCENC_CHROMA_IDC_444) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid codedChromaIdc, it's should be 0 ~ 3\n");
    return ENCHW_NOK;
  }

  /* check bit depth for main10 profile */
  if (((cfg_profile == VCENC_HEVC_MAIN_10_PROFILE) ||
       (cfg_profile == VCENC_H264_HIGH_10_PROFILE)) &&
  ...
  if (((IS_HEVC(enc_cfg->codecFormat) &&
        ((enc_cfg->level > VCENC_HEVC_MAX_LEVEL) ||
         (enc_cfg->level < VCENC_HEVC_LEVEL_1))) ||
       (IS_H264(enc_cfg->codecFormat) &&
        ((enc_cfg->level > VCENC_H264_MAX_LEVEL) ||
         (enc_cfg->level < VCENC_H264_LEVEL_1)) &&
  ...
  if ((enc_cfg->exp_of_input_alignment < 4 &&
       enc_cfg->exp_of_input_alignment > 0) ||
      (enc_cfg->exp_of_ref_alignment < 4 &&
       enc_cfg->exp_of_ref_alignment > 0) ||
      (enc_cfg->exp_of_ref_ch_alignment < 4 &&
       enc_cfg->exp_of_ref_ch_alignment > 0)) {
  ...
  if (IS_HEVC(enc_cfg->codecFormat) && enc_cfg->num_tile_columns > 1 &&
    ...
    APITRACEERR(
        "VCEncCheckCfg: Invalid parallelCoreNum, multi-tile-columns encoding "
        "only support parallelCoreNum = 1 when num_tile_columns > 1. \n");
    return ENCHW_NOK;
  }

  const EWLHwConfig_t *cfg = EncAsicGetAsicConfig(client_type, ctx);
  /* is HEVC encoding supported */
  if (!cfg) {
    APITRACEERR("VCEncCheckCfg: ERROR Null argument\n");
    return ENCHW_NOK;
  }
  if (IS_HEVC(enc_cfg->codecFormat) &&
      cfg->hevcEnabled == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid format, hevc not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }
  ...
  if (IS_H264(enc_cfg->codecFormat) &&
      cfg->h264Enabled == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid format, h264 not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }
  ...
  if (IS_AV1(enc_cfg->codecFormat) &&
      cfg->av1Enabled == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid format, av1 not supported by HW coding core\n");
    return ENCHW_NOK;
  }
  ...
  if (IS_VP9(enc_cfg->codecFormat) &&
      cfg->vp9Enabled == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid format, VP9 not supported by HW coding core\n");
    return ENCHW_NOK;
  }

  if ((cfg->cuInforVersion == 0x07) && enc_cfg->enableOutputCuInfo) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid enableOutputCuInfo, not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }
  ...
  if (enc_cfg->num_tile_columns == 1 &&
      ((IS_H264(enc_cfg->codecFormat)
            ? cfg->maxEncodedWidthH264
            : cfg->maxEncodedWidthHEVC) < enc_cfg->width)) {
    APITRACEERR(
  ...
  /* P010 Ref format */
  if (enc_cfg->P010RefEnable &&
      cfg->P010RefSupport == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: Invalid format, P010Ref not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }
  ...
  if (enc_cfg->num_tile_rows == 1 && (height > VCENC_MAX_ENC_HEIGHT) &&
    ...
    APITRACEERR(
        "VCEncCheckCfg: Invalid height, height extension not supported by HW "
        "coding core\n");
    return ENCHW_NOK;
  }

  /*disable recon write to DDR*/
  if (enc_cfg->writeReconToDDR == 0 &&
      cfg->disableRecWtSupport == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: disable recon write to DDR not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }

  /* Check ctb bits  */
  if (enc_cfg->enableOutputCtbBits &&
      cfg->CtbBitsOutSupport == EWL_HW_CONFIG_NOT_SUPPORTED) {
    APITRACEERR(
        "VCEncCheckCfg: ctb output encoded bits not supported by HW coding "
        "core\n");
    return ENCHW_NOK;
  }
  ...
  if (enc_cfg->enableTMVP) {
    i32 tmvpSupport =
        IS_VP9(enc_cfg->codecFormat) ||
        (asic_cfg->hevcTemporalMvpSupport && IS_HEVC(enc_cfg->codecFormat)) ||
        (asic_cfg->av1TemporalMvpSupport && IS_AV1(enc_cfg->codecFormat));
    if (!tmvpSupport) {
      APITRACEERR(
          "VCEncCheckCfg: TMVP is not supported for this codec format\n");
      return ENCHW_NOK;
    }
  }

  if (enc_cfg->refRingBufEnable &&
      (asic_cfg->refRingBufEnable == EWL_HW_CONFIG_NOT_SUPPORTED)) {
  ...
  if (enc_cfg->refRingBufEnable && (enc_cfg->parallelCoreNum != 1)) {
    APITRACEERR("VCEncCheckCfg: ringbuffer is only supported for single core\n");
    return ENCHW_NOK;
  }

  /* frame info and luma info support*/
  if (enc_cfg->enableFrameInfo && asic_cfg->frameInfoSupport == EWL_HW_CONFIG_NOT_SUPPORTED)
  {
    APITRACEERR("VCEncCheckCfg: FrameInfo is not supported\n");
    return ENCHW_NOK;
  }

  if (enc_cfg->enablelumaInfo && (enc_cfg->enableFrameInfo == 0 ||
      asic_cfg->frameInfoSupport == EWL_HW_CONFIG_NOT_SUPPORTED ||
      asic_cfg->prpAnalyzerSupport == EWL_HW_CONFIG_NOT_SUPPORTED))
  {
    APITRACEERR("VCEncCheckCfg: lumaInfo is not supported\n");
    return ENCHW_NOK;
  }

#ifdef MULTI_FRAME_SUPPORT
  /* use true/false can check all errors */
  ret = ret && vcencBatchCheckCfg(enc_cfg, asic_cfg, ctx);
#endif /* MULTI_FRAME_SUPPORT */
...
static VCEncLevel calculate_level(const VCEncConfig *pEncCfg) {
  ...
  u64 sample_per_second =
      sample_per_picture * pEncCfg->frameRateNum / pEncCfg->frameRateDenom;
  ...
  for (i = 0; i < maxLevel; i++) {
    if (sample_per_picture <= getMaxPicSize(pEncCfg->codecFormat, i)) break;
  }
  for (j = 0; j < maxLevel; j++) {
    if (sample_per_second <= getMaxSBPS(pEncCfg->codecFormat, j)) break;
  }
...
bool_e SetParameter(struct vcenc_instance *inst, const VCEncConfig *pEncCfg) {
  ...
  inst->asic.regs.recon_chroma_compress =
      ((pEncCfg->compressor & 2) &&
       (!((VCENC_CHROMA_IDC_400 == pEncCfg->codedChromaIdc) &&
          (inst->asic.regs.asicCfg->MonoChromeSupport ==
           EWL_HW_CONFIG_ENABLED))))
          ? 1
          : 0;

  /* frame Info */
  inst->asic.regs.enableFrameInfo = pEncCfg->enableFrameInfo;

  /* luma Info */
  inst->asic.regs.enablelumaInfo = pEncCfg->enablelumaInfo;
  ...
  if (((i32)inst->asic.regs.asicCfg->cuInforVersion < pEncCfg->cuInfoVersion) ||
      (pEncCfg->cuInfoVersion == 0 &&
       inst->asic.regs.asicCfg->cuInforVersion != 0)) {
  ...
  if (inst->asic.regs.asicCfg->cuInforVersion >= 2) {
    ...
    if (pEncCfg->pass == 1)
      inst->asic.regs.cuInfoVersion =
          inst->asic.regs.asicCfg->bMultiPassSupport
              ? inst->asic.regs.asicCfg->cuInforVersion
              : 1;
    else
      /* if cuinfo is enabled and cuinfoVersion != -1, use customer's config,
         else set to 2 for motion score */
      inst->asic.regs.cuInfoVersion =
          (pEncCfg->enableOutputCuInfo && pEncCfg->cuInfoVersion != -1)
              ? pEncCfg->cuInfoVersion
              : 2;
  } else
  ...
  /* Ctb encoded bits output */
  inst->asic.regs.enableOutputCtbBits =
      pEncCfg->enableOutputCtbBits && pEncCfg->pass != 1;
  ...
  inst->levelIdx = EncGetLevelIdx(inst->codecFormat, inst->level);
  ...
  if (IS_H264(inst->codecFormat) && pEncCfg->rdoLevel > 0) {
    APITRACEERR(
        "WARNING: rdoLevel 1/2 NOT support in H264, set rdoLevel to 0\n");
    inst->rdoLevel = inst->asic.regs.rdoLevel = 0;
  }

  /* VCENC_CHROMA_IDC_422: HW TODO */
  inst->asic.regs.codedChromaIdc =
      VCEncGetChromaIdc(pEncCfg->codedChromaIdc, inst);
  inst->profile = VCEncGetProfile(pEncCfg, 1, NULL);
  if ( VCENC_ADAPTIVE_PROFILE == pEncCfg->profile ) {
    inst->adaptive_profile = inst->profile;
  } else {
    inst->adaptive_profile = -1;
  }
  /* Monochrome not support HEVC_MAIN_STILL_PICTURE profile */
  if (inst->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_400 &&
      (inst->profile == VCENC_HEVC_MAIN_STILL_PICTURE_PROFILE ||
      inst->profile == VCENC_HEVC_MAIN_10_STILL_PICTURE_PROFILE)) {
    APITRACEERR(
        "ERROR: 'HEVC_MAIN_STILL_PICTURE_PROFILE' not Supported for "
        "Monochrome\n");
    return ENCHW_NOK;
  }

  /* codedChromaIdc 420 invalid for 'Main 422 10' and monochrome 8/10 profile */
  if (IS_HEVC(inst->codecFormat) &&
      (inst->profile == VCENC_HEVC_MONOCHROME_PROFILE ||
      inst->profile == VCENC_HEVC_MONOCHROME_10_PROFILE ||
        inst->profile == VCENC_HEVC_MAIN_422_10_PROFILE ||
        inst->profile == VCENC_HEVC_MAIN_444_10_PROFILE ||
      inst->profile == VCENC_HEVC_SCC_444_10_PROFILE) &&
      (VCENC_CHROMA_IDC_420 == pEncCfg->codedChromaIdc)) {
    APITRACEERR(
        "ERROR: 'Invalid chromaIdc for 'monochrome 8/10' ,'Main 422 10' or '444 10' profile\n");
    return ENCHW_NOK;
  }

  /* VCENC_CHROMA_IDC_422: HW TODO  */
  /* VCENC_CHROMA_IDC_422 not support HEVC_MAIN_STILL_PICTURE profile */
  if (inst->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_422 &&
      inst->profile == VCENC_HEVC_MAIN_STILL_PICTURE_PROFILE) {
    APITRACEERR(
        "ERROR: 'HEVC_MAIN_STILL_PICTURE_PROFILE' not Supported for "
        "codedChromaIdc 422\n");
  ...
  /* VCENC_CHROMA_IDC_422 must be VCENC_HEVC_MAIN_422_10_PROFILE profile */
  if (inst->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_422 &&
      inst->profile != VCENC_HEVC_MAIN_422_10_PROFILE) {
    APITRACEERR(
        "WARNING: Profile force to be 'VCENC_HEVC_MAIN_422_10_PROFILE' for "
        "codedChromaIdc 422\n");
    inst->profile = VCENC_HEVC_MAIN_422_10_PROFILE;
  }

  /* VCENC_CHROMA_IDC_444 must be VCENC_HEVC_MAIN_444_10_PROFILE or
     VCENC_HEVC_SCC_444_10_PROFILE profile */
  if (IS_HEVC(inst->codecFormat) &&
      inst->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_444 &&
      (inst->profile != VCENC_HEVC_MAIN_444_10_PROFILE &&
       inst->profile != VCENC_HEVC_SCC_444_10_PROFILE)) {
      if(inst->enableTS) {
        APITRACEERR(
            "WARNING: Profile force to be 'VCENC_HEVC_SCC_444_10_PROFILE' for codedChromaIdc 444\n");
        inst->profile = VCENC_HEVC_SCC_444_10_PROFILE;
      } else {
        APITRACEERR(
            "WARNING: Profile force to be 'VCENC_HEVC_MAIN_444_10_PROFILE' for codedChromaIdc 444\n");
        inst->profile = VCENC_HEVC_MAIN_444_10_PROFILE;
      }
  }

  /* Monochrome for hevc must be HEVC_REXT or h264 must be H264_HIGH profile */
  if (inst->asic.regs.codedChromaIdc == VCENC_CHROMA_IDC_400) {
    if (IS_HEVC(inst->codecFormat) && !(inst->profile == VCENC_HEVC_MONOCHROME_PROFILE ||
        inst->profile == VCENC_HEVC_MONOCHROME_10_PROFILE)) {
      APITRACEERR(
          "WARNING: Profile force to be 'VCENC_HEVC_MONOCHROME_PROFILE' for "
          "Monochrome; force to be 'VCENC_HEVC_MONOCHROME_10_PROFILE' for Monochrome 10\n");
      inst->profile = VCENC_HEVC_MONOCHROME_PROFILE;
      if (2 == inst->sps->bit_depth_luma_minus8) {
        inst->profile = VCENC_HEVC_MONOCHROME_10_PROFILE;
      }
    } else if (IS_H264(inst->codecFormat) &&
              inst->profile < VCENC_H264_HIGH_PROFILE) {
      APITRACEERR(
          "WARNING: Profile force to be 'VCENC_H264_HIGH_PROFILE' for "
          "Monochrome\n");
      inst->profile = VCENC_H264_HIGH_PROFILE;
    }
  }
  /* enforce maximum frame size in level */
  if ((u32)(inst->width * inst->height) >
      getMaxPicSize(inst->codecFormat, inst->levelIdx)) {
    APITRACEERR("WARNING: MaxFS violates level limit\n");
  }

  /* enforce Max luma sample rate in level */
  {
    u32 sample_rate = (pEncCfg->frameRateNum * inst->width * inst->height) /
  ...
  if (((HW_PRODUCT_SYSTEM60(inst->asic.regs.asicHwId)) ||
       (HW_PRODUCT_VC9000LE(inst->asic.regs.asicHwId))) &&
      (IS_H264(pEncCfg->codecFormat))) /* VCE6.0 and h264 */
  {
  ...
  bps = EncGetMaxBR(inst->codecFormat, inst->levelIdx, inst->profile, inst->tier, NAL_HRD_PARA_FLAG);

  {
...
#ifdef RCP_FIXED_POINT_OPT
    rc->reciprocalOfNumBlk8 =
        (RCP_32bit)((Q31_FIXED_ONE +
                     (rc->ctbPerPic * (ctb_size / 8) * (ctb_size / 8)) / 2) /
                    (rc->ctbPerPic * (ctb_size / 8) * (ctb_size / 8)));
#else
    rc->reciprocalOfNumBlk8 =
        1.0 / (rc->ctbPerPic * (ctb_size / 8) * (ctb_size / 8));
#endif
...

    {
      ...
      vb->bufferSize = EncGetMaxCPBS(inst->codecFormat, inst->levelIdx,
            inst->profile, inst->tier, NAL_HRD_PARA_FLAG);
      CHECK_API_I(" level:profile:tier=%d:%d:%d, MaxCPBS=%d, bitRate=%d\n",
        inst->levelIdx, inst->profile, inst->tier, vb->bufferSize, vb->bitRate);
    }
  ...
  inst->preProcess.scanType = VCENC_RASTER_SCAN;
  ...
  inst->preProcess.inLoopDSRatio =
      (pEncCfg->pass == 1) ? pEncCfg->inLoopDSRatio : 0;
...
u32 VCEncGetMaxWidth(const EWLHwConfig_t *asic_cfg,
                     VCEncVideoCodecFormat codec_format) {
  if (IS_HEVC(codec_format)) return asic_cfg->maxEncodedWidthHEVC;
  if (IS_H264(codec_format)) return asic_cfg->maxEncodedWidthH264;
...
void IntraLamdaQp(int qp, u32 *lamda_sad, enum slice_type type, int poc,
                  float dQPFactor, bool depth0) {
  ...
  // pre-compute lambda and QP values for all possible QP candidates
  {
    ...
    if (!depth0) lambda *= sqrtQpDiv6[qp];
  }
...
void InterLamdaQp(int qp, unsigned int *lamda_sse, unsigned int *lamda_sad,
                  enum slice_type type, float dQPFactor, bool depth0,
                  u32 asicId) {
  float recalQP = (float)qp;
  float lambda;
  ...
  if ((HW_ID_MAJOR_NUMBER(asicId)) >= 5 ||
      !HW_PRODUCT_H2(asicId)) /* H2V5 rev01 */
  {
...
void IntraLamdaQpFixPoint(int qp, u32 *lamda_sad, enum slice_type type, int poc,
...
void InterLamdaQpFixPoint(int qp, unsigned int *lamda_sse,
                          unsigned int *lamda_sad, enum slice_type type,
                          u32 qpFactorSad, u32 qpFactorSse, bool depth0,
                          u32 asicId) {
  i32 shiftSad =
      LAMBDA_FIX_POINT + QPFACTOR_FIX_POINT - MOTION_LAMBDA_SAD_SHIFT;
  i32 shiftSse =
      LAMBDA_FIX_POINT + QPFACTOR_FIX_POINT - MOTION_LAMBDA_SSE_SHIFT;
  u32 roundSad = 1 << (shiftSad - 1);
...
void LamdaTableQp(regValues_s *regs, int qp, enum slice_type type, int poc,
                  double dQPFactor, bool depth0, u32 ctbRc) {
  int qpoffset, lamdaIdx;
#ifdef FIX_POINT_LAMBDA
  u32 qpFactorSad = (u32)((dQPFactor * (1 << QPFACTOR_FIX_POINT)) + 0.5f);
  u32 qpFactorSse =
      (u32)((dQPFactor * dQPFactor * (1 << QPFACTOR_FIX_POINT)) + 0.5f);
#endif
#ifndef CTBRC_STRENGTH
...
#ifdef FIX_POINT_LAMBDA
      IntraLamdaQpFixPoint(qp - qpoffset,
                           regs->lambda_satd_ims + (lamdaIdx & 0xf), type, poc,
                           qpFactorSad, depth0);
      InterLamdaQpFixPoint(qp - qpoffset,
                           regs->lambda_sse_me + (lamdaIdx & 0xf),
                           regs->lambda_satd_me + (lamdaIdx & 0xf), type,
                           qpFactorSad, qpFactorSse, depth0, regs->asicHwId);
#else
      IntraLamdaQp(qp - qpoffset, regs->lambda_satd_ims + (lamdaIdx & 0xf),
                   type, poc, dQPFactor, depth0);
      InterLamdaQp(qp - qpoffset, regs->lambda_sse_me + (lamdaIdx & 0xf),
                   regs->lambda_satd_me + (lamdaIdx & 0xf), type, dQPFactor,
...
#ifdef FIX_POINT_LAMBDA
      IntraLamdaQpFixPoint(qp - qpoffset, regs->lambda_satd_ims + lamdaIdx,
                           type, poc, qpFactorSad, depth0);
      InterLamdaQpFixPoint(qp - qpoffset, regs->lambda_sse_me + lamdaIdx,
                           regs->lambda_satd_me + lamdaIdx, type, qpFactorSad,
                           qpFactorSse, depth0, regs->asicHwId);
#else
      IntraLamdaQp(qp - qpoffset, regs->lambda_satd_ims + lamdaIdx, type, poc,
                   dQPFactor, depth0);
      InterLamdaQp(qp - qpoffset, regs->lambda_sse_me + lamdaIdx,
                   regs->lambda_satd_me + lamdaIdx, type, dQPFactor, depth0,
                   regs->asicHwId);
#endif
    }
  }
  ...
  for (qpoffset = -16, lamdaIdx = 16 /* - regs->offsetSliceQp*/;
       qpoffset <= +15; qpoffset++, lamdaIdx++) {
#ifdef FIX_POINT_LAMBDA
    IntraLamdaQpFixPoint(CLIP3(0, 51, qp - qpoffset + regs->offsetSliceQp),
                         regs->lambda_satd_ims + (lamdaIdx & 0x1f), type, poc,
                         qpFactorSad, depth0);
    InterLamdaQpFixPoint(CLIP3(0, 51, qp - qpoffset + regs->offsetSliceQp),
                         regs->lambda_sse_me + (lamdaIdx & 0x1f),
                         regs->lambda_satd_me + (lamdaIdx & 0x1f), type,
                         qpFactorSad, qpFactorSse, depth0, regs->asicHwId);
#else
    ...
    InterLamdaQp(CLIP3(0, 51, qp - qpoffset + regs->offsetSliceQp),
                 regs->lambda_sse_me + (lamdaIdx & 0x1f),
                 regs->lambda_satd_me + (lamdaIdx & 0x1f), type, dQPFactor,
                 depth0, regs->asicHwId);
#endif
  }
  ...
  if (regs->asicCfg->roiAbsQpSupport) {
    ...
    regs->qpFactorSse =
        (u32)((dQPFactor * dQPFactor * (1 << QPFACTOR_FIX_POINT)) + 0.5);
...
static void VCEncSetQuantParameters(struct vcenc_instance *vcenc_instance,
  ...
  static unsigned short VCEncIntraPenaltyTu4[52] = {
      7,    7,    8,    10,   11,   12,   14,   15,  17,  20,  22,
      25,   28,   31,   35,   40,   44,   50,   56,  63,  71,  80,
      89,   100,  113,  127,  142,  160,  179,  201, 226, 254, 285,
      320,  359,  403,  452,  508,  570,  640,  719, 807, 905, 1016,
      1141, 1281, 1438, 1614, 1811, 2033, 2282, 2562};  //max*3=13bit
  ...
  static unsigned short VCEncIntraPenaltyTu16[52] = {
      9,    11,   12,   14,   15,   17,   19,   22,   24,   28,   31,
      35,   39,   44,   49,   56,   62,   70,   79,   88,   99,   112,
      125,  141,  158,  177,  199,  224,  251,  282,  317,  355,  399,
      448,  503,  564,  634,  711,  799,  896,  1006, 1129, 1268, 1423,
      1598, 1793, 2013, 2259, 2536, 2847, 3196, 3587};  //max*3=14bit
  ...
  static unsigned short VCEncIntraPenaltyModeA[52] =
      {
          ...
          30, 33, 38, 42, 47, 53, 60, 67, 76, 85, 95, 107, 120};  //max*3=9bit
  ...
  static unsigned short VCEncIntraPenaltyModeB[52] = {
      ...
      13, 14, 16, 18, 21,  23,  26,  29,  33,  37, 42, 47, 53, 59,
      66, 75, 84, 94, 106, 119, 133, 150, 168, 189};  ////max*3=10bit

  /* Intra Penalty for Mode c */
  static unsigned short VCEncIntraPenaltyModeC[52] = {
      1,   1,   1,   1,   1,   1,   2,   2,   2,   3,  3,  3,  4,   4,
      5,   6,   6,   7,   8,   9,   10,  12,  13,  15, 17, 19, 21,  24,
      27,  31,  34,  39,  43,  49,  55,  62,  69,  78, 87, 98, 110, 124,
      139, 156, 175, 197, 221, 248, 278, 312, 351, 394};  //max*3=11bit
  ...
  switch (HW_ID_MAJOR_NUMBER(asic->regs.asicHwId)) {
    case 1: /* H2V1 rev01 */
      ...
      u32 roi1_qp =
          CLIP3(0, 51, ((int)asic->regs.qp - (int)asic->regs.roi1DeltaQp));
      ...
      u32 roi2_qp =
          CLIP3(0, 51, ((int)asic->regs.qp - (int)asic->regs.roi2DeltaQp));
      ...
      asic->regs.intraMPMPenaltyRoi21 =
          VCEncIntraPenaltyModeA[roi2_qp];  //need to change
      asic->regs.intraMPMPenaltyRoi22 =
          VCEncIntraPenaltyModeB[roi2_qp];  //need to change
      asic->regs.intraMPMPenaltyRoi23 =
          VCEncIntraPenaltyModeC[roi2_qp];  //need to change

      /*inter prediction parameters*/
#ifndef FIX_POINT_LAMBDA
      InterLamdaQp(asic->regs.qp, &asic->regs.lamda_motion_sse,
                   &asic->regs.lambda_motionSAD, type, qp_factor, is_depth0,
                   asic->regs.asicHwId);
      InterLamdaQp(asic->regs.qp - asic->regs.roi1DeltaQp,
                   &asic->regs.lamda_motion_sse_roi1,
                   &asic->regs.lambda_motionSAD_ROI1, type, qp_factor,
                   is_depth0, asic->regs.asicHwId);
      InterLamdaQp(asic->regs.qp - asic->regs.roi2DeltaQp,
                   &asic->regs.lamda_motion_sse_roi2,
                   &asic->regs.lambda_motionSAD_ROI2, type, qp_factor,
                   is_depth0, asic->regs.asicHwId);
#else
      u32 qpFactorSad = (u32)((qp_factor * (1 << QPFACTOR_FIX_POINT)) + 0.5f);
      u32 qpFactorSse =
          (u32)((qp_factor * qp_factor * (1 << QPFACTOR_FIX_POINT)) + 0.5f);
      InterLamdaQpFixPoint(asic->regs.qp, &asic->regs.lamda_motion_sse,
                           &asic->regs.lambda_motionSAD, type, qpFactorSad,
                           qpFactorSse, is_depth0, asic->regs.asicHwId);
      InterLamdaQpFixPoint(asic->regs.qp - asic->regs.roi1DeltaQp,
                           &asic->regs.lamda_motion_sse_roi1,
                           &asic->regs.lambda_motionSAD_ROI1, type, qpFactorSad,
                           qpFactorSse, is_depth0, asic->regs.asicHwId);
      InterLamdaQpFixPoint(asic->regs.qp - asic->regs.roi2DeltaQp,
                           &asic->regs.lamda_motion_sse_roi2,
                           &asic->regs.lambda_motionSAD_ROI2, type, qpFactorSad,
                           qpFactorSse, is_depth0, asic->regs.asicHwId);
#endif
    ...
    case 2: /* H2V2 rev01 */
    case 3: /* H2V3 rev01 */
      LamdaTableQp(&asic->regs, asic->regs.qp, type, asic->regs.poc, qp_factor,
                   is_depth0, enable_ctu_rc);
...

  {
    ...
    asic->regs.lamda_SAO_chroma =
        MIN(asic->regs.lamda_SAO_chroma, maxSaoLambda);
  }

  /*seperate qpfactorSAD and qpfactorSSE setting if modSubjPrefer!=0,
    increased qpfactor only used in refine stage*/
  if (vcenc_instance->asic.regs.asicCfg->modSubjPrefer)
  {
    asic->regs.qpFactorSad = (u32)((qp_factor * (1 << QPFACTOR_FIX_POINT)) + 0.5);
    asic->regs.qpFactorSse = (u32)((vcenc_instance->qpfactorSSE * (1 << QPFACTOR_FIX_POINT)) + 0.5);
  }

  /* For experiment, lambda setting for subjective quality */
  if (vcenc_instance->rateControl.visualLmdTuning &&
      asic->regs.asicCfg->roiAbsQpSupport) {
    if (vcenc_instance->rateControl.qpFactor <= 0)
      LambdaTuningSubjective(asic, pic, pEncIn);
  }

  //all intra frames using fixed sadFactor and sseFactor.
  if (pEncIn->gopConfig.idr_interval == 1) {
    double factorSad = 1.6;
    double factorSse = 0.6;
    asic->regs.qpFactorSad = (u32)((factorSad * (1 << QPFACTOR_FIX_POINT)) + 0.5);
    asic->regs.qpFactorSse = (u32)((factorSse * (1 << QPFACTOR_FIX_POINT)) + 0.5);
  }

  /* For experiment, tuning rdoq lambda for subjective quality */
  true_e rdoqMapEnable = asic->regs.rdoqMapEnable &&
                         (asic->regs.asicCfg->roiMapVersion == 4) &&
  ...
  if (vcenc_instance->bRdoqLambdaAdjust &&
      ...
      (IS_H264(vcenc_instance->codecFormat) ||
       IS_HEVC(vcenc_instance->codecFormat)) &&
      asic->regs.asicCfg->roiAbsQpSupport &&
      asic->regs.asicCfg->tuneToolsSet2Support) {
    ...
    if (factorIntra < asic->regs.qpFactorSse) {
      vcenc_instance->asic.regs.av1_plane_rdmult[0][0] =
          vcenc_instance->asic.regs.av1_plane_rdmult[0][1] =
              ((factorIntra << 4) + asic->regs.qpFactorSse / 2) /
              asic->regs.qpFactorSse;
    }

    if (factorInter < asic->regs.qpFactorSse) {
      vcenc_instance->asic.regs.av1_plane_rdmult[1][0] =
          vcenc_instance->asic.regs.av1_plane_rdmult[1][1] =
              ((factorInter << 4) + asic->regs.qpFactorSse / 2) /
              asic->regs.qpFactorSse;
    }
  }
}
...
void VCEncSetNewFrame(VCEncInst inst) {
  ...
  /* Enable slice ready interrupts if defined by config and slices in use */
  regs->enable_slice_irq = vcenc_instance->inputLineBuf.enable_slice_irq;
  regs->sliceReadyInterrupt =
      regs->enable_slice_irq & (regs->sliceNum > 1);
  /* HW base address for NAL unit sizes is affected by start offset
   * and SW created NALUs. */
  regs->sizeTblBase =
      asic->sizeTbl[vcenc_instance->jobCnt % vcenc_instance->parallelCoreNum]
          .busAddress +
      vcenc_instance->numNalus[vcenc_instance->jobCnt %
                               vcenc_instance->parallelCoreNum] *
          4;
  ...
  regs->lineBufferHwHandShake =
      vcenc_instance->inputLineBuf.inputLineBufHwModeEn;
  regs->lineBufferLoopBackEn =
      vcenc_instance->inputLineBuf.inputLineBufLoopBackEn;
  ...
  regs->segmentUnitHeight = vcenc_instance->inputLineBuf.segmentUnitHeight;

#ifdef LOW_LATENCY_SLICEINFO_SUPPORT
    /* configure poll input sliceinfo registers for low latency */
    regs->sliceinfo_poll_enable = vcenc_instance->inputSliceInfoPollEn;
#endif
...
static void VCEncHEVCDnfInit(struct vcenc_instance *vcenc_instance) {
  ...
  vcenc_instance->iSigmaCur = vcenc_instance->iFirstFrameSigma =
      11 << FIX_POINT_BIT_WIDTH;

  /* DENOISE 3DNR NLM */
  vcenc_instance->noiseReductionStrength_IntraY = 7;
  vcenc_instance->noiseReductionStrength_IntraU = 7;
  vcenc_instance->noiseReductionStrength_IntraV = 7;
  vcenc_instance->noiseReductionStrength_InterY = 7;
  vcenc_instance->noiseReductionStrength_InterU = 7;
  vcenc_instance->noiseReductionStrength_InterV = 7;
  vcenc_instance->noiseReduction_ChromaMaxMV = 4;
  //printf("init seq noiseL = %d, SigCur = %d, first = %d\n\n\n\n\n",pEncInst->iNoiseL, regs->nrSigmaCur,  pCodeParams->firstFrameSigma);
#endif
}

static void VCEncHEVCDnfGetParameters(struct vcenc_instance *inst,
                                      VCEncCodingCtrl *pCodeParams) {
...
static void VCEncHEVCDnfPrepare(struct vcenc_instance *vcenc_instance, VCEncPictureCodingType codingType) {
...
#if defined(DENOISE_3DNR)
  if (codingType == VCENC_INTRA_FRAME)
  {
    asic->regs.noiseReductionEnable = (vcenc_instance->lastPicNLM == NULL)? 0: vcenc_instance->uiNoiseReductionEnable;
  }
  else
  {
    asic->regs.noiseReductionEnable = vcenc_instance->uiNoiseReductionEnable;
  }
#endif
...
#ifdef SYSTEM_BUILD
#ifndef SYSTEM60_BUILD
  hwNrSigmaCur = vcenc_instance->iSigmaCur;
#endif
#endif
#endif
#if defined(DENOISE_3DNR) && DENOISE_3DNR_INTRAUSENLM3D
  if (vcenc_instance->uiNoiseReductionEnable && vcenc_instance->lastPicNLM) {
    if (codingType == VCENC_INTRA_FRAME) {
      /* use lastPicNLM as 3DNR reference */
      asic->regs.pRefPic_recon_l0[0][0] = vcenc_instance->lastPicNLM->recon.lum;
      asic->regs.pRefPic_recon_l0[1][0] = vcenc_instance->lastPicNLM->recon.cb;
      asic->regs.pRefPic_recon_l0[2][0] = vcenc_instance->lastPicNLM->recon.cr;
      asic->regs.pRefPic_recon_l0_4n[0] = vcenc_instance->lastPicNLM->recon_4n_base;
      asic->regs.l0_delta_poc[0] = vcenc_instance->lastPicNLM->poc - vcenc_instance->poc;
      asic->regs.ref_l0_luma_compressed[0] =
          vcenc_instance->lastPicNLM->recon_compress.lumaCompressed;
      asic->regs.ref_l0_luma_compress_tbl_base[0] =
          vcenc_instance->lastPicNLM->recon_compress.lumaTblBase;
      asic->regs.ref_l0_chroma_compressed[0] =
          vcenc_instance->lastPicNLM->recon_compress.chromaCompressed;
      asic->regs.ref_l0_chroma_compress_tbl_base[0] =
          vcenc_instance->lastPicNLM->recon_compress.chromaTblBase;
    } else {
      /* release lastPicNLM if unused */
      // vcenc_instance->lastPicNLM->ref_cnt--; // call this earlier after init_poc_list(), for case 8740
      // vcenc_instance->lastPicNLM = NULL;
    }
  }
#endif
...
static void VCEncHEVCDnfUpdate(struct vcenc_instance *vcenc_instance, struct sw_picture *pic) {
  ...
  if (((vcenc_instance->uiNoiseReductionEnable == 0) &&
       (!vcenc_instance->smartModeNoiseEn)) ||
      vcenc_instance->stream.byteCnt == 0)
    return;
#if defined(DENOISE_3DNR) && DENOISE_3DNR_INTRAUSENLM3D
  /* delayed release of lastPicNLM */
  if (vcenc_instance->lastPicNLM) {
    vcenc_instance->lastPicNLM->ref_cnt--;
    vcenc_instance->lastPicNLM = NULL;
  }
  /* save current pic, which may be used by NLM if next is I frame */
  vcenc_instance->lastPicNLM = pic;
  /* delay release of lastPicNLM */
  pic->ref_cnt++;
#endif

  if (uiFrmNum == 0) {
    vcenc_instance->FrmNoiseSigmaSmooth[0] =
        vcenc_instance->iFirstFrameSigma;
    //(pvcenc_instance->asic.regs.firstFrameSigma<< FIX_POINT_BIT_WIDTH);
    //pvcenc_instance->iFirstFrmSigma;//(double)((51-QpSlc-5)/2);
  }
  int iFrmSigmaRcv = vcenc_instance->iSigmaCalcd;//first(65535) or pic->swctrl->nrFrameSigmaCalced(iSigmaCurFrm)
  ...
  vcenc_instance->FrmNoiseSigmaSmooth[(uiFrmNum + 1) % SIGMA_SMOOTH_NUM] =
      iFrmSigmaRcv;
  ...
  CurSigmaTmp = (dSumFrmNoiseSigma *
                 SigmaSmoothFactor[MIN(SIGMA_SMOOTH_NUM - 1, uiFrmNum + 1)]) >>
                10;
  vcenc_instance->iSigmaCur =
      CLIP3(0, (SIGMA_MAX << FIX_POINT_BIT_WIDTH), CurSigmaTmp);
  vcenc_instance->iThreshSigmaCur = vcenc_instance->iThreshSigmaPrev =
      iThSigmaRcv;
...
u32 VCEncGetEncodedMbLines(VCEncInst inst) {
  ...
  if (vcenc_instance->inputLineBuf.inputLineBufHwModeEn) {
    ...
    lines = EncAsicGetRegisterValue(vcenc_instance->asic.ewl,
                                    vcenc_instance->asic.regs.regMirror,
                                    HWIF_CTB_ROW_RD_PTR);
...
VCEncRet VCEncSetInputMBLines(VCEncInst inst, u32 lines) {
  ...
  if (!vcenc_instance->asic.regs.bVCMDEnable) {
    /* the regMirror[] maybe not update well, so read it from the register
     * firstly */
    u32 offset = BASE_HWIF_CTB_ROW_WR_PTR;
    vcenc_instance->asic.regs.regMirror[offset/4] =
                   EWLReadReg(vcenc_instance->asic.ewl, offset);
    EncAsicWriteRegisterValue(vcenc_instance->asic.ewl,
                            vcenc_instance->asic.regs.regMirror,
                            HWIF_CTB_ROW_WR_PTR, lines);
  } else {
    EncAsicWriteRegisterValueByVcmd(vcenc_instance->asic.ewl,
                            vcenc_instance->asic.regs.regMirror,
                            HWIF_CTB_ROW_WR_PTR, lines);
  }
...
void VCEncGenSliceHeaderRps(struct vcenc_instance *vcenc_instance,
  ...
  i32 i32NegDeltaPoc[VCENC_MAX_REF_FRAMES],
      i32PosDeltaPoc[VCENC_MAX_REF_FRAMES];
  i32 i32NegDeltaPocUsed[VCENC_MAX_REF_FRAMES],
      i32PosDeltaPocUsed[VCENC_MAX_REF_FRAMES];
  u32 used_by_cur;
  ...
  regs->short_term_ref_pic_set_sps_flag =
      pHwRps->u1short_term_ref_pic_set_sps_flag;
...
------------------------------------------------------------------------------*/
void VCEncGenPicRefConfig(struct container *c, VCEncGopPicConfig *cfg,
                          struct sw_picture *pCurPic, i32 curPicPoc) {
  struct sw_picture *p;
  struct node *n;
  i32 i, j;
  i32 i32PicPoc[VCENC_MAX_REF_FRAMES] = {0};
  ...
  for (n = c->picture.tail; n; n = n->next) {
    p = (struct sw_picture *)n;
    if ((p->long_term_flag == HANTRO_FALSE) && (p->reference == HANTRO_TRUE) &&
        (p->poc > -1) && cfg->numRefPics < VCENC_MAX_REF_FRAMES) {
...
void VCEncTrace(const char *msg) {
  ...
  if (fp == NULL) fp = fopen("api.trc", "wt");
#endif
  if (fp) fprintf(fp, "%s\n", msg);
}
 ...
 */
static i32 VCEncWriteFillerDataForCBR(struct vcenc_instance *vcenc,
                                      i32 length) {
  i32 has_startcode =
      (vcenc->asic.regs.streamMode == VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO;
  i32 nalu_overhead;
  i32 nalu_length = 0;
  ...
  if (length < nalu_overhead) {
    ...
    vcenc->rateControl.virtualBuffer.bucketFullness +=
        (nalu_overhead - length) * 8;
    vcenc->rateControl.virtualBuffer.realBitCnt += (nalu_overhead - length) * 8;
    vcenc->rateControl.virtualBuffer.bucketLevel +=
        (nalu_overhead - length) * 8;
  } else {
...
static void VCEncParameterSetHandle(struct vcenc_instance *vcenc_instance,
  ...
  if (pEncIn->sendAUD) {
    tmp = vcenc_instance->stream.byteCnt;
    if (IS_H264(vcenc_instance->codecFormat)) {
      H264AccessUnitDelimiter(&vcenc_instance->stream,
                              vcenc_instance->asic.regs.streamMode,
                              pEncIn->codingType);
    } else {
      HEVCAccessUnitDelimiter(&vcenc_instance->stream,
                              vcenc_instance->asic.regs.streamMode,
                              pEncIn->codingType);
    }
    pEncOut->streamSize += vcenc_instance->stream.byteCnt - tmp;
  ...
  struct sps *s_br =
      (struct sps *)get_parameter_set(c, SPS_NUT, vcenc_instance->sps_id);
  if (s_br == NULL) return;
  ...
  u32 new_bitrate =
      round_this_value((u32)vcenc_instance->rateControl.virtualBuffer.bitRate);

  i32 old_frame_rate_num = vcenc_instance->rateControl.virtualBuffer.timeScale;
  i32 old_frame_rate_denom =
      vcenc_instance->rateControl.virtualBuffer.unitsInTic;
  ...
  u32 old_hrd =
      vcenc_instance->rateControl.sei.hrd;  //This hrd has not been update.
  u32 new_hrd =
      vcenc_instance->rateControl
          .hrd;  //This value already has been update in VCEncSetRateCtrl.

  if (old_bitrate == 0)  //hrd == 0
  {
    s_br->vui.bitRate = new_bitrate;  //save this value.
  }
  u32 frame_rate_change = (new_frame_rate_num != old_frame_rate_num ||
       new_frame_rate_denom != old_frame_rate_denom);
  if (pEncIn->resendSPS ||
      ...
      (new_hrd != old_hrd))  //hrd conformance change
  {
    ...
    VCEncSpsSetVuiAspectRatio(s_br, vcenc_instance->sarWidth,
                              vcenc_instance->sarHeight);
    VCEncSpsSetVuiVideoInfo(s_br, vcenc_instance->vuiVideoFullRange);

    if (vcenc_instance->vuiVideoSignalTypePresentFlag)
      VCEncSpsSetVuiSignalType(
          s_br, vcenc_instance->vuiVideoSignalTypePresentFlag,
          vcenc_instance->vuiVideoFormat, vcenc_instance->vuiVideoFullRange,
          vcenc_instance->vuiColorDescription.vuiColorDescripPresentFlag,
          vcenc_instance->vuiColorDescription.vuiColorPrimaries,
          vcenc_instance->vuiColorDescription.vuiTransferCharacteristics,
    ...
    if ((new_bitrate != old_bitrate) &&
        (old_bitrate != 0))  //bitrate change, update vui.
    {
      VCEncSpsSetVuiHrd(s_br, vcenc_instance->rateControl.hrd);
      VCEncSpsSetVuiHrdBitRate(s_br, new_bitrate);
      VCEncSpsSetVuiHrdCpbSize(
          s_br, vcenc_instance->rateControl.virtualBuffer.bufferSize);
      update_pps_after_sps = 1;
    }

    if (new_frame_rate_num != old_frame_rate_num ||
        new_frame_rate_denom !=
            old_frame_rate_denom)  //frame rate change, update vui.
    {
      rcVirtualBuffer_s *vb = &vcenc_instance->rateControl.virtualBuffer;
    ...
    {
      vcenc_instance->rateControl.sei.hrd =
          (u32)vcenc_instance->rateControl.hrd;
      VCEncSpsSetVuiHrd(s_br, vcenc_instance->rateControl.hrd);
      update_pps_after_sps = 1;
    }
    if(frame_rate_change && !vcenc_instance->bSendSPSonFPSAdjust) {
      /* dont send SPS&PPS */
    ...
    vcenc_instance->stream.byteCnt = tmp;
#ifdef INTERNAL_TEST
    if ((vcenc_instance->testId == TID_FPS_ADJUST) &&
        (frame_rate_change && vcenc_instance->bSendSPSonFPSAdjust))
    {
       VCEncAdjustFPSTest(NULL, s_br, 1);
    }
#endif
  ...
  if (pEncIn->resendPPS || update_pps_after_sps) {
    if (update_pps_after_sps) update_pps_after_sps = 0;
    struct pps *p;
  ...
  if (vcenc_instance->insertNewPPS) {
    ...
    p = (struct pps *)get_parameter_set(c, PPS_NUT,
                                        vcenc_instance->insertNewPPSId);
    if (p == NULL) return;
    p->ps.b = vcenc_instance->stream;
    tmp = vcenc_instance->stream.byteCnt;
    //printf("byteCnt=%d\n",vcenc_instance->stream.byteCnt);
...
static u32 getCpbFactor(VCEncVideoCodecFormat codecFormat, i32 profile, true_e nalHrdParaFlag) {
  u32 CpbFactor;
  ASSERT(profile >= 0);
  switch (codecFormat) {
    case VCENC_VIDEO_CODEC_HEVC:
      ASSERT(profile < VCENC_HEVC_PROFILE_NUM);
      CpbFactor = (nalHrdParaFlag)? VCEncCpbNalFactorHevc[profile]
                            : VCEncCpbVclFactorHevc[profile];
      break;
    case VCENC_VIDEO_CODEC_H264:
      if (VCENC_H264_HIGH_PROFILE == profile) {
        CpbFactor = (nalHrdParaFlag)? 1500 : 1250;
      } else if (VCENC_H264_HIGH_10_PROFILE == profile) {
        CpbFactor = (nalHrdParaFlag)? 3600 : 3000;
      } else {
        CpbFactor = 1000;
      }
      break;
    case VCENC_VIDEO_CODEC_AV1:
      if (VCENC_AV1_HIGH_PROFILE == profile) {
        CpbFactor = 2;
      } else if (VCENC_AV1_PROFESSIONAL_PROFILE == profile) {
        CpbFactor = 3;
      } else {
        CpbFactor = 1;
      }
...
u32 EncGetMaxCPBS(VCEncVideoCodecFormat codecFormat, i32 levelIdx, i32 profile,
  ...
  i32 level = MAX(0, levelIdx);
  u32 maxCBPS = 0;
  u32 cpbFactor = getCpbFactor(codecFormat, profile, nalHrdParaFlag);;

  switch (codecFormat) {
    case VCENC_VIDEO_CODEC_HEVC:
      ASSERT(level < HEVC_LEVEL_NUM);
      level = MIN(level, (HEVC_LEVEL_NUM - 1));
      maxCBPS = (tier == VCENC_HEVC_HIGH_TIER) ?
                VCEncMaxCPBSHighTierHevc[level] * cpbFactor
                : VCEncMaxCPBSMainTierHevc[level] * cpbFactor;
      break;

    case VCENC_VIDEO_CODEC_H264:
      ASSERT(level < H264_LEVEL_NUM);
      level = MIN(level, (H264_LEVEL_NUM - 1));
      maxCBPS = VCEncMaxCPBSH264[level] * cpbFactor;
      break;

    case VCENC_VIDEO_CODEC_AV1:
      level = MIN(level, (AV1_VALID_MAX_LEVEL - 1));
      maxCBPS = ((tier == VCENC_HEVC_HIGH_TIER) || (level > 7))
                    ? VCEncMaxCPBSHighTierAV1[level] * cpbFactor
                    : VCEncMaxCPBSAV1[level] * cpbFactor;
      break;
...
u32 EncGetMaxBR(VCEncVideoCodecFormat codecFormat, i32 levelIdx, i32 profile,
  ...
  i32 level = MAX(0, levelIdx);
  u32 maxBR = 0;
  // MaxBRSize = BrVclFactor * MaxBR
  // (BrVclFactor = CpbVclFactor * HbrFactor), but we only support HbrFactor=1
  // so BrFactor equal to cpbFactor
  u32 brFactor = getCpbFactor(codecFormat, profile, nalHrdParaFlag);

  switch (codecFormat) {
    case VCENC_VIDEO_CODEC_HEVC:
      ASSERT(level < HEVC_LEVEL_NUM);
      level = MIN(level, (HEVC_LEVEL_NUM - 1));
      maxBR = (tier == VCENC_HEVC_HIGH_TIER) ? VCEncMaxBRHighTierHevc[level] * brFactor
                                             : VCEncMaxBRMainHevc[level] * brFactor;
      break;

    case VCENC_VIDEO_CODEC_H264:
      ASSERT(level < H264_LEVEL_NUM);
      level = MIN(level, (H264_LEVEL_NUM - 1));
      maxBR = VCEncMaxBRH264[level] * brFactor;
      break;

    case VCENC_VIDEO_CODEC_AV1:
      level = MIN(level, (AV1_VALID_MAX_LEVEL - 1));
      maxBR = ((tier == VCENC_HEVC_HIGH_TIER) || (level > 7))
                  ? VCEncMaxCPBSHighTierAV1[level] * brFactor
                  : VCEncMaxCPBSAV1[level] * brFactor;
      break;
...
i32 VCEncGetEncodeMaxDelay(VCEncInst inst) {
  ...
  if (pEncInst->pass)
    ...
    delay =
        pEncInst->gopSize ? pEncInst->gopSize - 1 : MAX_ADAPTIVE_GOP_SIZE - 1;
...
------------------------------------------------------------------------------------*/
void VCEncCollectEncodeStats(struct vcenc_instance *vcenc_instance,
                             VCEncIn *pEncIn, VCEncOut *pEncOut,
                             VCEncPictureCodingType codingType,
                             VCEncStatisticOut *enc_statistic) {
  asicData_s *asic = &vcenc_instance->asic;
  u32 uiIntraCu8Num = asic->regs.intraCu8Num;
  double dIntraVsInterskip =
      (double)uiIntraCu8Num /
      (double)((vcenc_instance->width / 8) * (vcenc_instance->height / 8));
  u32 curPicSize = vcenc_instance->stream.byteCnt * 8;

  enc_statistic->kEwl = asic->ewl;
  enc_statistic->avg_qp_y = asic->regs.qp;
  enc_statistic->secure_mode = asic->secure_mode;
  ...
  enc_statistic->output_buffer_over_flow =
      vcenc_instance->output_buffer_over_flow;
  enc_statistic->encodeTimeout = vcenc_instance->encodeTimeout;
  enc_statistic->regValueAXITotalPending = asic->regs.regValueAXITotalPending;
  enc_statistic->regValueAXIWritePending = asic->regs.regValueAXIWritePending;
  enc_statistic->regValueAXIReadPending = asic->regs.regValueAXIReadPending;
  enc_statistic->frameNum = vcenc_instance->frameNum;
#ifdef SUPPORT_AV1
  /* Some parameters need to be reset for AV1 */
  if (IS_AV1(vcenc_instance->codecFormat)) {
    if (pEncIn->bIsIDR && codingType == VCENC_INTRA_FRAME)
      enc_statistic->POCtobeDisplayAV1 = 0;
    else {
      vcenc_instance_av1 *av1_inst = &vcenc_instance->av1_inst;
      enc_statistic->POCtobeDisplayAV1 = av1_inst->prePOCtobeDisplayAV1;
    }
  }
#endif
  enc_statistic->scene_change = 0;
  if (dIntraVsInterskip >= 0.5 && codingType != VCENC_INTRA_FRAME &&
      IS_CTBRC_FOR_BITRATE(vcenc_instance->rateControl.ctbRc) &&
      enc_statistic->output_buffer_over_flow == 0) {
    if (vcenc_instance->reEncodeCnt == 0) {
      enc_statistic->scene_change = 1;
      enc_statistic->avg_qp_y = asic->regs.sumOfQP / asic->regs.sumOfQPNumber;
      enc_statistic->frame_min_size = asic->regs.minPicSize;
      enc_statistic->frame_max_size = asic->regs.maxPicSize;
      enc_statistic->targetQp = asic->regs.qp;
      vcenc_instance->reEncodePreSize = curPicSize;
      vcenc_instance->reEncodeCnt++;
    } else if (vcenc_instance->reEncodeCnt == 1 &&
               curPicSize > vcenc_instance->reEncodePreSize) {
      vcenc_instance->reEncodeCnt = 0;
      enc_statistic->scene_change = 0;
    } else {
      vcenc_instance->reEncodeCnt = 0;
    }
...
------------------------------------------------------------------------------------*/
VCEncRet VCEncResetEncodeStatus(struct vcenc_instance *vcenc_instance,
                                struct container *c, VCEncIn *pEncIn,
                                struct sw_picture *pic, VCEncOut *pEncOut,
                                const VCEncStatisticOut *enc_statistic) {
#ifdef SUPPORT_AV1
  if (IS_AV1(vcenc_instance->codecFormat)) {
      vcenc_instance->av1_inst.POCtobeDisplayAV1 = enc_statistic->POCtobeDisplayAV1;
      vcenc_instance->frameNum = enc_statistic->frameNum;
    if (VCENC_OK !=
        VCEncGenAV1Config(vcenc_instance, c, pEncIn, pic, pEncOut->codingType))
      return VCENC_ERROR;
  }
#endif
  vcenc_instance->output_buffer_over_flow = 0;
  vcenc_instance->encodeTimeout = 0;

  return VCENC_OK;
}

/*----------------------------------------------------------------------------------
    Function name : VCEncSetReEncodeParameters
    Description   : Set the ReEncode parameters with new parameters
    Return type   : void
    Argument      : vcenc_instance - encoder instance
    Argument      : pEncIn - user provided input parameters
    Argument      : pEncOut - place where output info is returned
...
------------------------------------------------------------------------------------*/
void VCEncSetReEncodeParameters(struct vcenc_instance *vcenc_instance,
                              VCEncIn *pEncIn, VCEncOut *pEncOut,
                              VCEncSliceReady *slice_callback,
                              NewEncodeParams *new_params,
                              regValues_s *regs_for2nd_encode) {
  ...
  if (new_params->strategy == NEW_QP) {
    ...
    vcenc_instance->stream.stream =
        ((u8 *)new_params->output_buffer_mem[0].virtualAddress) +
        vcenc_instance->stream.byteCnt;
    vcenc_instance->stream.stream_bus =
        new_params->output_buffer_mem[0].busAddress;
    ...
    slice_callback->streamBufs =
        vcenc_instance->streamBufs[(vcenc_instance->jobCnt + 1) %
                                   vcenc_instance->parallelCoreNum];

    regs_for2nd_encode->outputStrmBase[0] =
        (ptr_t)vcenc_instance->stream.stream_bus +
        vcenc_instance->stream.byteCnt;
    regs_for2nd_encode->outputStrmSize[0] =
        vcenc_instance->stream.size - vcenc_instance->stream.byteCnt;
  } else if (new_params->strategy == NEW_TARGET_BIT) {
    regs_for2nd_encode->targetPicSize = new_params->targetPicSize;
    regs_for2nd_encode->maxPicSize = new_params->maxPicSize;
  } else if (new_params->strategy == NEW_RE_ENCODE_FOR_TIMEOUT) {
    //do noting
  }
...
/** check if configure support batch mode */
static true_e vcencBatchCheckCfg(const VCEncConfig *enc_cfg,
                                 const EWLHwConfig_t *asic_cfg,
                                 const void *ctx)
{
  true_e ret = true;
  if (enc_cfg->enc_mode.batch_flag) {
    if (0==EWLGetVCMDSupport(ctx)) {
      APITRACEERR("vcencBatchCheckCfg: Batch Mode only work with VCMD\n");
      ret = false;
    }
    if (enc_cfg->lookaheadDepth > 0) {
      APITRACEERR("vcencBatchCheckCfg: Batch Mode don't work with lookahead\n");
      ret = false;
    }
  }
...
/** check if batch mode related coding parameter is valid or not */
static VCEncRet vcencBatchGetCodingCtrl(struct vcenc_instance *pEncInst,
                            VCEncCodingCtrl *pCodeParams)
{
  if (pEncInst->batchEnable) {
    pCodeParams->batchCount = pEncInst->batchCount;
  } else {
    pCodeParams->batchCount = 0;
  }
  return VCENC_OK;
}

/**
 * in batch mode, try cache the task or drive the hardware in batch
 */
static VCEncRet vcencBatchRunCmdbuf(struct vcenc_instance *vcenc_instance,
                    asicData_s *asic, VcmdDes_t *vcmd,
                    u32 client_type)
{
  VCEncRet ret = VCENC_OK;
  u32 sendCount = vcenc_instance->batchCount;
//  printf("vcencBatchRunCmdbuf: vcenc_instance->cacheCnt = %u, vcenc_instance->batchCount = %u\n", vcenc_instance->cacheCnt, vcenc_instance->batchCount);

  if (sendCount == 0) sendCount = 1;

  if (vcmd) {
    /* save current job in cache */
    int index= vcenc_instance->jobCnt % vcenc_instance->parallelCoreNum;
    VcmdJob *vjob = &vcenc_instance->vjobs[index];
    memcpy(&vjob->cmdbuf, vcmd, sizeof(*vcmd));
    /* set manual interrupt mode */
    vjob->interrupt_ctrl = ((u64)1<<31);
    vjob->interrupt_ctrl &= ~(u64)1;
    vcenc_instance->cacheCnt++;
  }

  while(vcenc_instance->cacheCnt >= sendCount) {
    for(int i = 0; i< sendCount; i++) {
      int index = vcenc_instance->sendCnt % vcenc_instance->parallelCoreNum;
      VcmdJob *vjob = &vcenc_instance->vjobs[index];

      ASSERT(vjob->interrupt_ctrl==((u64)1<<31));

      vjob->interrupt_ctrl |= ((u64)sendCount<<32);
      /* set IE flag for last job in current batch */
      if (i == sendCount-1) {
        vjob->interrupt_ctrl |= (u64)1;
      }
      EncSetReseveInfo(asic->ewl, vjob->interrupt_ctrl, client_type);

      EncLinkRunCmdbuf(asic->ewl, &vjob->cmdbuf);
      vcenc_instance->sendCnt++;
    }
    vcenc_instance->cacheCnt -=sendCount;
    ret |= VCENC_FRAME_ENQUEUE;
  }

  if (vcenc_instance->cacheCnt > 0)
    ret |= VCENC_FRAME_CACHE;
...
{
    struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
    p[0] = vcenc_instance->asic.regs.intraCu8Num;
    p[1] = vcenc_instance->asic.regs.skipCu8Num;
    p[2] = vcenc_instance->asic.regs.PBFrame4NRdCost;
}

u32 GetReservedCore(const void *inst)
{
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  return vcenc_instance->reservedCore;
}
