...
#define NUM_REF_BUF 5
  ...
  i32 traceRef4N[4] = {BaseRef4nLum0, BaseRef4nLum1, BaseRef4nLum2,
                       BaseRef4nLum3};
  ...
  if (vcenc_instance->preProcess.inputFormat != VCENC_YUV420_SEMIPLANAR &&
      ...
      vcenc_instance->preProcess.inputFormat != VCENC_YUV422_INTERLEAVED_VYUY &&
      vcenc_instance->preProcess.inputFormat != VCENC_YUV422_INTERLEAVED_YVYU &&
      vcenc_instance->preProcess.inputFormat !=
          VCENC_YUV420_PLANAR_10BIT_P010 &&
      vcenc_instance->preProcess.inputFormat !=
          VCENC_YVU420_PLANAR_10BIT_P010 &&
      vcenc_instance->preProcess.inputFormat !=
    ...
    switch (vcenc_instance->preProcess.inputFormat) {
      ...
      case VCENC_YUV420_SEMIPLANAR_VU:
        ...
        channel_cfg.trace_end_addr =
            asic->regs.input_luma_stride * vcenc_instance->height;
        ...
        ret = EnableCacheChannel(&vcenc_instance->cache,
                                 &vcenc_instance->channel_idx, &channel_cfg,
                                 ENCODER_VCE, dir);
        break;
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010_TILE_4_4:
        ...
        if (vcenc_instance->preProcess.inputFormat ==
                VCENC_YUV420_SEMIPLANAR_8BIT_TILE_4_4 ||
            vcenc_instance->preProcess.inputFormat ==
                VCENC_YUV420_SEMIPLANAR_VU_8BIT_TILE_4_4)
          byte_per_compt = 1;
    ...
    switch (vcenc_instance->preProcess.inputFormat) {
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010_TILE_4_4:
        ...
        channel_cfg.end_addr =
            (asic->regs.inputCbBase +
             (asic->regs.input_chroma_stride *
              ((vcenc_instance->preProcess.lumHeightSrc[tileId] / 2) + 3) /
              4)) >>
            4;
  ...
  u32 reference_4n_size = asic->regs.ref_ds_luma_stride *
                          ((vcenc_instance->height + 63) & (~63)) / 4 / 4;
  ...
  if (pic->sliceInst->type != I_SLICE) {
    ...
    channel_cfg.trace_end_addr =
        ((vcenc_instance->width + alignment) & (~alignment)) *
        ((vcenc_instance->height + 63) & (~63));

    channel_cfg.start_addr = asic->regs.pRefPic_recon_l0[0][0] >> 4;
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l0[0][0] + reference_luma_size) >> 4;

    ret =
        EnableCacheChannel(&vcenc_instance->cache, &vcenc_instance->channel_idx,
                           &channel_cfg, ENCODER_VCE, dir);
  ...
  if (pic->sliceInst->type != I_SLICE) {
    ...
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l0[1][0] + reference_chroma_size) >> 4;
  ...
  if (pic->sliceInst->type == B_SLICE) {
    ...
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l1[0][0] + reference_luma_size) >> 4;
  ...
  if (pic->sliceInst->type == B_SLICE) {
    ...
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l1[1][0] + reference_chroma_size) >> 4;
  ...
  if (pic->sliceInst->type != I_SLICE) {
    ...
    channel_cfg.trace_end_addr =
        ((vcenc_instance->height / 4 + alignment) & (~alignment)) *
        vcenc_instance->height / 4;

    channel_cfg.start_addr = asic->regs.pRefPic_recon_l0_4n[0] >> 4;
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l0_4n[0] + reference_4n_size) >> 4;
  ...
  if (pic->sliceInst->type == B_SLICE) {
    ...
    channel_cfg.end_addr =
        (asic->regs.pRefPic_recon_l1_4n[0] + reference_4n_size) >> 4;
  ...
  channel_cfg.pad_e = 1;
  channel_cfg.rfc_e = 0;
  ret = EnableCacheChannel(&vcenc_instance->cache,&vcenc_instance->channel_idx,&channel_cfg,ENCODER_VCE,dir);
...
VCEncRet EnableCache(struct vcenc_instance *vcenc_instance, asicData_s *asic,
  ...
  GenCacheChannelModeReg(&vcenc_instance->cache, CACHE_RD, &cacheRegInfo,
                         CACHE_ENABLE);
  ...
  for (i = 0; i < CWL_CACHE_REG_NUM_MAX; i++) {
    if (0 != cacheRegInfo.cacheRegs[i].flag && i != 1) {
      EWLWriteRegbyClientType(asic->ewl, cacheRegInfo.cacheRegs[i].offset,
                              cacheRegInfo.cacheRegs[i].val,
                              EWL_CLIENT_TYPE_L2CACHE);
    }
  }
...
void DisableCache(const void *ewl, void *cache_data) {
  ...
  GenCacheChannelModeReg(cacheData->cache, CACHE_RD, &cacheRegInfo,
                         CACHE_DISABLE);
