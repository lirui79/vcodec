 ...
 */
static VCEncRet VCEncChangeResolutionCheck(VCEncInst inst, void *ctx, int w,
                                           int h, int exteralReconAlloc) {
  ...
  if (!cfg) {
    APITRACEERR("VCEncChangeResolution: ERROR Null argument");
  ...
  if (IS_AV1(vcenc_instance->codecFormat) ||
    ...
    if (width > 4096) {
      APITRACEERR(
          "VCEncChangeResolution: Invalid width, need 4096 or smaller when AV1 "
          "or VP9");
      return VCENC_INVALID_ARGUMENT;
    }
    if (width * height > 4096 * 2304) {
      APITRACEERR(
          "VCEncChangeResolution: Invalid area, need 4096*2304 or below when "
          "AV1 or VP9");
      return VCENC_INVALID_ARGUMENT;
    }
  }
  ...
  if (width < VCENC_MIN_ENC_WIDTH || width > VCENC_MAX_ENC_WIDTH ||
      (width & 0x1) != 0) {
  ...
  if (height < VCENC_MIN_ENC_HEIGHT || height > VCENC_MAX_ENC_HEIGHT_EXT ||
      (height & 0x1) != 0) {
  ...
  if ((IS_H264(vcenc_instance->codecFormat)
           ...
           : cfg->maxEncodedWidthHEVC) < width) {
    APITRACEERR(
        "VCEncChangeResolution: Invalid width, not supported by HW coding "
        "core");
    return VCENC_INVALID_ARGUMENT;
  }

  /*extend video coding height */
  if ((height > VCENC_MAX_ENC_HEIGHT) &&
      (cfg->videoHeightExt == EWL_HW_CONFIG_NOT_SUPPORTED)) {
    APITRACEERR(
        "VCEncChangeResolution: Invalid height, height extension not supported "
        "by HW coding core");
    return VCENC_INVALID_ARGUMENT;
  }

  if (width > vcenc_instance->ori_width &&
      height > vcenc_instance->ori_height) {
    APITRACEERR(
        "VCEncChangeResolution: width and height exceeded max(ori) value.");
    return VCENC_INVALID_ARGUMENT;
  }

  if (asic->regs.codingType == ASIC_JPEG ||
      asic->regs.codingType == ASIC_CUTREE)
    return VCENC_OK;
  ...
  EncAsicGetSizeAndSetRegs(&asic_tmp, &allocCfg, &internalImageLumaSize_ori,
                           &lumaSize_ori, &lumaSize4N_ori, &chromaSize_ori,
                           &u32FrameContextSize_ori);

  allocCfg.width = w;
  allocCfg.height = h;
  EncAsicGetSizeAndSetRegs(&asic_tmp, &allocCfg, &internalImageLumaSize,
                           &lumaSize, &lumaSize4N, &chromaSize,
                           &u32FrameContextSize);

  if (internalImageLumaSize > internalImageLumaSize_ori ||
      lumaSize > lumaSize_ori || lumaSize4N > lumaSize4N_ori ||
      chromaSize > chromaSize_ori) {
...
static VCEncRet VCEncChangeResolution(VCEncInst inst, void *ctx, int w, int h,
  ...
  allocCfg.frameInfo = vcenc_instance->frameInfo;
  allocCfg.enablelumaInfo = vcenc_instance->enablelumaInfo;
  ...
  allocCfg.numRefBuffsLum = allocCfg.numRefBuffsChr =
      vcenc_instance->sps->max_dec_pic_buffering[0] - 1 +
      vcenc_instance->parallelCoreNum - 1;
  allocCfg.writeReconToDDR = vcenc_instance->writeReconToDDR;
  ...
  allocCfg.refRingBufEnable = vcenc_instance->refRingBufEnable;
  allocCfg.RefRingBufExtendHeight = vcenc_instance->RefRingBufExtendHeight;
  ...
  allocCfg.codedChromaIdc = VCEncGetChromaIdc(vcenc_instance->asic.regs.codedChromaIdc, vcenc_instance);
  ...
  vcenc_instance->ctbPerFrame =
      vcenc_instance->ctbPerRow * vcenc_instance->ctbPerCol;
  ...
  if ((c = get_container(vcenc_instance))) {
    ...
    for (n = c->picture.tail; n; n = n->next) {
      ...
      if (sw_init_image(p, &p->input[0], w, h, SW_IMAGE_U8, HANTRO_FALSE, 0, 0))
        goto out;
      if (sw_init_image(p, &p->recon, WIDTH_64_ALIGN(width), height,
                        SW_IMAGE_U8, HANTRO_FALSE, 0, 0))
        goto out;
    }
  out:
    if (n != NULL) {
  ...
  if (vcenc_instance->lookaheadDepth > 0 &&
      vcenc_instance->lookahead.priv_inst) {
      int w_pass1 = 0;
      int h_pass1 = 0;
    if (w < 272 || h < 256) {
      w_pass1 = w;
    ...
    ret = VCEncChangeResolution(vcenc_instance->lookahead.priv_inst, ctx,
                                w_pass1, h_pass1, 0);
    if (ret != VCENC_OK) return ret;
  }
...
static void VCEncReleaseCoreInst(VCEncInst inst) {
  ...
  struct vcenc_instance *pEncInst_pass1 =
...
/**
 * @brief VCEncSetStrmCtrl(), Set stream control.In previous VCE software, there's no way to
...
VCEncRet VCEncSetStrmCtrl(VCEncInst inst, VCEncStrmCtrl *params) {
  ...
  /*resolution change*/
  if ((ret = VCEncChangeResolution(inst, vcenc_instance->ctx, params->width,
                                   params->height, 0)) != VCENC_OK) {
    return ret;
  }

  /*reset rate control*/
  i32 ctb_size = (IS_H264(vcenc_instance->codecFormat) ? 16 : 64);
  vcenc_instance->rateControl.picArea = ((vcenc_instance->width + 7) & (~7)) *
                                        ((vcenc_instance->height + 7) & (~7));
  ...
  vcenc_instance->rateControl.reciprocalOfNumBlk8 =
      (Q31_FIXED_ONE+(vcenc_instance->rateControl.ctbPerPic * (ctb_size / 8) * (ctb_size / 8))/2)/
      (vcenc_instance->rateControl.ctbPerPic * (ctb_size / 8) * (ctb_size / 8));
#else
  vcenc_instance->rateControl.reciprocalOfNumBlk8 =
      1.0 /
      (vcenc_instance->rateControl.ctbPerPic * (ctb_size / 8) * (ctb_size / 8));
  ...
  vcenc_instance->rateControl.ctbRateCtrl.models[0]
      ...
      vcenc_instance->rateControl.ctbRateCtrl.models[0].started =
          vcenc_instance->rateControl.ctbRateCtrl.models[0].preFrameMad =
              vcenc_instance->rateControl.ctbRateCtrl.models[1]
                  .x0 = vcenc_instance->rateControl.ctbRateCtrl.models[1].x1 =
                  vcenc_instance->rateControl.ctbRateCtrl.models[1].started =
                      vcenc_instance->rateControl.ctbRateCtrl.models[1]
                          .preFrameMad = vcenc_instance->rateControl.ctbRateCtrl
                                             .models[2]
                                             .x0 =
                          vcenc_instance->rateControl.ctbRateCtrl.models[2].x1 =
                              vcenc_instance->rateControl.ctbRateCtrl.models[2]
                                  .started =
                                  vcenc_instance->rateControl.ctbRateCtrl
                                      .models[2]
                                      .preFrameMad = 0;

  if (vcenc_instance->lookaheadDepth && vcenc_instance_pass1) {
    vcenc_instance_pass1->rateControl.picArea =
        ((vcenc_instance_pass1->width + 7) & (~7)) *
        ((vcenc_instance_pass1->height + 7) & (~7));
    vcenc_instance_pass1->rateControl.ctbPerPic =
        vcenc_instance_pass1->ctbPerFrame;

#ifdef RCP_FIXED_POINT_OPT
    vcenc_instance_pass1->rateControl.reciprocalOfNumBlk8 =
        (Q31_FIXED_ONE + (vcenc_instance_pass1->rateControl.ctbPerPic * (ctb_size / 8) * (ctb_size / 8))/2)
        / (vcenc_instance_pass1->rateControl.ctbPerPic * (ctb_size / 8) * (ctb_size / 8));
#else
    vcenc_instance_pass1->rateControl.reciprocalOfNumBlk8 =
    ...
    vcenc_instance_pass1->rateControl.ctbRateCtrl.models[0]
        .x0 = vcenc_instance_pass1->rateControl.ctbRateCtrl.models[0].x1 =
        vcenc_instance_pass1->rateControl.ctbRateCtrl.models[0]
            .started = vcenc_instance_pass1->rateControl.ctbRateCtrl.models[0]
                           .preFrameMad =
            vcenc_instance_pass1->rateControl.ctbRateCtrl.models[1].x0 =
                vcenc_instance_pass1->rateControl.ctbRateCtrl.models[1].x1 =
                    vcenc_instance_pass1->rateControl.ctbRateCtrl.models[1]
                        .started = vcenc_instance_pass1->rateControl.ctbRateCtrl
                                       .models[1]
                                       .preFrameMad =
                        vcenc_instance_pass1->rateControl.ctbRateCtrl.models[2]
                            .x0 = vcenc_instance_pass1->rateControl.ctbRateCtrl
                                      .models[2]
                                      .x1 =
                            vcenc_instance_pass1->rateControl.ctbRateCtrl
                                .models[2]
                                .started =
                                vcenc_instance_pass1->rateControl.ctbRateCtrl
                                    .models[2]
  ...
  if (vcenc_instance->lookaheadDepth && vcenc_instance_pass1) {
    if ((ret = cuTreeChangeResolution(vcenc_instance_pass1,
                                      &vcenc_instance_pass1->cuTreeCtl)) !=
        VCENC_OK) {
      return ret;
    }
  }
...
#ifdef TEST_DATA //mainly to update the pic->recon.width and pic->recon.height when resolution change
    vcenc_instance->created_pic_num = 0;
    sw_free_pictures(get_container(vcenc_instance));
#endif
