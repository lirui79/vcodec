...
#define CUTREE_BUFFER_NUM \
  (MIN(2 * MAX_GOP_SIZE,  \
       16))  // output ROI map buffr count limited by bits in command
...
#define IS_CODING_TYPE_I(x) \
  ((x) == VCENC_FRAME_TYPE_I || (x) == VCENC_FRAME_TYPE_IDR)
#define IS_CODING_TYPE_B(x) \
  ((x) == VCENC_FRAME_TYPE_B || (x) == VCENC_FRAME_TYPE_BREF)
#define IS_CODING_TYPE_P(x) \
  ((x) == VCENC_FRAME_TYPE_P || (x) == VCENC_FRAME_TYPE_BLDY)
#define VCENC_CODING_TYPE(x)                                                \
  (IS_CODING_TYPE_I(x) ? VCENC_INTRA_FRAME                                  \
                       : (IS_CODING_TYPE_B(x) ? VCENC_BIDIR_PREDICTED_FRAME \
...
typedef struct {
  ...
  void *pRateCtrlParam;      //pointer to rate ctrl parameters of this frame
} VCEncLookaheadJob;
...
struct cuTreeCtr {
  ...
  EWLLinearMem_t propagateCostMemFactory[CUTREE_BUFFER_CNT(VCENC_LOOKAHEAD_MAX,
                                                           MAX_GOP_SIZE)];
  ...
  i32 rem_frames;
  u64 commands
      [CUTREE_BUFFER_CNT(VCENC_LOOKAHEAD_MAX, MAX_GOP_SIZE) +
  ...
  size_t
      outRoiMapSegmentCountOffset; /* Offset inside roibuffer for vp9 segment */
...
#define GetLookaheadOutput(lookahead, bFlush, vcenc_instance) (NULL)
...
VCEncRet cuTreeAddFrame(VCEncInst inst, VCEncLookaheadJob *job,
                        void *agopInfoBuf, i32 gopSizeFromUser);
...
VCEncLookaheadJob *GetLookaheadOutput(VCEncLookahead *lookahead, bool bFlush, struct vcenc_instance *vcenc_instance);
