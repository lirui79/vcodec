...
static VCEncRet VCEncGetCuInfoV0(bits_buffer_s *b, VCEncCuInfo *pEncCuInfo,
  ...
  if (!IS_H264(codecFormat)) {
    pEncCuInfo->cuLocationY =
        get_value(b, CU_INFO_LOC_Y_BITS, HANTRO_FALSE) * 8;
    pEncCuInfo->cuLocationX =
        get_value(b, CU_INFO_LOC_X_BITS, HANTRO_FALSE) * 8;
    pEncCuInfo->cuSize =
        (1 << (get_value(b, CU_INFO_SIZE_BITS, HANTRO_FALSE) + 3));
  } else {
  ...
  if (pEncCuInfo->cuMode == 0) {
    ...
    pEncCuInfo->interPredIdc =
        get_value(b, CU_INFO_INTER_PRED_IDC_BITS, HANTRO_FALSE);
    for (i = 0; i < 2; i++) {
      pEncCuInfo->mv[i].refIdx =
          get_value(b, CU_INFO_MV_REF_IDX_BITS, HANTRO_FALSE);
     ...
     */
    pEncCuInfo->intraPartMode =
        get_value(b,
                  IS_H264(codecFormat) ? CU_INFO_INTRA_PART_BITS_H264
                                       : CU_INFO_INTRA_PART_BITS,
                  HANTRO_FALSE);
 ...
 */
static int getCuNumLocationV2(struct vcenc_instance *pEncInst, u32 ctuNum,
                              u32 cuNum, u32 *cuX, u32 *cuY) {
  ...
  if (ctuNum % pEncInst->ctbPerRow == pEncInst->ctbPerRow - 1)
    nCuInCtuX =
        ((pEncInst->width + 15) / 16 - 1) % (pEncInst->max_cu_size / 16) + 1;
  if (ctuNum / pEncInst->ctbPerRow == pEncInst->ctbPerCol - 1)
    nCuInCtuY =
        ((pEncInst->height + 15) / 16 - 1) % (pEncInst->max_cu_size / 16) + 1;
  if (cuX)
    *cuX = ctuNum % pEncInst->ctbPerRow * (pEncInst->max_cu_size / 16) +
           cuNum % nCuInCtuX;
  if (cuY)
    *cuY = ctuNum / pEncInst->ctbPerRow * (pEncInst->max_cu_size / 16) +
           cuNum / nCuInCtuX;
 ...
 * \param [in]  pEncCuOutData - structure containning ctu table and cu information stream output by HW
...
VCEncRet VCEncGetCuInfo(VCEncInst inst, VCEncCuOutData *pEncCuOutData,
  ...
  if (version == 2) {
    ...
    cuStream = pEncCuOutData->cuData +
               (cuX + cuY * ((pEncInst->width + 15) / 16)) * infoSize;
  }
  if (cuNum >= nCuInCtu) {
    APITRACEWRN(
        "VCEncGetCuInfo: WARNING CU number is beyond the boundary of given "
        "CTU\n");
    return VCENC_INVALID_ARGUMENT;
  }
  ...
  if (version != 2) cuStream += cuNum * infoSize;
  b.cache = 0;
  b.bit_cnt = 0;
  b.stream = cuStream;
  b.accu_bits = 0;
