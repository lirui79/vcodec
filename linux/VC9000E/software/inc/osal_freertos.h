...
#define PTHREAD_ONCE_INIT \
  (((pthread_once_t){.done = 0, .once_mutex = PTHREAD_MUTEX_INITIALIZER}))
...
static av_unused int pthread_attr_setinheritsched(pthread_attr_t *attr,
                                                  int inheritsched) {
  //TODO...
  (void)attr;
  (void)inheritsched;
...
static av_unused void osal_usleep(unsigned long usec) { usleep(usec); }
...
static av_unused void osal_open_memstream(struct stream_trace *stream_trace) {
  ...
  fm = CreateFileMapping(h, NULL, PAGE_READWRITE | SEC_RESERVE, 0,
                         16 * 1024 * 1024, NULL);
  if (fm == NULL) {
    fprintf(stderr, "Could not access memory space! %sn",
  ...
  if (stream_trace->buffer = NULL) {
    fprintf(stderr, "Could not fill memory space! %sn",
            strerror(GetLastError()));
    exit(GetLastError());
  }
#else
