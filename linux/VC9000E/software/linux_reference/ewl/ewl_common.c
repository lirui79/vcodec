...
#include "enc_log.h"
#include <stdio.h>
static const char *busTypeName[7] = {"UNKNOWN", "AHB",    "OCP",   "AXI",
                                     "PCI",     "AXIAHB", "AXIAPB"};
...
i32 EWLGetCoreSignature(u32 *regs, EWLCoreSignature_t *signature) {
  if (!regs || !signature) return EWL_ERROR;
  ...
  if (hw_id == 0x90004200 && signature->hw_build_id == 0) {
    signature->fuse[4] |= 0x10000000;
  }
 ...
 */
static i32 EWLParseCoreConfig(EWLCoreSignature_t *signature,
                              EWLHwConfig_t *cfg_info) {
  if (!signature || !cfg_info) return EWL_ERROR;
  ...
  if (!cfg_info->encSliceSizeBits) cfg_info->encSliceSizeBits = 7;
  ...
  cfg_info->prpLowLatencySupport = (cfgval >> 19) & 1;
  ...
  cfg_info->maxEncodedWidthH264 = cfg_info->maxEncodedWidthJPEG =
  ...
  /* alwayse set the ctbRc as 1 for legacy core */
  cfg_info->ctbRcSupport = 1;
...
#define SUPPORT_NO_BUILD_ID
  ...
  if (hw_build_id == 0) {
    ...
    for (i = 0; i < EWL_MAX_CORES; i++) {
      if ((signature_list[i].hw_asic_id == signature->hw_asic_id) &&
          (0 == EWLmemcmp(signature_list[i].fuse, signature->fuse,
                          sizeof(u32) * EWL_MAX_CORES))) {
    ...
    for (i = 0; i < EWL_MAX_CORES; i++) {
      if (signature_list[i].hw_build_id == 0) break;
    }

    if (i == EWL_MAX_CORES) {
      /* no empty slot for parsed configure, need to set cfg_info pointer as NULL */
      *cfg_info = NULL;
      return EWL_ERROR;
    }
  ...
  for (i = 0; i < list_num; i++) {
    if (hw_build_id == cfg->hw_build_id) break;
    cfg++;
  }

  if (i == list_num) {
    /* not found build id ,need to set cfg_info pointer as NULL */
    *cfg_info = NULL;
    return EWL_ERROR;
  }

  if (list_num > 0)  //Mark for klocwork : if list_num > 0 then feature_list/cfg have more than 1 item
    *cfg_info = cfg;
...
{
  #define DEF_NAME(name, attr) #name,

  static const char *mem_hint[] = {
    EWL_HINT_LIST(DEF_NAME)
  };

  return mem_hint[(mem_type & EWL_MEM_USAGE_HINT_MASK)>>EWL_MEM_USAGE_HINT_SHIFT]+sizeof("EWL_MEM_USAGE");
}
#endif
#endif /* SYSTEMSHARED */
