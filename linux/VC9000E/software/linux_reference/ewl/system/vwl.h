...
struct cmdbuf_obj {
  ...
  /* interrupt control.
   * - bit[31] : interrupt mode. 0-adaptive; 1-manual
   * - bit[ 0] : in manual mode, value to insert JMP command as IE flag
   * - bit[39:32] is batch count. */
  u64 interrupt_ctrl;
  ...
  u16 core_mask; //core_mask for user to select core.
};
...
typedef struct {
  ...
  u32 jpeg_shared;
  u32 *VirtualPlatformBuf[ASIC_SWREG_AMOUNT];
  u32 *CmodelUseOrigBuf[ASIC_SWREG_AMOUNT];
} ewlSysInstance;
...
struct hantrovcmd_dev {
  ...
  struct HwVcmdCoreConfig
      vcmd_core_cfg;  //config of each core,such as base addr, irq,etc
  ...
  u32 *
      vcmd_reg_mem_virtual_address;  //start virtual address of vcmd registers memory of  CMDBUF.
  ptr_t
      vcmd_reg_mem_bus_address;  //start physical address of vcmd registers memory of  CMDBUF.
