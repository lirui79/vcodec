...
i32 EWLMemSyncAllocHostBufferSys(const void *instance, u32 size, u32 alignment,
  ...
  if (buff->mem_type & CPU_WR || buff->mem_type & CPU_RD ||
      buff->mem_type & EXT_WR || buff->mem_type & EXT_RD) {
    ...
    EWLTRACE_I((void *)instance,
             ("EWLMemSyncAllocHostBufferSys: %p, aligned %p\n",
  ...
  if (buff->virtualAddress != NULL) {
    for (i = 0; i < inst->totalChunks; i++) {
      if (inst->alignedChunks[i] == buff->virtualAddress) {
        EWLTRACE_I((void *)instance,
                 "EWLMemSyncFreeHostBufferSys virtualAddress \t%p\n",
...
i32 EWLSyncMemDataSys(EWLLinearMem_t *mem, u32 offset, u32 length,
  ...
  if (mem->virtualAddress == NULL || mem->busAddress == 0) return EWL_PAR_ERROR;
