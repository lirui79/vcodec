...
static u32 vcmd_read_reg_cb(void* core, u32 reg_idx, u32 core_type,
                            void* dev_id);
static void vcmd_write_reg_cb(void* core, u32 reg_idx, u32 value, u32 core_type,
                              void* dev_id);
...
static pthread_mutex_t cmdbuf_used_mutex;
static u16 cmdbuf_used_pos;
static u16 cmdbuf_used_residual;

static struct hantrovcmd_dev* vcmd_manager[MAX_VCMD_TYPE]
                                          [MAX_SAME_MODULE_TYPE_CORE_NUMBER];
...
static u32* basemem_reg[MAX_VCMD_TYPE][MAX_SAME_MODULE_TYPE_CORE_NUMBER] = {
    {NULL}};
...
static inline void vwl_set_register_mirror_value(u32* reg_mirror,
  ...
  PDEBUG("(field->mask >> field->lsb) >= value = %d/n",
...
static inline u32 vwl_get_register_mirror_value(u32* reg_mirror,
                                                regVcmdName name) {
  const regVcmdField_s* field;
  u32 regVal;
...
void vwl_write_register_value(const void* vcmd_core, u32* reg_mirror,
                              regVcmdName name, u32 value) {
  const regVcmdField_s* field;
  u32 regVal;
  field = &asicVcmdRegisterDesc[name];
  ...
  AsicHwVcmdCoreWriteRegister(vcmd_core, field->base,
                              reg_mirror[field->base / 4]);
}

u32 vwl_get_register_value(const void* vcmd_core, u32* reg_mirror,
                           regVcmdName name) {
  const regVcmdField_s* field;
  u32 value;
  ...
  value = reg_mirror[field->base / 4] =
      AsicHwVcmdCoreReadRegister(vcmd_core, field->base);
...
#define vwl_set_addr_register_value(vcmd_core, reg_mirror, name, value) \
  do {                                                                  \
    if (sizeof(ptr_t) == 8) {                                           \
      vwl_write_register_value((vcmd_core), (reg_mirror), name,         \
                               (u32)((ptr_t)value));                    \
...
#define VWLGetAddrRegisterValue(vcmd_core, reg_mirror, name)                  \
  ((sizeof(ptr_t) == 8)                                                       \
       ? ((((ptr_t)vwl_get_register_value((vcmd_core), (reg_mirror), name)) | \
           (((ptr_t)vwl_get_register_value((vcmd_core), (reg_mirror),         \
                                           name##_MSB))                       \
            << 32)))                                                          \
       : ((ptr_t)vwl_get_register_value((vcmd_core), (reg_mirror), (name))))
...
static u32 calculate_executing_time_after_node_high_priority(
    bi_list_node* exe_cmdbuf_node) {
  u32 time_run_all = 0;
  struct cmdbuf_obj* cmdbuf_obj_temp = NULL;
  if (exe_cmdbuf_node == NULL) return time_run_all;
  cmdbuf_obj_temp = (struct cmdbuf_obj*)exe_cmdbuf_node->data;
  time_run_all += cmdbuf_obj_temp->executing_time;
  exe_cmdbuf_node = exe_cmdbuf_node->next;
  while (1) {
    if (exe_cmdbuf_node == NULL) break;
    cmdbuf_obj_temp = (struct cmdbuf_obj*)exe_cmdbuf_node->data;
    if (cmdbuf_obj_temp->priority == CMDBUF_PRIORITY_NORMAL) break;
    time_run_all += cmdbuf_obj_temp->executing_time;
    exe_cmdbuf_node = exe_cmdbuf_node->next;
  }
  return time_run_all;
...
static i32 allocate_cmdbuf(struct VwlMem* new_cmdbuf_addr,
  ...
  pthread_mutex_lock(&cmdbuf_used_mutex);
  while (1) {
    if (cmdbuf_used[cmdbuf_used_pos] == 0 &&
        (global_cmdbuf_node[cmdbuf_used_pos] == NULL)) {
      cmdbuf_used[cmdbuf_used_pos] = 1;
      cmdbuf_used_residual -= 1;
      new_cmdbuf_addr->cmdbuf_id = cmdbuf_used_pos;
      new_cmdbuf_addr->virtual_address = vcmd_buf_mem_pool.virtual_address +
                                         cmdbuf_used_pos * CMDBUF_MAX_SIZE / 4;
      new_cmdbuf_addr->bus_address = (ptr_t)new_cmdbuf_addr->virtual_address +
      ...
      new_status_cmdbuf_addr->bus_address =
          (ptr_t)new_status_cmdbuf_addr->virtual_address +
          cmdbuf_used_pos * CMDBUF_MAX_SIZE;
      new_status_cmdbuf_addr->size = CMDBUF_MAX_SIZE;
      new_status_cmdbuf_addr->cmdbuf_id = cmdbuf_used_pos;
      cmdbuf_used_pos++;
      if (cmdbuf_used_pos >= TOTAL_DISCRETE_CMDBUF_NUM) cmdbuf_used_pos = 0;
      pthread_mutex_unlock(&cmdbuf_used_mutex);
      return 0;
    } else {
      cmdbuf_used_pos++;
      if (cmdbuf_used_pos >= TOTAL_DISCRETE_CMDBUF_NUM) cmdbuf_used_pos = 0;
...
static bi_list_node* get_cmdbuf_node_in_list_by_addr(ptr_t cmdbuf_addr,
                                                     bi_list* list) {
  bi_list_node* new_cmdbuf_node = NULL;
  struct cmdbuf_obj* cmdbuf_obj = NULL;
  new_cmdbuf_node = list->head;
  while (1) {
    if (new_cmdbuf_node == NULL) return NULL;
...
static int is_supported_core(u32 core_mask, u16 dev_pos) {
  if (core_mask && ((core_mask >> dev_pos) & 0x1)) {
...
static i32 select_vcmd(bi_list_node* new_cmdbuf_node) {
  ...
  cmdbuf_obj = (struct cmdbuf_obj*)new_cmdbuf_node->data;
  core_mask = cmdbuf_obj->core_mask;

  //there is an empty vcmd to be used
  while (1) {
    ret = is_supported_core(core_mask, vcmd_position[cmdbuf_obj->module_type]);
    if (ret != 0) {
      ...
      if (list->tail == NULL) {
        ...
        if (vcmd_position[cmdbuf_obj->module_type] >=
            vcmd_core_num[cmdbuf_obj->module_type])
    ...
    counter++;
    if (counter >= vcmd_core_num[cmdbuf_obj->module_type]) break;
  }
  ...
  while (1) {
    ...
    if (ret != 0) {
      ...
      if (dev->hw_version_id <= VCMD_HW_ID_1_0_C)
        hw_rdy_cmdbuf_num = vwl_get_register_value(
            dev->vcmd_core, dev->reg_mirror, HWIF_VCMD_EXE_CMDBUF_COUNT);
      else {
        hw_rdy_cmdbuf_num =
            *(dev->vcmd_reg_mem_virtual_address + VCMD_EXE_CMDBUF_COUNT);
        if (hw_rdy_cmdbuf_num != dev->sw_cmdbuf_rdy_num) hw_rdy_cmdbuf_num += 1;
      }
      curr_cmdbuf_node = list->tail;
      if (curr_cmdbuf_node == NULL) {
        bi_list_insert_node_tail(list, new_cmdbuf_node);
  ...
  if (cmdbuf_obj->priority == CMDBUF_PRIORITY_NORMAL) {
    ...
    while (1) {
      ...
      if (ret != 0) {
        ...
        //read executing cmdbuf address
        if (dev->hw_version_id <= VCMD_HW_ID_1_0_C) {
          exe_cmdbuf_addr = VWLGetAddrRegisterValue(
              dev->vcmd_core, dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR);
          list = &dev->list_manager;
          ...
          //cmdbuf_id = vcmd_get_register_value((const void *)dev->hwregs,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ID);
          cmdbuf_id =
              *(dev->vcmd_reg_mem_virtual_address + EXECUTING_CMDBUF_ID_ADDR + 1);
          ...
          if (curr_cmdbuf_node == NULL) {
            ...
            while (1) {
              if (curr_cmdbuf_node == NULL) break;
              cmdbuf_obj_temp = (struct cmdbuf_obj*)curr_cmdbuf_node->data;
    ...
    while (1) {
      ...
      if (ret != 0) {
        ...
        if (dev->hw_version_id <= VCMD_HW_ID_1_0_C) {
          ...
          //cmdbuf_id = vcmd_get_register_value((const void *)dev->hwregs,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ID);
          cmdbuf_id =
              *(dev->vcmd_reg_mem_virtual_address + EXECUTING_CMDBUF_ID_ADDR);
          if (cmdbuf_id >= TOTAL_DISCRETE_CMDBUF_NUM ||
              (cmdbuf_id == 0 && vcmd_run_once[dev->core_id])) {
            printf("cmdbuf_id greater than the ceiling !!\n");
            return -1;
...
static i32 reserve_cmdbuf(struct exchange_parameter* input_para) {
  ...
  if (new_cmdbuf_node == NULL) return -1;

  //set info from ewl
  cmdbuf_obj = (struct cmdbuf_obj*)new_cmdbuf_node->data;
  cmdbuf_obj->module_type = input_para->module_type;
  cmdbuf_obj->priority = EXCH_G_BIT(input_para->input_mask, EXCH_PRIO_BIT);
  cmdbuf_obj->executing_time = input_para->interrupt_ctrl&(0x7fffffff);
  cmdbuf_obj->interrupt_ctrl = input_para->interrupt_ctrl;
  ...
  cmdbuf_obj->core_mask = input_para->core_mask;
...
static long release_cmdbuf_node_cleanup(bi_list* list) {
  ...
  while (1) {
    new_cmdbuf_node = list->head;
    if (new_cmdbuf_node == NULL) return 0;
...
static bi_list_node* find_last_linked_cmdbuf(bi_list_node* current_node) {
  ...
  while (1) {
    if (new_cmdbuf_node == NULL) return last_cmdbuf_node;
    cmdbuf_obj = (struct cmdbuf_obj*)new_cmdbuf_node->data;
...
static i32 release_cmdbuf(u16 cmdbuf_id) {
  ...
  while (1) {
    ...
    if (last_cmdbuf_node == NULL) break;
...
static inline void vcmd_set_jmp_ie(u32* jmp_addr)
{
  u32 operation_code;

  operation_code = *(jmp_addr);
  operation_code = JMP_IE_1 | operation_code;
  *(jmp_addr) = operation_code;
}

static inline void vcmd_clr_jmp_ie(u32* jmp_addr)
{
  u32 operation_code;
...
static inline void vcmd_process_jmp(struct hantrovcmd_dev* dev,
...
static void vcmd_link_cmdbuf(struct hantrovcmd_dev* dev,
    ...
    if (cmdbuf_obj->cmdbuf_data_linked == 0) {
      ...
      if (cmdbuf_obj->has_end_cmdbuf == 0) {
        if (cmdbuf_obj->no_normal_int_cmdbuf == 1) {
          vcmd_process_jmp(dev, cmdbuf_obj);
        }
      }
    }
  }
  ...
  while (1) {
    if (new_cmdbuf_node == NULL) break;
    /* first cmdbuf has been added.*/
    if (new_cmdbuf_node->next == NULL) break;
    ...
    if (cmdbuf_obj->has_end_cmdbuf == 0) {
      ...
      if (sizeof(ptr_t) == 8) {
        *(jmp_addr - 2) =
            (u32)(((u64)(ptr_t)next_cmdbuf_obj->cmdbuf_busAddress >> 32));
      } else {
        *(jmp_addr - 2) = 0;
      }
      ...
      *(jmp_addr - 4) = (u32)(operation_code | JMP_RDY_1 |
      ...
      if (next_cmdbuf_obj->has_end_cmdbuf == 0) {
        if (next_cmdbuf_obj->no_normal_int_cmdbuf == 1) {
          vcmd_process_jmp(dev, next_cmdbuf_obj);
        }
      } else {
        dev->duration_without_int = 0;
      }
...
#ifdef VCMD_DEBUG_INTERNAL
      {
        ...
        for (i = cmdbuf_obj->cmdbuf_size / 4 - 8;
          ...
          printf("current linked cmdbuf data %u =0x%x\n", i,
...
static void vcmd_start(struct hantrovcmd_dev* dev,
  ...
  if (dev->working_state == WORKING_STATE_IDLE) {
    if ((first_linked_cmdbuf_node != NULL) && dev->sw_cmdbuf_rdy_num) {
      ...
      //0x40
      vwl_set_register_mirror_value(dev->reg_mirror,
      ...
      vwl_set_register_mirror_value(dev->reg_mirror,
                                    HWIF_VCMD_CORE_CLK_GATE_DISABLE, 1);
      ...
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_START_TRIGGER,
                                    1);
      //0x48
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_JMP_EN,
                                      1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_ABORT_EN, 1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_CMDERR_EN,
                                    1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_TIMEOUT_EN,
                                    1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_BUSERR_EN,
                                    1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_ENDCMD_EN,
                                    1);
      //0x4c
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_TIMEOUT_ENABLE, 1);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_TIMEOUT_CYCLES,
                                    0x1dcd6500);
      vwl_set_register_mirror_value(
          dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR,
          (u32)((ptr_t)cmdbuf_obj->cmdbuf_virtualAddress));
      if (sizeof(ptr_t) == 8) {
        vwl_set_register_mirror_value(
            dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR_MSB,
            (u32)(((u64)(ptr_t)cmdbuf_obj->cmdbuf_virtualAddress) >> 32));
      } else {
        vwl_set_register_mirror_value(dev->reg_mirror,
                                      HWIF_VCMD_CMDBUF_EXE_ADDR_MSB, 0);
      }
      vwl_set_register_mirror_value(dev->reg_mirror,
      ...
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_RDY_CMDBUF_COUNT,
                                    dev->sw_cmdbuf_rdy_num);
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_MAX_BURST_LEN,
                                    0x10);
      ...
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_START_TRIGGER,
                                    1);
      AsicHwVcmdCoreWriteRegister(dev->vcmd_core, 0x40,
...
static i32 link_and_run_cmdbuf(struct exchange_parameter* input_para) {
  ...
  u16 batchcount = ((input_para->interrupt_ctrl >> 32) & 0xff);
  new_cmdbuf_node = global_cmdbuf_node[cmdbuf_id];
  if (new_cmdbuf_node == NULL) {
    //should not happen
...
#ifdef VCMD_DEBUG_INTERNAL
  {
    ...
    for (i = 0; i < cmdbuf_obj->cmdbuf_size / 4; i++) {
      printf("current cmdbuf data %u =0x%x\n", i,
             *(cmdbuf_obj->cmdbuf_virtualAddress + i));
    }
  }
#endif
  ...
  cmdbuf_obj->no_normal_int_cmdbuf =
      0;  //0: interrupt when JMP,1 not interrupt when JMP
  ...
  if (opCode == OPCODE_JMP) {
    //jmp
    if (cmdbuf_obj->interrupt_ctrl>>31) {
      cmdbuf_obj->no_normal_int_cmdbuf = 0;
      if (cmdbuf_obj->interrupt_ctrl&1)
        vcmd_set_jmp_ie(jmp_addr - 4);
      else
        vcmd_clr_jmp_ie(jmp_addr - 4);
    } else {
      opCode = tempOpcode;
      opCode &= 0x02000000;
      if (opCode == JMP_IE_1) {
  ...
  PDEBUG("Allocate cmd buffer [%d] to core [%d]\n", cmdbuf_id,
         input_para->core_id);		//cppcheck-suppress preprocessorErrorDirective
  if (dev->hw_version_id > VCMD_HW_ID_1_0_C) {
    ...
    jmp_addr = cmdbuf_obj->cmdbuf_virtualAddress;
    if (sizeof(ptr_t) == 8) {
      *(jmp_addr + 2) = (u32)((u64)(dev->vcmd_reg_mem_bus_address +
    ...
    *(jmp_addr + 1) = (u32)(
        (dev->vcmd_reg_mem_bus_address + (EXECUTING_CMDBUF_ID_ADDR + 1) * 4));
  ...
  if (dev->working_state == WORKING_STATE_IDLE) {
    ...
    if ((batchcount > 0 && (cmdbuf_obj->interrupt_ctrl&1) && vcmd_core_num[VCMD_TYPE_ENCODER] == 1)
        || batchcount == 0 || vcmd_core_num[VCMD_TYPE_ENCODER] > 1) {
...
static int check_cmdbuf_irq(struct hantrovcmd_dev* dev,
                            struct cmdbuf_obj* new_cmdbuf_obj,
                            u32* irq_status_ret) {
  int rdy = 0;
  if (new_cmdbuf_obj->cmdbuf_run_done) {
    ...
    *irq_status_ret =
        new_cmdbuf_obj
            ->executing_status;  //need to decide how to assign this variable
  }
  return rdy;
}

/******************************************************************************/
static int check_mc_cmdbuf_irq(struct cmdbuf_obj* new_cmdbuf_obj,
                               u32* irq_status_ret) {
  int k;
  struct hantrovcmd_dev* dev = NULL;
  ...
  for (k = 0; k < TOTAL_DISCRETE_CMDBUF_NUM; k++) {
    ...
    if (new_cmdbuf_node == NULL) continue;
...
static unsigned int wait_cmdbuf_ready(u16* cmd_buf_id) {
  ...
  if ((*cmd_buf_id != ANY_CMDBUF_ID) && *cmd_buf_id < ARRAY_SIZE(global_cmdbuf_node)) {
    ...
    while (check_cmdbuf_irq(dev, new_cmdbuf_obj, &irq_status) == 0) {
      ...
      pthread_cond_timedwait(&global_vcmd_cond_wait, &global_vcmd_mutex,
                             &wait_timer);
      //            pthread_cond_wait(&global_vcmd_cond_wait,&global_vcmd_mutex);
      if (global_vcmd_release == 1) break;
    }
...
static void create_read_all_registers_cmdbuf(
  ...
  //u32 *status_base_virt_addr=vcmd_status_buf_mem_pool.virtualAddress + input_para->cmdbuf_id*CMDBUF_MAX_SIZE/4+(vcmd_manager[input_para->module_type][0]->vcmd_core_cfg.status_main_addr /4+0);
  struct config_parameter params;
  params.module_type = input_para->module_type;
  CmodelIoctlGetVcmdParameter(&params);
  ptr_t status_base_phy_addr =
      vcmd_status_buf_mem_pool.bus_address +
      input_para->cmdbuf_id * CMDBUF_MAX_SIZE +
      params.status_main_addr;
  u32 offset_inc = 0;
  u32 offset_inc_dec400 = 0;
  if (vcmd_manager[input_para->module_type][input_para->core_id]
          ->hw_version_id > VCMD_HW_ID_1_0_C) {
    PDEBUG("vcx_vcmd_driver:create cmdbuf data when hw_version_id = 0x%x\n",
           vcmd_manager[input_para->module_type][input_para->core_id]
               ->hw_version_id);	//cppcheck-suppress preprocessorErrorDirective

    //read vcmd executing cmdbuf id registers to ddr for balancing core load.
    *(set_base_addr + 0) =
        (OPCODE_RREG) | (1 << 16) | (EXECUTING_CMDBUF_ID_ADDR * 4);
    ...
    *(set_base_addr + 7) = 0;
    ...
    if (vcmd_manager[input_para->module_type][0]
      ...
      offset_inc = 4;
      status_base_phy_addr = vcmd_status_buf_mem_pool.bus_address +
                             input_para->cmdbuf_id * CMDBUF_MAX_SIZE +
                             params.status_L2Cache_addr;
      //map_status_base_phy_addr=vcmd_status_buf_mem_pool.mmu_bus_address + input_para->cmdbuf_id*CMDBUF_MAX_SIZE+(vcmd_manager[input_para->module_type][0]->vcmd_core_cfg.status_L2Cache_addr/2+0);
...
static void read_main_module_all_registers(u32 core_id) {
  ...
  /* normal priority */
  input_para.input_mask = 0;
  input_para.module_type = main_module_type;
  input_para.cmdbuf_size = 0;
  input_para.cmdbuf_reserve = 1;
  input_para.core_id = manager_id;
  input_para.core_mask = 1 << vcmd_manager[main_module_type][manager_id]->id_in_type;
  ret = reserve_cmdbuf(&input_para);
  (void)(ret);
  vcmd_manager[main_module_type][manager_id]->status_cmdbuf_id =
      input_para.cmdbuf_id;
  ...
  status_base_virt_addr =
      ...
      input_para.cmdbuf_id * CMDBUF_MAX_SIZE / 4 +
       0;
  printf("vcx_vcmd_driver: main module register 0:0x%x\n",
         *status_base_virt_addr);
  printf("vcx_vcmd_driver: main module register 80:0x%x\n",
         *(status_base_virt_addr + 80));
  printf("vcx_vcmd_driver: main module register 214:0x%x\n",
         *(status_base_virt_addr + 214));
  printf("vcx_vcmd_driver: main module register 226:0x%x\n",
         *(status_base_virt_addr + 226));
  printf("vcx_vcmd_driver: main module register 287:0x%x\n",
         *(status_base_virt_addr + 287));
  printf("vcx_vcmd_driver: main module register 509:0x%x\n",
...
i32 CmodelIoctlReleaseCmdbuf(u32 cmd_buf_id) {
  PDEBUG("release cmdbuf %u\n", cmd_buf_id);
  return release_cmdbuf(cmd_buf_id);
}

i32 CmodelIoctlGetCmdbufParameter(
  ...
  vcmd_mem_params->virt_status_cmdbuf_addr =
      vcmd_status_buf_mem_pool.virtual_address;
...
static void get_subsys_offset_in_status_buf(struct config_parameter *param)
{
	...
	if (param->submodule_axife_addr[0] != 0xffff) {
		...
		size = ALIGN_64(AXIFE_REGISTER_SIZE * 4);
		offset += size;
	}
	...
	if (param->submodule_ufbc_addr != 0xffff) {
		...
		size = ALIGN_64(UFBC_REGISTER_SIZE * 4);
		offset += size;
	}
}
...
i32 CmodelIoctlGetVcmdParameter(struct config_parameter* vcmd_params) {
  int i=0;
  //get vcmd info
  if (vcmd_core_num[vcmd_params->module_type]) {  //cppcheck-suppress ctuuninitvar
    ...
    vcmd_params->submodule_ufbc_addr =
        vcmd_manager[vcmd_params->module_type][0]
            ->vcmd_core_cfg.vcmd_core_config_ptr->submodule_ufbc_addr;
    ...
    vcmd_params->vcmd_hw_version_id = vcmd_manager[vcmd_params->module_type][0]
                                          ->vcmd_core_cfg.vcmd_hw_version_id;
    for(i = 0; i < vcmd_params->vcmd_core_num; i++){
        vcmd_params->vcmd_priority[i] = vcmd_manager[vcmd_params->module_type][i]->vcmd_core_cfg.vcmd_core_config_ptr->priority;
    }
    get_subsys_offset_in_status_buf(vcmd_params);
  } else {
    ...
    vcmd_params->submodule_ufbc_addr = 0xffff;
    vcmd_params->config_status_cmdbuf_id = 0;
    vcmd_params->vcmd_core_num = 0;
    for(i = 0; i < vcmd_params->vcmd_core_num; i++) {
      vcmd_params->vcmd_priority[i] = 0;
    }
  }
...
i32 CmodelVcmdGetCoreConfig() {
  ...
  for (i = 0; i < CoreInfo.cores_per_slice; i++) {
    if (CoreInfo.Cfg[i].has_vcmd == HasExtHw) {
      ...
      //DEV
      if (cfg->h264Enabled && cfg->hevcEnabled && cfg->jpegEnabled &&
          cfg->av1Enabled && cfg->vp9Enabled && cfg->cuTreeSupport) {
        //h264/hevc/jpeg/av1/vp9/cutree
        if (i < 2) {
        ...
        if (cfg->h264Enabled || cfg->hevcEnabled || cfg->av1Enabled || cfg->vp9Enabled) {
          vcmd_core_array[vcmd_core_cont] = vcmd_core_all_array[VCEcont];
          vcmd_core_cont++;
          VCEcont++;
  ...
  total_vcmd_core_num =
      (vcmd_core_cont > MAX_HWCORE_NUM) ? MAX_HWCORE_NUM : vcmd_core_cont;
  return 0;
}
...
i32 CmodelVcmdInit() {
  ...
  hantrovcmd_data = (struct hantrovcmd_dev*)malloc(
  ...
  vcmd_status_buf_mem_pool.virtual_address =
      (u32*)malloc(CMDBUF_POOL_TOTAL_SIZE);
  ...
  vcmd_status_buf_mem_pool.bus_address =
      (ptr_t)vcmd_status_buf_mem_pool.virtual_address;
  vcmd_status_buf_mem_pool.cmdbuf_id = 0xFFFF;

  vcmd_registers_mem_pool.virtual_address =
      (u32*)malloc(CMDBUF_VCMD_REGISTER_TOTAL_SIZE);
  ...
  vcmd_registers_mem_pool.bus_address =
      (ptr_t)vcmd_registers_mem_pool.virtual_address;
  ...
  for (i = 0; i < total_vcmd_core_num; i++) {
    if (total_vcmd_core_num == 1 &&
        vcmd_core_array[i].sub_module_type ==VCMD_TYPE_JPEG_ENCODER) {
      vcmd_core_array[i].sub_module_type = VCMD_TYPE_ENCODER;
    }
    ...
    dev->id_in_type = core;
    if (dev->hwregs != NULL) hantrovcmd_data[i].hw_version_id = *(u32*)dev->hwregs;
  ...
  pthread_mutex_init(&cmdbuf_used_mutex, NULL);
...
void CmodelVcmdRelease() {
  ...
  pthread_mutex_destroy(&cmdbuf_used_mutex);
...
static int vcmd_isr_cb(int irq, void* dev_id) {
  ...
  if (dev->list_manager.head == NULL) {
    ...
    irq_status = AsicHwVcmdCoreReadRegister(dev->vcmd_core,
                                            VCMD_REGISTER_INT_STATUS_OFFSET);
    AsicHwVcmdCoreWriteRegister(dev->vcmd_core, VCMD_REGISTER_INT_STATUS_OFFSET,
                                irq_status);
    return IRQ_HANDLED;
  }
  ...
  if ((dev->hw_version_id > VCMD_HW_ID_1_0_C) && (irq_status & 0x3f)) {
    ...
    cmdbuf_id = vwl_get_register_value(dev->vcmd_core, dev->reg_mirror,
                                       HWIF_VCMD_CMDBUF_EXE_ID);
    if (cmdbuf_id >= TOTAL_DISCRETE_CMDBUF_NUM) {
...
#ifdef VCMD_DEBUG_INTERNAL
    {
      ...
      for (i = 0; i < ASIC_VCMD_SWREG_AMOUNT; i++) {
        ...
        printf("ddr vcmd register %u:0x%x\n", i, fordebug);
      }
    }
#endif
  ...
  if (vwl_get_register_mirror_value(dev->reg_mirror, HWIF_VCMD_IRQ_ABORT)) {
    ...
    if (dev->hw_version_id > VCMD_HW_ID_1_0_C) {
      ...
      exe_cmdbuf_bus_address = VWLGetAddrRegisterValue(
          dev->vcmd_core, dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR);
    ...
    while (1) {
      ...
      if ((cmdbuf_obj->cmdbuf_run_done == 0)) {
        ...
        vcmd_run_once[((struct cmdbuf_obj*)(global_cmdbuf_node[cmdbuf_id]
                                                ->data))
                          ->core_id] = 1;
      } else
        break;
...
static void vcmd_write_reg_cb(void* core, u32 reg_idx, u32 value, u32 core_type,
  ...
  u32 offset;
  offset = reg_idx * 4;
  if (core_type == MODULE_MAIN_CORE || core_type == MODULE_UFBC) {
...
static void vcmd_reset_asic(struct hantrovcmd_dev* dev) {
  ...
  for (n = 0; n < total_vcmd_core_num; n++) {
    if (dev[n].hwregs != NULL) {
      ...
      AsicHwVcmdCoreWriteRegister(dev[n].vcmd_core,
                                  VCMD_REGISTER_INT_CTL_OFFSET, 0x0000);
      //reset all
      AsicHwVcmdCoreWriteRegister(dev[n].vcmd_core,
                                  VCMD_REGISTER_CONTROL_OFFSET, 0x0002);
      //read status register
      result = AsicHwVcmdCoreReadRegister(dev[n].vcmd_core,
                                          VCMD_REGISTER_INT_STATUS_OFFSET);
      ...
      AsicHwVcmdCoreWriteRegister(dev[n].vcmd_core,
                                  VCMD_REGISTER_INT_CTL_OFFSET, 0xffffffff);
    }
  }
}
...
static void vcmd_reset_current_asic(struct hantrovcmd_dev* dev) {
  ...
  if (dev->hwregs != NULL) {
    //disable interrupt at first
    AsicHwVcmdCoreWriteRegister(dev->vcmd_core, VCMD_REGISTER_INT_CTL_OFFSET,
                                0x0000);
    //reset all
    AsicHwVcmdCoreWriteRegister(dev->vcmd_core, VCMD_REGISTER_CONTROL_OFFSET,
                                0x0002);
    //read status register
    result = AsicHwVcmdCoreReadRegister(dev->vcmd_core,
                                        VCMD_REGISTER_INT_STATUS_OFFSET);
    //clean status register
    AsicHwVcmdCoreWriteRegister(dev->vcmd_core, VCMD_REGISTER_INT_STATUS_OFFSET,
                                result);
