...
//#define OBJECT_DETECTION_SUPPORT
...
typedef struct commandLine_s {
  ...
  char *osdDec400CompTableInput[MAX_OVERLAY_NUM];

  i32 useVcmd;  /* Whether to enable VCMD, only valid for cmodel */
  i32 useDec400;
  i32 useL2Cache;
  i32 useAXIFE;
  ...
  i32 intraPeriod; /* nonIDR intra interval */
  ...
  i32 fpsAdjustFrame[MAX_FPS_ADJUST];
  i32 fpsAdjustRateNum[MAX_FPS_ADJUST];
  i32 fpsAdjustRateDenom[MAX_FPS_ADJUST];
  i32 fpsAdjustDelay[MAX_FPS_ADJUST];
  i32 smoothPsnrInGOP;
  ...
  u32 lowdelayB; /**< enable low delay B to replace regular P frame */
  u32 gopLowdelay;
  u32 gopMaxBSize;
  i32 outReconFrame;
  u32 longTermGap;
  ...
  u32 ltrInterval;
  i32 longTermQpDelta;
  u32 enableLeadingPictures; /* insert leading pictures before IDR */

  char *flexRefs; /**< flexible reference description file. */

  i32 gdrDuration;

  /** enable or disable anti intra flicker feature */
  u32 aifEnable;
  /** qp delta between AIF frame and its reference frame */
  i32 aifQpDelta;
  ...
  i32 noiseSigmaU;
  i32 noiseSigmaV;
  i32 noiseReductionStrength_IntraY;
  i32 noiseReductionStrength_IntraU;
  i32 noiseReductionStrength_IntraV;
  i32 noiseReductionStrength_InterY;
  i32 noiseReductionStrength_InterU;
  i32 noiseReductionStrength_InterV;
  i32 noiseReduction_ChromaMaxMV;

  i32 bitDepthLuma;
  i32 bitDepthChroma;

  /* frameInfo of hevc or h264*/
  u32 enableFrameInfoVersion;
  ...
  /* static region detection threshold */
  i32 SRDThreshold;
  /* static region detection mode, 0:off, 1:on, 2:dynamic */
  i32 SRDEnable;
  ...
  u32 ctbRcSkinCbRange[2];
  u32 ctbRcSkinCrRange[2];
  u32 ctbRcSkinLumRange[2];
...
typedef struct {
  i32 gopBits[MAX_BITS_STAT]; /* record all gops encoded bits upto 1024 gops */
  i32 gopPos;
  i32 nGop;
  i8 nGopFrame;
  double sumDeviation;
...
/* buffer1 will bind with inputMem */
typedef struct {
  ptr_t inputMem;
  void *buf;
  //maybe exist multi sub buffers internal struct of buf
  u32 countInBuf;
  u8 isUsed;
} BufferSet;
...
struct test_bench {
  ...
  EWLLinearMem_t *osdDec400TableMem[MAX_OVERLAY_NUM];
  ...
  EWLLinearMem_t
      outbufMemFactory[MAX_CORE_NUM][HEVC_MAX_TILE_COLS]
  ...
  EWLLinearMem_t osdDec400CmpTableMemFactory[MAX_DELAY_NUM][MAX_OVERLAY_NUM];
  EWLLinearMem_t mvReplaceBuffer;
  EWLLinearMem_t SliceInfoMemFactory[MAX_DELAY_NUM]; /* Sliceinfo data for low latency */
  EWLLinearMem_t osdMapMemFactory[MAX_DELAY_NUM];
  ...
  u32 roiMapDeltaQpMemFlags
      [MAX_DELAY_NUM];  //flags for input roiMapDeltaQp buffers
  u32 roiMapCuCtrlInfoMemFlags
      [MAX_DELAY_NUM];  //flags for input roiMapCuCtrlInfo buffers
  u32 overlayInputFlags[MAX_DELAY_NUM];  //flags for overlayInput buffers
  u32 osdMapInputFlags[MAX_DELAY_NUM];
  ...
  FILE *extSEIFile;
  BufferSet bindInputExternalSei[MAX_DELAY_NUM];
  const char *jsonSEIFile;
...
#ifdef HEIF_SUPPORT
  bool heif_enable;
  VCEncHeifInst *heif_inst;
#endif
  u32 ufbcMode;
  u32 ufbcBlockType;
  void *bufmgr; //for libva used
  u32 dec400Enable;
  u32 dec400TSHeaderEnable;
};
