...
void InitPicConfig(VCEncIn *pEncIn, struct test_bench *tb, commandLine_s *cml) {
  ...
  for (i = 0; i < pEncIn->gopConfig.special_size; i++) {
    ...
    if (pEncIn->gopConfig.pGopPicSpecialCfg[i].i32Ltr == 0)
      ...
      for (k = 0; k < (i32)pEncIn->gopConfig.pGopPicSpecialCfg[i].numRefPics;
           k++) {
        i32 i32LTRIdx =
  ...
  if (cml->hrdConformance) pEncIn->vui_timing_info_enable = 1;
  ...
  if (cml->enableLeadingPictures) {
    pEncIn->last_idr_picture_cnt = pEncIn->picture_cnt = cml->gopSize - 1;
  }
}
