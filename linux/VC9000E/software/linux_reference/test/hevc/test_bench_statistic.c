...
void StatisticFrameBits(VCEncInst inst, struct test_bench *tb, ma_s *ma,
  ...
  u32 newGop = type == 0 || rc->predLayerId == 1;  //predLayerId == LAYER_P;
  i32 movingBitRate = -1;
  ...
  if (ma->count >= ma->length) {
    ...
    tb->bitStat.maxMovingBitRate =
        MAX(tb->bitStat.maxMovingBitRate, ratePercent);
    /* assume that real bitRate can not be zero */
    tb->bitStat.minMovingBitRate =
        tb->bitStat.minMovingBitRate == 0
            ? ratePercent
  ...
  if (newGop) {
    ...
    if (tb->bitStat.nGop < MAX_BITS_STAT) tb->bitStat.nGop++;
  }
  ...
  u32 actualBitrate = totalBits * rc->outRateNum /
                      ((tb->picture_encoded_cnt + 1) * rc->outRateDenom);
  float bpsDeviation = (float)actualBitrate / targetBitrate - 1;

  printf("[RC] Pic %i ", tb->picture_encoded_cnt);
  if (movingBitRate >= 0) printf("movingBitrate: %d ", movingBitRate);

  printf(
      "targetBits=%d actualBits=%u deviation=%.2f%% | targetBitrate=%d "
      "actualBitrate=%u deviation=%.2f%%\n",
      rc->targetPicSize, frameBits,
      (frameBits * 100.0f) / rc->targetPicSize - 100, rc->virtualBuffer.bitRate,
      actualBitrate, bpsDeviation * 100);
  //#else
...
float GetBitsDeviation(struct test_bench *tb) {
  ...
  if (tb->bitStat.nGop) {
    ...
    i64 gopAvgBits = 0;
    double gopSquareDiff = 0;
    for (i = 0; i < tb->bitStat.nGop; i++) gopAvgBits += tb->bitStat.gopBits[i];

    if (gopAvgBits) {
      gopAvgBits /= tb->bitStat.nGop;
      for (i = 0; i < tb->bitStat.nGop; i++) {
        double diff = tb->bitStat.gopBits[i] - gopAvgBits;
        gopSquareDiff += diff * diff;
      }
      theta = sqrt(gopSquareDiff / tb->bitStat.nGop);
...
/*------------------------------------------------------------------------------
    ...
    Return type     : void
------------------------------------------------------------------------------*/
void EncTraceProfile(VCEncInst inst, struct test_bench *tb, VCEncOut *pEncOut,
                     u64 bits, u8 force) {
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
  asicData_s *asic = &vcenc_instance->asic;
  struct container *c;
  ...
  */
  ...
  if (c != NULL) pic = get_picture(c, poc);
  if (!pic) return;
  ...
  if (vcenc_instance->rateControl.frameCoded == ENCHW_NO) return;
  ...
  if (vcenc_instance->num_tile_columns > 1) {
    ...
    prof.ssim = (asic->regs.codedChromaIdc == 0)
                    ? pEncOut->ssim[0]
                    : (pEncOut->ssim[0] * 0.8 +
                       0.1 * (pEncOut->ssim[1] + pEncOut->ssim[2]));
    prof.ssim_y = pEncOut->ssim[0];
    prof.ssim_u = pEncOut->ssim[1];
    prof.ssim_v = pEncOut->ssim[2];
  } else {
    EWLLinearMem_t *compress_coeff_SACN =
        &asic->compress_coeff_SACN[(vcenc_instance->jobCnt + 1) %
                                   vcenc_instance->parallelCoreNum];
    EWLSyncMemData(compress_coeff_SACN, 0, sizeof(prof), DEVICE_TO_HOST);

    void *prof_data =
        (u8 *)(asic->compress_coeff_SACN[(vcenc_instance->jobCnt + 1) %
                                         vcenc_instance->parallelCoreNum]
                   .virtualAddress);
  ...
  fprintf(fp,
          "POC %d QP %2.2f  %llu bits [Y %.4f dB  U %.4f dB  V %.4f dB] [SSIM "
          "%.4f]  [SSIM Y %.4f  U %.4f  V %.4f] [%s]\n",
          poc, asic->regs.sumOfQP * 1.0 / asic->regs.sumOfQPNumber, (unsigned long long)bits,
          prof.psnr_y, prof.psnr_u, prof.psnr_v, prof.ssim, prof.ssim_y,
          prof.ssim_u, prof.ssim_v,
