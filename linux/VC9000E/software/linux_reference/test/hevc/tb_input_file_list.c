...
/* create a link list.*/
static inputFileLinkList *inputFileLinkListCreat(char *in, char *out, u32 a,
                                                 u32 b, u32 w, u32 h) {
...
static void inputFileLinkListInsertTail(inputFileLinkList *list, char *in,
                                        char *out, u32 a, u32 b, u32 w, u32 h) {
...
#define MAX_LINE_LENGTH 1024
  ...
  while (1) {
    if (feof(tb->inputFileList)) break;

    achParserBuffer[0] = '\0';
    char *line =
        fgets((char *)achParserBuffer, MAX_LINE_LENGTH, tb->inputFileList);
    if (!line) break;

    removeSpaceAndReturn(line);

    while (1) { //Mark for klocwork: ignore
      if (line == NULL) break;
      line = nextTokenMinus(line);
      if (line != NULL) {
        if (strlen(line) >= MAX_LINE_LENGTH) break;
        option = line[1];
        line = line + 2;
        switch (option) {
          case 'i':
            sscanf(line, "%1023[^-]", input_dir);
            break;
          case 'o':
            sscanf(line, "%1023[^-]", output_dir);
            break;
          case 'a':
            sscanf(line, "%10[^-]", tmp);
            a = atoi((char *)tmp);
    ...
    if (tb->inputFileLinkListNum == 0) {
      tb->inputFileLinkListP =   //Mark for klocwork : it will be free by inputFileLinkListDeleteHead()
          inputFileLinkListCreat(input_dir, output_dir, a, b, w, h);
    } else {
      inputFileLinkListInsertTail(tb->inputFileLinkListP, input_dir, output_dir,
                                  a, b, w, h);
    }
