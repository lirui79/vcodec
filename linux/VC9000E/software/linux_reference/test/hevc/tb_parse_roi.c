...
i32 parse_inter_dir(char *str) {
  if (0 == strcmp("L0", str)) return ROI_PRED_L0;
  if (0 == strcmp("L1", str)) return ROI_PRED_L1;
  if (0 == strcmp("B0", str)) return ROI_PRED_BI;
  if (0 == strcmp("B1", str)) return ROI_PRED_BI;
...
i8 parse_intra_cu_part(char *str) {
  if (0 == strcmp("2Nx2N", str)) return ROI_PART_2N_2N_I;
  if (0 == strcmp("NxN", str)) return ROI_PART_N_N_I;
  return -1;
}
...
i8 parse_inter_cu_part(char *str) {
  if (0 == strcmp("any", str)) return ROI_INTERNAL_AUTOMODE;
  if (0 == strcmp("2Nx2N", str)) return ROI_PART_2N_2N;
  if (0 == strcmp("2NxN", str)) return ROI_PART_2N_N;
  if (0 == strcmp("Nx2N", str)) return ROI_PART_N_2N;
  if (0 == strcmp("2NxnU", str)) return ROI_PART_2N_NU;
  if (0 == strcmp("2NxnD", str)) return ROI_PART_2N_ND;
  if (0 == strcmp("nLx2N", str)) return ROI_PART_NL_2N;
  if (0 == strcmp("nRx2N", str)) return ROI_PART_NR_2N;
...
u8 parse_cu_mode(char *str) {
  if (0 == strcmp("intra", str)) return CU_MODE_INTRA;
  if (0 == strcmp("inter", str)) return CU_MODE_INTER;
  if (0 == strcmp("skip", str)) return CU_MODE_SKIP;
  if (0 == strcmp("ipcm", str)) return CU_MODE_IPCM;
...
i32 fill_pic_roimap(const void *inst, FILE *roimapConfigFile, u8 roimap_format,
                    u8 *roimapDeltaQPmemory, u32 blk_side_length, u16 roiBlkSize) {
  static i32 pic_this = -1;
  RoiRect roi;
  u8 roi_valid = 0;
  char buffer[MAX_LINE_LENGTH_BLOCK];
  char *line;
  i32 left = 0, width = 0, top = 0, height = 0, last_left = 0, last_width = 0,
      last_top = 0, last_height = 0;
  char str0[MAX_LINE_LENGTH_BLOCK], str1[MAX_LINE_LENGTH_BLOCK];
  i32 value = 0;
  u32 rect_num =
      -1;  //rect_num is used to count the reading steps of rectange coordinate.
  while (pic_this == -1) {
    /* first entry */
    buffer[0] = '\0';
    ...
    switch (line[0]) {
      ...
      case ' ':
        switch (line[2]) {
          case 'q':
            /* qp */
            sscanf(line, "  qp=(%4095[^,], %d)", str0, &value);
          ...
          case 'm':
            /* mode */
            sscanf(line, "  mode=(%4095[^,], %4095[^)])", str0, str1);
            ...
            if (roi.mode == CU_MODE_INTRA) {
              ...
              if (((roi.part == ROI_PART_2N_2N_I) ||
                   (roi.part == ROI_PART_N_N_I)) == 0) {
                return NOK;
              }
            } else if (roi.mode == CU_MODE_INTER) {
              roi.part = parse_inter_cu_part(str1);
            } else if ((roi.mode == CU_MODE_IPCM) ||
                       (roi.mode == CU_MODE_SKIP)) {
              roi.intra_part_valid = 0;
            } else {
              printf("Error: Invalde cu mode: %s.\n", str0);
              return NOK;
            }
            break;
          ...
          case 'i':
            /* intra modes */
            sscanf(line, "  intra=(%d, %d, %d, %d, %d)",
                   &roi.info.intra.luma_mode[0], &roi.info.intra.luma_mode[1],
                   &roi.info.intra.luma_mode[2], &roi.info.intra.luma_mode[3],
                   &roi.info.intra.chroma_mode);
            break;
          case 'p':
            /* intra modes */
            {
              ...
              if (line[6] == '0') {
                part_idx = 0;
                sscanf(line, "  part0=(%4095[^,], %d, %d, %d)", str0, &ref, &mvx,
                       &mvy);
              } else {
                part_idx = 1;
                sscanf(line, "  part1=(%4095[^,], %d, %d, %d)", str0, &ref, &mvx,
                       &mvy);
              }
