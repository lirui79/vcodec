...
char *RpsLowdelayB_GOPSize_1[] = {
    "Frame1:  B        1   0        0.6     0      2        -1 -2       1 1",
    NULL,
};

char *RpsLowdelayB_GOPSize_2[] = {
    "Frame1:  B        2   0        0.6     0      2        -2 -4      1 1",
    "Frame2:  nrefB    1   0        0.68    0      2        -1 1       1 1",
    NULL,
};

char *RpsLowdelayB_GOPSize_3[] = {
...
char *Rps_lp = "Frame1:  nrefB    1   0        0.4624    0     1       1         1";

static FILE *yuv_out = NULL;

i32 FindInputBufferIdByBusAddr(EWLLinearMem_t *memFactory, const u32 MemNum,
                               const ptr_t busAddr);
...
u32 getAV1OBUPayloadLen(u8 *pStream) {
  ...
  do {
    nData = *pStreamLen;
    if ((nData & 0x7f)) len |= ((nData & 0x7f) << (offset * 7));
    pStreamLen++;
    offset++;
  } while ((nData & 0x80));
...
u32 parsingAV1TileGroupObu(VCEncStrmBufs *bufs, u32 streamSize, u32 offset) {
  ...
  do {
    ...
    if (obuType == OBU_TEMPORAL_DELIMITER) {
      ...
      u32 newTileGroupSizeLen =
          av1UlebEncode((pStream + obuHeaderLen), payload);
...
void writeIvfFrame(FILE *fout, VCEncStrmBufs *bufs, u32 streamSize, u32 offset,
                   i32 *ivfFrameCnt, bool bTileGroupSizeModified,
                   u8 frameNotShow, i32 *ivfSize, u32 bVp9) {
  i32 byteCnt = 0;
...
void getStreamBufs(VCEncStrmBufs *bufs, struct test_bench *tb,
  ...
  for (i = 0; i < MAX_STRM_BUF_NUM; i++) {
    bufs->buf[i] = tb->outbufMem[index][i]
                       ? (u8 *)tb->outbufMem[index][i]->virtualAddress
                       : NULL;
    bufs->bufLen[i] =
        tb->outbufMem[index][i] ? tb->outbufMem[index][i]->size : 0;
  }

#ifdef INTERNAL_TEST
  if (encoding && (cml->testId == TID_INT) && cml->streamBufChain &&
...
void WriteScaled(u32 *strmbuf, struct commandLine_s *cml) {
  ...
  u32 width = (cml->rotation && cml->rotation != 3 ? cml->scaledHeight
                                                   : cml->scaledWidth);
  u32 height = (cml->rotation && cml->rotation != 3 ? cml->scaledWidth
                                                    : cml->scaledHeight);
  u32 size = width * height * 2;
  if (format == 1) size = width * height * 3 / 2;

  if (!size) return;

  char fname[100];
  if (format == 0)
    sprintf(fname, "scaled_%ux%u.yuyv", width, height);
  else
    sprintf(fname, "scaled_%ux%u.yuv", width, height);
...
void writeFlags2Memory(char flag, u8 *memory, u16 column, u16 row,
                       u16 blockunit, u16 width, u16 ctb_size, u32 roiBlkSize,
                       u32 ctb_per_row, u32 ctb_per_column) {
  u32 blks_per_ctb = ctb_size / roiBlkSize;
  u32 blks_per_unit = 8 * (1 << (3 - blockunit)) / roiBlkSize;
...
void writeFlagsRowData2Memory(char *rowStartAddr, u8 *memory, u16 width,
                              u16 rowNumber, u16 blockunit, u16 ctb_size, u32 roiBlkSize,
                              u32 ctb_per_row, u32 ctb_per_column) {
  int i;
  i = 0;
  while (i < width) {
    writeFlags2Memory(*rowStartAddr, memory, i, rowNumber, blockunit, width,
                      ctb_size, roiBlkSize, ctb_per_row, ctb_per_column);
    rowStartAddr++;
    i++;
  }
}

void writeQpValue2Memory(char qpDelta, u8 *memory, u16 column, u16 row,
                         u16 blockunit, u16 width, u16 ctb_size, u8 roiBlkSize,
                         u32 ctb_per_row, u32 ctb_per_column) {
  u32 blks_per_ctb = ctb_size / roiBlkSize;
  u32 blks_per_unit = 8 * (1 << (3 - blockunit)) / roiBlkSize;
  u32 ctb_row_number = row * roiBlkSize * blks_per_unit / ctb_size;
  u32 ctb_column_number = column * roiBlkSize * blks_per_unit / ctb_size;
  u32 ctb_row_stride = ctb_per_row * blks_per_ctb * blks_per_ctb;
  u32 xoffset = (column * blks_per_unit) % blks_per_ctb;
  u32 yoffset = (row * blks_per_unit) % blks_per_ctb;
  u32 stride = blks_per_ctb;
  ...
  for (r = 0; r < rows; r++) {
    u8 *dst = memory + r * stride;
    for (c = 0; c < columns; c++) *dst++ = qpDelta;
  }
}

/* roiMapVersion 0 */
void writeQpDeltaData2Memory(char qpDelta, u8 *memory, u16 column, u16 row,
                             u16 blockunit, u16 width, u16 ctb_size, u8 roiBlkSize,
                             u32 ctb_per_row, u32 ctb_per_column) {
...

  {
    ...
    switch (blockunit) {
      ...
      case 3:
        ...
        curMemoryStartPtr =
            ctbMemoryStartPtr + ctb_row_offset * internal_ctb_stride + xoffset;
...
void writeQpDeltaRowData2Memory(char *qpDeltaRowStartAddr, u8 *memory,
                                u16 width, u16 rowNumber, u16 blockunit,
                                u16 ctb_size, u32 ctb_per_row, u8 roiBlkSize,
                                u32 ctb_per_column, i32 roiMapVersion) {
  i32 i = 0;
  while (i < width) {
    if (roiMapVersion >= 1)
      writeQpValue2Memory(*qpDeltaRowStartAddr, memory, i, rowNumber, blockunit,
                          width, ctb_size, roiBlkSize, ctb_per_row, ctb_per_column);
    else
      writeQpDeltaData2Memory(*qpDeltaRowStartAddr, memory, i, rowNumber,
                              blockunit, width, ctb_size, roiBlkSize, ctb_per_row,
                              ctb_per_column);
...
#define MAX_LINE_LENGTH_BLOCK (512 * 8)
  ...
  i32 ret = OK;
  u32 encWidth = (cml->rotation == 1 || cml->rotation == 2) ? cml->height : cml->width;
  u32 encHeight = (cml->rotation == 1 || cml->rotation == 2) ? cml->width : cml->height;
  u32 ctb_per_row = ((encWidth + cml->max_cu_size - 1) / (cml->max_cu_size));
  u32 ctb_per_column =
      ((encHeight + cml->max_cu_size - 1) / (cml->max_cu_size));
  ...
  u16 width =
      (((encWidth + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) +
       block_unit - 1) /
      block_unit;
  u16 height =
      (((encHeight + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) +
       block_unit - 1) /
      block_unit;
  ...
  while (line_idx < height) {
    ...
    // Read one line
    char *line =
        fgets((char *)achParserBuffer, MAX_LINE_LENGTH_BLOCK, roiMapFile);
    if (feof(roiMapFile)) {
    ...
    //if (line)
	{
      ...
      writeQpDeltaRowData2Memory(rowbuffer, memory, width, line_idx, blockunit,
                                 ctb_size, ctb_per_row, tb->hwCfg->roiBlkSize == 0 ? 8 : 16, ctb_per_column,
                                 roiMapVersion);
      line_idx++;
    }
  }
...
int copyFlagsMap2Memory(commandLine_s *cml, VCEncInst enc,
  ...
  bool bRoiVerSelectable =
      ...
      ((HW_ID_MAJOR_NUMBER(tb->hwid) >= 0x82) ||
       (HW_PRODUCT_VC9000(tb->hwid)) || HW_PRODUCT_SYSTEM6010(tb->hwid) ||
       (HW_PRODUCT_VC9000LE(tb->hwid)));
  ...
  width = (((encWidth + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) +
           block_unit_size - 1) /
          block_unit_size;
  height = (((encHeight + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) +
            block_unit_size - 1) /
           block_unit_size;
  ...
  while (line_idx < height) {
    ...
    //if (line)
	{
      ...
      writeFlagsRowData2Memory(rowbuffer, memory, width, line_idx, blockunit,
                               ctb_size, roiBlkSize, ctb_per_row, ctb_per_column);
      line_idx++;
    }
  }
...
void getAlignedPicSizebyFormat(VCEncPictureType type, u32 width, u32 height,
  ...
  VCEncGetAlignedStride(width, type, &luma_stride, &chroma_stride, alignment, scan_type);
  switch (type) {
    case VCENC_YUV420_PLANAR:
    ...
    case VCENC_BGRX1010102:
      lumaSize = luma_stride * height;
      if (scan_type == VCENC_SUPERTILEX_SCAN)  lumaSize = luma_stride * ((height + 63) / 64); //height align to 64 when supertile.
      chromaSize = 0;
      break;
    case VCENC_YUV420_PLANAR_10BIT_I010:
      lumaSize = luma_stride * height;
    ...
    case VCENC_YUV420SP10b:
      lumaSize = luma_stride * 10 / 8 * height;
      chromaSize = chroma_stride * 10 / 8 * height / 2;
      break;
    case VCENC_YUV422P10bWL:
      lumaSize = luma_stride * height;
      chromaSize = chroma_stride * height * 2;
...
void GetOsdDec400Size(const void *wl_instance, commandLine_s *cmdl, u8 idx,
                      u32 input_alignment, u32 dec400TableSize[],
                      u32 *dec400TotalTableSize, u32 dec400Enable) {
  ...
  EncGetDec400TsBufferSize(format, cmdl->olWidth[idx], cmdl->olHeight[idx],
    ...
    cmdl->scanType, dec400Enable, cmdl->dec400TSHeaderEnable);
...
void ChangeToCustomizedFormat(commandLine_s *cml,
  ...
  if ((cml->formatCustomizedType == 13) &&
      (cml->inputFormat == VCENC_YUV420_PLANAR)) {
...
u32 GetOsdmapReadHeight(commandLine_s *cml, struct test_bench *tb) {
  u32 blockSize = tb->hwCfg->OSDMapVersion ? cml->osdMapBlockSize : 8;//check version use 8 or 4;
  u32 fillSize = 16;
...
void ChangeCmlCustomizedFormat(commandLine_s *cml) {
  ...
  if (((cml->formatCustomizedType >= 2) && (cml->formatCustomizedType <= 11)) &&
    ...
    if (cml->testId == 16) cml->testId = 0;
    cml->rotation = 0;
  } else if (((cml->formatCustomizedType >= 12) &&
              (cml->formatCustomizedType <= 13)) &&
             (cml->inputFormat == VCENC_YUV420_PLANAR)) {
    if (cml->testId == 16) cml->testId = 0;
...
void transYUVtoTile32format(struct test_bench *tb, commandLine_s *cml) {
  ...
  if (IS_HEVC(cml->codecFormat))  //hevc format
  {
    ...
    for (i = 0; i < row_32; i++) {
      start_add =
          tb->lum + i * 32 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1)));
    ...
    for (i = 0; i < row_32; i++) {
      cb_start_add =
          tb->cb + i * 16 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1))) / 2;
      cr_start_add =
          tb->cr + i * 16 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1))) / 2;
  ...
  } else  //h264 format
  {
    ...
    for (i = 0; i < row_16; i++) {
      start_add =
          tb->lum + i * 16 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1)));
      cb_start_add =
          tb->cb + i * 8 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1))) / 2;
      cr_start_add =
          tb->cr + i * 8 * ((cml->lumWidthSrc + 16 - 1) & (~(16 - 1))) / 2;
  ...
  if (IS_HEVC(cml->codecFormat))  //hevc
  {
    ...
    getAlignedPicSizebyFormat(VCENC_YUV420_PLANAR_8BIT_TILE_32_32,
                              cml->lumWidthSrc, cml->lumHeightSrc, 0, &size_lum,
                              &size_ch, NULL, cml->scanType);
...
void transYUVtoTile4format(struct test_bench *tb, commandLine_s *cml) {
    ...
    for (x = 0; x < cml->lumWidthSrc / 4; x++) {
      for (y = 0; y < cml->lumHeightSrc; y++)
        memcpy(transform_buf + y % 4 * 4 * byte_per_compt + stride * (y / 4) +
                   x * 16 * byte_per_compt,
               tb->lum +
                   y * ((cml->lumWidthSrc + 15) & (~15)) * byte_per_compt +
                   x * 4 * byte_per_compt,
               4 * byte_per_compt);
    }
  ...
  {
    u32 size_lum = ((cml->lumWidthSrc * 4 * byte_per_compt + alignment - 1) &
                    (~(alignment - 1))) *
                   cml->lumHeightSrc / 4;
...
FILE *transYUVtoP101010format(struct test_bench *tb, commandLine_s *cml) {
  ...
  if (cml->exp_of_input_alignment != 0) {
    //static FILE *yuv_out = NULL;
    if (yuv_out == NULL) yuv_out = fopen("daj_transformed_format.yuv", "wb");
    if (yuv_out == NULL) return NULL;
    u32 transform_size = 0;

    transform_size = (cml->lumWidthSrc + 2) / 3 * 4 *
                     (cml->lumHeightSrc + cml->lumHeightSrc / 2);

    for (i = 0; i < transform_size; i++)
      fwrite((u8 *)(tb->transformMem->virtualAddress) + i, sizeof(u8), 1,
             yuv_out);
  } else {
...
FILE *transYUVto256BTileformat(struct test_bench *tb, commandLine_s *cml) {
  u32 x, y, i, k, j;
  u8 *luma_data = NULL, *cb_data = NULL, *cr_data = NULL, *data_out = NULL,
     *data = NULL;
  ...
  if (cml->formatCustomizedType == 3 || cml->formatCustomizedType == 4) {
    ...
    tile_width = 128;
    tile_height = 2;
  } else if (cml->formatCustomizedType == 10 ||
             cml->formatCustomizedType == 11) {
    tile_width = 96;
    tile_height = 2;
  }
  ...
  if (cml->inputFormat == VCENC_YUV420_PLANAR &&
      ...
      for (x = 0; x < (w + tile_width - 1) / tile_width; x++)
        for (i = 0; i < tile_height; i++) {
          if (cml->formatCustomizedType == 5) {
            ...
            memcpy(data_out,
                   luma_data + i * w + tile_width * x + y * tile_height * w,
                   tile_width);
            data_out += tile_width;
          }
        }
    ...
    if (cml->formatCustomizedType == 3 || cml->formatCustomizedType == 8) {
      cb_data = tb->cb;
      cr_data = tb->cr;
    } else if (cml->formatCustomizedType == 4 ||
               cml->formatCustomizedType == 5 ||
               cml->formatCustomizedType == 9) {
      cb_data = tb->cr;
      cr_data = tb->cb;
    }
    if (cb_data == NULL) return NULL;

    for (y = 0; y < (cml->lumHeightSrc / 2 + tile_height - 1) / tile_height;
         y++) {
      for (x = 0; x < (w + tile_width - 1) / tile_width; x++)
        for (i = 0; i < tile_height; i++)
          for (k = 0; k < tile_width / 2; k++) {
            if (cml->formatCustomizedType == 5) {
              ...
              *data_out++ = *(cr_data + k + i * w / 2 +
                              y * tile_height * w / 2 + x * (tile_width / 2));
              *data_out++ = *(cb_data + k + i * w / 2 +
                              y * tile_height * w / 2 + x * (tile_width / 2));
            }
          }
    }
      ...
      for (x = 0; x < (w + tile_width - 1) / tile_width; x++)
        for (i = 0; i < tile_height; i++) {
          for (k = 0; k < tile_width / 3; k++) {
            tmp = 0;
            data = luma_data + i * w + tile_width * x + y * tile_height * w +
                   k * 3;
            for (j = 0; j < 3; j++) {
    ...
    if (cml->formatCustomizedType == 6 || cml->formatCustomizedType == 10) {
      cb_data_tmp = tb->cb;
      cr_data_tmp = tb->cr;
    } else if (cml->formatCustomizedType == 7 ||
               cml->formatCustomizedType == 11) {
      cb_data_tmp = tb->cr;
      cr_data_tmp = tb->cb;
    }
    if (cb_data_tmp == NULL) return NULL;
    ...
    for (y = 0; y < (cml->lumHeightSrc / 2 + tile_height - 1) / tile_height;
      ...
      for (x = 0; x < (w + tile_width - 1) / tile_width; x++)
        for (i = 0; i < tile_height; i++) {
          j = 0;
          tmp = 0;
          cb_data = cb_data_tmp + i * w / 2 + y * tile_height * w / 2 +
                    x * (tile_width / 2);
          cr_data = cr_data_tmp + i * w / 2 + y * tile_height * w / 2 +
                    x * (tile_width / 2);
          for (k = 0; k < tile_width; k++) {
            if (j % 2 == 0)
  ...
  if (cml->exp_of_input_alignment != 0) {
    ...
    if ((cml->formatCustomizedType >= 3 && cml->formatCustomizedType <= 5) ||
      ...
      size_lum = (cml->lumWidthSrc + tile_width - 1) / tile_width * tile_width /
                 3 * 4 * STRIDE(cml->lumHeightSrc, tile_height);
...
i32 ChangeInputToTransYUV(struct test_bench *tb, VCEncInst encoder,
  ...
  while ((cml->exp_of_input_alignment > 0) &&
    ...
    if (read_picture(tb, cml->inputFormat, src_img_size, cml->lumWidthSrc,
      ...
      i32 inputInfoBufId = FindInputBufferIdByBusAddr(
          tb->pictureMemFactory, tb->buffer_cnt, tb->pictureMem->busAddress);
      if (NOK ==
          ReturnBufferById(tb->inputMemFlags, tb->outbuf_cnt, inputInfoBufId))
        return NOK;
    } else {
      i32 inputInfoBufId = FindInputBufferIdByBusAddr(
          tb->pictureMemFactory, tb->buffer_cnt, tb->pictureMem->busAddress);
  ...
  if (transfile != NULL) {
    ...
    u64 lumaSize = 0, chromaSize = 0, pictureSize = 0;
    getAlignedPicSizebyFormat(cml->inputFormat, cml->lumWidthSrc,
                              cml->lumHeightSrc, tb->input_alignment, &lumaSize,
                              &chromaSize, &pictureSize, cml->scanType);
    ...
    for (coreIdx = 0; coreIdx < tb->buffer_cnt; coreIdx++) {
      ...
      SET_MEM_USAGE(tb->pictureMemFactory[coreIdx].mem_type, EWL_MEM_USAGE_IN_SURFACE,
                    cml->secure_mode);
...
i32 readConfigFiles(struct test_bench *tb, commandLine_s *cml) {
  ...
  if ((cml->roiMapDeltaQpEnable) && (cml->roiMapDeltaQpFile != NULL)) {
    tb->roiMapFile = open_file(cml->roiMapDeltaQpFile, "r");
  ...
  if (cml->roiMapDeltaQpEnable && cml->roiMapDeltaQpBinFile) {
    tb->roiMapBinFile = open_file(cml->roiMapDeltaQpBinFile, "rb");
  ...
  if ((cml->ipcmMapEnable) && (cml->ipcmMapFile != NULL)) {
    tb->ipcmMapFile = open_file(cml->ipcmMapFile, "r");
  ...
  for (i = 0; i < 2; i++) {
    if (cml->gmvFileName[i]) {
      tb->gmvFile[i] = open_file(cml->gmvFileName[i], "r");
      if (!tb->gmvFile[i])
  ...
  if (cml->skipMapEnable && cml->skipMapFile && (tb->skipMapFile == NULL) &&
      (notRdoqVerFlag)) {
    tb->skipMapFile = open_file(cml->skipMapFile, "r");
  ...
  if (cml->roiMapInfoBinFile && (tb->roiMapInfoBinFile == NULL)) {
    tb->roiMapInfoBinFile = open_file(cml->roiMapInfoBinFile, "rb");
  ...
  if (cml->roiMapConfigFile && (tb->roiMapConfigFile == NULL)) {
    tb->roiMapConfigFile = open_file(cml->roiMapConfigFile, "rb");
  ...
  if (cml->RoimapCuCtrlInfoBinFile && (tb->RoimapCuCtrlInfoBinFile == NULL)) {
    tb->RoimapCuCtrlInfoBinFile = open_file(cml->RoimapCuCtrlInfoBinFile, "rb");
  ...
  if (cml->RoimapCuCtrlIndexBinFile && (tb->RoimapCuCtrlIndexBinFile == NULL)) {
    tb->RoimapCuCtrlIndexBinFile = open_file(cml->RoimapCuCtrlIndexBinFile, "rb");
  ...
  for (i = 0; i < MAX_OVERLAY_NUM; i++) {
    if ((cml->overlayEnables >> i) & 1) {
      u32 frameByte = 1;
      tb->overlayFile[i] = open_file(cml->olInput[i], "rb");
      ...
      if (tb->olFrameNume[i] == 0) {
        printf(
            "Overlay file %d: Error, file %s size is not suitable for region "
            "size\n",
            i + 1, cml->olInput[i]);
        return -1;
      }

      if (cml->osdDec400CompTableInput[i]) {
        tb->osdDec400TableFile[i] = fopen(cml->osdDec400CompTableInput[i], "rb");
        if (tb->osdDec400TableFile[i] == NULL) {
          printf("OSD DEC400 input: Error, Can Not Open File %s\n",
                 cml->osdDec400CompTableInput[i]);
          return -1;
        }
      }
    } else
  ...
  if (cml->osdMapEnable) {
    tb->osdMapFile = open_file(cml->osdMapInput, "rb");
    u32 frameByte = 1;
    u32 read_height = GetOsdmapReadHeight(cml, tb);
    fseeko(tb->osdMapFile, 0, SEEK_END);
    frameByte = cml->osdMapStride * read_height;
    tb->osdMapFrameNum = ftello(tb->osdMapFile) / frameByte;
    if (tb->osdMapFrameNum == 0) {
      printf(
  ...
  if (cml->extSEI != NULL) {
    tb->extSEIFile = open_file(cml->extSEI, "rb");
    if (!tb->extSEIFile)
      printf("extSEI: Error, Can Not Open File %s\n", cml->extSEI);
  }
  if (cml->jsonHDR != NULL) {
    tb->jsonSEIFile = cml->jsonHDR;
  }
  if (cml->p_t35_payload_file != NULL) {
    tb->p_t35_file = open_file(cml->p_t35_payload_file, "rb");
    if (!tb->p_t35_file)
      printf("%s, %d, t35_file: Error, Can Not Open File %s:\n", __FILE__,
             __LINE__, cml->p_t35_payload_file);
  }
...
#define MAX_LINE_LENGTH 1024
  ...
  while (1) {
    ...
    if (getSmartOpt(line, "smart_H264_Luma_DC_Th", &cml->smartH264LumDcTh) ==
        OK)
      continue;
    ...
    if (getSmartOpt(line, "smart_Hevc_CU8_Luma_DC_Th",
                    &(cml->smartHevcLumDcTh[0])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU16_Luma_DC_Th",
                    &(cml->smartHevcLumDcTh[1])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU32_Luma_DC_Th",
                    &(cml->smartHevcLumDcTh[2])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU8_Chroma_DC_Th",
                    &(cml->smartHevcChrDcTh[0])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU16_Chroma_DC_Th",
                    &(cml->smartHevcChrDcTh[1])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU32_Chroma_DC_Th",
                    &(cml->smartHevcChrDcTh[2])) == OK)
      continue;

    if (getSmartOpt(line, "smart_Hevc_CU8_Luma_AC_Num_Th",
                    &(cml->smartHevcLumAcNumTh[0])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU16_Luma_AC_Num_Th",
                    &(cml->smartHevcLumAcNumTh[1])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU32_Luma_AC_Num_Th",
                    &(cml->smartHevcLumAcNumTh[2])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU8_Chroma_AC_Num_Th",
                    &(cml->smartHevcChrAcNumTh[0])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU16_Chroma_AC_Num_Th",
                    &(cml->smartHevcChrAcNumTh[1])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Hevc_CU32_Chroma_AC_Num_Th",
                    &(cml->smartHevcChrAcNumTh[2])) == OK)
      continue;
    ...
    if (getSmartOpt(line, "smart_Mean_Th3", &(cml->smartMeanTh[3])) == OK)
      continue;
    if (getSmartOpt(line, "smart_Pixel_Num_Cnt_Th", &cml->smartPixNumCntTh) ==
        OK)
      continue;
  }
...
/**
 * \page framerate Frame Rate Conversion
 *
 * The test bench implements frame rate conversion by repeating and dropping frames according
 * to the settings of options <tt>inputRateNumer</tt>, <tt>outputRateDenom</tt>,
 * <tt>inputRateDenom</tt>, and <tt>outputRateNumer</tt>.
 *
 * <b>NOTE:</b> The video encoder does not support frame rate conversion in hardware.
 */
...
i32 read_dec400Data(struct test_bench *tb, u32 inputFormat, u32 src_width,
  ...
  if (tb->dec400TSHeaderEnable) {
    tb->dec400Table = tb->in;
    src_img_size = tb->dec400FrameTableSize + tb->lumaSize + tb->chromaSize;
  }
...
i32 read_picture(struct test_bench *tb, u32 inputFormat, u32 src_img_size,
...
#ifndef SYSTEM_BUILD
  if (tb->ufbcMode) {
    u32 ufbc_header_size, asic_format = 0;
    u64 src_img_size_64 = (u64)src_img_size;
    EncUfbcGetSize(inputFormat, tb->ufbcBlockType, src_width, src_height,
      tb->ufbcMode, &asic_format, &ufbc_header_size, &src_img_size_64);
    seek = ((u64)num) * (src_img_size_64 + ufbc_header_size);
    file_read(in, lum, seek, ufbc_header_size);
    seek += ufbc_header_size;
    lum += ufbc_header_size;
    if (asic_format == ASIC_INPUT_YUV420SEMIPLANAR || asic_format == ASIC_INPUT_P010)
      cb += ufbc_header_size;
    if (file_read(in, lum, seek, src_img_size_64)) return NOK;
    return OK;
  }
#endif
#endif
  ...
  switch (inputFormat) {
    ...
    case VCENC_RGB444:
    case VCENC_BGR444: {
      if (scan_type == VCENC_SUPERTILEX_SCAN) {
        u32 tile_stride = 0;
        tile_stride = STRIDE(src_width, 64) * 64 * 2;
        /* Lum */
        for (i = 0; i < STRIDE(src_height, 64) / 64; i++) {
    ...
    case VCENC_GRB888_24BIT: {
      for (i = 0; i < src_height; i++) {
        if (file_read(in, lum, seek, src_width * 3)) return NOK;
        seek += src_width * 3;
        lum += luma_stride;
      }
      break;
    ...
    case VCENC_BGRX1010102: {
      if (scan_type == VCENC_SUPERTILEX_SCAN) {
        u32 tile_stride = 0;
        tile_stride = STRIDE(src_width, 64) * 64 * 4;
        /* Lum */
        for (i = 0; i < STRIDE(src_height, 64) / 64; i++) {
          if (file_read(in, lum, seek, tile_stride)) return NOK;
          seek += tile_stride;
    ...
    case VCENC_YUV420SP10b: {
      ...
      /* CbCr */
      for (i = 0; i < src_height / 2; i++) {
        if (file_read(in, cb, seek, src_width * 10 / 8)) return NOK;
        seek += src_width * 10 / 8;
        cb += chroma_stride * 10 / 8;
      }
      break;
    ...
    case VCENC_YUV420_10BIT_TILE_8_8: {
      ...
      if (inputFormat == VCENC_YUV420_8BIT_TILE_8_8 ||
          inputFormat == VCENC_YVU420_8BIT_TILE_8_8) {
    ...
    case VCENC_YUV444_PLANAR: {
      ...
      /* Cr */
      for (i = 0; i < src_height; i++) {
        if (file_read(in, cr, seek, src_width)) return NOK;
        seek += src_width;
        cr += chroma_stride;
      }
      break;
    ...
    case VCENC_Y10bWH_TILE_8_8: {
      ...
      if (inputFormat == VCENC_Y8b_TILE_8_8) {
        tile_stride = STRIDE(src_width, 8) * 8;
      } else {
        tile_stride = STRIDE(src_width, 8) * 8 * 2;
      }
...
i32 read_stab_picture(struct test_bench *tb, u32 inputFormat, u32 src_img_size,
  ...
  switch (inputFormat) {
    ...
    case VCENC_GRB888_24BIT:
      for (i = 0; i < src_height; i++) {
        if (file_read(tb->in, lum_next, seek, src_width * 3)) return NOK;
        seek += src_width * 3;
        lum_next += luma_stride;
      }
      break;
...
/**
 * \page gopConfig GOP Configuration File Format

 The video encoder supports configuring a GOP structure by specifying a configuration file
 through the <tt>\-\-gopConfig</tt> option. The file must contain the following configurations:
 - [Normal GOP configurations](@ref normGopConfig)
 - [(Optional) Special GOP configurations](@ref specialGopConfig)

 If <tt>\-\-gopConfig</tt> is unspecified, the default configurations corresponding to the value
 of the <tt>\-\-gopSize</tt> option are applied.


\section normGopConfig Normal GOP Configuration Format

 The GOP configuration file must include <tt>gopSize</tt> normal configuration lines. Each
 line must contain the following fields in order:
...
- <b>FrameN</b>: the frame ID of the frame, where N is an integer ranging from 1 to <tt>gopSize</tt>.

  In the configuration file, frames must be listed in decoding order.
- <b>Type</b>: the slice type, which can be P, B, or nrefB.
- <b>POC</b>: the display order of the frame within the GOP, ranging from 1 to <tt>gopSize</tt>.
- <b>QPoffset</b>: the offset to be added to the QP parameter to obtain the final QP value used for
  the frame.
- <b>QPfactor</b>: the weight used during rate distortion optimization.
- <b>TemporalId</b>: the temporal layer ID.
...
- <b>ref_pics</b>: a list of num_ref_pics integers, each indicating the POC offset of the reference
  frame relative to the current frame or the LTR index.
- <b>used_by_cur</b>: a list of num_ref_pics binaries, each indicating whether the corresponding
 ...
 \section specialGopConfig Special GOP Configuration Format

 Each special GOP configuration line in the GOP configuration file must contain the following fields
 in order:
...
- <b>Frame0</b>: indicates that the current line contains special GOP configurations (for example,
  for long-term reference).
- <b>Type</b>: the slice type, which can be P, B, or I.

  The default value is -255, which indicates the video encoder uses the slice type
  specified in the normal configurations. The specified value is used only if it differs from the
  default.
- <b>QPoffset</b>: the offset to be added to the QP parameter to obtain the final QP.

  The default value is -255, which indicates the video encoder uses the QP offset specified
  in the normal configurations. The specified value is used only if it differs from the default.
- <b>QPfactor</b>: the factor used by rate distortion optimization.

  The default value is -255, which indicates the video encoder uses the QP factor specified
  in the normal configurations. The specified value is used only if it differs from the default.
- <b>TemporalId</b>: the temporal layer ID.

  The default value is -255, which indicates the video encoder uses the temporal layer ID
  specified in the normal configurations. The specified value is used only if it differs from the
  default.
- <b>num_ref_pics</b>: the number of reference frames kept for the current frame.
...
- <b>used_by_cur</b>: a list of num_ref_pics binaries, each indicating whether the corresponding
frame in the ref_pics list is used as a reference for encoding of the current frame.
- <b>LTR</b>: the LTR index of the frame.
  - Value 0 indicates a common frame that uses LTR.
  - A value in range from 1 to <tt>VCENC_MAX_LT_REF_FRAMES</tt> indicates an LTR frame.
- <b>Offset</b>:
  - If LTR equals 0, the Offset field indicates the POC delta between the LTR frame and the first
  subsequent frame that uses the LTR frame as reference.
  - If LTR falls in the range from 1 to <tt>VCENC_MAX_LT_REF_FRAMES</tt>, the Offset field indicates
  the POC delta between the first LTR frame and the first encoded frame.
- <b>Interval</b>:
  - If LTR equals 0, the Interval field indicates the POC delta between two adjacent frames that
  use the same LTR frame as reference.
  - If LTR falls in the range from 1 to <tt>VCENC_MAX_LT_REF_FRAMES</tt>, the Interval field
  indicates the POC delta between two adjacent frames with the same LTR index.


 \section gopConfig_ex GOP Configuration File Examples

 \subsection gopConfig_ex1 Example: Default GOP Structure of Size 4

 The following script shows the contents of the default GOP configuraion file for <tt>gopSize=4</tt>.
 Hierarchical B structure is used.
 \n
 \include default_gop4.cfg


 \subsection gopConfig_ex2 Example: GOP Structure of Size 1 with Long-Term Reference

 The following script shows the contents of an example GOP configuration file for <tt>gopSize=1</tt>
 with long-term reference:
 \n
 \include LTR_gop1.cfg

 */

int ParseGopConfigString(char *line, VCEncGopConfig *gopCfg, int frame_idx,
                         int gopSize) {
  if (!line) return -1;
...
int ReadGopConfig(char *fname, char **config, VCEncGopConfig *gopCfg,
...
int InitGopConfigs(int gopSize, commandLine_s *cml, VCEncGopConfig *gopCfg,
  ...
  u32 singleRefForP =
      (cml->lookaheadDepth && !bPass2) ||
      (cml->numRefP == 1);  //not enable multiref for P in pass-1

  char **rpsDefaultGop1 =
      singleRefForP ? RpsDefault_GOPSize_1 : Rps_2RefForP_GOPSize_1;
  if (IS_H264(cml->codecFormat))
    rpsDefaultGop1 =
        singleRefForP ? RpsDefault_H264_GOPSize_1 : Rps_2RefForP_H264_GOPSize_1;
  ...
  if (cml->lowdelayB &&
      ((cml->lookaheadDepth && bPass2) || cml->lookaheadDepth == 0))
  {
    default_configs[0] = RpsLowdelayB_GOPSize_1;
    default_configs[1] = RpsLowdelayB_GOPSize_2;
    default_configs[2] = RpsLowdelayB_GOPSize_3;
    default_configs[3] = RpsLowdelayB_GOPSize_4;
    default_configs[4] = RpsLowdelayB_GOPSize_5;
    default_configs[5] = RpsLowdelayB_GOPSize_6;
    default_configs[6] = RpsLowdelayB_GOPSize_7;
    default_configs[7] = RpsLowdelayB_GOPSize_8;
  }
  ...
  for (i = 1; i <= pre_load_num; i++) {
    if (ReadGopConfig(gopSize == i ? fname : NULL, default_configs[i - 1],
  ...
  // config RPS for leadig pictures
  if(cml->enableLeadingPictures) {
    gopCfg->gopCfgOffsetLP = gopCfg->size - 1; // gopPicIdx for leading pictures is 1-based
    ParseGopConfigString(Rps_lp, gopCfg, 0, gopSize);
    u32 idx = gopCfg->size - 1;
    gopCfg->pGopPicCfg[idx].poc = -1;
    gopCfg->pGopPicCfg[idx].refPics[0].ref_pic = 1;
    for(i = 2; i < gopSize; i++) {
      idx = gopCfg->size++;
      gopCfg->pGopPicCfg[idx] = gopCfg->pGopPicCfg[idx - 1];
      gopCfg->pGopPicCfg[idx].poc = -i;
      gopCfg->pGopPicCfg[idx].refPics[0].ref_pic = i;
    }
  }
  ...
  if ((DEFAULT != cml->ltrInterval) && (gopCfg->special_size == 0)) {
    if (cml->gopSize != 1) {
      printf(
          "GOP Config: Error, when using --LTR configure option, the gopsize "
          "alse should be set to 1!\n");
      return -1;
    }
  ...
  /* 6.0 software: merge */
  if (cml->hwBuild < 0x80006010 && IS_H264(cml->codecFormat) && gopCfg->ltrcnt > 0) {
...
u32 SetupInputBuffer(struct test_bench *tb, commandLine_s *cml,
  ...
  u32 width;
  bool isLumaOnly = cml->inputFormat == VCENC_Y8b ||
                    cml->inputFormat == VCENC_Y10bWL ||
                    cml->inputFormat == VCENC_Y10bWH ||
                    cml->inputFormat == VCENC_Y8b_TILE_8_8 ||
                    cml->inputFormat == VCENC_YUV400_PLANAR_10BIT_PACKED ||
                    cml->inputFormat == VCENC_Y10bWH_TILE_8_8 ;
  pEncIn->busLuma = tb->pictureMem->busAddress;
  tb->lum = (u8 *)tb->pictureMem->virtualAddress;

  if (isLumaOnly) {
    pEncIn->busChromaU = pEncIn->busChromaV = pEncIn->busLuma;
    tb->cb = tb->cr = tb->lum;

    if (cml->inputFormat == VCENC_Y8b ||
        cml->inputFormat == VCENC_Y10bWL ||
        cml->inputFormat == VCENC_Y10bWH )
      src_img_size = cml->lumWidthSrc * cml->lumHeightSrc *
                     VCEncGetBitsPerPixel(cml->inputFormat) / 8;
    else if (cml->inputFormat == VCENC_Y8b_TILE_8_8 ||
             cml->inputFormat == VCENC_Y10bWH_TILE_8_8)
      getAlignedPicSizebyFormat(cml->inputFormat, cml->lumWidthSrc,
                                cml->lumHeightSrc, 0, NULL, NULL,
                                &src_img_size, cml->scanType);
    else
      src_img_size = tb->lumaSize + tb->chromaSize;
  } else if ((tb->input_alignment == 0 || cml->formatCustomizedType != -1) &&
           0 != VCEncGetBitsPerPixel(cml->inputFormat)) {
    u64 size_lum = 0;
    u64 size_ch = 0;
    getAlignedPicSizebyFormat(cml->inputFormat, cml->lumWidthSrc,
                              cml->lumHeightSrc, 0, &size_lum, &size_ch, NULL, cml->scanType);
    pEncIn->busChromaU = pEncIn->busLuma + size_lum;
    tb->cb = tb->lum + (u32)size_lum;
    pEncIn->busChromaV = pEncIn->busChromaU + (u32)size_ch / 2;
    tb->cr = tb->cb + (u32)size_ch / 2;

    if (cml->scanType == VCENC_SUPERTILEX_SCAN)
      src_img_size = size_lum + size_ch;
    else
      src_img_size = cml->lumWidthSrc * cml->lumHeightSrc *
                     VCEncGetBitsPerPixel(cml->inputFormat) / 8;
  }
  ...
  if ((cml->interlacedFrame) &&
      ((pEncIn->picture_cnt & 1) == cml->fieldOrder)) {
    u32 luma_stride;
    u32 chroma_stride;
    u32 alignment;
    alignment = (tb->formatCustomizedType != -1 ? 0 : tb->input_alignment);
    VCEncGetAlignedStride(cml->lumWidthSrc, cml->inputFormat, &luma_stride,
                          &chroma_stride, alignment, cml->scanType);
    pEncIn->busLuma += luma_stride;
    pEncIn->busChromaU += chroma_stride;
    pEncIn->busChromaV += chroma_stride;
  }
  ...
  if (cml->halfDsInput) {
    ...
    getAlignedPicSizebyFormat(cml->inputFormat, cml->lumWidthSrc / 2,
                              cml->lumHeightSrc / 2, alignment, &size_lum,
                              &size_ch, NULL, cml->scanType);
    ...
    pEncIn->busLumaOrig = pEncIn->busChromaUOrig = pEncIn->busChromaVOrig =
        (ptr_t)NULL;
    tb->src_img_size_ds = 0;
  }
  if (!tb->picture_enc_cnt)
    printf("Reading input from file <%s>, frame size %llu bytes.\n", cml->input,
           (unsigned long long)src_img_size);
...
void SetupSliceInfoBuffer(struct test_bench *tb, VCEncIn *pEncIn) {
  pEncIn->SliceInfo_Base = tb->SliceInfoMem->busAddress;
}


/**
 * \page bw Bandwidth Saving
 *
 * To reduce consumed bandwidth, you can use the following methods:
 * - \subpage extsram
 * - \subpage dec400
 */

/**
 * \page extsram Using External SRAM to Reduce DDR Bandwidth
 *
 * Motion estimation consumes much bandwidth to repeatedly load reference data. In order to
 * eliminate reloads, each core can define their own external SRAM as a cache to store loaded
 * reference data.
 *
 * External SRAM is supported only if the value of <tt>EwlConfig.meExternSramSupport</tt> is <tt>1</tt>.
 */

/* Setup External SRAM */
void SetupExtSRAM(struct test_bench *tb, VCEncIn *pEncIn) {
  EWLLinearMem_t *pExtSRAMMem =
      &tb->extSRAMMemFactory[tb->picture_enc_cnt % tb->parallelCoreNum];
...
/**
 * \page dec400 Using DEC400 to Read Compressed Input Buffer
 *
 * In transcoding scenarios, the decoder output buffer serving as the input of the video encoder
 * can be compressed with the DEC400 encoder in the video decoder. The DEC400 encoder in the video
 * encoder can read and descrompress the compressed data to reduce bandwidth consumption.
 *
 * This method to reduce bandwidth consumption is available only if the subsystem includes the
 * DEC400 decoder.
 */
...
void SetupDec400CompTable(struct test_bench *tb, VCEncIn *pEncIn) {
  ...
  pEncIn->dec400CbTableBase =
      tb->Dec400CmpTableMem->busAddress + tb->dec400LumaTableSize;
  pEncIn->dec400CrTableBase =
      pEncIn->dec400CbTableBase + tb->dec400ChrTableSize / 2;
}

void SetupDec400CompTable_osd(struct test_bench *tb, commandLine_s *cml, VCEncIn *pEncIn) {
  for (u32 i = 0; i < MAX_OVERLAY_NUM; i++) {
    if(tb->osdDec400TableSize[i][0] != 0) {
      pEncIn->osdDec400TableBase[i][0] = tb->osdDec400TableMem[i]->busAddress;
      pEncIn->osdDec400TableBase[i][1] =
          tb->osdDec400TableMem[i]->busAddress + tb->osdDec400TableSize[i][0];
      pEncIn->osdDec400TableBase[i][2] =
          pEncIn->osdDec400TableBase[i][1] + tb->osdDec400TableSize[i][1];
    }
  }
}

void SetupROIMapVer3(struct test_bench *tb, commandLine_s *cml, VCEncIn *pEncIn,
                     VCEncInst encoder) {
  u8 *memory;
  u8 u8RoiData[24];
  ...
  i32 size = 0;
  i32 ret = NOK;
  i32 block_num, i, j, k, m, n, block_width, block_height, mb_width,
      last_mb_height;
  ...
  if ((tb->roiMapFile) && (cml->RoiQpDeltaVer < 3)) {
    ...
    if (cml->RoiQpDeltaVer == 4) {
      ...
      if (num <= tb->lastPic && roiBlkSize == 8) {
        ret = file_read(tb->roiMapInfoBinFile, (u8 *)memory,
                        (((u64)num) * ((u64)size)), size);
      } else {
        u8* temp_buf = (u8*)malloc(tb->roiMapDeltaQpMem->size * 4);
        if (temp_buf == NULL) {
          ret = NOK;
          printf("SetupROIMapVer3 Error: fail to alloc buffer!\n");
        } else {
          memset(temp_buf, 0, tb->roiMapDeltaQpMem->size * 4);
          ret = file_read(tb->roiMapInfoBinFile, (u8 *)temp_buf,
                          (((u64)num) * ((u64)size)), size);
          for (i = 0; i < block_height / 2; i++) {
            for (j = 0; j < block_width / 2; j++) {
              memory[i * block_width / 2 + j] = temp_buf[2 * j + i * block_width * 2];
            }
          }
          free(temp_buf);
        }
      ...
      if (num <= tb->lastPic) {
        switch (cml->roiMapDeltaQpBlockUnit) {
          case 0:
            block_num = 64 * 64 / roiBlkSize / roiBlkSize;
            break;
          case 1:
            block_num = 32 * 32 / roiBlkSize / roiBlkSize;
            break;
          case 2:
            block_num = 16 * 16 / roiBlkSize / roiBlkSize;
            break;
          case 3:
            block_num = 8 * 8 / roiBlkSize / roiBlkSize;
            break;
          default:
            block_num = 64 * 64 / roiBlkSize / roiBlkSize;
            break;
        ...
        mb_width =
            ((cml->width + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) /
            roiBlkSize;
        block_height = (((cml->height + cml->max_cu_size - 1) &
                         (~(cml->max_cu_size - 1))) +
                        blkSize - 1) /
                       blkSize;
        size = block_width * block_height;
        block_base_addr = ((u64)num) * ((u64)size);
        block_cu8_cnum = blkSize / roiBlkSize;
        last_block_cu8_cnum =
            (((cml->width + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) -
             (block_width - 1) * blkSize) /
            roiBlkSize;
        last_mb_height = (((cml->height + cml->max_cu_size - 1) &
                           (~(cml->max_cu_size - 1))) -
        ...
        if (IS_H264(cml->codecFormat)) {
          ...
          for (i = 0; i < size; i++) {
            ret = file_read(tb->roiMapInfoBinFile, u8RoiData,
                            (block_base_addr + i), 1);
            if (ret == NOK) break;
          ...
          cu8NumInCtb =
              ctb_block_num * ctb_block_num * block_cu8_cnum * block_cu8_cnum;
    ...
    if (ret == NOK)
      memset((u8 *)tb->roiMapDeltaQpMem->virtualAddress, 0,
             tb->roiMapDeltaQpMem->size);
  }
  ...
  if (roiMapEnable && (tb->roiMapConfigFile != NULL)) {
    ...
    i32 ret = fill_pic_roimap(encoder, tb->roiMapConfigFile, roimap_format,
                              roimapDeltaQPmemory, blkSize, roiBlkSize);
    if (ret == NOK) memset(roimapDeltaQPmemory, 0, roimap_Qp_or_CU_Mem_size);
  ...
  if (tb->RoimapCuCtrlIndexBinFile != NULL) {
    ...
    num = next_picture(tb, tb->encIn.picture_cnt) + tb->firstPic;
    size = (((cml->width + blkSize - 1) / blkSize) *
                ((cml->height + blkSize - 1) / blkSize) +
            7) >>
           3;
    ret = NOK;
    if (num <= tb->lastPic) {
  ...
  if (tb->RoimapCuCtrlInfoBinFile != NULL) {
    ...
    if (num <= tb->lastPic && roiBlkSize == 8) {
      ...
      if (ret == NOK) {
      memset((u8 *)tb->RoimapCuCtrlInfoMem->virtualAddress, 0,
             tb->RoimapCuCtrlInfoMem->size);
      EWLSyncMemData(tb->RoimapCuCtrlInfoMem, 0, size, HOST_TO_DEVICE);
      }
    } else if (num <= tb->lastPic && roiBlkSize == 16) {
      u8* temp_buf = (u8*)malloc(block_height * block_width * block_num);
      if (temp_buf == NULL) {
          ret = NOK;
          printf("SetupROIMapVer3 Error: fail to alloc buffer!\n");
      } else {
        memset(temp_buf, 0, block_height * block_width * block_num);
        for (i = 0; i < size; i += read_size) {
          if ((size - i) > 1024)
            read_size = 1024;
          else
          ...
          ret = file_read(tb->RoimapCuCtrlInfoBinFile, (u8 *)(temp_buf + i),
                          (((u64)num) * ((u64)size) + i), read_size);
          if (ret == NOK) break;
        }
        for (i = 0; i < block_height * 8 / cml->max_cu_size; i++)
          for (j = 0; j < block_width * 8 / cml->max_cu_size; j++)
            for (k = 0; k < cml->max_cu_size / 16; k++)
              for (m = 0; m < cml->max_cu_size / 16; m++)
                for (n = 0; n < block_num; n++) {
                  memory[i * block_width * 8 / cml->max_cu_size * block_num * (cml->max_cu_size / 16 * cml->max_cu_size / 16) +
                        j * block_num * (cml->max_cu_size / 16 * cml->max_cu_size / 16) + k * block_num * cml->max_cu_size / 16 + m * block_num + n]
                        = temp_buf[i * block_width * 8 / cml->max_cu_size * block_num * (cml->max_cu_size / 8 * cml->max_cu_size / 8) + j * block_num *
                          (cml->max_cu_size / 8 * cml->max_cu_size / 8) + k * block_num * cml->max_cu_size / 8 * 2 + m * block_num * 2 + n];
                }
        EWLSyncMemData(tb->RoimapCuCtrlInfoMem, 0, size / 4, HOST_TO_DEVICE);
        free(temp_buf);
        if (ret == NOK) {
          memset((u8 *)tb->RoimapCuCtrlInfoMem->virtualAddress, 0,
                tb->RoimapCuCtrlInfoMem->size);
...
void GenerateRoiVer4InputQpDelta(commandLine_s *cml, u8 *pRoiMapDelta,
                                 VCEncInst encoder) {
  // #ifdef INTERNAL_TEST
  if (!pRoiMapDelta || !cml) return;
  ...
  for (y = 0; y < roiBlock_height; y++) {
    for (x = 0; x < roiBlock_width; x++) {
      ...
      pRoiMapDelta[y * roiBlock_width + x] =
          pRoiMapDelta[yy * block_width + xx];
    }
  }
  // #else
...
void SetupROIMapBuffer(struct test_bench *tb, commandLine_s *cml,
  ...
  if ((cml->lookaheadDepth) && tb->roiMapBinFile) {
    ...
    if (cml->RoiQpDeltaVer == 4) {
      /* when RoiQpDeltaVer==4, the size of info in the cfg file is 8x8 block or 16x16 block, and the number should be whole CTU's */
      blkSize = tb->hwCfg->roiBlkSize ? 16 : 8; // 8 or 16
      block_width =
          (((cml->width + cml->max_cu_size - 1) & (~(cml->max_cu_size - 1))) +
           blkSize - 1) /
          blkSize;
    ...
    if (num <= tb->lastPic)
      ret = file_read(tb->roiMapBinFile, (u8 *)pEncIn->pRoiMapDelta,
                      ((u64)num) * ((u64)size), size);
    ...
    if (ret != OK) memset(pEncIn->pRoiMapDelta, 0, size);
  }
  ...
  true_e notRdoqVerFlag =
      !((tb->hwCfg->roiMapVersion == 4) && (cml->RoiQpDeltaVer == 4));
  ...
  true_e skipFileValid =
      cml->skipMapEnable && tb->skipMapFile && notRdoqVerFlag;
...
page LoadUserMV MV Binary File Input
...
The MVs are grouped with 32x32 blocks in raster scan order. if the picture size is not aligned with 32, the block number should be calculated by the size after alignment.
...
<table>
 <caption id='tab_MVOrder'>The Order of the 2 MVs</caption>
 <tr>
	<th align="center"> Description
	<th align="center"> Type
	<th align="center"> Bits
 <tr>
    <td align="center"> L0 Horizontal MV
    <td align="center"> i16
    <td align="center"> 16
 <tr>
    <td align="center"> L0 Vertical MV
    <td align="center"> int16
    <td align="center"> 16
 <tr>
    <td align="center"> L1 Horizontal MV
    <td align="center"> int16
    <td align="center"> 16
 <tr>
    <td align="center"> L1 Vertical MV
    <td align="center"> int16
    <td align="center"> 16
</table>
*/
...
void SetupMvReplaceBuffer(struct test_bench *tb, commandLine_s *cml,
  ...
  u32 frameSize = 2 * 2 * 2 * 21 * (STRIDE(cml->lumWidthSrc, 32) >> 5) *
                  (STRIDE(cml->lumHeightSrc, 32) >> 5);
  ...
  if (file_read(tb->replaceMvFile, (u8 *)tb->mvReplaceBuffer.virtualAddress,
                seek, frameSize)) {
...
void SetupOverlayBuffer(struct test_bench *tb, commandLine_s *cml,
  ...
  for (i = 0; i < MAX_OVERLAY_NUM; i++) {
	...
	(tb->heif_inst->heif_config->grid_mode >= HEIF_GRID_INPUT_PIC))
	  pEncIn->overlayEnable[i] = tb->heif_inst->heif_config->OSD_config.overlayArea[i].enable;
	else
...
#ifdef HEIF_SUPPORT
	  static int grid_num[MAX_OVERLAY_NUM] = {0};
	  if ((tb->heif_enable == false)
		|| (tb->heif_inst->heif_config->grid_mode < HEIF_GRID_INPUT_PIC)) {
	  	grid_num[i] = next_picture(tb, tb->encIn.picture_cnt % tb->olFrameNume[i]);
	  } else if (tb->heif_inst->input_numbers == 0){  //Only support one picture for gird mode 2 & 3
	  	grid_num[i] = next_picture(tb, tb->encIn.picture_cnt % tb->olFrameNume[i]);
	 }
	 num = grid_num[i];
#else
	 num = next_picture(tb, tb->encIn.picture_cnt % tb->olFrameNume[i]);
#endif
      ...
      switch (cml->olFormat[i]) {
        case 0:  //ARGB
          ...
          if (tb->osdDec400TableMem[i]) {
            seek = num * (cml->olHeight[i] * cml->olYStride[i]);
            if (file_read(tb->overlayFile[i], lum, seek, cml->olHeight[i] * cml->olYStride[i])) {
        ...
        case 1:  //NV12
          ...
          pEncIn->overlayInputUAddr[i] =
              pEncIn->overlayInputYAddr[i] + size_luma;  //Cb and Cr
          cb = lum + size_luma;
          if (tb->osdDec400TableMem[i]) {
            seek = num * (cml->olHeight[i] * cml->olYStride[i] + cml->olYStride[i] * cml->olHeight[i] / 2);
            if (file_read(tb->overlayFile[i], lum, seek, cml->olHeight[i] * cml->olYStride[i] +
              cml->olYStride[i] * cml->olHeight[i] / 2)) {
              printf("Error: fail to read overlay\n");
              pEncIn->overlayEnable[i] = 0;
              break;
            }
  ...
  for (i = 0; i < MAX_OVERLAY_NUM; i++) {
    ...
    if (tb->osdDec400TableMem[i]) {
      ...
      dec400TotalTableSize = tb->osdDec400TableSize[i][0] + tb->osdDec400TableSize[i][1] +
        tb->osdDec400TableSize[i][2];
      seek = ((u64)num) * ((u64)dec400TotalTableSize);
      pEncIn->osdDec400TableBase[i][0] = tb->osdDec400TableMem[i]->busAddress;
      lum = (u8 *)tb->osdDec400TableMem[i]->virtualAddress;

      if (file_read(tb->osdDec400TableFile[i], lum, seek, dec400TotalTableSize)) {
        printf("Error: fail to read osd dec400 table file\n");
        pEncIn->osdDec400Enable[i] = 0;
      }
      EWLSyncMemData(tb->osdDec400TableMem[i], 0, dec400TotalTableSize,
                    HOST_TO_DEVICE);
    } else {
  ...
  if (cml->osdMapEnable) {
    pEncIn->osdMapEnable = cml->osdMapEnable;
    u32 read_height = GetOsdmapReadHeight(cml, tb);
    int num = next_picture(tb, tb->encIn.picture_cnt % tb->osdMapFrameNum);
    u32 osdMapSize = 0;

    pEncIn->osdMapInputAddr = tb->osdMapMem->busAddress;
    lum = (u8 *)tb->osdMapMem->virtualAddress;
    src_width = cml->osdMapStride;//unit: byte; 4bits per 8x8 or 4x4 block.
    seek = num * src_width * read_height;
    for (j = 0; j < read_height; j++) {
      if (file_read(tb->osdMapFile, lum, seek, src_width)) {
    ...
    EWLSyncMemData(tb->osdMapMem, 0, osdMapSize, HOST_TO_DEVICE);
  }
}
...
i32 FindInputPicBufIdByBusAddr(struct test_bench *tb, const ptr_t busAddr,
  ...
  if (bTrans) {
    bufferIndex = FindInputBufferIdByBusAddr(tb->transformMemFactory,
                                             tb->buffer_cnt, busAddr);
  } else {
    bufferIndex = FindInputBufferIdByBusAddr(tb->pictureMemFactory,
                                             tb->buffer_cnt, busAddr);
  }
...
/*------------------------------------------------------------------------------
    ...
    Argument        : const ptr_t busAddr                             [in]   buffer bus address
------------------------------------------------------------------------------*/
i32 FindOverlayInputBufIdByBusAddr(EWLLinearMem_t memFactory[][MAX_OVERLAY_NUM],
                                   const u32 MemNum, const ptr_t busAddr,
                                   i32 ibuf) {
  i32 bufferIndex = -1;
  if (ibuf < 0 || ibuf >= MAX_OVERLAY_NUM) return bufferIndex;
...
i32 ReturnBufferById(u32 *memsRefFlag, const u32 MemNum, i32 bufferIndex) {
  if (bufferIndex < MemNum && bufferIndex >= 0) {
    if (memsRefFlag[bufferIndex] > 0) memsRefFlag[bufferIndex]--;
    return OK;
  } else
    return NOK;
}
...
i32 ReturnIOBuffer(struct test_bench *tb, commandLine_s *cml,
  ...
  if (0 != consumedAddrs->inputbufBusAddr) {
    ...
    if ((cml->interlacedFrame) &&
        ((picture_cnt & 1) == cml->fieldOrder)) {  //interlace
      u32 luma_stride;
      u32 chroma_stride;
      u32 alignment;
      alignment = (tb->formatCustomizedType != -1 ? 0 : tb->input_alignment);
      VCEncGetAlignedStride(cml->lumWidthSrc, cml->inputFormat, &luma_stride,
                            &chroma_stride, alignment, cml->scanType);
      consumedAddrs->inputbufBusAddr -= luma_stride;
    }

    inputPicBufIndex = FindInputPicBufIdByBusAddr(
        tb, consumedAddrs->inputbufBusAddr, cml->formatCustomizedType != -1);

    if (NOK ==
        ReturnBufferById(tb->inputMemFlags, tb->buffer_cnt, inputPicBufIndex))
      iRet = NOK;
  }

  //put output buffer back to pool
  if (0 != consumedAddrs->outbufBusAddr) {
    i32 outputBufferIndex = FindOutputBufferIdByBusAddr(
        tb->outbufMemFactory, tb->outbuf_cnt, consumedAddrs->outbufBusAddr);
    if (NOK ==
        ReturnBufferById(tb->outputMemFlags, tb->outbuf_cnt, outputBufferIndex))
      iRet = NOK;
  }
  ...
  if (0 != consumedAddrs->roiMapDeltaQpBusAddr) {
    i32 inputInfoBufId = FindInputBufferIdByBusAddr(
        tb->roiMapDeltaQpMemFactory, tb->inputInfoBuf_cnt,
        consumedAddrs->roiMapDeltaQpBusAddr);
    if (NOK == ReturnBufferById(tb->roiMapDeltaQpMemFlags, tb->inputInfoBuf_cnt,
                                inputInfoBufId))
      iRet = NOK;
  }
  ...
  if (0 != consumedAddrs->roimapCuCtrlInfoBusAddr) {
    i32 inputInfoBufId = FindInputBufferIdByBusAddr(
        tb->RoimapCuCtrlInfoMemFactory, tb->inputInfoBuf_cnt,
        consumedAddrs->roimapCuCtrlInfoBusAddr);
    if (NOK == ReturnBufferById(tb->roiMapCuCtrlInfoMemFlags,
                                tb->inputInfoBuf_cnt, inputInfoBufId))
      iRet = NOK;
  }
  ...
  for (i32 ibuf = 0; ibuf < MAX_OVERLAY_NUM; ibuf++) {
    if (0 != consumedAddrs->overlayInputBusAddr[ibuf]) {
      i32 inputInfoBufId = FindOverlayInputBufIdByBusAddr(
          tb->overlayInputMemFactory, tb->inputInfoBuf_cnt,
          consumedAddrs->overlayInputBusAddr[ibuf], ibuf);
      if (NOK == ReturnBufferById(tb->overlayInputFlags, tb->inputInfoBuf_cnt,
                                  inputInfoBufId))
        iRet = NOK;
      break;
    }
  ...
  //put osdMap buffer back to pool
  if (0 != consumedAddrs->osdMapInputBusAddr) {
    i32 inputInfoBufId = FindInputBufferIdByBusAddr(
        tb->osdMapMemFactory, tb->inputInfoBuf_cnt,
        consumedAddrs->osdMapInputBusAddr);
    if (NOK == ReturnBufferById(tb->osdMapInputFlags, tb->inputInfoBuf_cnt,
                                inputInfoBufId))
      iRet = NOK;
  }
...
i32 ReuseInputBuffer(struct test_bench *tb, ptr_t busAddr) {
  ...
  inputBufferIndex = FindInputBufferIdByBusAddr(tb->pictureMemFactory,
                                                tb->buffer_cnt, busAddr);
  if (inputBufferIndex >= tb->buffer_cnt) return NOK;
...
i32 GetFreeInputPicBuffer(struct test_bench *tb) {
  ...
  if (inputBufferIndex < 0 || inputBufferIndex >= tb->buffer_cnt) return NOK;

  //find input buffer of multi-cores
  tb->pictureMem = &(tb->pictureMemFactory[inputBufferIndex %
                                           ARRAY_SIZE(tb->pictureMemFactory)]);
  tb->pictureDSMem =
      &(tb->pictureDSMemFactory[inputBufferIndex %
                                ARRAY_SIZE(tb->pictureDSMemFactory)]);

  /* find slice info  buffer of multi-core */
#ifdef LOW_LATENCY_SLICEINFO_SUPPORT
  tb->SliceInfoMem = &(tb->SliceInfoMemFactory[inputBufferIndex %
                        ARRAY_SIZE(tb->pictureDSMemFactory)]);
#endif

  //find dec400table buffer of multi-cores/multi-delay-frames
  if (tb->dec400Table || tb->dec400TSHeaderEnable)
    tb->Dec400CmpTableMem = &(
        tb->Dec400CmpTableMemFactory[inputBufferIndex %
                                     ARRAY_SIZE(tb->Dec400CmpTableMemFactory)]);
  else
    tb->Dec400CmpTableMem = NULL;

  //find transform buffer of multi-cores
  tb->transformMem =
      &(tb->transformMemFactory[inputBufferIndex %
                                ARRAY_SIZE(tb->transformMemFactory)]);
...
i32 GetFreeInputInfoBuffer(struct test_bench *tb) {
  ...
  inputInfoIndex =
      GetFreeBufferId(tb->roiMapDeltaQpMemFlags, tb->inputInfoBuf_cnt);
  if (inputInfoIndex < 0 || inputInfoIndex >= tb->inputInfoBuf_cnt) return NOK;
  ...
  if (!tb->roiMapDeltaQpMemFactory[inputInfoIndex].busAddress)
    ReturnBufferById(tb->roiMapDeltaQpMemFlags, tb->inputInfoBuf_cnt,
                     inputInfoIndex);

  inputInfoIndex = GetFreeBufferId(
      tb->roiMapCuCtrlInfoMemFlags,
      MIN(tb->inputInfoBuf_cnt, ARRAY_SIZE(tb->roiMapCuCtrlInfoMemFlags)));
  if (inputInfoIndex < 0 || inputInfoIndex >= tb->inputInfoBuf_cnt) return NOK;
  ...
  if (!tb->RoimapCuCtrlInfoMemFactory[inputInfoIndex].busAddress)
    ReturnBufferById(tb->roiMapCuCtrlInfoMemFlags, tb->inputInfoBuf_cnt,
                     inputInfoIndex);
  ...
  inputInfoIndex = GetFreeBufferId(
      tb->overlayInputFlags,
      MIN(tb->inputInfoBuf_cnt, ARRAY_SIZE(tb->overlayInputFlags)));
  if (inputInfoIndex < 0 || inputInfoIndex >= tb->inputInfoBuf_cnt) return NOK;
  i8 bOverlayEnable = HANTRO_FALSE;
  for (iBuf = 0; iBuf < MAX_OVERLAY_NUM; iBuf++) {
    if (tb->olEnable[iBuf]) {
      tb->overlayInputMem[iBuf] =
          &(tb->overlayInputMemFactory[inputInfoIndex][iBuf]);
#ifdef HEIF_SUPPORT
      if ((tb->heif_enable) &&
  	     (tb->heif_inst->heif_config->grid_mode >= HEIF_GRID_INPUT_PIC)) {
	        if (tb->heif_inst->heif_config->OSD_config.overlayArea[iBuf].enable) {
  ...
  if (bOverlayEnable) {
    for (iBuf = 0; iBuf < MAX_OVERLAY_NUM; iBuf++) {
      if (tb->osdDec400TableFile[iBuf]) {
        tb->osdDec400TableMem[iBuf] =
            &(tb->osdDec400CmpTableMemFactory[inputInfoIndex][iBuf]);
        bOverlayEnable = HANTRO_TRUE;
      } else {
        tb->osdDec400TableMem[iBuf] = NULL;
      }
    }
  }
  //if bOverlay is not enable, reset the flag.
  else
    ReturnBufferById(tb->overlayInputFlags, tb->inputInfoBuf_cnt,
                     inputInfoIndex);

  //find osdMap buffer of multi-cores
  inputInfoIndex =
      GetFreeBufferId(tb->osdMapInputFlags, tb->inputInfoBuf_cnt);
  if (inputInfoIndex < 0 || inputInfoIndex >= tb->inputInfoBuf_cnt) return NOK;
  tb->osdMapMem = &(tb->osdMapMemFactory[inputInfoIndex]);
  //if the busAddress is 0,reset the flag.
  if (!tb->osdMapMemFactory[inputInfoIndex].busAddress)
    ReturnBufferById(tb->osdMapInputFlags, tb->inputInfoBuf_cnt,
                     inputInfoIndex);
...
i32 GetFreeOutputBuffer(struct test_bench *tb) {
  ...
  if (outputBufferIndex < 0 || outputBufferIndex >= tb->outbuf_cnt) return NOK;

  //load core level output buffer address
  for (tileId = 0; tileId < tb->tileColumnNum; tileId++) {
    for (iBuf = 0; iBuf < tb->streamBufNum; iBuf++) {
      tb->outbufMem[tileId][iBuf] =
          &(tb->outbufMemFactory[outputBufferIndex][tileId][iBuf]);
    }
  }
  return OK;
}
...
void InitSliceCtl(struct test_bench *tb, struct commandLine_s *cml) {
  i32 ctbSize = (cml->codecFormat==VCENC_VIDEO_CODEC_H264)?16:64;
  i32 sliceSize = cml->sliceSize;
  tb->sliceCtl.multislice_encoding =
      (sliceSize != 0 && (((cml->height + ctbSize -1) / ctbSize) > sliceSize)) ? 1 : 0;
  tb->sliceCtl.output_byte_stream = cml->byteStream ? 1 : 0;
  tb->sliceCtl.outStreamFile = tb->out;
  tb->sliceCtl.streamPos = 0;
  tb->sliceCtl.enable_slice_irq = cml->sliceIrqEnable;
}
...
void InitStreamSegmentCrl(struct test_bench *tb, struct commandLine_s *cml) {
  ...
  tb->streamSegCtl.streamBase =
      (u8 *)tb->outbufMemFactory[0][0][0].virtualAddress;
  ...
  if (tb->streamSegCtl.streamMultiSegEn) {
    ...
    tb->streamSegCtl.segmentSize =
        ((tb->streamSegCtl.segmentSize + 16 - 1) &
...
u64 uTimeDiffLL(struct timeval end, struct timeval start) {
  return (u64)(end.tv_sec - start.tv_sec) * 1000000 +
         (u64)(end.tv_usec - start.tv_usec);
}

/*
 * Read external sei data from file stream and pass to encoder;
 *
 * \see extern_sei for detail.
 *
...
u8 ReadExternalSeiData(ExternalSEI **pExternalSEI, FILE *file) {
  ...
  *pExternalSEI = NULL;
  ...
  size_t ret = fread(&seiCnt, sizeof(u32), 1, file);
  if (ret < 1) return 0;
  if (seiCnt > 65535) return 0;

  *pExternalSEI = (ExternalSEI *)malloc(sizeof(ExternalSEI) * seiCnt);
  //memset(pExternalSEI, 0, sizeof(ExternalSEI) * seiCnt);
  if (*pExternalSEI == NULL) return 0;
  for (int k = 0; k < seiCnt; k++) {
    ...
    ret = fread((*pExternalSEI)[k].pPayloadData, sizeof(u8),
                (*pExternalSEI)[k].payloadDataSize, file);
  }

  printf("ExternalSEI count: %u \n", seiCnt);

  return seiCnt;
}
void *InitVaDev(const commandLine_s *cml) {
#ifdef USE_LIBVA
  int fd0 = drmOpen(cml->vaDriverName, NULL);
  ASSERT_GE(fd0, 0);
  drm_hantro_bufmgr *bufmgr = drm_hantro_bufmgr_open(fd0);
  ASSERT_NE(0u, (__u64)bufmgr);
  EWLAttach((void *)bufmgr, cml->sliceNode, 0);
  return bufmgr;
#else
  return NULL;
#endif
...
void ReleaseVaDev(void *bufmgr)
{
  ...
  EWLDetach();
  if(bufmgr)
    drm_hantro_bufmgr_destroy((drm_hantro_bufmgr *)bufmgr);
#endif
}
...
const void *InitEwlInst(const commandLine_s *cml, void *bufmgr) {
  ...
  param.context = bufmgr;
  param.enc_dev = cml->encDevice;
  param.mem_dev = cml->memDevice;
  param.clientType = EWL_CLIENT_TYPE_MEM;  //buffer operation
  param.useVcmd = cml->useVcmd;
...
#include <sys/types.h>
#include <unistd.h>
int process_object_detection(struct commandLine_s *cml, struct test_bench *tb)
{
  int pipefds_input[2], pipefds_output[2], pipefds_error[2];
  int status;
  pid_t pid;
  char *input = "/tmp/vce_frame";
  char cmd[128];
  ...
  u32 luma_stride;
  u32 chroma_stride;
  int alignment = (tb->formatCustomizedType != -1 ? 0 : tb->input_alignment);
  VCEncGetAlignedStride(cml->lumWidthSrc, cml->inputFormat, &luma_stride, &chroma_stride,
                        alignment, cml->scanType);

  if (!(tb->formatCustomizedType == 0 && cml->inputFormat == VCENC_YUV420_PLANAR))
  ...
  u32 src_width = cml->lumWidthSrc;
  u32 src_height = cml->lumHeightSrc;
  for (i = 0; i < src_height; i++) {
    if (fwrite(lum, 1, src_width, fp) < src_width) {
		fclose(fp);
		return NOK;
    }
    lum += luma_stride;
  ...
  u32 w = ((src_width + 1) >> 1);
  u32 h = ((src_height + 1) >> 1);
  /* Cb */
  for (i = 0; i < h; i++) {
    if (fwrite(cb, 1, w, fp) < w) {
	  fclose(fp);
	  return NOK;
    }
    cb += chroma_stride;
  ...
  fclose(fp);
  sprintf(cmd, "python3 obj_detect_frame.py -W %d -H %d -i %d -p %s -t %u %s",
      cml->lumWidthSrc, cml->lumHeightSrc, tb->picture_enc_cnt, input, cml->objDetect-1, cml->objPlot?"-p":"");
  if(system(cmd) < 0) {
      perror("Error exec");
      return -1;
  }
