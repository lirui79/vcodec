                        ...
                        while(pStr < pStrLimit && *pStr)
                        {
                            while(pStr < pStrLimit && *pStr != ';')
                            {
                                ...
                                if (pStr + ulPNMStrLen < pStrLimit && *pStr == '$')
                                {
                                    ...
                                    if (!strncmp(pszPNMStr, pStr, ulPNMStrLen))
...
/* default: packet-by-packet & callback */
...
HX_RESULT rv_depacki_send_current_frame(rv_depack_internal* pInt)
{
    ...
    if (pInt && pInt->fpAvail)
    {
        ...
        if (pInt->pCurFrame)
        {
            ...
            if (bDoSend)
            {
				/* Set the ucCodType of current frame */
				pInt->pCurFrame->ucCodType = rv_depacki_get_pictype(pInt, pInt->pCurFrame);
				/* update KeyFrame Table -caijin @2008.06.01*/
				if (RV_IS_KEY_FRAME(pInt->pCurFrame->ucCodType))
				{
					/*
					* Is a key frame or not?
					* It should be same as the result from (pInt->pCurFrame->usFlags & HX_KEYFRAME_FLAG)
					*/
					if (!(pInt->pCurFrame->usFlags & HX_KEYFRAME_FLAG))
					{
						printf("Warning: something seems wrong! is it keyframe or not?!!\n");
					}
					pInt->pCurFrame->bIsKeyframe = 1;
					pInt->pKeyFrameTable[pInt->ulKeyFrameTableOffset] =	pInt->pCurFrame->ulTimestamp;
					pInt->ulKeyFrameTableOffset++;
					pInt->ulKeyFrameTableCount++;
					if (pInt->ulKeyFrameTableOffset > KEY_FRAME_TIMESTAMP_TABLE_COUNT)
						printf("\n\npKeyFrameTable length overflow!!!!!!!\n\n");
				}
				else
				{
					pInt->pCurFrame->bIsKeyframe = 0;
				}
                              ...
                              rv_frame**          ppFrame)
{
    if (pInt && ppFrame && *ppFrame)
    {
        ...
        if ((*ppFrame)->pData)
        {
			if (pInt->fpFree_ncnb)
			{
				/* NCNB mode -caijin @2008.06.04 */
				//HAL_FREE_MEM((*ppFrame)->pData);
				rv_depacki_destroy_ncnb(pInt, &(*ppFrame)->pStream);
			}
			else
			{
                                  ...
                                  rv_frame**          ppFrame)
{
        ...
        pFrameHdr->ulFrameSize && pFrameHdr->ulNumPackets)
    {
        ...
        if (pFrame)
        {
			...
			if (pInt->fpMalloc_ncnb)
			{
				/* NCNB mode -caijin @2008.06.04 */
				//pFrame->pData = HAL_MALLOC_MEM(pFrameHdr->ulFrameSize);
				pFrame->pStream = (rm_linear_memory *)rv_depacki_create_ncnb(pInt, pFrameHdr->ulFrameSize);
				if (pFrame->pStream == NULL)
					return  HXR_FAIL;
				pFrame->pData = (BYTE *)pFrame->pStream->virtualAddress;
				pFrame->ulDataBusAddress = pFrame->pStream->busAddress;
			}
			else
			{
				pFrame->pData = (BYTE *)rv_depacki_malloc(pInt, pFrameHdr->ulFrameSize);
				pFrame->ulDataBusAddress = (G1_ADDR_T)pFrame->pData;
			}
            ...
            if (pFrame->pData)
            {
				...
				/* Allocate space for segment array LMX modified*/
                ulSize = (pFrameHdr->ulNumPackets+1) * sizeof(rv_segment);
                ...
                if (pFrame->pSegment)
                {
                    ...
                    pFrame->bLastPacket   = FALSE;	/* when it be true? how to use it? -caijin */
                     ...
                     */
					/*
					 * Comment: It seems that there will be never more than one keyframe
					 * encoded in a single packet. right ? -caijin
					 */
					...
					/* Assign the bIsKeyframe field. -caijin */
					pFrame->bIsKeyframe = ulFlags & HX_KEYFRAME_FLAG;
					/* Assign the ucCodType field. -caijin */
					pFrame->ucCodType = 0xFF;
                    ...
                    else
                    {
                         ...
                         * number in the RV frame may not be 0.
                         */
...
void* rv_depacki_create_ncnb(rv_depack_internal* pInt, UINT32 ulSize)
{
	...
	if (pInt && pInt->fpMalloc_ncnb)
	{
		pRet = pInt->fpMalloc_ncnb(pInt->pUserMem, ulSize);
	}
...
{
	if (pInt && pInt->fpFree_ncnb)
	{
		pInt->fpFree_ncnb(pInt->pUserMem, pMem);
	}
}
										...
										UINT32				ulDepackMode)
{
	...
	if (pInt && ppBuf && pulLen && pPacket && pFrameHdr && *ppBuf)
	{
		...
		*/
		if (pInt->pCurFrame &&
			pInt->pCurFrame->ulTimestamp != pFrameHdr->ulTimestamp)
		{
			/* Send the current frame */
			retVal = rv_depacki_send_current_frame_ex(pInt, ulDepackMode);
		}
		/* Do we have a current frame? */
		if (retVal == HXR_OK && !pInt->pCurFrame)
		{
	...
	if (pInt)
	{
		...
		if (pInt->pCurFrame)
		{
			...
			if (bDoSend)
			{
				...
				if (RM_DEPACK_IS_CALLBACK(ulDepackMode))
				{
					/* Call the frame available function pointer */
					if (pInt->fpAvail)
					{
					...
					{
						printf("Warning: no fpAvail, pCurFrame %u discarded directly!!\n",
							pInt->pCurFrame->ulTimestamp);
					}
				}
				else if (RM_DEPACK_IS_FIFO(ulDepackMode))
				{
					/* Send the block to frame fifo*/
					retVal = rv_depack_attach_frame(pInt, pInt->pCurFrame);
					if (retVal)
					{
						/* Free the frame */
						rv_depacki_cleanup_frame(pInt, &pInt->pCurFrame);
						printf("rv_depack_attach_frame error!\n");
					}
				}
				else if (RM_DEPACK_IS_SWAP(ulDepackMode))
				{
					if (pInt->pReadyFrame == NULL)
					{
						pInt->pReadyFrame = pInt->pCurFrame;
						retVal = HXR_OK;
					}
					else
					{
						retVal = HXR_FAIL;
						printf("The last frame has not been processed!\n");
					}
				}
			}
			else
								 ...
								 rv_frame**          ppFrame)
{
	...
	{
		/* Free the encoded frame buffer */
		if (!(*ppFrame)->bIsExternalBuf && (*ppFrame)->pData)
		{
			if (pInt->fpFree_ncnb)
			{
				/* NCNB mode -caijin @2008.06.04 */
									 ...
									 rv_frame**          ppFrame)
{
		...
		pFrameHdr->ulFrameSize && pFrameHdr->ulNumPackets)
	{
		...
		if (pFrame)
		{
			/* NULL out the memory */
			memset(pFrame, 0, sizeof(rv_frame));
			/* the stream buffer is allocated externally or internally? */
			if (pInt->pFrameBufAddr && (pFrameHdr->ulFrameSize <= pInt->ulFrameBufLen))
			{
				pFrame->bIsExternalBuf = TRUE;
				pFrame->pData = (BYTE *)pInt->pFrameBufAddr;
				pFrame->ulDataBusAddress = (G1_ADDR_T)pFrame->pData;
			}
			else
			{
				pFrame->bIsExternalBuf = FALSE;
				/* Allocate enough space for the data */
				if (pInt->fpMalloc_ncnb)
				{
					/* NCNB mode -caijin @2008.06.04 */
...
/*
 * return : pictype(0,1,2,3) or 0xFF (fail)
 */
UINT8 rv_depacki_get_pictype(rv_depack_internal *pInt, rv_frame *pFrame)
{
	UINT8 retVal = 0xFF;

	if (pInt && pFrame && pFrame->pData)
	{
		BYTE ch = *(pFrame->pData);
		/* To determine pictype is different for Rv8 and Rv9 */
		/* Rv8:3bit,__2bit__(pictype) */
		/* Rv9:1bit,__2bit__(pictype) */
		if (pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == HX_RV30VIDEO_ID)
		{
			ch >>= 3;
		}
		else if(pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == HX_RV40VIDEO_ID ||
			pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == HX_RV89COMBO_ID)
		{
			ch >>= 5;
		}
		else
...
HX_RESULT rv_depacki_set_frame_buffer(rv_depack_internal *pInt, BYTE *bufAddr, UINT32 bufLen)
{
	...
	if (pInt && bufAddr)
	{
		pInt->pFrameBufAddr = bufAddr;
		pInt->ulFrameBufLen = bufLen;
		retVal = HXR_OK;
	}
...
/*
* read-frame(s) implemented in rm_parser
* because we need more informations than rv_depack & rm_packet
*/
HX_RESULT rv_depacki_read_oneframe(rv_depack_internal * pInt, rm_packet * pPacket, rv_frame * pFrame)
{
	HX_RESULT retVal = HXR_FAIL;
	if (pInt && pPacket && pFrame)
	{
		retVal = HXR_OK;
	}
	return retVal;
...
/* rv_depacki_add_packet: packet-by-packet & callback */
static HX_RESULT rv_depacki_add_packet_packetbypacket_callback(rv_depack_internal* pInt,
															   rm_packet*          pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
			...
			!pInt->bIgnoreSubStream[ulSubStream])
		{
			...
			while (ulLen && retVal == HXR_OK)
			{
				...
				if (retVal == HXR_OK)
				{
					...
					case RVFrameTypePartial:
					case RVFrameTypeLastPartial:
						retVal = rv_depacki_handle_partial_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_CALL);
						break;
					case RVFrameTypeMultiple:
					case RVFrameTypeWhole:
						retVal = rv_depacki_handle_one_frame_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_CALL);
						break;
					}
				}
			}
			/* Assume all data of the current packet were processed over. */
			pPacket->bIsValid = FALSE;
		}
	}
...
/* rv_depacki_add_packet: packet-by-packet & fifo */
static HX_RESULT rv_depacki_add_packet_packetbypacket_fifo(rv_depack_internal* pInt,
														   rm_packet*          pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
			...
			!pInt->bIgnoreSubStream[ulSubStream])
		{
			...
			while (ulLen && retVal == HXR_OK)
			{
				...
				if (retVal == HXR_OK)
				{
					...
					case RVFrameTypePartial:
					case RVFrameTypeLastPartial:
						//retVal = rv_depacki_handle_partial_fifomode(pInt, &pBuf, &ulLen, pPacket, &hdr);
						retVal = rv_depacki_handle_partial_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_FIFO);
						break;
					case RVFrameTypeMultiple:
					case RVFrameTypeWhole:
						//retVal = rv_depacki_handle_one_frame_fifomode(pInt, &pBuf, &ulLen, pPacket, &hdr);
						retVal = rv_depacki_handle_one_frame_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_FIFO);
						break;
					}
				}
			}
...
/* rv_depacki_add_packet: frame-by-frame & callback */
static HX_RESULT rv_depacki_add_packet_framebyframe_callback(rv_depack_internal * pInt, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pPacket && pPacket->bIsValid)
	{
		/* Init local variables */
		BYTE *pBuf = rm_packet_get_cur_data(pPacket);
		UINT32 ulLen = rm_packet_get_cur_data_length(pPacket);
		UINT32 ulSubStream = 0;
		rv_frame_hdr hdr;
		/* Assume all packet data will be processed over */
		pPacket->bIsValid = FALSE;
		/* Clear the return value */
		retVal = HXR_OK;
		/* Make sure we are not ignoring this substream */
		...
		if (!pInt->bStreamSwitchable || !pInt->bIgnoreSubStream[ulSubStream])
		{
			...
			if (ulLen)
			{
				pPacket->bIsValid = TRUE;
			}
			else
			{
				pPacket->bIsValid = FALSE;
			}
			pPacket->pCurData = pBuf;
			pPacket->usCurDataLen = (UINT16)ulLen;
		}
	}
...
/* rv_depacki_add_packet: frame-by-frame & fifo */
static HX_RESULT rv_depacki_add_packet_framebyframe_fifo(rv_depack_internal * pInt, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
...
/* rv_depacki_add_packet: frame-by-frame & fifo */
static HX_RESULT rv_depacki_add_packet_framebyframe_swap(rv_depack_internal * pInt, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pPacket && pPacket->bIsValid)
	{
		...
		if (!pInt->bStreamSwitchable || !pInt->bIgnoreSubStream[ulSubStream])
		{
			...
			if (retVal == HXR_OK)
			{
				...
				case RVFrameTypePartial:
				case RVFrameTypeLastPartial:
					retVal = rv_depacki_handle_partial_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_SWAP);
					break;
				case RVFrameTypeMultiple:
				case RVFrameTypeWhole:
					retVal = rv_depacki_handle_one_frame_ex(pInt, &pBuf, &ulLen, pPacket, &hdr, RM_DEPACK_SWAP);
					break;
				}
			}
			if (ulLen)
...
HX_RESULT rv_depacki_add_packet_ex(rv_depack_internal * pInt, rm_packet * pPacket, UINT32 ulDepackMode)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pPacket)
	{
		if (RM_DEPACK_IS_PACKET_BY_PACKET(ulDepackMode))
		{
			if (RM_DEPACK_IS_CALLBACK(ulDepackMode))
			{
				retVal = rv_depacki_add_packet_packetbypacket_callback(pInt, pPacket);
			}	// callback
			else if (RM_DEPACK_IS_FIFO(ulDepackMode))
			{
				retVal = rv_depacki_add_packet_packetbypacket_fifo(pInt, pPacket);
			}	// fifo
			else if (RM_DEPACK_IS_SWAP(ulDepackMode))
			{
				/* SWAP mode does not supported in packet-by-packet */
				retVal = HXR_FAIL;
			}
		}	// packet-by-packet
		else
		{
			if (RM_DEPACK_IS_CALLBACK(ulDepackMode))
			{
				retVal = rv_depacki_add_packet_framebyframe_callback(pInt, pPacket);
			}	// callback
			else if (RM_DEPACK_IS_FIFO(ulDepackMode))
			{
				retVal = rv_depacki_add_packet_framebyframe_fifo(pInt, pPacket);
			}	// fifo
			else if (RM_DEPACK_IS_SWAP(ulDepackMode))
			{
				retVal = rv_depacki_add_packet_framebyframe_swap(pInt, pPacket);
			}
		}	// frame-by-frame
	}
...
void rv_depacki_cleanup_frame_buffer(rv_depack_internal *pInt)
{
	if (pInt)
	{
		...
		if (pInt->pReadyFrame)
		{
			rv_depacki_cleanup_frame(pInt, &pInt->pReadyFrame);
		}
	}
}

#if 0
/* The following API's will be removed later. -caijin @2008.06.03 */
											 ...
											 rv_frame_hdr*       pFrameHdr)
{
	...
	if (pInt && ppBuf && pulLen && pPacket && pFrameHdr && *ppBuf)
	{
		...
		*/
		...
		{
			/* Send the current frame */
			retVal = rv_depacki_send_current_frame_fifomode(pInt);
		}
		/* Do we have a current frame? */
		if (retVal == HXR_OK && !pInt->pCurFrame)
		{
...
HX_RESULT rv_depacki_send_current_frame_fifomode(rv_depack_internal * pInt)
{
	...
	if (pInt)
	{
		...
		if (pInt->pCurFrame)
		{
			...
			if (bDoSend)
			{
				/* Call the frame available function pointer,__ORIGINAL__,masked by Zhang Xuecheng,2007.11.19 */
				//retVal = pInt->fpAvail(pInt->pAvail,
				//                  0,//pInt->ulActiveSubStream,
				//                  pInt->pCurFrame);
				...
				/* the rv.type filed is different for Rv8 and Rv9 */
				/* Rv8:3bit,__2bit__(rv.type) */
				/* Rv9:1bit,__2bit__(rv.type) */
				/* so we have to judge the bitstream version first,Zhang Xuecheng,2008-3-10 14:55:46 */
				/*
				t = *(unsigned char *)(pInt->pCurFrame->pData);
				if (pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == 0x52563330)
				{
				t &= 0x18;
				t <<= 2;
				}
				else if(pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == 0x52563430 ||
				pInt->pSubStreamHdr[pInt->ulActiveSubStream].ulSubMOFTag == 0x54524F4D)
				t &= 0x60;
				else
				t = 0xff;
				...
				if (t == 0x20 || t == 0x00)
				{
				pInt->pKeyFrameTable[pInt->ulKeyFrameTableOffset] =
				pInt->pCurFrame->ulTimestamp;
				pInt->ulKeyFrameTableOffset++;
				pInt->ulKeyFrameTableCount++;
				if (pInt->ulKeyFrameTableOffset > KEY_FRAME_TIMESTAMP_TABLE_COUNT)
