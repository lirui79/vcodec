...
HX_RESULT rv_depack_attach_frame(rv_depack * pDepack, rv_frame * pFrame)
{
	...
	if (pInt)
	{
		...
		pInt->pRvDepackFifo[pInt->uRvFifoHead] = (unsigned int)((G1_ADDR_T)pFrame);
		pInt->uRvFifoHead++;
...
rv_frame *rv_depack_deattach_frame(rv_depack * pDepack)
{
	...
	if (pInt)
	{
		...
		pFrame = (rv_frame *)((G1_ADDR_T) pInt->pRvDepackFifo[pInt->uRvFifoTail]);
		pInt->uRvFifoTail++;
...
UINT32 rv_depack_fifo_time(rv_depack * pDepack)
{
	...
	if (pInt)
	{
		if (pInt->uRvFifoCnt)
		{
			pFrameHead = (rv_frame *)((G1_ADDR_T) pInt->pRvDepackFifo[pInt->uRvFifoHead - 1]);
			pFrameTail = (rv_frame *)((G1_ADDR_T) pInt->pRvDepackFifo[pInt->uRvFifoTail]);
