...
HX_RESULT ra_depacki_init(ra_depack_internal * pInt, rm_stream_header * hdr)
{
	...
	if (pInt && hdr)
	{
		...
		{
			//-----------------------------------------------------------------------------
			// Xiaocheng Wang   2008-02-28
			// add if(pInt->ulEndTime > 0) to avoid using 0 endtime
			if (pInt->ulEndTime > 0)
	...
	rm_rule_map * pMap, BYTE ** ppBuf, UINT32 * pulLen)
{
	...
	if (pInt)
	{
		retVal = rm_unpack_rule_map(ppBuf, pulLen,
			pInt->fpMalloc, pInt->fpFree, pInt->pUserMem, pMap);
	}
				...
				if (retVal == HXR_OK)
				{
						...
						!pHdr->bSeeked)
					{
						...
						retVal = ra_depacki_generate_and_send_loss(pInt,
							ulSubStream, pHdr->ulLastSentEndTime, pPacket->ulTime);
						if (retVal == HXR_OK)
						{
	...
	if (pInt)
	{
		...
		if (pBlock)
		{
			...
			if (retVal == HXR_OK)
			{
				...
				//                       ulSubStream,
				//                       pBlock);
				/* Zhang Xuecheng,2008-3-5 11:13:47 */
				retVal = ra_depack_attach_block(pInt, pBlock);
				if (retVal)
					...
					ulPacketOffset + ulAUFragSize <= ulPacketSize)
				{
					...
					if (pHdr->ulFragBufferOffset >= pHdr->ulFragBufferAUSize)
					{
						...
						retVal = ra_depacki_send_block(pInt, ulSubStream,
							pHdr->pFragBuffer,
							pHdr->ulFragBufferAUSize, pHdr->ulFragBufferTime, 0xFFFFFFFF);
			...
			if (ulBufOffset + 1 < ulLen)
			{
				...
				if (ulBufOffset + ulAUSize <= ulLen)
				{
					...
					retVal = ra_depacki_send_block(pInt,
						ulSubStream,
						pBuf + ulBufOffset, ulAUSize, pPacket->ulTime + ulTSOffset, 0xFFFFFFFF);
					if (retVal == HXR_OK)
					{
...
HX_RESULT ra_depacki_seek(ra_depack_internal * pInt, UINT32 ulTime)
{
	...
	if (pInt && pInt->pSubStreamHdr)
	{
		 ...
		 */
		UINT32 i = 0;
		ra_substream_hdr *pHdr = NULL;
		for (i = 0; i < pInt->multiStreamHdr.ulNumSubStreams; i++)
		{
			pInt->pSubStreamHdr[i].bSeeked = TRUE;
		}

		pHdr = &pInt->pSubStreamHdr[0];

		/* Zhang Xuecheng,2008-3-9 20:53:49 */
		/* Clear the interleaving buffers */
		/* only cook audio has interleave buffer,so before we clear the buffers,a check is required */
		if (pHdr->pIBuffer)
			memset(pHdr->pIBuffer, 0, pHdr->ulSuperBlockSize);
		if (pHdr->pDBuffer)
			memset(pHdr->pDBuffer, 0, pHdr->ulSuperBlockSize);
				...
				if (pHdr->pulGENRBlockOffset)
				{
					...
					if (pHdr->pulGENRPattern)
					{
						...
						if (pHdr->pulInterleavePattern)
						{
							...
							memcpy(pHdr->pulGENRPattern, pHdr->pulInterleavePattern, ulSize);
							/*
							...
							else
							{
								...
								while (ulCount < pHdr->ulNumCodecFrames)
								{
									pHdr->pulGENRPattern[ulCount] =
										ulBlockIndx * ulFramesPerBlock + ulFrameIndx;
									ulCount++;
									ulBlockIndx += 2;
