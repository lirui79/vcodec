...
HX_RESULT rm_parseri_read_next_header(rm_parser_internal* pInt, UINT32* pulID)
{
    ...
    if (pInt && pulID)
    {
        ...
        if (ulNumBytesRead == ulBytesToRead)
        {
            ...
            else if (genHdr.id == RM_CONTENT_OBJECT )
            {
                UINT32 bytesRead;
                UINT32 tmp;
                BYTE buffer[512];
                BYTE *pBuf = buffer;
                FILE *fp = (FILE*)pInt->pUserRead;
                UINT32 ulLen;
                UINT32 pos;
                UINT32 totalLen = 0;

                /* do trick here to get one failing case pass (corrupt
                 * headers) */
                if( genHdr.version == 0 )
                {
                    pos = ftell(fp);
                    bytesRead = fread( buffer, 1, 512, fp );
                    ulLen = bytesRead;

                    totalLen = RM_PARSER_GENERIC_HDR_SIZE;

                    tmp = rm_unpack16(&pBuf, &ulLen);
                    totalLen += 2 + tmp;
                    pBuf += tmp;
                    tmp = rm_unpack16(&pBuf, &ulLen);
                    totalLen += 2 + tmp;
...
HX_RESULT rm_parseri_setup_interleaved_streams(rm_parser_internal* pInt)
{
    ...
    if (pInt && pInt->pMediaPropsHdr && pInt->ulNumMediaPropsHdrs)
    {
        ...
        if (pInterleaved)
        {
            ...
            /*
             * Now run through all the logical streams
            ...
            for (i = 0; i < pInt->ulNumMediaPropsHdrs; i++)
            {
				...
				/*
				* zhang xuecheng,2008-2-18 16:25:29
				* the un-interleaved media porps header should be freed now
				* because they are not used any more.
				*/
				else
				{
					rm_parseri_cleanup_media_props_hdr(pInt, &pInt->pMediaPropsHdr[i]);
            ...
            if (ulNumInterleaved)
            {
                ...
                if (pTmp)
                {
                    ...
                    for (i = 0; i < pInt->ulNumMediaPropsHdrs; i++)
                    {
						...
						{
							/* Make sure to destroy the useless info */
							rm_parseri_cleanup_media_props_hdr(pInt, &pInt->pMediaPropsHdr[i]);
						}
                    }
                                         ...
                                         rm_property* pProp, UINT32 ulNumProp)
{
    ...
    if (pInt && pProp && ulNumProp)
    {
        ...
        retVal = rm_parseri_set_rm_property(pInt, &pProp[ulIndex++], "StreamCount",
                                            ...
                                            (void*) (G1_ADDR_T)pInt->ulNumStreams, 0);

        if (retVal == HXR_OK)
        {
            retVal = rm_parseri_set_rm_property(pInt, &pProp[ulIndex++], "Flags",
                                                RM_PROPERTY_TYPE_UINT32,
                                                (void*)(G1_ADDR_T)pInt->propHdr.flags, 0);
        }
        if (retVal == HXR_OK &&
    ...
    if (pInt && i < pInt->ulNumStreams && hdr)
    {
        ...
        if (pProp)
        {
            rm_parseri_set_rm_property(pInt, &pProp[ulRet], "StreamNumber",
                                       RM_PROPERTY_TYPE_UINT32, (void*)(G1_ADDR_T) i, 0);
        }
        ulRet++;
        ...
        if (pInt->pMediaPropsHdr[i].max_bit_rate)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "MaxBitRate",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) pInt->pMediaPropsHdr[i].max_bit_rate, 0);
            }
            ulRet++;
        }
        ...
        if (pInt->pMediaPropsHdr[i].avg_bit_rate)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "AvgBitRate",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T)pInt->pMediaPropsHdr[i].avg_bit_rate, 0);
            }
            ulRet++;
        }
        ...
        if (pInt->pMediaPropsHdr[i].max_pkt_size)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "MaxPacketSize",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) pInt->pMediaPropsHdr[i].max_pkt_size, 0);
            }
            ulRet++;
        }
        ...
        if (pInt->pMediaPropsHdr[i].avg_pkt_size)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "AvgPacketSize",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) pInt->pMediaPropsHdr[i].avg_pkt_size, 0);
            }
            ulRet++;
        }
        ...
        if (pInt->pMediaPropsHdr[i].start_time)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "StartTime",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) pInt->pMediaPropsHdr[i].start_time, 0);
            }
            ulRet++;
        }
        ...
        if (pInt->pMediaPropsHdr[i].duration > pInt->propHdr.duration)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "EndTime",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) pInt->propHdr.duration, 0);
            }
            ulRet++;
        }
        ...
        if (ulStreamingPreroll)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "StreamingPreroll",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) ulStreamingPreroll, 0);
            }
            ulRet++;
        }
        ...
        if (ulPreDecBufSize)
        {
            if (pProp)
            {
                rm_parseri_set_rm_property(pInt, &pProp[ulRet], "X-PreDecBufSize",
                                           RM_PROPERTY_TYPE_UINT32,
                                           (void*)(G1_ADDR_T) ulPreDecBufSize, 0);
            }
            ulRet++;
        }
    ...
    if (pInt && pbHasPreDataProps)
    {
        ...
        if (pRet)
        {
            ...
            else if (bIsRealEvent || bHasOpaqueData)
            {
                ...
                {
                    /* RealEvent stream */
                    sprintf(pRet, "%s", g_pszRuleBook[6]);
                }
                else if (bIsVBR)
                {
            ...
            else
            {
                ...
                if (bIsVBR)
                {
                    ...
                    {
                        sprintf(pRet, "%s", g_pszRuleBook[4]);
                    }
                }
                else
                {
...
HX_RESULT rm_parseri_read_next_packet(rm_parser_internal* pInt, rm_packet** ppPacket)
{
    ...
    if (pInt && ppPacket)
    {
        ...
        if (pInt->ulCurFileOffset < pInt->propHdr.data_offset + pInt->dataHdr.size)
        {
			...
			//UINT32     ulNextPacketTime = 0;	/* added by caijin */
            do
            {
                retVal = rm_parseri_read_next_packet_header(pInt, &hdr);
                if (retVal == HXR_OK)
                        ...
                        if (pInfo)
                        {
                            ...
                            if (pInt->ulKeyframesNeeded)
                            {
                                ...
                                else
                                {
									...
									bSkipPacket = TRUE;
									/* Seek past the data of this packet */
									/* chaoyanggou.rmvb,if seek to 1hour+,header member will be all 0 */
									/* so I add a judge,let parser can go on,not spinning here */
									/* zhang xuecheng,2008-5-19 14:48:15 */
            ...
            if (HX_SUCCEEDED(retVal))
            {
				...
				/*
				 * Get the next packet time,
				 * which may be required to compute the frame duration
				 * caijin @2008.06.01
				 */
				//retVal = rm_parseri_check_next_packet_timestamp(pInt, ulStreamNum, ulNextPacketTime);
				//if (!HX_SUCCEEDED(retVal))
				//{
				//	ulNextPacketTime = 0xFFFFFFFF;	/* set the fail flag */
				//}
                ...
                if (pInt->pStreamInfo[ulStreamNum].bIsRealVideo)
                {
                     ...
                     * the timestamp of the last packet and either this packet
                ...
                if (pPacket)
                {
                    ...
                    if (pPacket->pData)
                    {
                        ...
                        if (ulBytesRead == ulDataSize)
                        {
							...
							/*
							 * added to support frame-by-frame depacketization mode.
							 * caijin @2008.05.27
							 */
							pPacket->bIsValid = TRUE;
							pPacket->usCurDataLen = pPacket->usDataLen;
							pPacket->pCurData = pPacket->pData;
							/* next packet time may be required while computing frame duration */
							//pPacket->ulNextTimestamp = ulNextPacketTime;
			...
			if (HX_SUCCEEDED(retVal))
			{
				...
				*/
				...
				if (pPacket)
				{
					...
					{
						/* Read the data into the buffer */
						ulBytesRead = rm_parseri_file_read_buffer2(pInt, &pPacket->pData, ulDataSize);
						if (ulBytesRead == ulDataSize)
						{
							/* Assign the packet parameters */
							pPacket->ulTime     = hdr.timestamp;
...
UINT32 rm_parseri_file_read_buffer2(rm_parser_internal* pInt, BYTE** ppBuf, UINT32 ulBytesToRead)
{
	...
	if (pInt && pInt->fpRead2)
	{
		/* Read in ulBytesToRead bytes */
		ulRet = pInt->fpRead2(pInt->pUserRead, ppBuf, ulBytesToRead);
		/* Advance the current file offset */
		pInt->ulCurFileOffset += ulRet;
	}
                                            ...
                                            UINT32* pulFoundTime, UINT32* pulFoundOffset)
{
        ...
        pInt->ulNumStreams && pInt->pStreamInfo)
    {
        ...
        for (i = 0; i < pInt->ulNumStreams; i++)
        {
            ...
            if (pInfo)
            {
                ...
                if (status == HXR_OK)
                {
                    ...
                    //}
					if (pInfo->keyFramePacket.ulTimestamp < ulMinTime)
					{
					    ulMinTime = pInfo->keyFramePacket.ulTimestamp;
					}
					if (pInfo->keyFramePacket.ulFileOffset < ulMinOffset)
					{
					    ulMinOffset = pInfo->keyFramePacket.ulFileOffset;
					}
                }
                /*
        ...
        if (HX_SUCCEEDED(retVal))
        {
			...
			*pulFoundTime   = ulMinTime;
			*pulFoundOffset = ulMinOffset;
        }
    }
...
HX_RESULT rm_parseri_search_index_chunk(rm_parser_internal* pInt, UINT32 ulSeekTime)
{
        ...
        pInt->ulNumStreams)
    {
        ...
        {
			/*
			* Avoid to seek beyond the file length.
			* If fpTell is not available, this function cannot step forward.
			* -caijin @2008.07.02
			*/
			UINT32 ulFileLength = 0;
			if (pInt->fpTell)
			{
				pInt->fpSeek(pInt->pUserRead, 0, HX_SEEK_ORIGIN_END);
				ulFileLength = pInt->fpTell(pInt->pUserRead);
			}
			if (ulFileLength <= ulIndexOffset)
			{
				retVal = HXR_FAIL;
				break;
			}
...
HX_RESULT rm_parseri_seek(rm_parser_internal* pInt, UINT32 ulSeekTime)
{
    ...
    if (pInt && pInt->ulNumStreams && pInt->pStreamInfo)
    {
		...
		else if (pInt->propHdr.index_offset)		//-caijin@2008.07.02
        {
            /*
			...
			if (retVal)
			{
				ulDataOffset = pInt->propHdr.data_offset +
					RM_PARSER_DATA_CHUNK_HEADER_SIZE;
				rm_parseri_file_seek(pInt, ulDataOffset, HX_SEEK_ORIGIN_SET);
			}
        }
        ...
        for (i = 0; i < pInt->ulNumStreams; i++)
        {
                ...
                pInt->pStreamInfo[i].keyFramePacket.bValid)
            {
                    ...
                    pInt->pStreamInfo[i].keyFramePacket.ulFileOffset < ulLowOffset)
                {
					...
					if (pInt->pStreamInfo[i].bIsRealVideo)
					{
						pInt->ulRvTimeStampAfterSeek =
							pInt->pStreamInfo[i].keyFramePacket.ulTimestamp;
					}
					else if (pInt->pStreamInfo[i].bIsRealAudio)
					{
						pInt->ulRaTimeStampAfterSeek =
							pInt->pStreamInfo[i].keyFramePacket.ulTimestamp;
					}
                }
            }
...
HX_RESULT rm_parseri_get_fileoffset(rm_parser_internal * pInt, UINT32 *ulCurFileOffset)
{
	...
	if (pInt && ulCurFileOffset)
	{
		*ulCurFileOffset = pInt->ulCurFileOffset;
		retVal = HXR_OK;
	}
...
/*
 * Function: Get the timestamp of next packet which belongs to same stream
 * Return:   HXR_OK, HXR_FAIL, HXR_AT_END
 */
HX_RESULT rm_parseri_check_next_packet_timestamp(rm_parser_internal * pInt, UINT32 ulStreamNum, UINT32 * ulTimestamp)
{
	...
	struct rm_stream_info* pInfo = &pInt->pStreamInfo[ulStreamNum];
	if (pInt && ulTimestamp && pInfo)
	{
		struct rm_pkt_hdr hdr;
		UINT32 ulCurFileOffset = 0;

		/* save file offset for recovery before return*/
		retVal = rm_parseri_get_fileoffset(pInt, &ulCurFileOffset);

		/* Now keep getting packets, until we find a packet with the same StreamNum as given */
		while (pInt->ulCurFileOffset < pInt->propHdr.data_offset + pInt->dataHdr.size)
		{
			/* Read the next packet header into the read buffer*/
			retVal = rm_parseri_read_next_packet_header(pInt, &hdr);
			if (retVal != HXR_OK)
			{
				break;
			}
			/* Is this the stream we are focused on */
			if (ulStreamNum != rm_parseri_translate_stream_number(pInt, hdr.stream_num))
			{
				continue;
			}

			/*
			* we need not a keyframe? or,
			* Is this a keyframe, and do we need a keyframe
			* for this stream?
			*/
			if (!pInfo->bNeedKeyframe ||
				...
				hdr.timestamp >= pInfo->keyFramePacket.ulTimestamp)))
			{
				/* Save the timestamp of the current packet */
				*ulTimestamp  = hdr.timestamp;
				/* Break out of the packet loop */
				break;
			}
			else
		...
		if (pInt->ulCurFileOffset >= pInt->propHdr.data_offset + pInt->dataHdr.size)
		{
			*ulTimestamp = pInt->propHdr.duration;
			retVal = HXR_AT_END;
		}

		/* recover the file offset */
		rm_parseri_set_fileoffset(pInt, ulCurFileOffset);
	}
...
HX_RESULT rm_parseri_get_timestamp_after_seek_time(rm_parser_internal * pInt, UINT16 usStreamNum, UINT32 *ulTimeStamp)
{
	...
	if (pInt && pInt->pStreamInfo && (usStreamNum < pInt->ulNumStreams) && ulTimeStamp)
	{
		*ulTimeStamp = pInt->pStreamInfo[usStreamNum].keyFramePacket.ulTimestamp;
		retVal = HXR_OK;
	}
