...
#include "basetype.h"
#include "dwlthread.h"
#include "../subsys_driver/vcmdswhwregisters.h"
#include "asic.h"
...
#define VCMD_HW_ID_1_4_1                0x43421401
#define ANY_CMDBUF_ID 0xFFFF

#ifdef USE_LIBVA
#define CmodelIoctlReserveCmdbuf DecCmodelIoctlReserveCmdbuf
#define CmodelIoctlEnableCmdbuf DecCmodelIoctlEnableCmdbuf
#define CmodelIoctlWaitCmdbuf DecCmodelIoctlWaitCmdbuf
#define CmodelIoctlReleaseCmdbuf DecCmodelIoctlReleaseCmdbuf
#define CmodelIoctlGetCmdbufParameter DecCmodelIoctlGetCmdbufParameter
#define CmodelIoctlGetVcmdParameter DecCmodelIoctlGetVcmdParameter
#define CmodelVcmdInit DecCmodelVcmdInit
#define CmodelVcmdRelease DecCmodelVcmdRelease
#define asicVcmdRegisterDesc DecasicVcmdRegisterDesc
#endif
...
struct exchange_parameter
{
  ...
  u32 core_mask;                      //core_mask for user to select cores: [0,15]core mask, [16,31]client type.
  u8  cmdbuf_reserve;
};
...
struct cmdbuf_obj
{
  u32 module_type; 		          //current CMDBUF type: input vce=0,IM=1,vcd=2，jpege=3, jpegd=4
  ...
  addr_t cmdbuf_bus_address;		  //current CMDBUF start physical address.
  u32 *status_virtual_address;   //current status CMDBUF start virtual address.
  addr_t status_bus_address;		  //current status CMDBUF start physical address.
...
struct hantrovcmd_dev
{
    ...
    addr_t vcmd_reg_mem_bus_address;  //start physical address of vcmd registers memory of  CMDBUF.
