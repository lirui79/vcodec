...
#define VWL_CLIENT_TYPE_VVC_DEC      0x00010000
...
extern void DWLVirtualPlatformTestIn(i32 core_id, Core instance, HwEnableStatus hw_status);
static u32 AsicHwVcmdCoreGetCmdBufId(void* instance);
#endif
...
static pthread_mutex_t vcmd_release_mutex;
...
static int vcmd_run_once[MAX_CORE_NUMBER] = {0};
...
{
#include "../subsys_driver/vcmdregistertable.h"
};

static void init_bi_list(bi_list* list);
static bi_list_node* bi_list_create_node(void);
static void bi_list_free_node(bi_list_node* node);
static void bi_list_insert_node_tail(bi_list* list,bi_list_node* current_node);
static void bi_list_insert_node_before(bi_list* list,bi_list_node* base_node,bi_list_node* new_node);
static void bi_list_remove_node(bi_list* list,bi_list_node* current_node);
...
static void vwl_write_register_value(const void *vcmd_core,u32* reg_mirror,regVcmdName name, u32 value) {
  const regVcmdField_s *field;
  u32 regVal;
...
static u32 vwl_get_register_value(const void *vcmd_core, u32* reg_mirror,regVcmdName name) {
  const regVcmdField_s *field;
  u32 value;
...
#define vwl_set_addr_register_value(vcmd_core, reg_mirror, name, value) do {\
    if(sizeof(addr_t) == 8){\
      vwl_write_register_value((vcmd_core), (reg_mirror),name, (u32)((u64)(addr_t)value));  \
      vwl_write_register_value((vcmd_core), (reg_mirror),name##_MSB, (u32)(((u64)(addr_t)value) >> 32)); \
    } else {\
      vwl_write_register_value((vcmd_core),(reg_mirror), name, (u32)((addr_t)value));  \
    }\
} while (0)

#define VWLGetAddrRegisterValue(vcmd_core, reg_mirror,name)  \
     ((sizeof(addr_t) == 8) ? (\
     (((addr_t)vwl_get_register_value((vcmd_core),(reg_mirror), name)) |  \
     (((addr_t)vwl_get_register_value((vcmd_core), (reg_mirror),name##_MSB)) << 32))\
    ) : ((addr_t)vwl_get_register_value((vcmd_core),(reg_mirror), (name))))
...
void PrintInstr(u32 i, u32 instr, u32 *size) {
  if ((instr & 0xF8000000) == OPCODE_WREG) {
    ...
    printf("current cmdbuf data %u = 0x%08x => [%s %s %d 0x%x]\n", i, instr, "WREG", ((instr >> 26) & 0x1) ? "FIX" : "",
           length, (instr & 0xFFFF));
    *size = ((length+2)>>1)<<1;
  } else if ((instr & 0xF8000000) == OPCODE_END) {
    printf("current cmdbuf data %u = 0x%08x => [%s]\n", i, instr, "END");
    *size = 2;
...
static bi_list_node* get_cmdbuf_node_in_list_by_addr(addr_t  cmdbuf_addr,bi_list* list)
{
  ...
  while(1) {
    ...
    if(ret != 0) {
      ...
      if(dev->hw_version_id <= VCMD_HW_ID_1_0_C )
        hw_rdy_cmdbuf_num = vwl_get_register_value(dev->vcmd_core, dev->reg_mirror, HWIF_VCMD_EXE_CMDBUF_COUNT);
      else {
  ...
  if(cmdbuf_obj->priority==CMDBUF_PRIORITY_NORMAL)
  {
    ...
    while(1)
    {
      ...
      if(ret != 0) {
        ...
        if(dev->hw_version_id <= VCMD_HW_ID_1_0_C )
        {
          exe_cmdbuf_addr = VWLGetAddrRegisterValue(dev->vcmd_core,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ADDR);
          list=&dev->list_manager;
          pthread_mutex_lock(&global_vcmd_mutex);
    ...
    bi_list_insert_node_tail(list,new_cmdbuf_node);
    pthread_mutex_unlock(&global_vcmd_mutex);
    if (smallest_dev != NULL) cmdbuf_obj->core_id = smallest_dev->core_id;
    return 0;
  }
  else
  {
    ...
    if (smallest_dev != NULL) {
		vwl_write_register_value(smallest_dev->vcmd_core, smallest_dev->reg_mirror, HWIF_VCMD_START_TRIGGER, 0);
    //FIXME
...
static void vcmd_start(struct hantrovcmd_dev *dev, bi_list_node* first_linked_cmdbuf_node) {
  ...
  if(dev->working_state == WORKING_STATE_IDLE) {
    if((first_linked_cmdbuf_node != NULL) && dev->sw_cmdbuf_rdy_num) {
      ...
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR, (u32)((addr_t)cmdbuf_obj->cmdbuf_virtual_address));
      if(sizeof(addr_t) == 8) {
        vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR_MSB, (u32)(((u64)(addr_t)cmdbuf_obj->cmdbuf_virtual_address)>>32));
      } else {
        vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_ADDR_MSB, 0);
      }
      vwl_set_register_mirror_value(dev->reg_mirror, HWIF_VCMD_CMDBUF_EXE_LENGTH, (u32)((cmdbuf_obj->cmdbuf_size+7)/8));
      ...
      if(dev->hw_version_id > VCMD_HW_ID_1_0_C ) {
        vwl_write_register_value(dev->vcmd_core,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ID,(u32)cmdbuf_obj->cmdbuf_id);
      }

      AsicHwVcmdCoreWriteRegister(dev->vcmd_core, 0x44, AsicHwVcmdCoreReadRegister(dev->vcmd_core, 0x44));
      for(i = 0; i < 7; i++)
      {
        AsicHwVcmdCoreWriteRegister(dev->vcmd_core, 0x48+i*4, dev->reg_mirror[(0x48+i*4)/4]);
      }
...
static i32 link_and_run_cmdbuf(struct exchange_parameter* input_para) {
  ...
  if(dev->working_state == WORKING_STATE_IDLE) {
    ...
    if(record_last_cmdbuf_rdy_num != dev->sw_cmdbuf_rdy_num)
      vwl_write_register_value(dev->vcmd_core,dev->reg_mirror,HWIF_VCMD_RDY_CMDBUF_COUNT,dev->sw_cmdbuf_rdy_num);
  }
  pthread_mutex_unlock(&global_vcmd_mutex);
...
/******************************************************************************/
static int check_cmdbuf_irq(struct hantrovcmd_dev* dev,struct cmdbuf_obj* new_cmdbuf_obj,u32 *irq_status_ret)
{
  ...
  {
    rdy = 1;
    *irq_status_ret=new_cmdbuf_obj->executing_status;//need to decide how to assign this variable
  }
  return rdy;
}
...
static unsigned int wait_cmdbuf_ready(u16 *cmd_buf_id)
{
    ...
    pthread_mutex_lock(&vcmd_release_mutex);
    if (*cmd_buf_id != ANY_CMDBUF_ID) {
        new_cmdbuf_obj=(struct cmdbuf_obj*)global_cmdbuf_node[*cmd_buf_id&255]->data;
...
//            pthread_cond_wait(&global_vcmd_cond_wait,&global_vcmd_mutex);
    ...
    pthread_mutex_unlock(&vcmd_release_mutex);
    return 0;
}
...
static void create_read_all_registers_cmdbuf(struct exchange_parameter* input_para)
{
    ...
    addr_t status_base_phy_addr=vcmd_status_buf_mem_pool.bus_address + input_para->cmdbuf_id*CMDBUF_MAX_SIZE+(vcmd_manager[input_para->module_type][0]->vcmd_core_cfg.vcmd_core_config_ptr->submodule_main_addr+0);
    u32 offset_inc=0;
...
static void read_main_module_all_registers(u32 core_id)
{
  ...
  input_para.core_mask = 1 << core_id;
  ret = reserve_cmdbuf(&input_para);
  (void)(ret);
  vcmd_manager[main_module_type][core_id]->status_cmdbuf_id = input_para.cmdbuf_id;
  ...
  status_base_virt_addr=vcmd_status_buf_mem_pool.virtual_address + input_para.cmdbuf_id*CMDBUF_MAX_SIZE/4+(vcmd_manager[main_module_type][core_id]->vcmd_core_cfg.vcmd_core_config_ptr->submodule_main_addr/4+0);
...
i32 CmodelIoctlGetVcmdParameter(struct config_parameter *vcmd_params) {
  ...
  if(vcmd_core_num[vcmd_params->module_type]) {
    ...
    vcmd_params->submodule_MMUWrite_addr = vcmd_manager[vcmd_params->module_type][0]->vcmd_core_cfg.vcmd_core_config_ptr->submodule_MMUWrite_addr;
    vcmd_params->submodule_axife_addr = vcmd_manager[vcmd_params->module_type][0]->vcmd_core_cfg.vcmd_core_config_ptr->submodule_axife_addr;
    ...
    vcmd_params->submodule_MMUWrite_addr = 0xffff;
    vcmd_params->submodule_axife_addr = 0xffff;
...
i32 CmodelVcmdInit(){
  ...
  for(i = 0; i < total_vcmd_core_num; i++) {
    ...
    hantrovcmd_data[i].vcmd_core_cfg.vcmd_hw_version_id = VCMD_HW_ID_1_4_1;
    ...
    vcmd_manager[type][i] = dev;
    if (dev->hwregs) hantrovcmd_data[i].hw_version_id = *(u32*)dev->hwregs;
  ...
  pthread_mutex_init(&vcmd_release_mutex, NULL);
...
void CmodelVcmdRelease() {
  ...
  pthread_mutex_destroy(&vcmd_release_mutex);
...
static int vcmd_isr_cb(int irq, void *dev_id) {
  ...
  if((dev->hw_version_id > VCMD_HW_ID_1_0_C) && (irq_status&0x3f)) {
    ...
    cmdbuf_id = vwl_get_register_value(dev->vcmd_core,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ID);
    if(cmdbuf_id>=TOTAL_DISCRETE_CMDBUF_NUM) {
      DTRACE_E("%s","vcmd_isr_cb error cmdbuf_id greater than the ceiling !!\n");
      return IRQ_HANDLED;
    }
  ...
  if(vwl_get_register_mirror_value(dev->reg_mirror,HWIF_VCMD_IRQ_ABORT)) {
    ...
    if(dev->hw_version_id > VCMD_HW_ID_1_0_C) {
      new_cmdbuf_node = global_cmdbuf_node[cmdbuf_id];
      if(new_cmdbuf_node==NULL) {
        DTRACE_E("%s","vcmd_isr_cb error cmdbuf_id !!\n");
        return IRQ_HANDLED;
      }
    } else {
      exe_cmdbuf_bus_address = VWLGetAddrRegisterValue(dev->vcmd_core,dev->reg_mirror,HWIF_VCMD_CMDBUF_EXE_ADDR);
    ...
    while(1) {
      ...
      if((cmdbuf_obj->cmdbuf_run_done==0)) {
        ...
        vcmd_run_once[((struct cmdbuf_obj*)(global_cmdbuf_node[cmdbuf_id]->data))->core_id] = 1;
      } else
        break;
      new_cmdbuf_node = new_cmdbuf_node->previous;
    }
...
#ifdef VIRTUAL_PLATFORM_TEST
    if (reg_idx == MAX_REG_COUNT-1) {
      HwEnableStatus hw_status;
      hw_status.cmodel_enable_hw_bit = 0;
      hw_status.driver_enable_hw_bit = 1;
      u32 cmdbuf_id = AsicHwVcmdCoreGetCmdBufId(core);
      DWLVirtualPlatformTestIn(cmdbuf_id, (Core)core, hw_status);
    }
#endif
    if(reg_idx == 1 && (value & 0x01))
    {
...
#ifdef VIRTUAL_PLATFORM_TEST
    u32 cmdbuf_id = AsicHwVcmdCoreGetCmdBufId(core);
    DWLVirtualPlatformTestOut(cmdbuf_id, (Core)core);
#endif
  }
  else
  {
...
#ifdef VIRTUAL_PLATFORM_TEST
static u32 AsicHwVcmdCoreGetCmdBufId(void* instance) {
  struct HwVcmdCore* vcmd_core = (struct HwVcmdCore*)instance;
  return vcmd_core->running_cmdbuf_id;
}
#endif
